//! Types used to define loaded and effective Peregrine configuration values.
...
pub use peregrine_types::config_types::AltScreenMode;
pub use peregrine_types::config_types::ApprovalsReviewer;
use peregrine_types::config_types::EnvironmentVariablePattern;
pub use peregrine_types::config_types::ModeKind;
pub use peregrine_types::config_types::Personality;
pub use peregrine_types::config_types::ServiceTier;
use peregrine_types::config_types::ShellEnvironmentPolicy;
use peregrine_types::config_types::ShellEnvironmentPolicyInherit;
pub use peregrine_types::config_types::WebSearchMode;
...
/// Determine where Peregrine should store CLI auth credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in PEREGRINE_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in PEREGRINE_HOME.
    Auto,
    /// Store credentials in memory only for the current process.
    Ephemeral,
}

/// Determine where Peregrine should store and read MCP credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum OAuthCredentialsStoreMode {
    ...
    #[default]
    Auto,
    /// PEREGRINE_HOME/.credentials.json
...
pub struct AnalyticsConfigToml {
    /// When `false`, disables analytics across Peregrine product surfaces in this profile.
...
pub struct FeedbackConfigToml {
    /// When `false`, disables the feedback flow across Peregrine product surfaces.
    pub enabled: Option<bool>,
}
...
pub struct MemoriesToml {
    ...
    /// Minimum remaining percentage required in Peregrine rate-limit windows before memory startup runs.
...
pub struct AppConfig {
    /// When `false`, Peregrine does not surface this app.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
/// Fallback resize-reflow row cap when Peregrine cannot identify a terminal-specific scrollback size.
...
pub struct Tui {
    ...
    /// Use `/theme` in the TUI or see `$PEREGRINE_HOME/themes` for custom themes.
    #[serde(default)]
    pub theme: Option<String>,

    /// Pet id to preselect in the terminal pet picker.
    ///
    /// Custom pet ids resolve against PEREGRINE_HOME/pets/<pet-id>/pet.json.
    #[serde(default)]
    ...
    /// transcript exceeds this cap. Omit to use Peregrine's terminal-specific default. Set to `0` to
...
/// (primarily the Peregrine IDE extension). NOTE: these are different from
...
pub struct Notice {
    ...
    /// Tracks whether the user opted out of Peregrine-managed fast defaults.
    pub fast_default_opt_out: Option<bool>,
    /// Tracks whether the user opted out of the rate limit model switch reminder.
    pub hide_rate_limit_model_nudge: Option<bool>,
    /// Tracks whether the user has seen the model migration prompt
    pub hide_gpt5_1_migration_prompt: Option<bool>,
    /// Tracks whether the user has seen the gpt-5.1-peregrine-max migration prompt
    #[serde(rename = "hide_gpt-5.1-peregrine-max_migration_prompt")]
    pub hide_gpt_5_1_peregrine_max_migration_prompt: Option<bool>,
...
pub struct PluginMcpServerConfig {
    /// When `false`, Peregrine skips initializing this plugin MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
pub struct MarketplaceConfig {
    /// Last time Peregrine successfully added or refreshed this marketplace.
    #[serde(default)]
    pub last_updated: Option<String>,
    /// Git revision Peregrine last successfully activated for this marketplace.
    #[serde(default)]
...
impl From<SandboxWorkspaceWrite> for peregrine_app_server_protocol::SandboxSettings {
