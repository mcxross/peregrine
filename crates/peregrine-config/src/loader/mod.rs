...
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/peregrine/config.toml";
...
/// - system    `/etc/peregrine/requirements.toml` (Unix) or
///   `%ProgramData%\Peregrine\requirements.toml` (Windows)
///
...
///
/// - admin:    managed preferences (*)
/// - system    `/etc/peregrine/config.toml` (Unix) or
///   `%ProgramData%\Peregrine\config.toml` (Windows)
/// - user      `${PEREGRINE_HOME}/config.toml`
/// - profile   `${PEREGRINE_HOME}/<name>.config.toml`, when selected
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.peregrine/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.peregrine/config.toml` (loaded but disabled when untrusted)
...
pub async fn load_config_layers_state(
    ...
    let loaded_config_layers =
        layer_io::load_config_layers_internal(fs, peregrine_home, overrides.clone(), strict_config)
            .await?;
    if !ignore_managed_requirements {
    ...
    // contain overrides.
    let active_user_file = overrides.user_config_path(peregrine_home)?;
    let base_user_file =
        AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, peregrine_home);
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        // value for base_dir, so peregrine_home is as good a value as any.
        let managed_config =
            resolve_relative_paths_in_config_toml(config.managed_config, peregrine_home)?;
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/peregrine/requirements.toml"))
}
...
#[cfg(windows)]
fn windows_peregrine_system_dir() -> PathBuf {
...
impl ProjectTrustContext {
    ...
    fn root_checkout_hooks_folder_for_dir(&self, dir: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
        ...
        Some(repo_root.join(relative_dir).join(".peregrine"))
    }
}

fn project_layer_entry(
    dot_peregrine_folder: &AbsolutePathBuf,
    config: TomlValue,
    disabled_reason: Option<String>,
    hooks_config_folder_override: Option<AbsolutePathBuf>,
) -> ConfigLayerEntry {
    let source = ConfigLayerSource::Project {
        dot_peregrine_folder: dot_peregrine_folder.clone(),
    };
...
fn project_ignored_config_keys_warning(
    ...
    let config_path = dot_peregrine_folder.join(CONFIG_TOML_FILE);
...
async fn load_project_layers(
    ...
    let peregrine_home_abs = AbsolutePathBuf::from_absolute_path(peregrine_home)?;
    let peregrine_home_normalized = normalize_path(peregrine_home_abs.as_path())
        .unwrap_or_else(|_| peregrine_home_abs.to_path_buf());
    let mut dirs = cwd
        .ancestors()
        .scan(false, |done, a| {
            if *done {
    ...
    for dir in dirs {
        let dot_peregrine_abs = dir.join(".peregrine");
        if !fs
            .get_metadata(&dot_peregrine_abs, /*sandbox*/ None)
            .await
        ...
        let dot_peregrine_normalized = normalize_path(dot_peregrine_abs.as_path())
            .unwrap_or_else(|_| dot_peregrine_abs.to_path_buf());
        if dot_peregrine_abs == peregrine_home_abs
            || dot_peregrine_normalized == peregrine_home_normalized
        {
            continue;
        }
        let config_file = dot_peregrine_abs.join(CONFIG_TOML_FILE);
            ...
            Ok(contents) => {
                ...
                if disabled_reason.is_none() && strict_config {
                    validate_config_toml_strictly(
                        ...
                        &contents,
                        &config,
                        dot_peregrine_abs.as_path(),
                    )?;
                }
                let ignored_project_config_keys = sanitize_project_config(&mut config);
                let config =
                    resolve_relative_paths_in_config_toml(config, dot_peregrine_abs.as_path())?;
...
/// from a file like `/etc/peregrine/managed_config.toml` that has the same
...
impl From<LegacyManagedConfigToml> for ConfigRequirementsToml {
    fn from(legacy: LegacyManagedConfigToml) -> Self {
        ...
        if let Some(sandbox_mode) = sandbox_mode {
            ...
            // Allowing read-only is a requirement for Peregrine to function correctly.
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("Peregrine").join("config.toml"))
        );
    }
}
