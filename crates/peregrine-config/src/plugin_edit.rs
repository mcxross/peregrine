...
pub async fn clear_user_plugin(peregrine_home: &Path, plugin_key: String) -> std::io::Result<()> {
    apply_user_plugin_config_edits(peregrine_home, vec![PluginConfigEdit::Clear { plugin_key }])
        .await
}
...
pub async fn apply_user_plugin_config_edits(
    ...
    let peregrine_home = peregrine_home.to_path_buf();
    task::spawn_blocking(move || apply_user_plugin_config_edits_blocking(&peregrine_home, edits))
        .await
...
mod tests {
    ...
    async fn set_user_plugin_enabled_writes_plugin_entry() {
        ...
        let config = read_config(peregrine_home.path());
...
"#,
        ...
        clear_user_plugin(peregrine_home.path(), "demo@market".to_string())
            .await
            .unwrap();

        assert_eq!(
            fs::read_to_string(peregrine_home.path().join(CONFIG_TOML_FILE)).unwrap(),
            ""
        );
    }
    ...
    async fn clear_user_plugin_missing_entry_does_not_create_config() {
        ...
        assert!(!peregrine_home.path().join(CONFIG_TOML_FILE).exists());
    }
    ...
    async fn set_user_plugin_enabled_follows_config_symlink() {
        ...
        let peregrine_home = TempDir::new().unwrap();
        let target_path = peregrine_home.path().join("target_config.toml");
        symlink(&target_path, peregrine_home.path().join(CONFIG_TOML_FILE)).unwrap();
...
enabled = true
    ...
    fn read_config(peregrine_home: &Path) -> toml::Value {
        toml::from_str(&fs::read_to_string(peregrine_home.join(CONFIG_TOML_FILE)).unwrap()).unwrap()
    }
}
