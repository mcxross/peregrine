...
use peregrine_app_server_protocol::ConfigLayer;
use peregrine_app_server_protocol::ConfigLayerMetadata;
...
impl LoaderOverrides {
    ...
    pub fn without_managed_config_for_tests() -> Self {
        let base = std::env::temp_dir().join("peregrine-config-tests");
        Self {
    ...
    pub fn user_config_path(&self, peregrine_home: &Path) -> std::io::Result<AbsolutePathBuf> {
...
impl ConfigLayerEntry {
    ...
    // Get the `.peregrine/` folder associated with this config layer, if any.
    pub fn config_folder(&self) -> Option<AbsolutePathBuf> {
        match &self.name {
            ...
            } => Some(dot_peregrine_folder.clone()),
    ...
    /// Returns the `.peregrine/` folder that should be used for hook declarations.
    ///
...
pub struct ConfigLayerStack {
    ...
    /// the base `$PEREGRINE_HOME/config.toml` layer followed by the profile override
...
impl ConfigLayerStack {
    ...
    /// the base `$PEREGRINE_HOME/config.toml` layer because the active layer is the
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<Option<usize>> {
    ...
    let mut previous_project_dot_peregrine_folder: Option<&AbsolutePathBuf> = None;
    for (index, layer) in layers.iter().enumerate() {
        ...
        if let ConfigLayerSource::Project {
            dot_peregrine_folder: current_project_dot_peregrine_folder,
        } = &layer.name
        {
            if let Some(previous) = previous_project_dot_peregrine_folder {
                ...
                if previous == current_project_dot_peregrine_folder
                    || !current_project_dot_peregrine_folder
                        .as_path()
                        .ancestors()
            ...
            previous_project_dot_peregrine_folder = Some(current_project_dot_peregrine_folder);
        }
    }
