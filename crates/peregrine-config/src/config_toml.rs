//! Schema-heavy configuration TOML types used by Peregrine.
...
use peregrine_types::config_types::WebSearchToolConfig;
...
/// Base config deserialized from ~/.peregrine/config.toml.
...
pub struct ConfigToml {
    ...
    /// sanctioned by Peregrine will likely degrade model performance.
    ...
    /// file: Use a file in the Peregrine home directory.
    ...
    /// Defaults to `$PEREGRINE_SQLITE_HOME` when set. Otherwise uses `$PEREGRINE_HOME`.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where Peregrine writes log files. Setting this value explicitly
    /// also enables the TUI text log in this directory.
    /// Defaults to `$PEREGRINE_HOME/log`.
    ...
    /// Optional product SKU forwarded on host-owned Peregrine Apps MCP requests.
    ...
    /// directories for `.peregrine` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
pub struct DebugConfigLockToml {
    ...
    /// Allow replaying a lock generated by a different Peregrine version.
    pub allow_peregrine_version_mismatch: Option<bool>,
...
pub use peregrine_types::protocol::RealtimeConversationVersion as RealtimeWsVersion;
pub use peregrine_types::protocol::RealtimeVoice;
