...
use peregrine_sui_move_model::{MoveModule, MoveStructSignature};
use serde::Serialize;
...
use crate::{
    ...
    facts::{BytecodeFunctionFact, BytecodePackageFacts, load_bytecode_package_facts},
};
...
mod tests {
    ...
    fn detects_objects_from_bytecode_when_source_is_unavailable() {
        ...
        let build_root = Path::new(env!("CARGO_MANIFEST_DIR")).join(
            "../peregrine-sui-indexer/tests/fixtures/sui/struct_abilities/build/struct_abilities",
        );
        let input = ScanInput {
            package_model: &package,
            package_root: None,
