...
//!
//! - read CA material from `PEREGRINE_CA_CERTIFICATE`, falling back to `SSL_CERT_FILE`
...
//!   because Peregrine itself commonly runs spawned test processes under seatbelt, so this is not just
...
pub const PEREGRINE_CA_CERT_ENV: &str = "PEREGRINE_CA_CERTIFICATE";
pub const SSL_CERT_FILE_ENV: &str = "SSL_CERT_FILE";
const CA_CERT_HINT: &str = "If you set PEREGRINE_CA_CERTIFICATE or SSL_CERT_FILE, ensure it points to a PEM file containing one or more CERTIFICATE blocks, or unset it to use system roots.";
...
/// Builds a reqwest client that honors Peregrine custom CA environment variables.
///
/// Callers supply the baseline builder configuration they need, and this helper layers in custom
/// CA handling before finally constructing the client. `PEREGRINE_CA_CERTIFICATE` takes precedence
...
/// easy mistake to make when adding a new outbound Peregrine HTTP path, and the resulting bug only
...
/// Builds a rustls client config when a Peregrine custom CA bundle is configured.
///
/// This is the websocket-facing sibling of [`build_reqwest_client_with_custom_ca`]. When
/// `PEREGRINE_CA_CERTIFICATE` or `SSL_CERT_FILE` selects a CA bundle, the returned config starts from
...
fn maybe_build_rustls_client_config_with_env(
    ...
    // they would get from tungstenite's default rustls connector, then layer in the Peregrine custom
...
fn build_reqwest_client_with_env(
    ...
    info!(
        peregrine_ca_certificate_configured = false,
...
trait EnvSource {
    ...
    ///
    /// `PEREGRINE_CA_CERTIFICATE` wins over `SSL_CERT_FILE` because it is the Peregrine-specific override.
    ...
    fn configured_ca_bundle(&self) -> Option<ConfiguredCaBundle> {
        self.non_empty_path(PEREGRINE_CA_CERT_ENV)
            .map(|path| ConfiguredCaBundle {
                source_env: PEREGRINE_CA_CERT_ENV,
                path,
            })
            .or_else(|| {
...
impl ConfiguredCaBundle {
    ...
    /// Loads every certificate block from a PEM file intended for Peregrine CA overrides.
    ///
    /// This accepts a few common real-world variants so Peregrine behaves like other CA-aware tooling:
...
impl NormalizedPem {
    ...
    ///
    /// Peregrine only needs certificate DER bytes to seed `reqwest`'s root store, but operators may
...
mod tests {
    ...
    use super::EnvSource;
    use super::PEREGRINE_CA_CERT_ENV;
    ...
    #[test]
    fn ca_path_prefers_peregrine_env() {
        let env = map_env(&[
            (PEREGRINE_CA_CERT_ENV, "/tmp/codex.pem"),
    ...
    fn rustls_config_uses_custom_ca_bundle_when_configured() {
        ...
        let env = map_env(&[(PEREGRINE_CA_CERT_ENV, cert_path.to_string_lossy().as_ref())]);
