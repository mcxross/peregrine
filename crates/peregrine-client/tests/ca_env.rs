...
//! detailed explanation of what "hermetic" means here lives in `peregrine_client::custom_ca`; these
...
const PEREGRINE_CA_CERT_ENV: &str = "PEREGRINE_CA_CERTIFICATE";
...
fn probe_command() -> Command {
    ...
    cmd.env_remove(PEREGRINE_CA_CERT_ENV);
...
#[test]
fn uses_peregrine_ca_cert_env() {
    let temp_dir = TempDir::new().expect("tempdir");
    let cert_path = write_cert_file(&temp_dir, "ca.pem", TEST_CERT_1);

    let output = run_probe(&[(PEREGRINE_CA_CERT_ENV, cert_path.as_path())]);
...
#[test]
fn prefers_peregrine_ca_cert_over_ssl_cert_file() {
    let temp_dir = TempDir::new().expect("tempdir");
    let cert_path = write_cert_file(&temp_dir, "ca.pem", TEST_CERT_1);
    let bad_path = write_cert_file(&temp_dir, "bad.pem", "");

    let output = run_probe(&[
        (PEREGRINE_CA_CERT_ENV, cert_path.as_path()),
...
fn rejects_empty_pem_file_with_hint() {
    ...
    assert!(stderr.contains("PEREGRINE_CA_CERTIFICATE"));
