...
use chrono::Utc;
use http::HeaderMap;
use peregrine_types::auth::PlanType;
use peregrine_types::error::PeregrineErr;
use peregrine_types::error::RetryLimitReachedError;
use peregrine_types::error::UnexpectedResponseError;
use peregrine_types::error::UsageLimitReachedError;
use serde::Deserialize;
use serde_json::Value;

pub fn map_api_error(err: ApiError) -> PeregrineErr {
    match err {
        ApiError::ContextWindowExceeded => PeregrineErr::ContextWindowExceeded,
        ApiError::QuotaExceeded => PeregrineErr::QuotaExceeded,
        ApiError::UsageNotIncluded => PeregrineErr::UsageNotIncluded,
        ApiError::Retryable { message, delay } => PeregrineErr::Stream(message, delay),
        ApiError::Stream(msg) => PeregrineErr::Stream(msg, None),
        ApiError::ServerOverloaded => PeregrineErr::ServerOverloaded,
        ApiError::Api { status, message } => {
            PeregrineErr::UnexpectedStatus(UnexpectedResponseError {
                status,
                body: message,
                url: None,
                cf_ray: None,
        ...
        ApiError::Transport(transport) => match transport {
            TransportError::Http {
                ...
                {
                    return PeregrineErr::ServerOverloaded;
                }
                ...
                if status == http::StatusCode::BAD_REQUEST {
                            ...
                            == Some(CYBER_POLICY_ERROR_CODE)
                    {
                        ...
                        PeregrineErr::CyberPolicy { message }
                    } else if body_text
                        .contains("The image data you provided does not represent a valid image")
                    {
                        PeregrineErr::InvalidImageRequest()
                    } else {
                        PeregrineErr::InvalidRequest(body_text)
                    }
                } else if status == http::StatusCode::INTERNAL_SERVER_ERROR {
                    PeregrineErr::InternalServerError
                    ...
                    if let Ok(err) = serde_json::from_str::<UsageErrorResponse>(&body_text) {
                        if err.error.error_type.as_deref() == Some("usage_limit_reached") {
                            ...
                            return PeregrineErr::UsageLimitReached(UsageLimitReachedError {
                            ...
                            return PeregrineErr::UsageNotIncluded;
                        }
                    }

                    PeregrineErr::RetryLimit(RetryLimitReachedError {
                        status,
            ...
            TransportError::RetryLimit => PeregrineErr::RetryLimit(RetryLimitReachedError {
                status: http::StatusCode::INTERNAL_SERVER_ERROR,
                request_id: None,
            }),
            TransportError::Timeout => PeregrineErr::RequestTimeout,
            TransportError::Network(msg) | TransportError::Build(msg) => {
                PeregrineErr::Stream(msg, None)
            }
        },
        ApiError::RateLimit(msg) => PeregrineErr::Stream(msg, None),
    }
}

const ACTIVE_LIMIT_HEADER: &str = "x-peregrine-active-limit";
