...
pub(crate) fn subagent_header(source: &Option<SessionSource>) -> Option<String> {
    ...
    match sub {
        peregrine_types::protocol::SubAgentSource::Review => Some("review".to_string()),
        peregrine_types::protocol::SubAgentSource::Compact => Some("compact".to_string()),
        peregrine_types::protocol::SubAgentSource::MemoryConsolidation => {
            Some("memory_consolidation".to_string())
        }
        peregrine_types::protocol::SubAgentSource::ThreadSpawn { .. } => {
            Some("collab_spawn".to_string())
        }
        peregrine_types::protocol::SubAgentSource::Other(label) => Some(label.clone()),
    }
}
