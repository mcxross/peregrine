...
const X_PEREGRINE_TURN_STATE_HEADER: &str = "x-peregrine-turn-state";
...
async fn connect_websocket(
    ...
    // If a Peregrine-specific CA bundle is configured, build an explicit rustls connector so this
    ...
    if let Some(turn_state) = turn_state
        && let Some(header_value) = response
            .headers()
            .get(X_PEREGRINE_TURN_STATE_HEADER)
            .and_then(|value| value.to_str().ok())
    {
...
mod tests {
    ...
    fn parse_wrapped_websocket_error_event_maps_to_transport_http() {
        ...
        assert_eq!(
            headers
                .get("x-peregrine-primary-used-percent")
                .and_then(|value| value.to_str().ok()),
            Some("100.0")
        );
        assert_eq!(
            headers
                .get("x-peregrine-primary-window-minutes")
                .and_then(|value| value.to_str().ok()),
            Some("15")
        );
