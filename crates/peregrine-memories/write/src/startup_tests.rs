...
async fn memories_startup_phase2_tracks_workspace_diff_across_runs() -> anyhow::Result<()> {
    ...
    let test = build_test_peregrine(&server, home.clone()).await?;
    trigger_memories_startup(&test).await;
...
-> anyhow::Result<()> {
    ...
    let context = crate::runtime::MemoryStartupContext::new(
        ...
        Arc::clone(&test.peregrine),
        &test.config,
        config_snapshot.session_source.clone(),
    );
    let request_context = context
        .stage_one_request_context(&test.config, test.config.model.as_deref())
        .await;
    assert_eq!(
...
async fn trigger_memories_startup(test: &TestPeregrine) {
    let config_snapshot = test.peregrine.config_snapshot().await;
...
async fn seed_stage1_output(
    ...
    let mut metadata_builder = codex_state::ThreadMetadataBuilder::new(
        thread_id,
        peregrine_home.join(format!("rollout-{thread_id}.jsonl")),
        updated_at,
        SessionSource::Cli,
    );
    metadata_builder.cwd = peregrine_home.join(format!("workspace-{rollout_slug}"));
...
) -> anyhow::Result<peregrine_core::ThreadConfigSnapshot> {
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        let config_snapshot = test.peregrine.config_snapshot().await;
...
async fn shutdown_test_peregrine(test: &TestPeregrine) -> anyhow::Result<()> {
