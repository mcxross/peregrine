...
pub async fn run(context: Arc<MemoryStartupContext>, config: Arc<Config>) {
    ...
    let root = memory_root(&config.peregrine_home);
...
mod agent {
    ...
    pub(super) fn get_config(config: &Config) -> Option<Config> {
        ...
        if let Some(consolidation_model) = config.memories.consolidation_model.clone() {
            agent_config.model = Some(consolidation_model);
        }
    ...
    async fn loop_agent(
        ...
        token: String,
        thread_id: ThreadId,
        thread: &peregrine_core::PeregrineThread,
    ) -> AgentStatus {
