...
async fn build_request_context(
    ...
    context
        .stage_one_request_context(config, config.memories.extract_model.as_deref())
        .await
}
...
mod job {
    ...
    pub(super) fn serialize_filtered_rollout_response_items(
        items: &[RolloutItem],
    ) -> peregrine_types::error::Result<String> {
        let filtered = items
            .iter()
            .filter_map(|item| {
        ...
        let serialized = serde_json::to_string(&filtered).map_err(|err| {
            PeregrineErr::InvalidRequest(format!("failed to serialize rollout memory: {err}"))
        })?;
        ...
        let serialized =
            job::serialize_filtered_rollout_response_items(&[RolloutItem::ResponseItem(
                ResponseItem::FunctionCallOutput {
                    ...
                    output: peregrine_types::models::FunctionCallOutputPayload {
                        body: peregrine_types::models::FunctionCallOutputBody::Text(
