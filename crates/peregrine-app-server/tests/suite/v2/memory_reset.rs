...
async fn memory_reset_clears_memory_files_and_rows_preserves_threads() -> Result<()> {
    ...
    create_config_toml(peregrine_home.path())?;
    ...
    let thread_id = seed_stage1_output(&state_db, peregrine_home.path()).await?;
...
async fn seed_stage1_output(
    ...
    let mut builder = ThreadMetadataBuilder::new(
        thread_id,
        peregrine_home.join("sessions").join("test.jsonl"),
        now,
        SessionSource::Cli,
    );
...
fn create_config_toml(peregrine_home: &Path) -> std::io::Result<()> {
