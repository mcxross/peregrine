...
async fn marketplace_add_local_directory_source() -> Result<()> {
    ...
    let source = peregrine_home.path().join("marketplace");
    std::fs::create_dir_all(source.join(".agents/plugins"))?;
    std::fs::create_dir_all(source.join("plugins/sample/.peregrine-plugin"))?;
    std::fs::write(
        source.join(".agents/plugins/marketplace.json"),
        r#"{"name":"debug","plugins":[]}"#,
    )?;
    std::fs::write(
        source.join("plugins/sample/.peregrine-plugin/plugin.json"),
        r#"{"name":"sample"}"#,
    )?;
