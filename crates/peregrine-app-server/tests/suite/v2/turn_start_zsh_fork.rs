...
use peregrine_app_server_protocol::CommandAction;
...
async fn turn_start_shell_zsh_fork_executes_command_v2() -> Result<()> {
    ...
    let mut mcp = create_zsh_test_mcp_process(&peregrine_home, &workspace, &zsh_path).await?;
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            model: Some("mock-model".to_string()),
            effort: Some(peregrine_types::openai_models::ReasoningEffort::Medium),
            summary: Some(peregrine_types::config_types::ReasoningSummary::Auto),
            ..Default::default()
        })
        .await?;
    ...
    assert!(
        command.starts_with(
            &command_packaged_zsh_path(&peregrine_home)
                .display()
                .to_string()
        )
    );
...
async fn turn_start_shell_zsh_fork_subcommand_decline_marks_parent_declined_v2() -> Result<()> {
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            cwd: Some(workspace.clone()),
            approval_policy: Some(peregrine_app_server_protocol::AskForApproval::UnlessTrusted),
            sandbox_policy: Some(
...
async fn create_zsh_test_mcp_process(
    ...
    let app_server = create_test_package_app_server(peregrine_home, zsh_path)?;
...
fn create_test_package_app_server(peregrine_home: &Path, zsh_path: &Path) -> Result<PathBuf> {
    let package_dir = peregrine_home.join("test-package");
    let bin_dir = package_dir.join("bin");
    let package_zsh_path = packaged_zsh_path(peregrine_home);
    ...
    std::fs::write(package_dir.join("peregrine-package.json"), "{}")?;

    let app_server = bin_dir.join("peregrine-app-server");
    copy_with_permissions(
        &codex_utils_cargo_bin::cargo_bin("peregrine-app-server")?,
        &app_server,
    )?;
...
fn packaged_zsh_path(peregrine_home: &Path) -> PathBuf {
    peregrine_home
        .join("test-package")
        .join("peregrine-resources")
        .join("zsh")
        .join("bin")
        .join("zsh")
}

fn command_packaged_zsh_path(peregrine_home: &Path) -> PathBuf {
    let path = packaged_zsh_path(peregrine_home);
...
fn find_test_zsh_path() -> Result<Option<std::path::PathBuf>> {
    ...
    let dotslash_zsh = repo_root.join("peregrine-rs/app-server/tests/suite/zsh");
