...
use peregrine_app_server::INPUT_TOO_LARGE_ERROR_CODE;
use peregrine_app_server::INVALID_PARAMS_ERROR_CODE;
use peregrine_app_server_protocol::AdditionalContextEntry;
use peregrine_app_server_protocol::AdditionalContextKind;
use peregrine_app_server_protocol::ByteRange;
use peregrine_app_server_protocol::ClientInfo;
use peregrine_app_server_protocol::CollabAgentStatus;
...
use peregrine_app_server_protocol::FileChangePatchUpdatedNotification;
...
use peregrine_app_server_protocol::TextElement;
...
use peregrine_core::personality_migration::PERSONALITY_MIGRATION_FILENAME;
...
const TEST_ORIGINATOR: &str = "peregrine_vscode";
...
async fn run_local_image_turn(detail: Option<ImageDetail>) -> Result<Vec<Value>> {
    // Two Peregrine turns hit the mock model (session start + turn/start).
    let responses = vec![
    ...
    let image_path = peregrine_home.path().join("image.png");
...
async fn turn_start_sends_originator_header() -> Result<()> {
    ...
    timeout(
        ...
        mcp.initialize_with_client_info(ClientInfo {
            ...
            title: Some("Peregrine VS Code Extension".to_string()),
...
async fn turn_start_emits_thread_scoped_warning_notification_for_trimmed_skills() -> Result<()> {
    ...
    write_models_cache(peregrine_home.path())?;
    let cache_path = peregrine_home.path().join("models_cache.json");
    ...
    write_test_skill(peregrine_home.path(), "alpha-skill")?;
    write_test_skill(peregrine_home.path(), "beta-skill")?;
...
async fn turn_start_tracks_turn_event_analytics() -> Result<()> {
    ...
    let event =
        wait_for_analytics_event(&server, DEFAULT_READ_TIMEOUT, "peregrine_turn_event").await?;
...
async fn turn_start_rejects_invalid_permission_selection_before_starting_turn() -> Result<()> {
    ...
    std::fs::write(
        peregrine_home.path().join("managed_config.toml"),
    ...
    let turn_req = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            environments: Some(vec![TurnEnvironmentParams {
                ...
                cwd: peregrine_home.path().to_path_buf().try_into()?,
            }]),
            ..Default::default()
        })
        .await?;
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    // Three Peregrine turns hit the mock model (session start + two turn/start calls).
    let responses = vec![
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
    ...
    assert_eq!(
        started.turn.status,
        peregrine_app_server_protocol::TurnStatus::InProgress
    );
    assert_eq!(started.turn.id, turn.id);
...
async fn turn_start_accepts_personality_override_v2() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(ThreadStartParams {
            model: Some("exp-peregrine-personality".to_string()),
            ..Default::default()
        })
        .await?;
    let thread_resp: JSONRPCResponse = timeout(
...
async fn turn_start_uses_migrated_pragmatic_personality_without_override_v2() -> Result<()> {
    ...
    let persisted_toml: ConfigToml = toml::from_str(&std::fs::read_to_string(
        peregrine_home.path().join("config.toml"),
    )?)?;
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let peregrine_home = tmp.path().to_path_buf();
    ...
    let mut mcp = McpProcess::new(peregrine_home.as_path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let second_turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(peregrine_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(peregrine_app_server_protocol::SandboxPolicy::DangerFullAccess),
            model: Some("mock-model".to_string()),
...
async fn turn_start_updates_sandbox_and_cwd_between_turns_v2() -> Result<()> {
    ...
    let mut mcp = McpProcess::new(&peregrine_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let first_turn = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            sandbox_policy: Some(
                peregrine_app_server_protocol::SandboxPolicy::WorkspaceWrite {
...
async fn turn_start_permission_profile_rebinds_runtime_workspace_roots_between_turns() -> Result<()>
{
    ...
    std::fs::write(
        peregrine_home.join("config.toml"),
        format!(
            r#"
model = "mock-model"
...
async fn turn_start_resolves_sticky_thread_local_environment_and_turn_overrides() -> Result<()> {
    ...
    std::fs::write(
        peregrine_home.join("environments.toml"),
        r#"
[[environments]]
id = "remote"
url = "ws://127.0.0.1:1"
...
"#;
    ...
    pretty_assertions::assert_eq!(
        started_changes,
        vec![peregrine_app_server_protocol::FileUpdateChange {
...
async fn turn_start_streams_apply_patch_change_updates_v2() -> Result<()> {
    ...
    write_models_cache(&peregrine_home)?;
    let cache_path = peregrine_home.join("models_cache.json");
    let mut cache: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(&cache_path)?)?;
    let models = cache["models"]
        .as_array_mut()
...
async fn turn_start_emits_spawn_agent_item_with_effective_role_model_metadata_v2() -> Result<()> {
    ...
    std::fs::write(
        peregrine_home.path().join("custom-role.toml"),
...
fn write_test_skill(peregrine_home: &Path, name: &str) -> std::io::Result<()> {
    let skill_dir = peregrine_home.join("skills").join(name);
