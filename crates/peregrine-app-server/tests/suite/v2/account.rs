...
use peregrine_app_server_protocol::ChatgptAuthTokensRefreshReason;
use peregrine_app_server_protocol::ChatgptAuthTokensRefreshResponse;
...
const LOGIN_ISSUER_ENV_VAR: &str = "PEREGRINE_APP_SERVER_LOGIN_ISSUER";
...
async fn logout_account_removes_auth_and_notifies() -> Result<()> {
    ...
    create_config_toml(peregrine_home.path(), CreateConfigTomlParams::default())?;
    ...
    assert!(peregrine_home.path().join("auth.json").exists());

    let mut mcp =
        McpProcess::new_with_env(peregrine_home.path(), &[("OPENAI_API_KEY", None)]).await?;
    ...
    assert!(
        !peregrine_home.path().join("auth.json").exists(),
...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(peregrine_app_server_protocol::ThreadStartParams {
    ...
    .await??;
    let thread = to_response::<peregrine_app_server_protocol::ThreadStartResponse>(thread_resp)?;

    let turn_req = mcp
        .send_turn_start_request(peregrine_app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            client_user_message_id: None,
            input: vec![peregrine_app_server_protocol::UserInput::Text {
