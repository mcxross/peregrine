...
use peregrine_app_server_protocol::PluginAuthPolicy;
...
const TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS: &str =
    "PEREGRINE_TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS";
const ALTERNATE_MARKETPLACE_RELATIVE_PATH: &str = ".claude-plugin/marketplace.json";
const ALTERNATE_PLUGIN_MANIFEST_RELATIVE_PATH: &str = ".claude-plugin/plugin.json";

fn write_plugins_enabled_config(peregrine_home: &std::path::Path) -> std::io::Result<()> {
    std::fs::write(
...
async fn plugin_list_skips_invalid_marketplace_file_and_reports_error() -> Result<()> {
    ...
    let home = peregrine_home.path().to_string_lossy().into_owned();
...
async fn plugin_installed_includes_installed_plugins_and_explicit_install_suggestions() -> Result<()>
{
    ...
    write_installed_plugin(&peregrine_home, "openai-curated", "linear")?;
    std::fs::write(
    ...
    assert_eq!(
        ...
        vec![PluginMarketplaceEntry {
            ...
            plugins: vec![PluginSummary {
                ...
                availability: peregrine_app_server_protocol::PluginAvailability::Available,
                interface: None,
...
#[tokio::test]
async fn plugin_list_returns_empty_when_workspace_peregrine_plugins_disabled() -> Result<()> {
    let peregrine_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
...
#[tokio::test]
async fn plugin_list_reuses_cached_workspace_peregrine_plugins_setting() -> Result<()> {
    let peregrine_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
        ...
        vec![PluginMarketplaceEntry {
            ...
            plugins: vec![
                PluginSummary {
                    ...
                    auth_policy: PluginAuthPolicy::OnInstall,
                    availability: peregrine_app_server_protocol::PluginAvailability::Available,
                    interface: Some(peregrine_app_server_protocol::PluginInterface {
...
async fn plugin_list_accepts_omitted_cwds() -> Result<()> {
    let peregrine_home = TempDir::new()?;
    std::fs::create_dir_all(peregrine_home.path().join(".agents/plugins"))?;
    write_plugins_enabled_config(peregrine_home.path())?;
    std::fs::write(
        peregrine_home
            .path()
...
async fn plugin_list_includes_install_and_enabled_state_from_config() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    write_installed_plugin(&peregrine_home, "peregrine-curated", "enabled-plugin")?;
    write_installed_plugin(&peregrine_home, "peregrine-curated", "disabled-plugin")?;
    std::fs::write(
        repo_root.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "peregrine-curated",
...
[plugins."enabled-plugin@peregrine-curated"]
enabled = true

[plugins."disabled-plugin@peregrine-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(marketplace.name, "peregrine-curated");
    assert_eq!(
        marketplace
            .interface
            .as_ref()
    ...
    assert_eq!(
        ...
        "enabled-plugin@peregrine-curated"
    );
    ...
    assert_eq!(
        ...
        "disabled-plugin@peregrine-curated"
    );
    ...
    assert_eq!(
        ...
        "uninstalled-plugin@peregrine-curated"
    );
...
async fn plugin_list_uses_home_config_for_enabled_state() -> Result<()> {
    let peregrine_home = TempDir::new()?;
    std::fs::create_dir_all(peregrine_home.path().join(".agents/plugins"))?;
    write_installed_plugin(&peregrine_home, "peregrine-curated", "shared-plugin")?;
    std::fs::write(
        peregrine_home
            .path()
            .join(".agents/plugins/marketplace.json"),
...
[plugins."shared-plugin@peregrine-curated"]
enabled = true
"#,
    )?;
  ...
  "plugins": [
    ...
    std::fs::create_dir_all(workspace_enabled.path().join(".peregrine"))?;
    std::fs::write(
        workspace_enabled.path().join(".peregrine/config.toml"),
        r#"[plugins."shared-plugin@peregrine-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(shared_plugin.id, "shared-plugin@peregrine-curated");
...
async fn plugin_list_returns_plugin_interface_with_absolute_asset_paths() -> Result<()> {
  ...
  "interface": {
    ...
    assert_eq!(plugin.id, "demo-plugin@peregrine-curated");
...
async fn app_server_startup_remote_plugin_sync_runs_once() -> Result<()> {
    ...
    write_openai_curated_marketplace(peregrine_home.path(), &["linear"])?;
  ...
  {"id":"1","name":"linear","marketplace_name":"openai-curated","version":"1.0.0","enabled":true}
    ...
    let marker_path = peregrine_home
        .path()
        .join(STARTUP_REMOTE_PLUGIN_SYNC_MARKER_FILE);

    {
        let mut mcp = McpProcess::new_with_plugin_startup_tasks(peregrine_home.path()).await?;
        timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn plugin_list_sync_upgrades_and_removes_remote_installed_plugin_bundles() -> Result<()> {
    ...
    let old_path = peregrine_home
        .path()
        .join("plugins/cache/openai-curated-remote/linear/1.0.0");
    let new_path = peregrine_home
        .path()
        .join("plugins/cache/openai-curated-remote/linear/1.2.3");
    let stale_path = peregrine_home
        .path()
...
async fn plugin_list_includes_remote_marketplaces_when_remote_plugin_enabled() -> Result<()> {
  ...
  "pagination": {
    ...
    assert_eq!(
        ...
        peregrine_app_server_protocol::PluginAvailability::Available
    );
    assert_eq!(
...
async fn plugin_list_marks_remote_plugin_disabled_by_admin() -> Result<()> {
  ...
  "pagination": {
    ...
    assert_eq!(
        plugin.availability,
        peregrine_app_server_protocol::PluginAvailability::DisabledByAdmin
    );
    Ok(())
}
...
async fn plugin_list_fetches_featured_plugin_ids_without_chatgpt_auth() -> Result<()> {
    ...
    write_openai_curated_marketplace(peregrine_home.path(), &["linear", "gmail"])?;
...
fn write_installed_plugin(
    ...
    write_installed_plugin_with_version(peregrine_home, marketplace_name, plugin_name, "local")
}
...
fn write_openai_curated_marketplace(
    ...
    let curated_root = peregrine_home.join(".tmp/plugins");
...
{plugins}
    ...
    std::fs::create_dir_all(peregrine_home.join(".tmp"))?;
    std::fs::write(
        peregrine_home.join(".tmp/plugins.sha"),
...
fn write_plugin_share_local_path_mapping(
    ...
    std::fs::create_dir_all(peregrine_home.join(".tmp"))?;
    std::fs::write(
        peregrine_home.join(".tmp/plugin-share-local-paths-v1.json"),
        format!("{contents}\n"),
    )
}
