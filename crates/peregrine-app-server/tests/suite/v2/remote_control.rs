...
use peregrine_app_server_protocol::RemoteControlDisableResponse;
...
async fn remote_control_enable_returns_connecting_status() -> Result<()> {
    let peregrine_home = TempDir::new()?;
    let _backend = BlockingRemoteControlBackend::start(peregrine_home.path()).await?;
    let mut mcp = McpProcess::new(peregrine_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn remote_control_status_read_returns_connecting_status_after_enable() -> Result<()> {
    let peregrine_home = TempDir::new()?;
    let mut backend = BlockingRemoteControlBackend::start(peregrine_home.path()).await?;
    let mut mcp = McpProcess::new(peregrine_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
impl BlockingRemoteControlBackend {
    async fn start(peregrine_home: &std::path::Path) -> Result<Self> {
