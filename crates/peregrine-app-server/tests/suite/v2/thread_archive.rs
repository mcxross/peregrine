...
use peregrine_core::find_archived_thread_path_by_id_str;
use peregrine_core::find_thread_path_by_id_str;
...
async fn thread_archive_requires_materialized_rollout() -> Result<()> {
    ...
    // Verify file moved.
    let archived_directory = peregrine_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
...
async fn thread_archive_succeeds_when_descendant_archive_fails() -> Result<()> {
    ...
    let child_rollout_path =
        find_thread_path_by_id_str(peregrine_home.path(), &child_id, /*state_db_ctx*/ None)
            .await?
            .expect("child rollout path");
    let archived_child_path = peregrine_home
        .path()
