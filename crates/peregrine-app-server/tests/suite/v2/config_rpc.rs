...
use peregrine_app_server_protocol::ForcedChatgptWorkspaceIds;
...
use peregrine_app_server_protocol::ToolsV2;
...
use peregrine_types::config_types::WebSearchLocation;
...
fn write_config(peregrine_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
...
"#,
    ...
    let peregrine_home_path = peregrine_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(peregrine_home_path.join("config.toml"))?;
...
async fn config_read_includes_project_layers_for_cwd() -> Result<()> {
    let peregrine_home = TempDir::new()?;
    write_config(&peregrine_home, r#"model = "gpt-user""#)?;
...
"#,
    ...
    assert_eq!(
        ...
        ConfigLayerSource::Project {
            dot_peregrine_folder: project_config
        }
    );
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let peregrine_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
"#,
    ...
    let expected_file_path =
        AbsolutePathBuf::resolve_path_against_base("config.toml", peregrine_home);
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let peregrine_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(peregrine_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
fn assert_layers_user_then_optional_system(
    layers: &[peregrine_app_server_protocol::ConfigLayer],
