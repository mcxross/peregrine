...
use peregrine_app_server_protocol::HookEventName;
    ...
    let request_id = mcp
        .send_plugin_read_request(PluginReadParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                peregrine_home.path().join("marketplace.json"),
            )?),
...
"#,
    ...
    write_installed_plugin(&peregrine_home, "openai-curated", "demo-plugin")?;
...
async fn plugin_read_returns_share_context_for_shared_local_plugin() -> Result<()> {
    ...
    write_plugin_share_local_path_mapping(peregrine_home.path(), "plugins_123", &plugin_path)?;
    Mock::given(method("GET"))
...
async fn plugin_read_falls_back_to_local_share_context_without_remote_auth() -> Result<()> {
    let peregrine_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    write_plugins_enabled_config(&peregrine_home)?;
    write_plugin_marketplace(
        repo_root.path(),
        "peregrine-curated",
        "demo-plugin",
...
async fn plugin_read_fails_on_malformed_share_mapping() -> Result<()> {
    ...
    write_plugin_source(repo_root.path(), "demo-plugin", &[])?;
    std::fs::create_dir_all(peregrine_home.path().join(".tmp"))?;
    std::fs::write(
        peregrine_home
            .path()
...
[plugins."demo-plugin@peregrine-curated"]
enabled = true

[hooks.state."demo-plugin@peregrine-curated:hooks/hooks.json:pre_tool_use:0:0"]
enabled = false
"#,
    )?;
    write_installed_plugin(&peregrine_home, "peregrine-curated", "demo-plugin")?;
    ...
    assert_eq!(response.plugin.marketplace_name, "peregrine-curated");
    assert_eq!(response.plugin.marketplace_path, Some(marketplace_path));
    assert_eq!(response.plugin.summary.id, "demo-plugin@peregrine-curated");
    assert_eq!(response.plugin.summary.name, "demo-plugin");
    assert_eq!(
        response.plugin.description.as_deref(),
    ...
    assert_eq!(
        ...
        vec![
            peregrine_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@peregrine-curated:hooks/hooks.json:pre_tool_use:0:0".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            peregrine_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@peregrine-curated:hooks/hooks.json:pre_tool_use:0:1".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            peregrine_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@peregrine-curated:hooks/hooks.json:session_start:0:0".to_string(),
...
async fn plugin_read_returns_app_needs_auth() -> Result<()> {
    ...
    let peregrine_home = TempDir::new()?;
    write_connectors_config(peregrine_home.path(), &server_url)?;
    write_chatgpt_auth(
        peregrine_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_plugins_enabled_config(peregrine_home: &TempDir) -> Result<()> {
    std::fs::write(
        peregrine_home.path().join("config.toml"),
        r#"[features]
plugins = true
