...
async fn plugin_install_writes_remote_plugin_to_cloud_and_cache() -> Result<()> {
    ...
    .await;
    configure_remote_plugin_test(peregrine_home.path(), &server)?;
    ...
    mount_remote_plugin_install_after_cache_write(
        &server,
        REMOTE_PLUGIN_ID,
        installed_path.join(".peregrine-plugin/plugin.json"),
    )
    .await;
...
#[tokio::test]
async fn plugin_install_rejects_when_workspace_peregrine_plugins_disabled() -> Result<()> {
    let peregrine_home = TempDir::new()?;
    ...
    assert!(
        err.error
            .message
            .contains("Peregrine plugins are disabled for this workspace")
    );
    Ok(())
}
...
async fn plugin_install_returns_invalid_request_for_missing_marketplace_file() -> Result<()> {
    ...
    let request_id = mcp
        .send_plugin_install_request(PluginInstallParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                peregrine_home.path().join("missing-marketplace.json"),
            )?),
            remote_marketplace_name: None,
...
async fn plugin_install_returns_invalid_request_for_disallowed_product_plugin() -> Result<()> {
  ...
  "plugins": [
    ...
    let mut mcp =
        McpProcess::new_with_args(peregrine_home.path(), &["--session-source", "atlas"]).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn plugin_install_tracks_analytics_event() -> Result<()> {
    ...
    let peregrine_home = TempDir::new()?;
    write_analytics_config(peregrine_home.path(), &analytics_server.uri())?;
    write_chatgpt_auth(
        peregrine_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
    ...
    assert_eq!(
        ...
        json!({
            "events": [{
                "event_type": "peregrine_plugin_installed",
                "event_params": {
...
fn write_analytics_config(peregrine_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        peregrine_home.join("config.toml"),
...
fn configure_remote_plugin_test(
    ...
    write_remote_plugin_catalog_config(peregrine_home, &format!("{}/backend-api/", server.uri()))?;
    write_chatgpt_auth(
        peregrine_home,
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
