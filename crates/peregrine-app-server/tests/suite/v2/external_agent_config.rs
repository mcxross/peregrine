...
-> Result<()> {
    ...
    let home_dir = peregrine_home.path().display().to_string();
    let mut mcp =
        McpProcess::new_with_env(peregrine_home.path(), &[("HOME", Some(home_dir.as_str()))])
            .await?;
...
{
    let peregrine_home = TempDir::new()?;
    let marketplace_root = peregrine_home.path().join("marketplace");
  ...
  "plugins": [
    ...
    std::fs::create_dir_all(peregrine_home.path().join(".claude"))?;
    ...
    std::fs::write(
        peregrine_home.path().join(".claude").join("settings.json"),
...
async fn external_agent_config_import_creates_session_rollouts() -> Result<()> {
    ...
    let project_root = peregrine_home.path().join("repo");
    let recent_timestamp = chrono::Utc::now().to_rfc3339_opts(chrono::SecondsFormat::Secs, true);
    let session_dir = peregrine_home.path().join(".claude/projects/repo");
...
-> Result<()> {
    ...
    let project_config_dir = project_root.join(".peregrine");
...
async fn external_agent_config_import_rejects_undetected_session_paths() -> Result<()> {
    ...
    let undetected_session_path = peregrine_home.path().join("outside.jsonl");
    std::fs::create_dir_all(&project_root)?;
    std::fs::create_dir_all(&session_dir)?;
