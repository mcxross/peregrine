...
async fn initialize_uses_client_info_name_as_originator() -> Result<()> {
    ...
    let expected_peregrine_home = AbsolutePathBuf::try_from(peregrine_home.path().canonicalize()?)?;
    ...
    let message = timeout(
        ...
        mcp.initialize_with_client_info(ClientInfo {
            name: "peregrine_vscode".to_string(),
    ...
    let InitializeResponse {
        user_agent,
        peregrine_home: response_peregrine_home,
        platform_family,
        platform_os,
    } = to_response::<InitializeResponse>(response)?;

    assert!(user_agent.starts_with("peregrine_vscode/"));
    assert_eq!(response_peregrine_home, expected_peregrine_home);
...
async fn initialize_probe_does_not_override_originator() -> Result<()> {
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "peregrine_app_server_daemon".to_string(),
            title: Some("Peregrine App Server Daemon".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
    ...
    assert!(user_agent.starts_with("peregrine_cli_rs/"));
    Ok(())
}

#[tokio::test]
async fn initialize_peregrine_backend_does_not_override_originator() -> Result<()> {
    let responses = Vec::new();
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let peregrine_home = TempDir::new()?;
    create_config_toml(peregrine_home.path(), &server.uri(), "never")?;
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "peregrine-backend".to_string(),
            title: Some("Peregrine Backend".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
...
async fn initialize_respects_originator_override_env_var() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_with_env(
        peregrine_home.path(),
        &[(
            "PEREGRINE_INTERNAL_ORIGINATOR_OVERRIDE",
            Some("peregrine_originator_via_env_var"),
        )],
    )
    .await?;
    ...
    assert!(user_agent.starts_with("peregrine_originator_via_env_var/"));
    assert_eq!(response_peregrine_home, expected_peregrine_home);
    assert_eq!(platform_family, std::env::consts::FAMILY);
    assert_eq!(platform_os, std::env::consts::OS);
    Ok(())
...
async fn initialize_rejects_invalid_client_name() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_with_env(
        peregrine_home.path(),
        &[("PEREGRINE_INTERNAL_ORIGINATOR_OVERRIDE", None)],
    )
    .await?;
...
async fn turn_start_notify_payload_includes_initialize_client_name() -> Result<()> {
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let peregrine_home = TempDir::new()?;
    let notify_file = peregrine_home.path().join("notify.json");
    let notify_capture = cargo_bin("peregrine-app-server-test-notify-capture")?;
...
fn create_config_toml(
    ...
    create_config_toml_with_extra(peregrine_home, server_uri, approval_policy, "")
}
