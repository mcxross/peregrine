...
use peregrine_config::MarketplaceConfigUpdate;
use peregrine_config::record_user_marketplace;
...
fn init_marketplace_repo(root: &Path, marketplace_name: &str, marker: &str) -> Result<String> {
    ...
    run_git(root, &["config", "user.name", "Peregrine Tests"])?;
...
fn disable_plugin_startup_tasks(peregrine_home: &Path) -> Result<()> {
...
fn marketplace_install_root(peregrine_home: &Path) -> std::path::PathBuf {
    peregrine_home.join(INSTALLED_MARKETPLACES_DIR)
}
...
fn expected_installed_root(
    ...
    AbsolutePathBuf::try_from(
        marketplace_install_root(&peregrine_home.canonicalize()?).join(marketplace_name),
    )
...
async fn marketplace_upgrade_all_configured_git_marketplaces() -> Result<()> {
    ...
    disable_plugin_startup_tasks(peregrine_home.path())?;

    let mut mcp = McpProcess::new(peregrine_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;

    let debug_root = expected_installed_root(peregrine_home.path(), "debug")?;
    let tools_root = expected_installed_root(peregrine_home.path(), "tools")?;
...
async fn marketplace_upgrade_named_marketplace_only() -> Result<()> {
    ...
    assert!(
        !marketplace_install_root(peregrine_home.path())
            .join("debug")
            .exists()
    );
    Ok(())
