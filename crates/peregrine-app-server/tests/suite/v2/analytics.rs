...
fn set_metrics_exporter(config: &mut peregrine_core::config::Config) {
...
async fn app_server_default_analytics_disabled_without_flag() -> Result<()> {
    ...
    let provider = peregrine_core::otel_init::build_provider(
        &config,
        SERVICE_VERSION,
        Some("peregrine-app-server"),
...
pub(crate) fn thread_initialized_event(payload: &Value) -> Result<&Value> {
    ...
    events
        .iter()
        .find(|event| event["event_type"] == "peregrine_thread_initialized")
        .ok_or_else(|| anyhow::anyhow!("peregrine_thread_initialized event should be present"))
}
