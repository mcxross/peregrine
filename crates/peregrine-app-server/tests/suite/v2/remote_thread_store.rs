...
//! the temporary `peregrine_home` must not contain rollout session files or sqlite
...
fn thread_start_with_non_local_thread_store_does_not_create_local_persistence() -> Result<()> {
    run_current_thread_test_with_stack(
        ...
        thread_start_with_non_local_thread_store_does_not_create_local_persistence_impl,
    )
}

async fn thread_start_with_non_local_thread_store_does_not_create_local_persistence_impl()
-> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let peregrine_home = TempDir::new()?;
    let store_id = Uuid::new_v4().to_string();
    // Plugin startup warmups may create `.tmp` under peregrine_home. Disable them
    // here so this regression stays focused on thread persistence artifacts.
    create_config_toml_with_thread_store(peregrine_home.path(), &server.uri(), &store_id)?;
    ...
    assert_no_local_persistence_artifacts(peregrine_home.path())?;

    Ok(())
}

fn assert_no_local_persistence_artifacts(peregrine_home: &Path) -> Result<()> {
    ...
    // the isolated test peregrine_home.
    assert!(
        !peregrine_home.join("sessions").exists(),
        "non-local thread persistence should not create local rollout sessions"
    );
    assert!(
        !peregrine_home.join("archived_sessions").exists(),
        "non-local thread persistence should not create archived rollout sessions"
    );
    assert!(
        !codex_state::state_db_path(peregrine_home).exists(),
        "non-local thread persistence should not create local thread sqlite"
    );

    let sqlite_artifacts = std::fs::read_dir(peregrine_home)?
    ...
    let mut entries = peregrine_home_entries(peregrine_home)?;
    // Bazel test runs may initialize shell snapshot storage under peregrine_home.
    ...
    assert_eq!(
        ...
        "non-local thread persistence should not create unexpected files in peregrine_home"
    );

    Ok(())
}

fn peregrine_home_entries(peregrine_home: &Path) -> Result<BTreeSet<String>> {
    Ok(std::fs::read_dir(peregrine_home)?
        .filter_map(|entry| {
            let entry = entry.ok()?;
