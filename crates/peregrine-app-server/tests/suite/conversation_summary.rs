...
fn get_conversation_summary_by_thread_id_reads_pathless_store_thread() -> Result<()> {
    run_current_thread_test_with_stack(
        "get_conversation_summary_by_thread_id_reads_pathless_store_thread",
        || async {
            let peregrine_home = TempDir::new()?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn get_conversation_summary_by_relative_rollout_path_resolves_from_peregrine_home()
-> Result<()> {
    let peregrine_home = TempDir::new()?;
    let conversation_id = create_fake_rollout(
        peregrine_home.path(),
    ...
    let relative_path = rollout_path
        .strip_prefix(peregrine_home.path())?
        .to_path_buf();
