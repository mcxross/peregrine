...
pub fn run_current_thread_test_with_stack<F, Fut>(name: &'static str, test: F) -> anyhow::Result<()>
where
...
pub fn to_response<T: DeserializeOwned>(response: JSONRPCResponse) -> anyhow::Result<T> {
    ...
    let peregrine_response = serde_json::from_value(value)?;
    Ok(peregrine_response)
}
