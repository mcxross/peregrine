...
use peregrine_app_server_protocol::ClientNotification;
...
use peregrine_app_server_protocol::FsCopyParams;
use peregrine_app_server_protocol::FsCreateDirectoryParams;
use peregrine_app_server_protocol::FsGetMetadataParams;
use peregrine_app_server_protocol::FsReadDirectoryParams;
use peregrine_app_server_protocol::FsReadFileParams;
use peregrine_app_server_protocol::FsRemoveParams;
use peregrine_app_server_protocol::FsUnwatchParams;
use peregrine_app_server_protocol::FsWatchParams;
use peregrine_app_server_protocol::FsWriteFileParams;
...
use peregrine_app_server_protocol::InitializeCapabilities;
use peregrine_app_server_protocol::InitializeParams;
use peregrine_app_server_protocol::JSONRPCError;
...
use peregrine_app_server_protocol::JSONRPCRequest;
...
use peregrine_app_server_protocol::ProcessResizePtyParams;
use peregrine_app_server_protocol::ProcessSpawnParams;
use peregrine_app_server_protocol::ProcessWriteStdinParams;
...
use peregrine_app_server_protocol::ThreadRealtimeListVoicesParams;
...
pub const DEFAULT_CLIENT_NAME: &str = "peregrine-app-server-tests";
pub const DISABLE_PLUGIN_STARTUP_TASKS_ARG: &str = "--disable-plugin-startup-tasks-for-tests";
const DISABLE_MANAGED_CONFIG_ENV_VAR: &str = "PEREGRINE_APP_SERVER_DISABLE_MANAGED_CONFIG";

impl McpProcess {
    pub async fn new(peregrine_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env_and_args(peregrine_home, &[], &[DISABLE_PLUGIN_STARTUP_TASKS_ARG]).await
    }

    pub async fn new_without_managed_config(peregrine_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(
            peregrine_home,
    ...
    pub async fn new_without_managed_config_with_env(
        ...
        Self::new_with_env(peregrine_home, &all_env_overrides).await
    }

    pub async fn new_with_plugin_startup_tasks(peregrine_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env_and_args(peregrine_home, &[], &[]).await
    }
    ...
    pub async fn new_with_env_and_plugin_startup_tasks(
        ...
        Self::new_with_env_and_args(peregrine_home, env_overrides, &[]).await
    }

    pub async fn new_with_args(peregrine_home: &Path, args: &[&str]) -> anyhow::Result<Self> {
        let mut all_args = vec![DISABLE_PLUGIN_STARTUP_TASKS_ARG];
        all_args.extend_from_slice(args);
        Self::new_with_env_and_args(peregrine_home, &[], &all_args).await
    }
    ...
    async fn new_with_env_and_args(
        ...
        let program = codex_utils_cargo_bin::cargo_bin("peregrine-app-server")
            .context("should find binary for peregrine-app-server")?;
        Self::new_with_program_env_and_args(peregrine_home, &program, env_overrides, args).await
    }
    ...
    async fn new_with_program_env_and_args(
        ...
        cmd.current_dir(peregrine_home);
        cmd.env("PEREGRINE_HOME", peregrine_home);
        cmd.env("RUST_LOG", "warn");
        // Keep integration tests isolated from host managed configuration.
        cmd.env(
            "PEREGRINE_APP_SERVER_MANAGED_CONFIG_PATH",
            peregrine_home.join("managed_config.toml"),
        );
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: peregrine_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `peregrine-app-server` child process.
        //
