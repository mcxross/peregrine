use super::*;
use peregrine_types::protocol::AdditionalContextEntry as CoreAdditionalContextEntry;
use peregrine_types::protocol::AdditionalContextKind as CoreAdditionalContextKind;
...
struct ThreadSettingsBuildParams {
    ...
    sandbox_policy: Option<peregrine_app_server_protocol::SandboxPolicy>,
...
impl TurnRequestProcessor {
    ...
    fn track_error_response(
        ...
        let _ = (request_id, error, error_type);
    }
    ...
    fn review_request_from_target(
        ...
        let hint = peregrine_core::review_prompts::user_facing_hint(&core_target);
    ...
    async fn turn_start_inner(
        ...
        if turn_has_input {
            ...
            peregrine_memories_write::start_memories_startup_task(
    ...
    async fn build_thread_settings_overrides(
    ...
    ) -> Result<peregrine_types::protocol::ThreadSettingsOverrides, JSONRPCErrorError> {
        ...
        let approval_policy =
            approval_policy.map(peregrine_app_server_protocol::AskForApproval::to_core);
        let approvals_reviewer =
            approvals_reviewer.map(peregrine_app_server_protocol::ApprovalsReviewer::to_core);
        ...
        if has_any_overrides {
            thread
                .preview_thread_settings_overrides(PeregrineThreadSettingsOverrides {
                    cwd: cwd.clone(),
        ...
        Ok(peregrine_types::protocol::ThreadSettingsOverrides {
            cwd,
    ...
    async fn thread_settings_update_inner(
        ...
        if thread_settings != peregrine_types::protocol::ThreadSettingsOverrides::default() {
            self.submit_core_op(
                request_id,
                thread.as_ref(),
                ...
                let (message, data, error_type) = match err {
                    ...
                    SteerInputError::ActiveTurnNotSteerable { turn_kind } => {
                        let (message, turn_steer_error) = match turn_kind {
                            peregrine_types::protocol::NonSteerableTurnKind::Review => (
                                "cannot steer a review turn".to_string(),
                                TurnSteerRequestError::NonSteerableReview,
                            ),
                            peregrine_types::protocol::NonSteerableTurnKind::Compact => (
    ...
    async fn prepare_realtime_conversation_thread(
        ...
        request_id: &ConnectionRequestId,
        thread_id: &str,
    ) -> Result<Option<(ThreadId, Arc<PeregrineThread>)>, JSONRPCErrorError> {
    ...
    async fn start_inline_review(
        &self,
        request_id: &ConnectionRequestId,
        parent_thread: Arc<PeregrineThread>,
