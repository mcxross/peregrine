...
//! the configured Peregrine executable as a subprocess, accepts only valid JSON from
...
pub(crate) struct DoctorFeedbackReport {
    /// JSON support report to upload as `peregrine-doctor-report.json`.
...
///
/// Failure to spawn Peregrine, finish before the timeout, or parse JSON means the
