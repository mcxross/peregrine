...
) -> Vec<peregrine_app_server_protocol::SkillMetadata> {
    skills
        .iter()
        .map(|skill| {
            let enabled = !disabled_paths.contains(&skill.path_to_skills_md);
            peregrine_app_server_protocol::SkillMetadata {
                ...
                dependencies: skill.dependencies.clone().map(|dependencies| {
                    peregrine_app_server_protocol::SkillDependencies {
                        tools: dependencies
                            .tools
                            .into_iter()
                            .map(|tool| peregrine_app_server_protocol::SkillToolDependency {
                                r#type: tool.r#type,
                                value: tool.value,
...
fn errors_to_info(
    errors: &[peregrine_core::skills::SkillError],
) -> Vec<peregrine_app_server_protocol::SkillErrorInfo> {
    errors
        .iter()
        .map(|err| peregrine_app_server_protocol::SkillErrorInfo {
...
impl CatalogRequestProcessor {
    ...
    async fn experimental_feature_list_response(
        ...
        let workspace_peregrine_plugins_enabled = self
            ...
            .map(|spec| {
                ...
                ApiExperimentalFeature {
                    ...
                    enabled: config.features.enabled(spec.id)
                        && (workspace_peregrine_plugins_enabled
                ...
                async move {
                    let (cwd_abs, config_layer_stack) = match self.resolve_cwd_config(&cwd).await {
                        ...
                        Err(message) => {
                            ...
                            return (
                                index,
                                peregrine_app_server_protocol::SkillsListEntry {
                                    cwd,
                                    skills: Vec::new(),
                                    errors: vec![peregrine_app_server_protocol::SkillErrorInfo {
                                        path: error_path,
                                        message,
                                    }],
                                },
                    ...
                    let effective_skill_roots = if workspace_peregrine_plugins_enabled {
                        let plugins_input = config.plugins_config_input();
                        let compat_config_layer_stack =
                            peregrine_config::codex_compat::config_layer_stack(&config_layer_stack);
                        plugins_manager
                            .effective_skill_roots_for_layer_stack(
                                &compat_config_layer_stack,
                                &plugins_input,
                            )
                            .await
                    } else {
                    ...
                    let skills_input = peregrine_core::skills::SkillsLoadInput::new(
                        cwd_abs.clone(),
    ...
    async fn hooks_list_response(
        ...
        for cwd in cwds {
                ...
                .await
            {
                ...
                Err(err) => {
                    let error_path = cwd.clone();
                    data.push(peregrine_app_server_protocol::HooksListEntry {
                        cwd,
                        hooks: Vec::new(),
                        warnings: Vec::new(),
                        errors: vec![peregrine_app_server_protocol::HookErrorInfo {
                            path: error_path,
            ...
            let workspace_peregrine_plugins_enabled = self
                ...
                .await;
            let plugins_enabled =
                config.features.enabled(Feature::Plugins) && workspace_peregrine_plugins_enabled;
            let plugin_outcome = if plugins_enabled {
                let plugins_input = config.plugins_config_input();
                let compat_config_layer_stack =
                    peregrine_config::codex_compat::config_layer_stack(&config.config_layer_stack);
                plugins_manager
                    .plugins_for_layer_stack(&compat_config_layer_stack, &plugins_input)
                    .await
            } else {
    ...
    async fn skills_config_write_response_inner(
        ...
        let edits = vec![edit];
        ConfigEditsBuilder::new(&self.config.peregrine_home)
            .with_edits(edits)
            .apply()
            .await
            .map(|()| {
