    ...
    async fn upload_feedback_response(
        ...
        if include_logs {
            ...
            if let Some(sandbox_log_attachment) =
                windows_sandbox_log_attachment(&self.config.peregrine_home)
...
#[cfg(target_os = "windows")]
fn windows_sandbox_log_attachment(peregrine_home: &Path) -> Option<FeedbackAttachmentPath> {
    let sandbox_log_path =
        codex_windows_sandbox::current_log_file_path_for_peregrine_home(peregrine_home);
    sandbox_log_path
        .is_file()
...
fn windows_sandbox_log_attachment(_peregrine_home: &Path) -> Option<FeedbackAttachmentPath> {
    None
}
...
mod tests {
    ...
    fn windows_sandbox_log_attachment_uses_current_log() {
        let peregrine_home = tempfile::tempdir().expect("create tempdir");
        let sandbox_dir = codex_windows_sandbox::sandbox_dir(peregrine_home.path());
        std::fs::create_dir_all(&sandbox_dir).expect("create sandbox dir");
        let sandbox_log_path =
            codex_windows_sandbox::current_log_file_path_for_peregrine_home(peregrine_home.path());
        std::fs::write(&sandbox_log_path, "sandbox log").expect("write sandbox log");

        let attachment = windows_sandbox_log_attachment(peregrine_home.path())
