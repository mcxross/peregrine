...
use peregrine_app_server_protocol::CommandMigration;
...
use peregrine_app_server_protocol::ExternalAgentConfigImportCompletedNotification;
...
use peregrine_app_server_protocol::ExternalAgentConfigMigrationItemType;
use peregrine_app_server_protocol::HookMigration;
use peregrine_app_server_protocol::JSONRPCErrorError;
use peregrine_app_server_protocol::McpServerMigration;
use peregrine_app_server_protocol::MigrationDetails;
use peregrine_app_server_protocol::PluginsMigration;
...
impl ExternalAgentConfigRequestProcessor {
    pub(crate) fn new(
        ...
        Self {
            outgoing,
            migration_service: ExternalAgentConfigService::new(peregrine_home.clone()),
            peregrine_home,
                ...
                .map(|migration_item| ExternalAgentConfigMigrationItem {
                    ...
                    details: migration_item.details.map(|details| MigrationDetails {
                        ...
                        sessions: details
                            .sessions
                            .into_iter()
                            .map(|session| peregrine_app_server_protocol::SessionMigration {
                                path: session.path,
                                cwd: session.cwd,
                                title: session.title,
                            })
                        ...
                        subagents: details
                            ...
                            .into_iter()
                            .map(
                                |subagent| peregrine_app_server_protocol::SubagentMigration {
                                    name: subagent.name,
                                },
                            )
                            .collect(),
    ...
    async fn import_external_agent_session(
        ...
        if let Some(title) = title
            && let Some(name) = peregrine_core::util::normalize_thread_name(&title)
        {
            imported_thread
                .thread
    ...
    fn prepare_validated_session_imports(
        ...
        prepare_validated_session_imports(&self.peregrine_home, sessions)
    }

    fn record_imported_session(&self, source_path: &std::path::Path, imported_thread_id: ThreadId) {
        if let Err(err) =
            record_imported_session(&self.peregrine_home, source_path, imported_thread_id)
        {
            tracing::warn!(
                error = %err,
