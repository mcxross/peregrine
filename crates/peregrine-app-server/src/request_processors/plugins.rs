...
use peregrine_app_server_protocol::PluginInstallPolicy;
use peregrine_app_server_protocol::PluginSharePrincipalRole;
use peregrine_app_server_protocol::PluginShareTargetRole;
...
fn plugin_skills_to_info(
    skills: &[peregrine_core::skills::SkillMetadata],
    ...
    skills
        ...
        .map(|skill| SkillSummary {
            ...
            interface: skill.interface.clone().map(|interface| {
                peregrine_app_server_protocol::SkillInterface {
...
fn marketplace_install_policy_to_api(
    value: codex_core_plugins::marketplace::MarketplacePluginInstallPolicy,
) -> PluginInstallPolicy {
    let codex_policy: codex_app_server_protocol::PluginInstallPolicy = value.into();
    protocol_convert(codex_policy)
}

fn marketplace_auth_policy_to_api(
    value: codex_core_plugins::marketplace::MarketplacePluginAuthPolicy,
) -> peregrine_app_server_protocol::PluginAuthPolicy {
    let codex_policy: codex_app_server_protocol::PluginAuthPolicy = value.into();
    protocol_convert(codex_policy)
}
...
fn convert_configured_marketplace_plugin_to_plugin_summary(
    ...
    PluginSummary {
        ...
        install_policy: marketplace_install_policy_to_api(plugin.policy.installation),
        auth_policy: marketplace_auth_policy_to_api(plugin.policy.authentication),
...
impl PluginRequestProcessor {
    ...
    async fn workspace_peregrine_plugins_enabled(
        &self,
        config: &Config,
        auth: Option<&CodexAuth>,
    ) -> bool {
        match workspace_settings::peregrine_plugins_enabled_for_workspace(
            config,
            auth,
        ...
        .await
        {
            ...
            Err(err) => {
                warn!(
                    "failed to fetch workspace Peregrine plugins setting; allowing Peregrine plugins: {err:#}"
                );
                true
            }
        }
    ...
    async fn plugin_list_response(
        ...
        if !self
            .workspace_peregrine_plugins_enabled(&config, auth.as_ref())
            .await
        {
            return Ok(empty_response());
        }
            ...
            match tokio::task::spawn_blocking(move || {
                ...
                Ok::<
                    (
                        ...
                        Vec<peregrine_app_server_protocol::MarketplaceLoadErrorInfo>,
                    ),
                    MarketplaceError,
                >((
                    outcome
                    ...
                    outcome
                        ...
                        .into_iter()
                        .map(
                            |err| peregrine_app_server_protocol::MarketplaceLoadErrorInfo {
            ...
            Ok(marketplace_path) => {
                ...
                PluginDetail {
                    ...
                    summary: PluginSummary {
                        ...
                        install_policy: marketplace_install_policy_to_api(
                            outcome.plugin.policy.installation,
                        ),
                        auth_policy: marketplace_auth_policy_to_api(
                            outcome.plugin.policy.authentication,
                        ),
                        availability: PluginAvailability::Available,
                    ...
                    hooks: outcome
                        ...
                        .hooks
                        .into_iter()
                        .map(|hook| peregrine_app_server_protocol::PluginHookSummary {
                            key: hook.key,
    ...
    async fn plugin_install_response(
        ...
        {
            return Err(invalid_request(
                "Peregrine plugins are disabled for this workspace",
            ));
        }
        ...
        Ok(PluginInstallResponse {
            auth_policy: marketplace_auth_policy_to_api(result.auth_policy),
            apps_needing_auth,
        })
    }
    ...
    async fn remote_plugin_install_response(
        ...
        if protocol_convert::<_, PluginAvailability>(remote_detail.summary.availability)
        ...
        if protocol_convert::<_, PluginInstallPolicy>(remote_detail.summary.install_policy)
        ...
        Ok(PluginInstallResponse {
            auth_policy: protocol_convert(remote_detail.summary.auth_policy),
            apps_needing_auth,
        })
    }
    ...
    async fn plugin_apps_needing_auth_for_install(
        ...
        let (accessible_connectors, peregrine_apps_ready) = match accessible_connectors_result {
            Ok(status) => (status.connectors, status.peregrine_apps_ready),
            Err(err) => {
                warn!(
                    plugin = plugin_id,
        ...
        if !peregrine_apps_ready {
            warn!(
                plugin = plugin_id,
                "peregrine_apps MCP not ready after plugin install; skipping appsNeedingAuth check"
            );
        }
    ...
    async fn start_plugin_mcp_oauth_logins(
        ...
        for (name, server) in plugin_mcp_servers {
            ...
            let store_mode = peregrine_core::config::codex_oauth_credentials_store_mode(
    ...
    async fn remote_plugin_uninstall_response(
        ...
        validate_remote_plugin_id(&plugin_id).map_err(|err| invalid_request(format!("{err:?}")))?;
...
async fn load_plugin_app_summaries(
    ...
    let accessible_connectors =
        ...
        .await
        {
            Ok(status) if status.peregrine_apps_ready => status.connectors,
            Ok(_) => {
                return plugin_connectors
                    .into_iter()
                    .map(|connector| app_summary_from_core_connector(connector, false))
                    .collect();
            }
            Err(err) => {
...
fn plugin_apps_needing_auth(
    all_connectors: &[peregrine_core::connectors::AppInfo],
    accessible_connectors: &[peregrine_core::connectors::AppInfo],
    plugin_apps: &[codex_plugin::AppConnectorId],
    peregrine_apps_ready: bool,
) -> Vec<AppSummary> {
    if !peregrine_apps_ready {
        return Vec::new();
    }
    ...
    all_connectors
        ...
        .cloned()
        .map(|connector| app_summary_from_core_connector(connector, true))
        .collect()
}

fn app_summary_from_core_connector(
    connector: peregrine_core::connectors::AppInfo,
    needs_auth: bool,
) -> AppSummary {
    AppSummary {
        id: connector.id,
...
fn remote_plugin_summary_to_info(summary: RemoteCatalogPluginSummary) -> PluginSummary {
    PluginSummary {
        ...
        install_policy: protocol_convert(summary.install_policy),
        auth_policy: protocol_convert(summary.auth_policy),
        availability: protocol_convert(summary.availability),
        interface: summary.interface.map(protocol_convert),
    ...
    PluginDetail {
        ...
        skills: detail
            ...
            .map(|skill| SkillSummary {
                ...
                interface: skill.interface.map(protocol_convert),
                path: None,
