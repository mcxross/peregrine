...
fn collect_resume_override_mismatches(
    ...
    if let Some(requested_review_policy) = request.approvals_reviewer.as_ref() {
        let active_review_policy: peregrine_app_server_protocol::ApprovalsReviewer =
    ...
    if let Some(requested_sandbox) = request.sandbox.as_ref() {
        ...
        let sandbox_matches = matches!(
            ...
            (
                ...
                peregrine_types::protocol::SandboxPolicy::DangerFullAccess
            ) | (
                SandboxMode::DangerFullAccess,
...
impl ThreadRequestProcessor {
    ...
    async fn instruction_sources_from_config(config: &Config) -> Vec<AbsolutePathBuf> {
        peregrine_core::AgentsMdManager::new(config)
    ...
    async fn load_thread(
        &self,
        thread_id: &str,
    ) -> Result<(ThreadId, Arc<PeregrineThread>), JSONRPCErrorError> {
    ...
    fn listener_task_context(&self) -> ListenerTaskContext {
        ListenerTaskContext {
            ...
            peregrine_home: self.config.peregrine_home.to_path_buf(),
    ...
    async fn request_trace_context(
        &self,
        request_id: &ConnectionRequestId,
    ) -> Option<peregrine_types::protocol::W3cTraceContext> {
    ...
    async fn thread_start_task(
        ...
        session_start_source: Option<peregrine_app_server_protocol::ThreadStartSource>,
        thread_source: Option<peregrine_types::protocol::ThreadSource>,
            ...
            && (requested_permissions_trust_project || effective_permissions_trust_project)
        {
            ...
            let cli_overrides_for_reload = if let Err(err) =
                peregrine_core::config::set_project_trust_level(
                    &listener_task_context.peregrine_home,
        ...
        let NewThread {
            ...
            .start_thread_with_options(StartThreadOptions {
                ...
                initial_history: match session_start_source
                    .unwrap_or(peregrine_app_server_protocol::ThreadStartSource::Startup)
                {
                    peregrine_app_server_protocol::ThreadStartSource::Startup => {
                        InitialHistory::New
                    }
                    peregrine_app_server_protocol::ThreadStartSource::Clear => {
            ...
            .map_err(|err| match err {
                PeregrineErr::InvalidRequest(message) => invalid_request(message),
    ...
    fn build_thread_config_overrides(
        ...
        approval_policy: Option<peregrine_app_server_protocol::AskForApproval>,
        approvals_reviewer: Option<peregrine_app_server_protocol::ApprovalsReviewer>,
        ...
        ConfigOverrides {
            ...
            approval_policy: approval_policy
                .map(peregrine_app_server_protocol::AskForApproval::to_core),
            approvals_reviewer: approvals_reviewer
                .map(peregrine_app_server_protocol::ApprovalsReviewer::to_core),
            sandbox_mode: sandbox.map(SandboxMode::to_core),
            peregrine_linux_sandbox_exe: self.arg0_paths.codex_linux_sandbox_exe.clone(),
    ...
    ) -> Result<(ThreadSetNameResponse, Option<ThreadNameUpdatedNotification>), JSONRPCErrorError>
    {
        ...
        let thread_id = ThreadId::from_string(&thread_id)
            .map_err(|err| invalid_request(format!("invalid thread id: {err}")))?;
        let Some(name) = peregrine_core::util::normalize_thread_name(&name) else {
    ...
    async fn memory_reset_response_inner(&self) -> Result<MemoryResetResponse, JSONRPCErrorError> {
        ...
        clear_memory_roots_contents(&self.config.peregrine_home)
            .await
            .map_err(|err| {
                internal_error(format!(
                    "failed to clear memory directories under {}: {err}",
                    self.config.peregrine_home.display()
                ))
            })?;
    ...
    async fn thread_resume_inner(
            ...
            .await
        {
            Ok(NewThread {
                thread_id,
                thread: peregrine_thread,
                session_configured,
                ..
            }) => {
                if let Err(err) = Self::set_app_server_client_info(
                    peregrine_thread.as_ref(),
                    app_server_client_name,
                    app_server_client_version,
                )
                .await
                ...
                thread.thread_source = peregrine_thread
                    .config_snapshot()
                    .await
                    .thread_source
                    .map(Into::into);
                ...
                let config_snapshot = peregrine_thread.config_snapshot().await;
                let sandbox = thread_response_sandbox_policy(
                    &config_snapshot.permission_profile,
                    config_snapshot.cwd.as_path(),
                );
                ...
                self.thread_goal_processor
                    .emit_resume_goal_snapshot_and_continue(thread_id, peregrine_thread.as_ref())
                    .await;
            }
            Err(err) => {
    ...
    async fn thread_fork_inner(
        ...
        let NewThread {
            ...
            .await
            .map_err(|err| match err {
                PeregrineErr::Io(_) | PeregrineErr::Json(_) => {
...
) -> Result<peregrine_app_server_protocol::TurnsPage, JSONRPCErrorError> {
    build_thread_turns_page_response(
        items,
        loaded_status,
        has_live_running_thread,
...
fn core_thread_write_error(operation: &str, err: PeregrineErr) -> JSONRPCErrorError {
    match err {
        PeregrineErr::ThreadNotFound(thread_id) => {
            invalid_request(format!("thread not found: {thread_id}"))
        }
        PeregrineErr::InvalidRequest(message) => invalid_request(message),
        PeregrineErr::UnsupportedOperation(message) => method_not_found(message),
...
fn summary_from_state_db_metadata(
    ...
    cli_version: String,
    source: String,
    _thread_source: Option<peregrine_types::protocol::ThreadSource>,
    ...
    let source = serde_json::from_str(&source)
        ...
        .unwrap_or(peregrine_types::protocol::SessionSource::Unknown);
...
fn preview_from_rollout_items(items: &[RolloutItem]) -> String {
    items
        ...
        .find_map(|item| match item {
            RolloutItem::ResponseItem(item) => match peregrine_core::parse_turn_item(item) {
                Some(peregrine_types::items::TurnItem::UserMessage(user)) => Some(user.message()),
                _ => None,
            },
            _ => None,
        })
...
fn requested_permissions_trust_project(overrides: &ConfigOverrides, cwd: &Path) -> bool {
    if matches!(
        ...
        Some(
            peregrine_types::config_types::SandboxMode::WorkspaceWrite
                | peregrine_types::config_types::SandboxMode::DangerFullAccess
        )
    ) {
        return true;
    }
...
fn permission_profile_trusts_project(
    profile: &peregrine_types::models::PermissionProfile,
    cwd: &Path,
) -> bool {
    match profile {
        peregrine_types::models::PermissionProfile::Disabled
        | peregrine_types::models::PermissionProfile::External { .. } => true,
        peregrine_types::models::PermissionProfile::Managed { .. } => profile
