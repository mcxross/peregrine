...
impl AppsRequestProcessor {
    ...
    async fn apps_list_task(
        ...
        let should_retry = result
            .as_ref()
            .is_ok_and(|(_, peregrine_apps_ready)| !peregrine_apps_ready);
        outgoing
        ...
        if should_retry && !retry_params.force_refetch {
            ...
            .await
            {
                warn!("failed to refresh app list after peregrine-apps readiness retry: {err:?}");
            }
        }
    }
    ...
    async fn apps_list_response(
        ...
        let mut peregrine_apps_ready = true;
        let mut last_notified_apps = None;

        if accessible_connectors.is_some() || all_connectors.is_some() {
            let merged = app_infos_to_api(connectors::with_app_enabled_state(
        ...
        loop {
            ...
            match result {
                AppListLoadResult::Accessible(Ok(status)) => {
                    ...
                    peregrine_apps_ready = status.peregrine_apps_ready;
                }
            ...
            if accessible_loaded && all_loaded {
                ...
                return Ok((response, peregrine_apps_ready));
            }
        }
    }
...
enum AppListLoadResult {
    ...
    Directory(Result<Vec<peregrine_core::connectors::AppInfo>, String>),
}

fn merge_loaded_apps(
    all_connectors: Option<&[peregrine_core::connectors::AppInfo]>,
    accessible_connectors: Option<&[peregrine_core::connectors::AppInfo]>,
) -> Vec<peregrine_core::connectors::AppInfo> {
    let all_connectors_loaded = all_connectors.is_some();
    let all = all_connectors.map_or_else(Vec::new, <[peregrine_core::connectors::AppInfo]>::to_vec);
    let accessible = accessible_connectors
        .map_or_else(Vec::new, <[peregrine_core::connectors::AppInfo]>::to_vec);
