...
fn extract_conversation_summary(
    ...
    let preview = head
        ...
        .find_map(|item| match peregrine_core::parse_turn_item(&item) {
...
pub(super) fn with_thread_spawn_agent_metadata(
    source: peregrine_types::protocol::SessionSource,
    agent_nickname: Option<String>,
    agent_role: Option<String>,
) -> peregrine_types::protocol::SessionSource {
    if agent_nickname.is_none() && agent_role.is_none() {
        return source;
    }

    match source {
        peregrine_types::protocol::SessionSource::SubAgent(
            peregrine_types::protocol::SubAgentSource::ThreadSpawn {
                parent_thread_id,
                depth,
                agent_path,
...
pub(crate) fn thread_response_active_permission_profile(
    active_permission_profile: Option<peregrine_types::models::ActivePermissionProfile>,
) -> Option<peregrine_app_server_protocol::ActivePermissionProfile> {
    active_permission_profile.map(Into::into)
}

pub(crate) fn thread_response_sandbox_policy(
    permission_profile: &peregrine_types::models::PermissionProfile,
    cwd: &Path,
) -> peregrine_app_server_protocol::SandboxPolicy {
...
pub(crate) fn thread_settings_from_core_snapshot(
    snapshot: peregrine_types::protocol::ThreadSettingsSnapshot,
) -> ThreadSettings {
    ThreadSettings {
        sandbox_policy: thread_response_sandbox_policy(
