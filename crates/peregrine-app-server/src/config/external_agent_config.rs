...
use peregrine_config::types::PluginConfig;
use peregrine_core::config::Config;
use peregrine_core::config::ConfigBuilder;
use peregrine_types::protocol::Product;
...
impl ExternalAgentConfigService {
    pub(crate) fn new(peregrine_home: PathBuf) -> Self {
    ...
    #[cfg(test)]
    fn new_for_test(peregrine_home: PathBuf, external_agent_home: PathBuf) -> Self {
        Self {
            peregrine_home,
            external_agent_home,
        }
    ...
    async fn detect_migrations(
        ...
        let target_config = repo_root.map_or_else(
            || self.peregrine_home.join("config.toml"),
            |repo_root| repo_root.join(".peregrine").join("config.toml"),
        );
        ...
        let target_hooks = repo_root.map_or_else(
            || self.peregrine_home.join("hooks.json"),
            |repo_root| repo_root.join(".peregrine").join("hooks.json"),
        );
        ...
        let target_subagents = repo_root.map_or_else(
            || self.peregrine_home.join("agents"),
            |repo_root| repo_root.join(".peregrine").join("agents"),
        );
        ...
        let target_agents_md = repo_root.map_or_else(
            || self.peregrine_home.join("AGENTS.md"),
        ...
        if let Some(settings) = settings.as_ref() {
            match ConfigBuilder::default()
                ...
                .fallback_cwd(Some(self.peregrine_home.clone()))
                .build()
                .await
            {
                Ok(config) => {
                    ...
                    let configured_marketplace_plugins = configured_marketplace_plugins(
                        &config,
                        &PluginsManager::new(self.peregrine_home.clone()),
                    )?;
        ...
        if repo_root.is_none() {
            let sessions = detect_recent_sessions(&self.external_agent_home, &self.peregrine_home)?;
    ...
    pub(crate) async fn import_plugins(
        ...
        let plugins_manager = PluginsManager::new(self.peregrine_home.clone());
        for plugin_group in plugins {
            ...
            let add_marketplace_outcome =
                add_marketplace(self.peregrine_home.clone(), request).await;
    ...
    fn import_subagents(&self, cwd: Option<&Path>) -> io::Result<usize> {
        let (source_agents, target_agents) = if let Some(repo_root) = find_repo_root(cwd)? {
            (
                ...
                repo_root.join(".peregrine").join("agents"),
            )
        } else if cwd.is_some_and(|cwd| !cwd.as_os_str().is_empty()) {
            return Ok(0);
        } else {
        ...
        let (source_external_agent_dir, target_hooks) =
            if let Some(repo_root) = find_repo_root(cwd)? {
                (
                    ...
                    repo_root.join(".peregrine").join("hooks.json"),
                )
            } else if cwd.is_some_and(|cwd| !cwd.as_os_str().is_empty()) {
                return Ok(());
            } else {
...
fn rewrite_external_agent_terms(content: &str) -> String {
    ...
    for from in [
        ...
        rewritten = replace_case_insensitive_with_boundaries(&rewritten, from, "Peregrine");
    }
    rewritten
}
