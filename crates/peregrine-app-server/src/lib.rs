...
use peregrine_app_server_protocol::TextPosition as AppTextPosition;
use peregrine_app_server_protocol::TextRange as AppTextRange;
use peregrine_config::ConfigLoadError;
use peregrine_config::TextRange as CoreTextRange;
use peregrine_core::ExecPolicyError;
use peregrine_core::check_execpolicy_for_warnings;
use peregrine_core::config::find_peregrine_home;
...
const OTEL_SERVICE_NAME: &str = "peregrine-app-server";
...
fn project_config_warning(config: &Config) -> Option<ConfigWarningNotification> {
    ...
    for layer in config.config_layer_stack.get_layers(
        ...
        disabled_folders.push((
            dot_peregrine_folder.as_path().display().to_string(),
...
pub async fn run_main_with_transport_options(
    ...
    let environment_manager = if loader_overrides.ignore_user_config {
        ...
        EnvironmentManager::from_codex_home(peregrine_home.clone(), Some(local_runtime_paths)).await
    }
    .map(Arc::new)
        ...
        .await
    {
        Ok(config) => {
            ...
            let auth_manager = AuthManager::shared_from_config(
                &config, /*enable_peregrine_api_key_env*/ false,
            )
            .await;
    ...
    let otel = peregrine_core::otel_init::build_provider(
        &config,
    ...
    peregrine_core::otel_init::record_process_start(otel.as_ref(), OTEL_SERVICE_NAME);
    peregrine_core::otel_init::install_sqlite_telemetry(otel.as_ref(), OTEL_SERVICE_NAME);
    let unix_socket_startup_lock = match &transport {
        AppServerTransport::UnixSocket { socket_path } => {
            let startup_lock_path = app_server_startup_lock_path(&peregrine_home)?;
    ...
    if should_run_personality_migration {
        ...
        match effective_toml.try_into() {
            Ok(config_toml) => {
                match peregrine_core::personality_migration::maybe_migrate_personality(
                ...
                .await
                {
                    Ok(peregrine_core::personality_migration::PersonalityMigrationStatus::Applied) => {
                    ...
                    Ok(
                        peregrine_core::personality_migration::PersonalityMigrationStatus::SkippedMarker
                        | peregrine_core::personality_migration::PersonalityMigrationStatus::SkippedExplicitPersonality
                        | peregrine_core::personality_migration::PersonalityMigrationStatus::SkippedNoSessions,
                    ) => {}
                    Err(err) => {
    ...
    let auth_manager =
        AuthManager::shared_from_config(&config, /*enable_peregrine_api_key_env*/ false).await;
