...
fn init_test_tracing() -> &'static TestTracing {
    ...
    TEST_TRACING.get_or_init(|| {
        ...
        let provider = SdkTracerProvider::builder()
            ...
            .build();
        let tracer = provider.tracer("peregrine-app-server-message-processor-tests");
...
impl TracingHarness {
    async fn new() -> Result<Self> {
        ...
        let config = Arc::new(build_test_config(peregrine_home.path(), &server.uri()).await?);
...
async fn build_test_config(peregrine_home: &Path, server_uri: &str) -> Result<Config> {
    ...
    loop {
        ...
        match envelope {
            crate::outgoing_message::OutgoingEnvelope::ToConnection {
                ...
                if matches!(
                    notification,
                    peregrine_app_server_protocol::ServerNotification::ThreadStarted(_)
                ) {
                    return;
                }
            }
