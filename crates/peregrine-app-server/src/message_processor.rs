...
use peregrine_app_server_protocol::ChatgptAuthTokensRefreshParams;
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let thread_store =
            peregrine_core::thread_store_from_config(config.as_ref(), state_db.clone());
    ...
    pub(crate) async fn process_request(
        ...
        Self::run_request_with_context(
            ...
            async {
                let peregrine_request = serde_json::to_value(&request)
                    .map_err(|err| invalid_request(format!("Invalid request: {err}")))
                    .and_then(|request_json| {
                        serde_json::from_value::<ClientRequest>(request_json)
                            .map_err(|err| invalid_request(format!("Invalid request: {err}")))
                    });
                let result = match peregrine_request {
                    Ok(peregrine_request) => {
    ...
    async fn handle_client_request(
        ...
        peregrine_request: ClientRequest,
        session: Arc<ConnectionSessionState>,
        ...
        if let ClientRequest::Initialize { request_id, params } = peregrine_request {
    ...
    async fn dispatch_initialized_client_request(
        ...
        if let Some(reason) = peregrine_request.experimental_reason()
        ...
        let serialization_scope = peregrine_request.serialization_scope();
        ...
        let request_future = Box::pin(async move {
        ...
        let request = QueuedInitializedRequest::new(rpc_gate, request_future.instrument(span));
        ...
        let result: Result<Option<ClientResponsePayload>, JSONRPCErrorError> =
            match peregrine_request {
                ...
                ClientRequest::AgentRoleList { params, .. } => self
                    ...
                    .await
                    .map(|response| Some(response.into())),
                ClientRequest::AgentRoleRead { params, .. } => self
                    .config_processor
                    .agent_role_read(params)
                    .await
                    .map(|response| Some(response.into())),
