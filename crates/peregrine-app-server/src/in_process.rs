...
//!
//! # Relationship to `peregrine-app-server-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `peregrine-app-server-client`,
...
use peregrine_app_server_protocol::Result;
...
/// through `peregrine-app-server-client`, which adds worker-task buffering,
...
pub struct InProcessClientHandle {
    ...
    #[cfg(test)]
    _test_peregrine_home: Option<tempfile::TempDir>,
}
...
impl InProcessClientHandle {
    ...
    pub async fn request(&self, request: ClientRequest) -> IoResult<PendingClientRequestResponse> {
        Box::pin(self.client.request(request)).await
    }
...
async fn start_uninitialized(args: InProcessStartArgs) -> IoResult<InProcessClientHandle> {
    ...
    let installation_id = resolve_installation_id(&args.config.peregrine_home).await?;
    let (client_tx, mut client_rx) = mpsc::channel::<InProcessClientMessage>(channel_capacity);
    let (event_tx, event_rx) = mpsc::channel::<InProcessServerEvent>(channel_capacity);

    let runtime_handle = tokio::spawn(Box::pin(async move {
        let (outgoing_tx, mut outgoing_rx) = mpsc::channel::<OutgoingEnvelope>(channel_capacity);
        let auth_manager = AuthManager::shared_from_config(
            args.config.as_ref(),
            args.enable_peregrine_api_key_env,
        )
        .await;
        ...
        let mut outbound_handle = tokio::spawn(Box::pin(async move {
        ...
        let config_manager = ConfigManager::new(
            args.config.peregrine_home.to_path_buf(),
            args.cli_overrides,
                    ...
                    command = processor_rx.recv() => {
                        ...
                        Some(ProcessorCommand::Request(request)) => {
                                ...
                                if !was_initialized && is_initialized {
                                    Box::pin(processor.send_initialize_notifications()).await;
                                }
                            }
                            Some(ProcessorCommand::Notification(notification)) => {
                                Box::pin(processor.process_client_notification(notification)).await;
                            }
                            None => {
                                break;
                            }
...
mod tests {
    ...
    async fn start_test_client_with_capacity(
        ...
        let config = Arc::new(build_test_config(peregrine_home.path()).await);
        ...
        let args = InProcessStartArgs {
            ...
            initialize: InitializeParams {
                client_info: ClientInfo {
                    name: "peregrine-in-process-test".to_string(),
                    title: None,
        ...
        client._test_peregrine_home = Some(peregrine_home);
        client
    }
    ...
    fn in_process_start_clamps_zero_channel_capacity() {
        run_in_process_test("in_process_start_clamps_zero_channel_capacity", || async {
            let client =
