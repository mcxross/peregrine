...
pub fn app_server_control_socket_path(peregrine_home: &Path) -> std::io::Result<AbsolutePathBuf> {
...
pub fn app_server_startup_lock_path(peregrine_home: &Path) -> std::io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(
        peregrine_home
            .join(APP_SERVER_CONTROL_SOCKET_DIR_NAME)
            ...
            let socket_path = if raw_socket_path.is_empty() {
                let peregrine_home = find_peregrine_home().map_err(|err| {
                    AppServerTransportParseError::InvalidUnixSocketPath {
                        ...
                        message: format!("failed to resolve PEREGRINE_HOME: {err}"),
                    }
                })?;
                app_server_control_socket_path(&peregrine_home).map_err(|err| {
                    AppServerTransportParseError::InvalidUnixSocketPath {
                        listen_url: listen_url.to_string(),
