...
use peregrine_app_server_protocol::DynamicToolSpec;
...
use peregrine_app_server_protocol::SandboxPolicy;
...
const OTEL_SERVICE_NAME: &str = "peregrine-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $PEREGRINE_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the Peregrine app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "Peregrine", version, about = "Bootstrap Peregrine app-server", long_about = None)]
struct Cli {
    /// Path to the `codex` CLI binary. When set, requests use stdio by
    /// spawning `codex app-server` as a child process.
    #[arg(long, env = "PEREGRINE_BIN", global = true)]
    peregrine_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--peregrine-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "PEREGRINE_APP_SERVER_URL", global = true)]
    url: Option<String>,
...
enum CliCommand {
    ...
    ///
    /// Logs are written to:
    ///   `/tmp/peregrine-app-server-test-client/`
    Serve {
    ...
    /// Send a user message through the Peregrine app-server.
    SendMessage {
        /// User message to send to Peregrine.
        user_message: String,
    },
    ...
    /// Fetch the current account rate limits from the Peregrine app-server.
    GetAccountRateLimits,
    /// List the available models from the Peregrine app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the Peregrine app-server.
    ...
    LiveElicitationTimeoutPause {
        ...
        #[arg(long, env = "PEREGRINE_E2E_MODEL", default_value = "gpt-5")]
        model: String,
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let peregrine_bin = peregrine_bin.unwrap_or_else(|| PathBuf::from("codex"));
            serve(&peregrine_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(peregrine_bin, url)?;
        ...
        CliCommand::ThreadIncrementElicitation { thread_id } => {
            ...
            let url =
                resolve_shared_websocket_url(peregrine_bin, url, "thread-increment-elicitation")?;
        ...
        CliCommand::ThreadDecrementElicitation { thread_id } => {
            ...
            let url =
                resolve_shared_websocket_url(peregrine_bin, url, "thread-decrement-elicitation")?;
...
fn resolve_endpoint(peregrine_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if peregrine_bin.is_some() && url.is_some() {
        bail!("--peregrine-bin and --url are mutually exclusive");
    }
    if let Some(peregrine_bin) = peregrine_bin {
        return Ok(Endpoint::SpawnPeregrine(peregrine_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if peregrine_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --peregrine-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(peregrine_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(peregrine_bin);
        if let Some(peregrine_bin_parent) = peregrine_bin.parent() {
            let mut path = OsString::from(peregrine_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", peregrine_bin.display()))?;
...
fn serve(
    ...
    let runtime_dir = PathBuf::from("/tmp/peregrine-app-server-test-client");
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,peregrine_=trace {}",
        shell_quote(&peregrine_bin.display().to_string())
    );
    for override_kv in config_overrides {
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnPeregrine(peregrine_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut PeregrineClient) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = PeregrineClient::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = PeregrineClient::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (peregrine_bin, url) {
        (Some(_), Some(_)) => bail!("--peregrine-bin and --url are mutually exclusive"),
        (Some(peregrine_bin), None) => {
            let server = BackgroundAppServer::spawn(&peregrine_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve peregrine-app-server-test-client binary path")?;
...
impl PeregrineClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnPeregrine(peregrine_bin) => {
                Self::spawn_stdio(peregrine_bin, config_overrides)
            }
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(peregrine_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        let peregrine_bin_display = peregrine_bin.display();
        let mut cmd = Command::new(peregrine_bin);
        if let Some(peregrine_bin_parent) = peregrine_bin.parent() {
            let mut path = OsString::from(peregrine_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut peregrine_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{peregrine_bin_display}` app-server"))?;

        let stdin = peregrine_app_server
            .stdin
            .take()
            .context("codex app-server stdin unavailable")?;
        let stdout = peregrine_app_server
            .stdout
            .take()
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `peregrine-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "peregrine-toy-app-server".to_string(),
                    title: Some("Peregrine Toy App Server".to_string()),
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            ...
            params: peregrine_app_server_protocol::LoginAccountParams::Chatgpt {
                peregrine_streamlined_login: false,
            },
        };
    ...
    fn login_account_chatgpt_device_code(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: peregrine_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = peregrine_core::otel_init::build_provider(
            &config,
