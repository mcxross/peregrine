...
use managed_install::managed_peregrine_bin;
#[cfg(unix)]
use managed_install::managed_peregrine_version;
use peregrine_app_server_protocol::RemoteControlConnectionStatus;
use peregrine_app_server_transport::app_server_control_socket_path;
...
pub struct LifecycleOutput {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub pid: Option<u32>,
    pub managed_peregrine_path: PathBuf,
    pub managed_peregrine_version: Option<String>,
...
struct Daemon {
    ...
    managed_peregrine_bin: PathBuf,
}

impl Daemon {
    fn from_environment() -> Result<Self> {
        let peregrine_home = find_peregrine_home().context("failed to resolve PEREGRINE_HOME")?;
        let socket_path = app_server_control_socket_path(peregrine_home.as_path())?
            .as_path()
            .to_path_buf();
        let state_dir = peregrine_home.as_path().join(STATE_DIR_NAME);
        Ok(Self {
            socket_path,
            ...
            managed_peregrine_bin: managed_peregrine_bin(peregrine_home.as_path()),
        })
    }
    ...
    async fn start(&self) -> Result<LifecycleOutput> {
        ...
        self.ensure_managed_peregrine_bin()?;
    ...
    pub(crate) async fn try_restart_if_running(
        ...
        let outcome = if let Some(backend) = self.running_backend_instance(&settings).await? {
            ...
            let managed_version = if info.is_some() {
                Some(managed_peregrine_version(managed_peregrine_bin).await?)
            } else {
                None
            };
            match restart_decision(mode, info.as_ref(), managed_version.as_deref()) {
                ...
                RestartDecision::Restart => {
                    backend.stop().await?;
                    let _ = self
                        .start_managed_backend_with_bin(&settings, managed_peregrine_bin)
                        .await?;
        ...
        if should_reexec_updater(updater_refresh_mode, outcome) {
            crate::update_loop::reexec_managed_updater(managed_peregrine_bin)?;
        }
    ...
    async fn append_daemon_app_server_context(&self, context: &mut String) {
        let managed_peregrine_version = self
            .managed_peregrine_version_best_effort()
            .await
            .unwrap_or_else(|| "unknown".to_string());
        context.push_str(&format!(
            "\n\nDaemon used app-server:\n  path: {}\n  version: {managed_peregrine_version}",
            self.managed_peregrine_bin.display()
        ));
    }
    ...
    async fn bootstrap_locked(&self, options: BootstrapOptions) -> Result<BootstrapOutput> {
        ...
        let info = self.wait_until_ready().await?;
        let managed_peregrine_version = self.managed_peregrine_version_best_effort().await;
        Ok(BootstrapOutput {
            status: BootstrapStatus::Bootstrapped,
            backend: BackendKind::Pid,
            auto_update_enabled: true,
            remote_control_enabled: settings.remote_control_enabled,
            managed_peregrine_path: self.managed_peregrine_bin.clone(),
            managed_peregrine_version,
    ...
    async fn start_managed_backend(&self, settings: &DaemonSettings) -> Result<Option<u32>> {
        self.start_managed_backend_with_bin(settings, &self.managed_peregrine_bin)
            .await
    }
    ...
    async fn start_managed_backend_with_bin(
        ...
        let backend =
            backend::pid_backend(self.backend_paths_with_bin(settings, managed_peregrine_bin));
        backend.start().await
    }
    ...
    fn ensure_managed_peregrine_bin(&self) -> Result<()> {
        if self.managed_peregrine_bin.is_file() {
            return Ok(());
        }

        let managed_peregrine_path = self.managed_peregrine_bin.display();
        Err(anyhow!(
    ...
    #[cfg(unix)]
    async fn managed_peregrine_version_best_effort(&self) -> Option<String> {
        managed_peregrine_version(&self.managed_peregrine_bin)
            .await
            .ok()
    }
    ...
    fn backend_paths(&self, settings: &DaemonSettings) -> BackendPaths {
        self.backend_paths_with_bin(settings, &self.managed_peregrine_bin)
    }
    ...
    fn backend_paths_with_bin(
        ...
        BackendPaths {
            peregrine_bin: managed_peregrine_bin.to_path_buf(),
    ...
    async fn not_ready_context_reports_daemon_app_server_before_stderr() {
        ...
        assert_eq!(
            ...
            format!(
                ...
                daemon.managed_peregrine_bin.display(),
                stderr_log.display()
            )
        );
    }
