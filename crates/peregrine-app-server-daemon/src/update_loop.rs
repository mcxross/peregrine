...
#[cfg(unix)]
use crate::managed_install::resolved_managed_peregrine_bin;
...
async fn update_once(
    ...
    let managed_peregrine_bin =
        resolved_managed_peregrine_bin(&daemon.managed_peregrine_bin).await?;
    let managed_identity = executable_identity(&managed_peregrine_bin).await?;
    ...
    loop {
        ...
        match daemon
            .try_restart_if_running(restart_mode, updater_refresh_mode, &managed_peregrine_bin)
            .await?
        {
...
#[cfg(unix)]
pub(crate) fn reexec_managed_updater(managed_peregrine_bin: &std::path::Path) -> Result<()> {
    let err = StdCommand::new(managed_peregrine_bin)
        .args(["app-server", "daemon", "pid-update-loop"])
        .exec();
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed Peregrine binary {}",
            managed_peregrine_bin.display()
        )
    })
}
