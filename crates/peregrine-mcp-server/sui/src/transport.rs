...
pub async fn run_sse_server(
    ...
    let service = StreamableHttpService::new(
        move || service_factory().map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string())),
        session_manager,
        config,
    );
    ...
    tracing::info!("Starting Peregrine MCP Server over SSE on 127.0.0.1:{}", port);
