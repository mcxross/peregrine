use crate::{
    analysis::{legacy_static_report, run_sui_analysis_blocking},
    pipeline::{
...
use peregrine_analysis::{AnalysisRequest, AnalysisStage, AnalysisTarget, ChainId};
use peregrine_sui_adapter::SuiAdapterSettings;
use peregrine_sui_static_analysis::{
    MovePackage, discover_move_project_fast, discover_move_project_shallow,
};
use serde_json::json;
    ...
    fn run(&self, context: &mut ProjectLoadContext) -> ProjectLoadStageReport {
        timed_stage(self.id(), self.label(), || {
            ...
            for package in valid_packages {
                ...
                request.stages = vec![AnalysisStage::Scan, AnalysisStage::Static];
                request.graph_kinds.clear();
                if let Some(plugin_root) = &context.options.analyzer_plugin_root {
                    request.options.insert(
                        "globalPluginRoot".to_string(),
                        json!(plugin_root.to_string_lossy()),
                    );
                }
                let report = match run_sui_analysis_blocking(request, SuiAdapterSettings::default())
                {
                    Ok(report) => legacy_static_report(&report),
                    Err(message) => {
