...
use peregrine_config::NetworkRequirementsToml;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    peregrine_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    peregrine_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}
...
#[ctor]
fn configure_hermetic_home_skills_for_tests() {
    // Safety: this ctor runs at process startup before test threads begin.
    unsafe {
        std::env::set_var("PEREGRINE_TEST_DISABLE_HOME_SKILLS", "1");
    }
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = codex_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("peregrine-rs"));
...
/// avoids clobbering a developer’s real `~/.peregrine`.
pub async fn load_default_config_for_test(peregrine_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        peregrine_linux_sandbox_exe: Some(
            find_peregrine_linux_sandbox_exe()
                .expect("should find binary for peregrine-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_peregrine_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    codex_utils_cargo_bin::cargo_bin("peregrine-linux-sandbox")
}

pub async fn wait_for_event<F>(
    codex: &PeregrineThread,
    predicate: F,
) -> peregrine_types::protocol::EventMsg
where
    F: FnMut(&peregrine_types::protocol::EventMsg) -> bool,
{
...
pub async fn wait_for_mcp_server(codex: &PeregrineThread, server_name: &str) -> anyhow::Result<()> {
...
pub async fn wait_for_event_match<T, F>(codex: &PeregrineThread, matcher: F) -> T
where
    F: Fn(&peregrine_types::protocol::EventMsg) -> Option<T>,
{
...
pub fn sandbox_env_var() -> &'static str {
    peregrine_core::spawn::PEREGRINE_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    peregrine_core::spawn::PEREGRINE_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

const REMOTE_ENV_ENV_VAR: &str = "PEREGRINE_TEST_REMOTE_ENV";
...
pub fn format_with_current_shell(command: &str) -> Vec<String> {
    peregrine_core::shell::default_user_shell()
...
macro_rules! skip_if_no_network {
    () => {{
        if ::std::env::var($crate::sandbox_network_env_var()).is_ok() {
            println!(
                "Skipping test because it cannot execute when network is disabled in a Peregrine sandbox."
            );
            return;
        }
    }};
...
#[macro_export]
macro_rules! peregrine_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_peregrine_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("peregrine-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }
