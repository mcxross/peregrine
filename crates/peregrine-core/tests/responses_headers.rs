...
async fn responses_stream_includes_subagent_header_on_review() {
    ...
    let peregrine_home = TempDir::new().expect("failed to create TempDir");
    ...
    assert_eq!(request.header("x-peregrine-parent-thread-id"), None);
    assert_eq!(
        request.body_json()["client_metadata"]["x-peregrine-installation-id"].as_str(),
        Some(TEST_INSTALLATION_ID)
    );
    assert_eq!(request.header("x-peregrine-sandbox"), None);
}
...
async fn responses_respects_model_info_overrides_from_config() {
    ...
    let thread_id = ThreadId::new();
    let auth_mode = peregrine_core::test_support::auth_manager_from_auth(
...
async fn responses_stream_includes_turn_metadata_header_for_git_workspace_e2e() {
    ...
    let initial_header = first_request
        ...
        .expect("x-peregrine-turn-metadata header should be present");
    let initial_parsed: serde_json::Value = serde_json::from_str(&initial_header)
        .expect("x-peregrine-turn-metadata should be valid JSON");
    ...
    assert!(
        ...
        "turn_id should not be empty in x-peregrine-turn-metadata"
    );
