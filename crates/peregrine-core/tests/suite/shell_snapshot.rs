...
const SNAPSHOT_MARKER_VAR: &str = "PEREGRINE_SNAPSHOT_POLICY_MARKER";
...
async fn wait_for_snapshot(peregrine_home: &Path) -> Result<PathBuf> {
    let snapshot_dir = peregrine_home.join("shell_snapshots");
...
async fn run_snapshot_command_with_options(
    ...
    let peregrine_home = test.home.path().to_path_buf();
    ...
    .await;
    let snapshot_path = wait_for_snapshot(&peregrine_home).await?;
...
async fn linux_unified_exec_uses_shell_snapshot() -> Result<()> {
    ...
    assert!(run.snapshot_path.starts_with(&run.peregrine_home));
...
async fn shell_command_snapshot_preserves_shell_environment_policy_set() -> Result<()> {
    ...
    let harness = TestPeregrineHarness::with_builder(builder).await?;
    let peregrine_home = harness.test().home.path().to_path_buf();
    ...
    assert!(snapshot_path.starts_with(peregrine_home));
...
async fn shell_snapshot_deleted_after_shutdown_with_skills() -> Result<()> {
    ...
    let peregrine_home = home.path().to_path_buf();
