...
use core_test_support::test_codex::TestPeregrine;
use core_test_support::test_codex::test_codex;
use core_test_support::wait_for_event;
use dunce::canonicalize as normalize_path;
use futures::StreamExt;
use peregrine_config::ConfigLayerStack;
use peregrine_core::ModelClient;
use peregrine_core::NewThread;
use peregrine_core::Prompt;
use peregrine_core::ResponseEvent;
use peregrine_core::ThreadManager;
use peregrine_core::resolve_installation_id;
...
use peregrine_types::config_types::ModelProviderAuthInfo;
...
use peregrine_types::models::FunctionCallOutputContentItem;
use peregrine_types::models::FunctionCallOutputPayload;
use peregrine_types::models::ImageDetail;
use peregrine_types::models::LocalShellAction;
use peregrine_types::models::LocalShellExecAction;
use peregrine_types::models::LocalShellStatus;
use peregrine_types::models::MessagePhase;
use peregrine_types::models::ReasoningItemContent;
use peregrine_types::models::ReasoningItemReasoningSummary;
use peregrine_types::models::ResponseItem;
use peregrine_types::models::WebSearchAction;
...
use peregrine_types::protocol::RolloutLine;
use peregrine_types::protocol::SessionMeta;
...
/// Writes an `auth.json` into the provided `peregrine_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        peregrine_home.path().join("auth.json"),
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = peregrine_types::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![peregrine_types::models::ContentItem::InputText {
    ...
    let prior_system = peregrine_types::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![peregrine_types::models::ContentItem::OutputText {
    ...
    let prior_item = peregrine_types::models::ResponseItem::Message {
        id: None,
    ...
    // Configure Peregrine to resume from our file
    let peregrine_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_codex()
        .with_home(peregrine_home.clone())
        .with_config(|config| {
            // Ensure user instructions are NOT delivered on resume.
            config.user_instructions = Some("be nice".to_string());
        });
    let test = builder
        .resume(&server, peregrine_home, session_path.clone())
        .await
        .expect("resume conversation");
    let codex = test.peregrine.clone();
...
async fn includes_session_id_thread_id_and_model_headers_in_request() {
    ...
    let mut builder = test_codex().with_auth(PeregrineAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
    ...
    let request_body = request.body_json();
    let installation_id =
        std::fs::read_to_string(test.peregrine_home_path().join(INSTALLATION_ID_FILENAME))
    ...
    assert_eq!(
        request_body["client_metadata"]["x-peregrine-installation-id"].as_str(),
...
async fn send_provider_auth_request(server: &MockServer, auth: ModelProviderAuthInfo) {
    ...
    let peregrine_home = TempDir::new().unwrap();
    let mut config = load_default_config_for_test(&peregrine_home).await;
    config.model_provider_id = provider.name.clone();
    config.model_provider = provider.clone();
    let effort = config.model_reasoning_effort;
    let summary = config.model_reasoning_summary;
    let model = peregrine_core::test_support::get_model_offline(config.model.as_deref());
    config.model = Some(model.clone());
    let config = Arc::new(config);
    let model_info =
        peregrine_core::test_support::construct_model_info_offline(model.as_str(), &config);
    ...
    let client = ModelClient::new(
        Some(AuthManager::from_auth_for_testing(
            PeregrineAuth::from_api_key("unused-api-key"),
        )),
        thread_id.into(),
        thread_id,
...
async fn includes_base_instructions_override_in_request() {
    ...
    let mut builder = test_codex()
        .with_auth(PeregrineAuth::from_api_key("Test API Key"))
        .with_config(|config| {
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut builder = test_codex()
        .with_auth(create_dummy_peregrine_auth())
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth_manager = match PeregrineAuth::from_auth_storage(
        peregrine_home.path(),
    ...
    .await
    {
        Ok(Some(auth)) => peregrine_core::test_support::auth_manager_from_auth(auth),
        Ok(None) => panic!("No PeregrineAuth found in peregrine_home"),
        Err(e) => panic!("Failed to load PeregrineAuth: {e}"),
    };
    let installation_id = resolve_installation_id(&config.peregrine_home)
        .await
...
async fn skills_append_to_developer_message() {
    ...
    let peregrine_home = Arc::new(TempDir::new().unwrap());
    let skill_dir = peregrine_home.path().join("skills/demo");
    ...
    let peregrine_home_path = peregrine_home.path().to_path_buf();
    let mut builder = test_codex()
        .with_home(peregrine_home.clone())
        .with_auth(PeregrineAuth::from_api_key("Test API Key"))
        .with_config(move |config| {
            config.cwd = peregrine_home_path.abs();
        });
    let codex = builder
        .build(&server)
        .await
    ...
    let expected_root = normalize_path(skill_dir.parent().expect("skill root")).unwrap();
    let expected_root_str = expected_root.to_string_lossy().replace('\\', "/");
    let has_aliased_path = skill_root_alias_for(&developer_text, &expected_root_str)
        .map(|alias| developer_text.contains(&format!("(file: {alias}/demo/SKILL.md)")))
        .unwrap_or(false);
    assert!(
        developer_text.contains(&expected_path_str) || has_aliased_path,
        "expected path {expected_path_str} or aliased path rooted at {expected_root_str} in developer message: {developer_messages:?}"
    );
    let _peregrine_home_guard = peregrine_home;
}
...
async fn skills_use_aliases_in_developer_message_under_budget_pressure() {
    ...
    let peregrine_home_parent = TempDir::new().unwrap();
    let long_home_parent = peregrine_home_parent
        .path()
        .join("peregrine-home-with-long-shared-prefix-for-skill-alias-budget-test");
    std::fs::create_dir_all(&long_home_parent).expect("create long home parent");
    let peregrine_home = Arc::new(TempDir::new_in(long_home_parent).unwrap());
    let skill_root = peregrine_home.path().join("skills");
    for index in 0..12 {
        let skill_dir = skill_root.join(format!("a{index:02}"));
        std::fs::create_dir_all(&skill_dir).expect("create skill dir");
        std::fs::write(
            skill_dir.join("SKILL.md"),
            format!("---\nname: a{index:02}\ndescription: d\n---\n\n# body\n"),
    ...
    let mut builder = test_codex()
        ...
        .with_config(move |config| {
            config.cwd = peregrine_home_path.abs();
            let user_config_path = peregrine_home_path.join("config.toml").abs();
    ...
    let expected_root_str = expected_root.to_string_lossy().replace('\\', "/");
    let skill_root_alias = skill_root_alias_for(&developer_text, &expected_root_str)
        .unwrap_or_else(|| {
            panic!("expected root alias for temp skill root in {developer_messages:?}")
        });
    assert!(
    ...
    assert!(
        developer_text.contains(&format!("- `{skill_root_alias}` = `{expected_root_str}`")),
        "expected root alias for {expected_root_str}: {developer_messages:?}"
    );
    assert!(
        developer_text.contains(&format!("- a00: (file: {skill_root_alias}/a00/SKILL.md)"))
            || developer_text
                .contains(&format!("- a00: d (file: {skill_root_alias}/a00/SKILL.md)")),
    ...
    let _peregrine_home_guard = peregrine_home;
    let _peregrine_home_parent_guard = peregrine_home_parent;
}

fn skill_root_alias_for(developer_text: &str, expected_root: &str) -> Option<String> {
    developer_text.lines().find_map(|line| {
        let line = line.strip_prefix("- `")?;
        let (alias, root) = line.split_once("` = `")?;
        let root = root.strip_suffix('`')?;
        (root == expected_root).then(|| alias.to_string())
    })
}
...
async fn includes_configured_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestPeregrine { codex, .. } = test_codex()
        .with_model("gpt-5.4")
        .with_config(|config| {
...
async fn includes_no_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestPeregrine { codex, .. } = test_codex().with_model("gpt-5.4").build(&server).await?;
...
async fn user_turn_collaboration_mode_overrides_model_and_effort() -> anyhow::Result<()> {
    ...
    .await;
    let TestPeregrine { codex, config, .. } =
...
async fn azure_responses_request_includes_store_and_reasoning_ids() {
    ...
    let model_info =
        peregrine_core::test_support::construct_model_info_offline(model.as_str(), &config);
    let thread_id = ThreadId::new();
    let auth_manager = peregrine_core::test_support::auth_manager_from_auth(
        PeregrineAuth::from_api_key("Test API Key"),
    );
    let session_telemetry = SessionTelemetry::new(
        thread_id,
        model.as_str(),
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-peregrine-primary-used-percent", "12.5")
        .insert_header("x-peregrine-secondary-used-percent", "40.0")
        .insert_header("x-peregrine-primary-window-minutes", "10")
        .insert_header("x-peregrine-secondary-window-minutes", "60")
        .insert_header("x-peregrine-primary-reset-at", "1704069000")
        .insert_header("x-peregrine-secondary-reset-at", "1704074400")
    ...
    let mut builder = test_codex()
        .with_auth(PeregrineAuth::from_api_key("test"))
        .with_config(move |config| {
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-peregrine-primary-used-percent", "100.0")
        .insert_header("x-peregrine-secondary-used-percent", "87.5")
        .insert_header("x-peregrine-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-peregrine-primary-window-minutes", "15")
        .insert_header("x-peregrine-secondary-window-minutes", "60")
        .set_body_json(json!({
            "error": {
    ...
    let mut builder = test_codex();
    let peregrine_fixture = builder.build(&server).await?;
    let codex = peregrine_fixture.peregrine.clone();
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let expected_context_window_message = PeregrineErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
fn create_dummy_peregrine_auth() -> PeregrineAuth {
    PeregrineAuth::create_dummy_chatgpt_auth_for_testing()
}
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Peregrine sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
