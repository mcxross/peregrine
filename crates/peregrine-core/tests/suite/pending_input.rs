...
async fn build_codex(server: &StreamingSseServer) -> Arc<PeregrineThread> {
    test_codex()
        .with_model("gpt-5.4")
        .build_with_streaming_server(server)
        .await
        .unwrap_or_else(|err| panic!("build streaming Peregrine test session: {err}"))
        .peregrine
}

async fn submit_user_input(codex: &PeregrineThread, text: &str) {
    codex
        .submit(Op::UserInput {
            environments: None,
...
async fn submit_danger_full_access_user_turn(test: &TestPeregrine, text: &str) {
...
async fn steer_user_input(codex: &PeregrineThread, text: &str) {
    codex
        .steer_input(
            vec![UserInput::Text {
                text: text.to_string(),
...
async fn submit_queue_only_agent_mail(codex: &PeregrineThread, text: &str) {
    codex
...
async fn wait_for_reasoning_item_started(codex: &PeregrineThread) {
    wait_for_event(codex, |event| {
        matches!(
            event,
...
async fn wait_for_agent_message(codex: &PeregrineThread, text: &str) {
...
async fn wait_for_turn_complete(codex: &PeregrineThread) {
