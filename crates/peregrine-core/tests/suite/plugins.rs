...
) -> Result<Arc<peregrine_core::PeregrineThread>> {
...
async fn capability_sections_render_in_developer_message_in_order() -> Result<()> {
    ...
    let peregrine_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(peregrine_home.as_ref());
    write_plugin_app_plugin(peregrine_home.as_ref());
    let codex = build_apps_enabled_plugin_test_codex(
        &server,
        Arc::clone(&peregrine_home),
    ...
    codex
        .submit(Op::UserInput {
            environments: None,
            items: vec![peregrine_types::user_input::UserInput::Text {
                text: "hello".into(),
...
async fn explicit_plugin_mentions_inject_plugin_guidance() -> Result<()> {
    ...
    write_plugin_skill_plugin(peregrine_home.as_ref());
    write_plugin_mcp_plugin(peregrine_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(peregrine_home.as_ref());

    let codex =
        build_apps_enabled_plugin_test_codex(&server, peregrine_home, apps_server.chatgpt_base_url)
            .await?;
    ...
    codex
        .submit(Op::UserInput {
            environments: None,
            items: vec![peregrine_types::user_input::UserInput::Mention {
                name: "sample".into(),
    ...
    assert!(
        request_tools
            .iter()
            .any(|name| name == "mcp__peregrine_apps__google_calendar"),
    ...
    let calendar_tool = request
        .tool_by_name("mcp__peregrine_apps__google_calendar", "_create_event")
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
    ...
    let peregrine_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(peregrine_home.as_ref());
    let codex = build_analytics_plugin_test_codex(&server, peregrine_home).await?;
        ...
        if let Some(event) = requests
            ...
            .find_map(|request| {
                ...
                payload["events"].as_array().and_then(|events| {
                    events
                        .iter()
                        .find(|event| event["event_type"] == "peregrine_plugin_used")
                        .cloned()
                })
            })
        {
