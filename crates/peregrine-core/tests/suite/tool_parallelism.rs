...
async fn run_turn(test: &TestPeregrine, prompt: &str) -> anyhow::Result<()> {
...
async fn run_turn_and_measure(test: &TestPeregrine, prompt: &str) -> anyhow::Result<Duration> {
...
#[allow(clippy::expect_used)]
async fn build_peregrine_with_test_tool(
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_peregrine_with_test_tool(&server).await?;
