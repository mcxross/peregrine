...
async fn responses_mode_stream_cli() {
    ...
    cmd.arg("exec")
        ...
        .arg(&repo_root)
        .arg("hello?");
    cmd.env("PEREGRINE_HOME", home.path())
...
async fn exec_cli_applies_model_instructions_file() {
    ...
    // Peregrine to use the Responses API with the dummy env var.
    let provider_override = format!(
        "model_providers.mock={{ name = \"mock\", base_url = \"{}/v1\", env_key = \"PATH\", wire_api = \"responses\" }}",
        server.uri()
    );
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    cmd.arg("exec")
        ...
        .arg(&prompt);
    cmd.env("PEREGRINE_HOME", home.path())
        .env(PEREGRINE_API_KEY_ENV_VAR, "dummy");
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("PEREGRINE_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy");

    let output2 = cmd2.output().unwrap();
    assert!(
        output2.status.success(),
        "resume peregrine-tui cli run failed"
    );
