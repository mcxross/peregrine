...
use peregrine_types::openai_models::ModelVisibility;
use peregrine_types::openai_models::ModelsResponse;
use peregrine_types::openai_models::ReasoningEffortPreset;
use peregrine_types::openai_models::TruncationPolicyConfig;
...
fn auto_approved_user_turn(fixture: &TestPeregrine, text: impl Into<String>) -> Op {
...
fn remote_aware_environment_id() -> String {
    ...
    std::env::var_os(remote_env_env_var())
        ...
        .unwrap_or_else(|| peregrine_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string())
}
...
fn unique_remote_path(binary_name: &str) -> anyhow::Result<String> {
    ...
    Ok(format!(
        "/tmp/peregrine-remote-env/{binary_name}-{}-{unique_suffix}",
        std::process::id()
    ))
}
...
async fn call_cwd_tool(
    ...
    wait_for_event(&fixture.peregrine, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
    let end_event = wait_for_event(&fixture.peregrine, |ev| {
...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    let fixture = test_codex()
        ...
        .await?;
    wait_for_mcp_server(&fixture.peregrine, server_name).await?;
    ...
    let begin_event = wait_for_event(&fixture.peregrine, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn stdio_mcp_parallel_tool_calls_default_false_runs_serially() -> anyhow::Result<()> {
    ...
    while call_events.len() < 4 {
        let event = wait_for_event(&fixture.peregrine, |ev| {
            matches!(
                ev,
        ...
        ModelsResponse {
            models: vec![ModelInfo {
                ...
                supported_reasoning_levels: vec![ReasoningEffortPreset {
                    effort: peregrine_types::openai_models::ReasoningEffort::Medium,
...
const REMOTE_EXEC_SERVER_URL_ENV_VAR: &str = "PEREGRINE_TEST_REMOTE_EXEC_SERVER_URL";
...
impl StreamableHttpTestServer {
    /// Returns the MCP endpoint URL that Peregrine should connect to.
    fn url(&self) -> &str {
        &self.server_url
    }
...
/// What this tests: Peregrine can discover and call a Streamable HTTP MCP tool in
...
async fn streamable_http_tool_call_round_trip() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Peregrine will call the MCP echo tool
    ...
    // Phase 3: configure Peregrine with the Streamable HTTP MCP server and build a
    ...
    // Phase 5: assert Peregrine begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.peregrine, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
/// This test writes to a fallback credentials file in PEREGRINE_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire PEREGRINE_HOME through the code.
#[test]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Peregrine will call the OAuth-backed
    ...
    // Phase 3: seed an isolated PEREGRINE_HOME with fallback OAuth tokens for this
    // server so the test does not share credentials with other suite cases.
    let temp_home = Arc::new(tempdir()?);
    let _peregrine_home_guard = EnvVarGuard::set("PEREGRINE_HOME", temp_home.path().as_os_str());
    ...
    // Phase 4: configure Peregrine with the OAuth-backed Streamable HTTP MCP
    ...
    // Phase 7: assert Peregrine begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.peregrine, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn start_remote_streamable_http_test_server(
    ...
    // remote HTTP before handing the URL to the Peregrine fixture.
