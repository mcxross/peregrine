...
use core_test_support::test_codex::TestPeregrineBuilder;
use core_test_support::test_codex::TestPeregrineHarness;
use core_test_support::test_codex::test_codex;
use core_test_support::wait_for_event;
use core_test_support::wait_for_event_match;
use core_test_support::wait_for_event_with_timeout;
use peregrine_core::compact::SUMMARY_PREFIX;
...
use peregrine_types::protocol::RealtimeConversationRealtimeEvent;
use peregrine_types::protocol::RealtimeEvent;
use peregrine_types::protocol::RealtimeOutputModality;
...
fn remote_realtime_test_peregrine_builder(
    realtime_server: &responses::WebSocketTestServer,
) -> TestPeregrineBuilder {
    let realtime_base_url = realtime_server.uri().to_string();
    test_codex()
        .with_auth(PeregrineAuth::from_api_key("dummy"))
...
async fn start_realtime_conversation(codex: &peregrine_core::PeregrineThread) -> Result<()> {
    codex
...
async fn close_realtime_conversation(codex: &peregrine_core::PeregrineThread) -> Result<()> {
...
async fn wait_for_turn_complete(codex: &peregrine_core::PeregrineThread) {
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    let harness = TestPeregrineHarness::with_builder(
        test_codex().with_auth(PeregrineAuth::create_dummy_chatgpt_auth_for_testing()),
    )
    .await?;
    let codex = harness.test().peregrine.clone();
    ...
    let compact_metadata: Value = serde_json::from_str(
        &compact_request
            .header("x-peregrine-turn-metadata")
    ...
    assert_eq!(
        ...
        compact_request.header("x-peregrine-window-id").as_deref()
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        follow_up_request.header("x-peregrine-window-id").as_deref()
    );
    assert_ne!(
...
async fn assert_remote_manual_compact_request_parity(
    ...
    let harness = TestPeregrineHarness::with_builder(builder).await?;
    let codex = harness.test().peregrine.clone();
    let image_url =
...
-> Result<()> {
    ...
    assert_remote_manual_compact_request_parity(
        PeregrineAuth::from_api_key("dummy"),
...
async fn remote_compact_v2_reuses_compaction_trigger_for_followups() -> Result<()> {
    ...
    let harness = TestPeregrineHarness::with_builder(
        test_codex()
            .with_auth(PeregrineAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
    ...
    assert!(
        compact_request
            .header("x-peregrine-beta-features")
            .as_deref()
...
async fn remote_compact_filters_deferred_dynamic_tools() -> Result<()> {
    ...
    let dynamic_tools = vec![
        DynamicToolSpec {
            namespace: Some("peregrine_app".to_string()),
    ...
    let new_thread = test
        ...
        .await?;
    test.peregrine = new_thread.thread;
    ...
    assert_eq!(
        namespace_child_tool_names(&first_response_body, "peregrine_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        namespace_child_tool_names(&compact_body, "peregrine_app"),
        vec![visible_tool.to_string()]
    );
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    let baseline_harness = TestPeregrineHarness::with_builder(
        test_codex()
            .with_auth(PeregrineAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
    ...
    .await?;
    let baseline_codex = baseline_harness.test().peregrine.clone();
    ...
    let override_harness = TestPeregrineHarness::with_builder(
        test_codex()
            .with_auth(PeregrineAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config({
    ...
    .await?;
    let override_codex = override_harness.test().peregrine.clone();
...
async fn remote_compact_and_resume_refresh_stale_developer_instructions() -> Result<()> {
    ...
    initial.peregrine.submit(Op::Compact).await?;
    wait_for_event(&initial.peregrine, |ev| {
        matches!(ev, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    initial.peregrine.submit(Op::Shutdown).await?;
    wait_for_event(&initial.peregrine, |ev| {
    ...
    resumed
        ...
        .await?;
    wait_for_event(&resumed.peregrine, |ev| {
        matches!(ev, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn remote_compact_refreshes_stale_developer_instructions_without_resume() -> Result<()> {
    ...
    test.peregrine
        ...
        .await?;
    wait_for_event(&test.peregrine, |ev| {
        matches!(ev, EventMsg::TurnComplete(_))
    })
    .await;

    test.peregrine.submit(Op::Compact).await?;
    wait_for_event(&test.peregrine, |ev| {
        matches!(ev, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn snapshot_request_shape_remote_pre_turn_compaction_restates_realtime_start() -> Result<()> {
    ...
    let mut builder =
        remote_realtime_test_peregrine_builder(&realtime_server).with_config(|config| {
    ...
    start_realtime_conversation(test.peregrine.as_ref()).await?;
    ...
    close_realtime_conversation(test.peregrine.as_ref()).await?;
...
async fn remote_request_uses_custom_experimental_realtime_start_instructions() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_peregrine_builder(&realtime_server).with_config({
...
async fn snapshot_request_shape_remote_manual_compact_restates_realtime_start() -> Result<()> {
    ...
    let server = wiremock::MockServer::start().await;
    let realtime_server = start_remote_realtime_server().await;
    let mut builder = remote_realtime_test_peregrine_builder(&realtime_server);
    let test = builder.build(&server).await?;
...
async fn snapshot_request_shape_remote_compact_resume_restates_realtime_end() -> Result<()> {
    ...
    start_realtime_conversation(initial.peregrine.as_ref()).await?;
    ...
    close_realtime_conversation(initial.peregrine.as_ref()).await?;
    ...
    for user in ["USER_ONE", "USER_TWO", "USER_THREE"] {
        if user == "USER_THREE" {
            core_test_support::submit_thread_settings(
                &codex,
                peregrine_types::protocol::ThreadSettingsOverrides {
