...
async fn responses_api_parent_and_subagent_requests_include_identity_headers() -> Result<()> {
    ...
    let parent_window_id = parent
        ...
        .ok_or_else(|| anyhow!("parent request missing x-peregrine-window-id"))?;
    let child_window_id = child
        .header("x-peregrine-window-id")
        .ok_or_else(|| anyhow!("child request missing x-peregrine-window-id"))?;
    ...
    assert_eq!(
        child.header("x-peregrine-parent-thread-id").as_deref(),
        Some(parent_thread_id)
    );
    let child_turn_metadata: serde_json::Value = serde_json::from_str(
        &child
            .header("x-peregrine-turn-metadata")
            .ok_or_else(|| anyhow!("child request missing x-peregrine-turn-metadata"))?,
    )?;
    assert_eq!(
...
async fn submit_turn_with_timeout(test: &TestPeregrine, prompt: &str) -> Result<()> {
