...
use peregrine_types::protocol::ElicitationAction;
...
fn set_calendar_approval_mode(config: &mut Config, approval_mode: AppToolApproval) {
    ...
    let user_config_path = config.peregrine_home.join("config.toml").abs();
        ...
        .submit(Op::UserInput {
            ...
            thread_settings: peregrine_types::protocol::ThreadSettingsOverrides {
                ...
                collaboration_mode: collaboration_mode.or({
                    Some(peregrine_types::config_types::CollaborationMode {
...
async fn approved_mcp_tool_call_metadata_records_prior_user_input_request() -> Result<()> {
    ...
    let EventMsg::ElicitationRequest(request) = wait_for_event(&test.peregrine, |event| {
    ...
    assert_eq!(
        apps_tool_call
            .pointer("/params/_meta/x-peregrine-turn-metadata/user_input_requested_during_turn"),
        Some(&json!(true))
    );
