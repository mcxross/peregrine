...
use peregrine_types::protocol::ExecCommandStatus;
...
async fn wait_for_raw_unified_exec_output(
    ...
    let content = wait_for_event_match(&test.peregrine, |event| match event {
...
async fn unified_exec_emits_exec_command_begin_event() -> Result<()> {
    ...
    let begin_event = wait_for_event_match(&test.peregrine, |msg| match msg {
...
async fn unified_exec_emits_exec_command_end_event() -> Result<()> {
    ...
    let end_event = wait_for_event_match(&test.peregrine, |msg| match msg {
...
async fn unified_exec_emits_output_delta_for_exec_command() -> Result<()> {
    ...
    let event = wait_for_event_match(&test.peregrine, |msg| match msg {
        EventMsg::ExecCommandEnd(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
...
async fn unified_exec_full_lifecycle_with_background_end_event() -> Result<()> {
    ...
    loop {
        let msg = wait_for_event(&test.peregrine, |_| true).await;
        match msg {
...
async fn unified_exec_network_denial_emits_failed_background_end_event() -> Result<()> {
    ...
    let args = json!({
        "cmd": "python3 -c \"import os, socket, time, urllib.parse; time.sleep(0.3); proxy = urllib.parse.urlparse(os.environ['HTTP_PROXY']); sock = socket.create_connection((proxy.hostname, proxy.port), timeout=2); sock.sendall(b'GET http://peregrine-network-denied.invalid/ HTTP/1.1\\r\\nHost: peregrine-network-denied.invalid\\r\\n\\r\\n'); sock.recv(1024); time.sleep(5)\"",
        "yield_time_ms": 50,
    });
    let response_mock =
...
async fn unified_exec_short_lived_network_denial_emits_failed_end_event() -> Result<()> {
    ...
    let args = json!({
        "cmd": "python3 -c \"import os, socket, urllib.parse; proxy = urllib.parse.urlparse(os.environ['HTTP_PROXY']); sock = socket.create_connection((proxy.hostname, proxy.port), timeout=2); sock.sendall(b'GET http://peregrine-short-network-denied.invalid/ HTTP/1.1\\r\\nHost: peregrine-short-network-denied.invalid\\r\\n\\r\\n'); sock.recv(1024)\"",
        "yield_time_ms": 1000,
    });
    let response_mock =
        mount_unified_exec_network_denial_responses(&server, call_id, &args).await?;
...
) -> Result<(TestPeregrine, PermissionProfile)> {
...
async fn wait_for_unified_exec_end(
...
) -> (peregrine_types::protocol::ExecCommandEndEvent, bool) {
    ...
    let end_event = loop {
        ...
        let event = match tokio::time::timeout(remaining, test.peregrine.next_event()).await {
...
async fn unified_exec_emits_one_begin_and_one_end_event() -> Result<()> {
    ...
    let mut task_completed = false;
    loop {
        let event_msg = wait_for_event(&test.peregrine, |_| true).await;
        match event_msg {
...
async fn unified_exec_timeout_and_followup_poll() -> Result<()> {
    ...
    loop {
        let event = test.peregrine.next_event().await.expect("event");
