...
use peregrine_types::dynamic_tools::DynamicToolCallOutputContentItem;
...
async fn run_code_mode_turn(
    ...
    prompt: &str,
    code: &str,
) -> Result<(TestPeregrine, ResponseMock)> {
...
async fn run_code_mode_turn_with_rmcp_config(
    ...
    let test = builder.build(server).await?;
    wait_for_mcp_server(&test.peregrine, "rmcp").await?;
...
const tool = ALL_TOOLS.find(
  ({ name }) => name === "mcp__peregrine_apps__calendar_timezone_option_99"
);
if (!tool) {
...
"#,
        visible_tool_name = "mcp__peregrine_apps__calendar_timezone_option_99",
...
async fn code_mode_can_call_hidden_dynamic_tools() -> Result<()> {
    ...
    let code = r#"
const tool = ALL_TOOLS.find(({ name }) => name === "peregrine_app__hidden_dynamic_tool");
const out = await tools.peregrine_app__hidden_dynamic_tool({ city: "Paris" });
text(
  JSON.stringify({
    name: tool?.name ?? null,
...
"#;
    ...
    let turn_id = wait_for_event_match(&test.peregrine, |event| match event {
        EventMsg::TurnStarted(event) => Some(event.turn_id.clone()),
        _ => None,
    })
    .await;
    let request = wait_for_event_match(&test.peregrine, |event| match event {
        EventMsg::DynamicToolCallRequest(request) => Some(request.clone()),
        _ => None,
    })
    .await;
    assert_eq!(request.namespace.as_deref(), Some("peregrine_app"));
    ...
    let peregrine_for_wait = std::sync::Arc::clone(&test.peregrine);
    let turn_complete = tokio::spawn(async move {
        wait_for_event(&peregrine_for_wait, |event| match event {
            EventMsg::TurnComplete(event) => event.turn_id == expected_turn_id,
            _ => false,
        })
        .await
    });
    ...
    turn_complete
        .await
        .expect("turn complete waiter should join");
    ...
    assert_eq!(
        parsed.get("name"),
        Some(&Value::String(
            "peregrine_app__hidden_dynamic_tool".to_string()
        ))
    );
    assert_eq!(
        parsed.get("out"),
        ...
        parsed
            ...
            .is_some_and(|description| {
                description.contains("A hidden dynamic tool.")
                    ...
                    && description.contains("peregrine_app__hidden_dynamic_tool(args:")
            })
    );
