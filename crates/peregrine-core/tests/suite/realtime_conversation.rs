...
use peregrine_types::protocol::RealtimeConversationVersion;
use peregrine_types::protocol::RealtimeEvent;
use peregrine_types::protocol::RealtimeNoopRequested;
...
const STARTUP_CONTEXT_HEADER: &str = "Startup context from Peregrine.";
...
const REALTIME_CONVERSATION_TEST_SUBPROCESS_ENV_VAR: &str =
    "PEREGRINE_REALTIME_CONVERSATION_TEST_SUBPROCESS";
...
async fn seed_recent_thread(
    ...
    let db = test.peregrine.state_db().context("state db enabled")?;
...
async fn conversation_start_audio_text_close_round_trip() -> Result<()> {
    ...
    let started = wait_for_event_match(&test.peregrine, |msg| match msg {
    ...
    let session_updated = wait_for_event_match(&test.peregrine, |msg| match msg {
    ...
    let audio_out = wait_for_event_match(&test.peregrine, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
    ...
    test.peregrine.submit(Op::RealtimeConversationClose).await?;
    let closed = wait_for_event_match(&test.peregrine, |msg| match msg {
...
async fn conversation_webrtc_start_posts_generated_session() -> Result<()> {
    ...
    let created = wait_for_event_match(&test.peregrine, |msg| match msg {
    ...
    assert_eq!(
        request
            ...
            .get("content-type")
            .and_then(|value| value.to_str().ok()),
        Some("multipart/form-data; boundary=peregrine-realtime-call-boundary")
    );
    ...
    assert_eq!(
        body,
        format!(
            "--peregrine-realtime-call-boundary\r\n\
...
async fn conversation_webrtc_close_while_sideband_connecting_drops_pending_join() -> Result<()> {
    ...
    let sdp = wait_for_event_match(&test.peregrine, |msg| match msg {
    ...
    let stale_event = timeout(Duration::from_millis(700), async {
        wait_for_event_match(&test.peregrine, |msg| match msg {
            EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
...
async fn conversation_webrtc_sideband_connect_failure_closes_with_error() -> Result<()> {
    ...
    let err = wait_for_event_match(&test.peregrine, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::Error(message),
        }) => Some(message.clone()),
        _ => None,
...
async fn conversation_start_uses_openai_env_key_fallback_with_chatgpt_auth() -> Result<()> {
    ...
    test.peregrine.submit(Op::RealtimeConversationClose).await?;
    let _closed = wait_for_event_match(&test.peregrine, |msg| match msg {
        EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
        _ => None,
    })
    .await;
...
async fn conversation_audio_before_start_emits_error() -> Result<()> {
    ...
    .await;
    assert_eq!(err.codex_error_info, Some(PeregrineErrorInfo::BadRequest));
    assert_eq!(err.message, "conversation is not running");
...
async fn conversation_second_start_replaces_runtime() -> Result<()> {
    ...
    test.peregrine
        ...
        .await?;
    let _ = wait_for_event_match(&test.peregrine, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::AudioOut(frame),
...
async fn conversation_rejects_voice_for_wrong_realtime_version() -> Result<()> {
    ...
    let error = wait_for_event_match(&test.peregrine, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::Error(message),
        }) => Some(message.clone()),
        _ => None,
...
async fn conversation_startup_context_current_thread_selects_many_turns_by_budget() -> Result<()> {
    ...
    let history = user_turns
        ...
        .collect::<Vec<_>>();
    test.peregrine.shutdown_and_wait().await?;
    let resumed_thread = test
        .thread_manager
        .resume_thread_with_history(
            test.config.clone(),
            InitialHistory::Forked(history),
            auth_manager_from_auth(PeregrineAuth::from_api_key("dummy")),
...
async fn conversation_startup_context_falls_back_to_workspace_map() -> Result<()> {
    ...
    let test = builder.build_with_websocket_server(&startup_server).await?;
    fs::create_dir_all(test.workspace_path("peregrine-rs/core"))?;
    ...
    assert!(startup_context.contains("peregrine-rs/"));
...
async fn conversation_user_text_turn_is_sent_to_realtime_when_active() -> Result<()> {
    ...
    wait_for_event(&test.peregrine, |event| {
