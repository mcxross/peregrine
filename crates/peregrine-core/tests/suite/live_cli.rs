...
//! `just test -p peregrine-core --test all --run-ignored only live_cli` provided they set a valid
//! `OPENAI_API_KEY`.
...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let peregrine_home = home.path().join(".peregrine");
    ...
    let mut cmd = Command::new(codex_utils_cargo_bin::cargo_bin("peregrine-rs").unwrap());
    ...
    let mut child = cmd.spawn().expect("failed to spawn peregrine-rs");
