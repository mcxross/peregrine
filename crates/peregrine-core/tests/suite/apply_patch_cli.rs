...
#[cfg(target_os = "linux")]
use codex_sandboxing::landlock::CODEX_LINUX_SANDBOX_ARG0 as PEREGRINE_LINUX_SANDBOX_ARG0;
...
pub async fn apply_patch_harness() -> Result<TestPeregrineHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestPeregrineBuilder) -> TestPeregrineBuilder,
    ...
    Box::pin(TestPeregrineHarness::with_remote_env_builder(builder)).await
}

async fn submit_without_wait(harness: &TestPeregrineHarness, prompt: &str) -> Result<()> {
...
async fn apply_patch_cli_uses_peregrine_self_exe_with_linux_sandbox_helper_alias() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let harness = apply_patch_harness().await?;
    let peregrine_linux_sandbox_exe = harness
        .test()
        .config
        .peregrine_linux_sandbox_exe
        .as_ref()
        .expect("linux test config should include peregrine-linux-sandbox helper");
    assert_eq!(
        peregrine_linux_sandbox_exe
            .file_name()
            .and_then(|name| name.to_str()),
        Some(PEREGRINE_LINUX_SANDBOX_ARG0),
    );
...
async fn apply_patch_custom_tool_streaming_emits_updated_changes() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some(&peregrine_types::protocol::FileChange::Add {
            content: String::new(),
        })
    );
    assert_eq!(
