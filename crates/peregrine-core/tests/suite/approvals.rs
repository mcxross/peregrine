...
use peregrine_types::protocol::ExecPolicyAmendment;
...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestPeregrine) -> (PathBuf, String) {
        match self {
    ...
    async fn prepare(
        ...
        match self {
            ...
            ActionKind::ApplyPatchShell { target, content } => {
                ...
                // Bazel may need to launch the configured Peregrine helper binary
...
impl Expectation {
    fn verify(&self, test: &TestPeregrine, result: &CommandResult) -> Result<()> {
        match self {
...
async fn expect_exec_approval(
    ...
    let event = wait_for_event(&test.peregrine, |event| {
        matches!(
            event,
...
async fn wait_for_completion_without_approval(test: &TestPeregrine) {
    let event = wait_for_event(&test.peregrine, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_completion(test: &TestPeregrine) {
...
async fn wait_for_spawned_thread(test: &TestPeregrine) -> Result<Arc<PeregrineThread>> {
...
async fn invalid_requested_prefix_rule_falls_back_for_compound_command() -> Result<()> {
    ...
    let command = "touch /tmp/peregrine-fallback-rule-test.txt && echo hello > /tmp/peregrine-fallback-rule-test.txt";
    let event = shell_event_with_prefix_rule(
        call_id,
        command,
        /*timeout_ms*/ 1_000,
...
"#,
    ...
    let fetch_command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://peregrine-network-test.invalid', timeout=30).read().decode(errors='replace'))""#
        .to_string();
