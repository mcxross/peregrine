...
const FIXED_CWD: &str = "/tmp/peregrine_remote_compaction_parity_workspace";
...
impl AuthCase {
    fn build(self) -> PeregrineAuth {
        match self {
            AuthCase::ChatGpt => PeregrineAuth::create_dummy_chatgpt_auth_for_testing(),
            AuthCase::ApiKey => PeregrineAuth::from_api_key("dummy"),
        }
    }
}
...
async fn run_manual_hook_session(mode: Mode) -> Result<Value> {
    ...
    let home = harness.test().peregrine_home_path();
...
async fn build_auto_harness(mode: Mode) -> Result<TestPeregrineHarness> {
    build_harness_inner(
        mode,
        RunSettings::default(),
        /*hooks*/ false,
...
async fn build_harness_inner(
    ...
    TestPeregrineHarness::with_builder(builder.with_config(move |config| {
        ...
        // These parity tests exercise mocked Responses compaction paths only.
        // Disable host-owned apps MCP so dummy ChatGPT auth does not trigger
        // live connector startup while building the model tool list.
        config
            .features
...
fn rollout_path(harness: &TestPeregrineHarness) -> PathBuf {
    harness
        .test()
        .session_configured
        .rollout_path
...
async fn capture_from_requests(
    mode: Mode,
    codex: &peregrine_core::PeregrineThread,
    rollout_path: &Path,
...
fn normalize_tmp_prefix_before_marker(text: &mut String, marker: &str) {
    ...
    while let Some(relative_marker_index) = text[search_start..].find(marker) {
        ...
        if let Some(start_index) = start {
            text.replace_range(start_index..marker_index, "<PEREGRINE_HOME>");
            search_start = start_index + "<PEREGRINE_HOME>".len() + marker.len();
        } else {
...
fn normalize_string_rewrites_linux_temp_skill_paths() {
    ...
    assert_eq!(
        text,
        "file: <PEREGRINE_HOME>/skills/.system/imagegen/SKILL.md and \
         <PEREGRINE_HOME>/skills/custom/SKILL.md"
    );
}
