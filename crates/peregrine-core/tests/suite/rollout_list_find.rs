...
use peregrine_core::RolloutRecorder;
use peregrine_core::RolloutRecorderParams;
use peregrine_core::config::ConfigBuilder;
use peregrine_core::find_archived_thread_path_by_id_str;
use peregrine_core::find_thread_meta_by_name_str;
...
fn write_minimal_rollout_with_id_in_subdir(
    ...
    let sessions = peregrine_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(peregrine_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(peregrine_home, "sessions", id)
}
...
async fn upsert_thread_metadata(
    ...
    let runtime = StateRuntime::init(peregrine_home.to_path_buf(), "test-provider".to_string())
        .await
        .unwrap();
    runtime
    ...
    builder.cwd = peregrine_home.to_path_buf();
...
#[tokio::test]
async fn find_handles_gitignore_covering_peregrine_home_directory() {
    let repo = TempDir::new().unwrap();
    let peregrine_home = repo.path().join(".peregrine");
    std::fs::create_dir_all(&peregrine_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".peregrine/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&peregrine_home, id);

    let found =
        find_thread_path_by_id_str(&peregrine_home, &id.to_string(), /*state_db_ctx*/ None)
            .await
            .unwrap();
