...
use peregrine_core::compact::SUMMARIZATION_PROMPT;
...
use peregrine_types::protocol::HookEventName;
use peregrine_types::protocol::HookRunStatus;
...
async fn assert_compaction_uses_turn_lifecycle_id(
    codex: &std::sync::Arc<peregrine_core::PeregrineThread>,
) {
...
async fn summarize_context_three_requests_and_instructions() {
    ...
    // Build config pointing to the mock server and spawn Peregrine.
    ...
    // Shut down Peregrine to flush rollout entries before inspecting the file.
...
async fn manual_compact_emits_api_and_local_token_usage_events() {
    ...
    let codex = builder.build(&server).await.unwrap().peregrine;
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        peregrine_types::models::ResponseItem::Message {
            id: None,
        ...
        peregrine_types::models::ResponseItem::Compaction {
    ...
    wait_for_event(&resumed.peregrine, |event| {
...
async fn pre_sampling_compact_runs_on_switch_to_smaller_context_model() {
    ...
    assert_compaction_uses_turn_lifecycle_id(&test.peregrine).await;
...
async fn pre_sampling_compact_runs_after_resume_and_switch_to_smaller_model() {
    ...
    wait_for_event(&initial.peregrine, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    assert_compaction_uses_turn_lifecycle_id(&resumed.peregrine).await;
...
async fn manual_compact_twice_preserves_latest_user_messages() {
    ...
    assert_eq!(
        ...
        requests[1].header("x-peregrine-window-id").as_deref()
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        requests[2].header("x-peregrine-window-id").as_deref()
    );
    assert_ne!(
