...
fn disabled_user_turn(test: &TestPeregrine, items: Vec<UserInput>, model: String) -> Op {
...
async fn view_image_tool_attaches_local_image() -> anyhow::Result<()> {
    ...
    wait_for_event_with_timeout(
        ...
        |event| match event {
            EventMsg::ItemStarted(event) => {
                if matches!(&event.item, peregrine_types::items::TurnItem::ImageView(_)) {
    ...
    match item_started.expect("view image item started event emitted") {
        peregrine_types::items::TurnItem::ImageView(item) => {
...
async fn view_image_routes_to_selected_remote_environment() -> anyhow::Result<()> {
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/peregrine-view-image-routing-{}",
