...
use peregrine_types::openai_models::ModelInstructionsVariables;
use peregrine_types::openai_models::ModelMessages;
...
async fn personality_does_not_mutate_base_instructions_without_template() {
    let peregrine_home = TempDir::new().expect("create temp dir");
    ...
    let model_info = peregrine_core::test_support::construct_model_info_offline("gpt-5.4", &config);
    assert_eq!(
...
async fn base_instructions_override_disables_personality_template() {
    ...
    let model_info =
        peregrine_core::test_support::construct_model_info_offline("gpt-5.3-codex", &config);
...
async fn user_turn_personality_some_adds_update_message() -> anyhow::Result<()> {
    ...
    .await;
    let mut builder = test_codex()
        .with_model("exp-peregrine-personality")
        .with_config(|config| {
            config
                .features
                .enable(Feature::Personality)
...
async fn remote_model_friendly_personality_instructions_with_feature() -> anyhow::Result<()> {
    ...
    let remote_slug = "peregrine-remote-default-personality";
...
async fn user_turn_personality_remote_model_template_includes_update_message() -> anyhow::Result<()>
{
    ...
    let remote_slug = "peregrine-remote-personality";
