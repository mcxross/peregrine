...
async fn window_id_advances_after_compact_persists_on_resume_and_resets_on_fork() -> Result<()> {
    ...
    let initial_thread = Arc::clone(&initial.peregrine);
    ...
    let resumed = builder
        ...
        .await?;
    submit_user_turn(&resumed.peregrine, "after resume").await?;
    shutdown_thread(&resumed.peregrine).await?;
...
async fn submit_user_turn(codex: &Arc<PeregrineThread>, text: &str) -> Result<()> {
    codex
        .submit(Op::UserInput {
            environments: None,
...
async fn submit_compact_turn(codex: &Arc<PeregrineThread>) -> Result<()> {
...
async fn shutdown_thread(codex: &Arc<PeregrineThread>) -> Result<()> {
...
fn window_id_parts(request: &ResponsesRequest) -> (String, u64) {
    let window_id = request
        ...
        .expect("missing x-peregrine-window-id header");
