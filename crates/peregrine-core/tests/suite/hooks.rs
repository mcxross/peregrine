...
use peregrine_core::config::Constrained;
use peregrine_types::items::parse_hook_prompt_fragment;
...
async fn stop_hook_can_block_multiple_times_in_same_turn() -> Result<()> {
    ...
    let hook_inputs = read_stop_hook_inputs(test.peregrine_home_path())?;
    ...
    let rollout_path = test.peregrine.rollout_path().expect("rollout path");
...
async fn session_start_hook_sees_materialized_transcript_path() -> Result<()> {
    ...
    let hook_inputs = read_session_start_hook_inputs(test.peregrine_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
    assert_eq!(
        hook_inputs[0]
            .get("transcript_path")
...
async fn session_start_runs_before_user_prompt_submit_on_first_turn() -> Result<()> {
    ...
    let hook_inputs = read_hook_order_inputs(test.peregrine_home_path())?;
    assert_eq!(
        hook_inputs
            .iter()
...
async fn resumed_thread_runs_resume_then_compact_session_start_hooks() -> Result<()> {
    ...
    let hook_inputs = read_session_start_hook_inputs(resumed.peregrine_home_path())?;
    assert_eq!(
        hook_inputs
            .iter()
...
async fn blocked_user_prompt_submit_persists_additional_context_for_next_turn() -> Result<()> {
    ...
    let hook_inputs = read_user_prompt_submit_hook_inputs(test.peregrine_home_path())?;
...
"#,
    ...
    let command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://peregrine-network-test.invalid', timeout=2).read().decode(errors='replace'))""#;
    let args = serde_json::json!({ "command": command });
    ...
    assert!(
        timeout(
            ...
            wait_for_event(&test.peregrine, |event| matches!(
                event,
    ...
    assert_single_permission_request_hook_input(
        test.peregrine_home_path(),
        command,
        Some("network-access http://peregrine-network-test.invalid:80"),
    )?;

    test.peregrine.submit(Op::Shutdown {}).await?;
    wait_for_event(&test.peregrine, |event| {
...
async fn pre_tool_use_blocks_shell_command_before_execution() -> Result<()> {
    ...
    let hook_inputs = read_pre_tool_use_hook_inputs(test.peregrine_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
...
async fn plugin_pre_tool_use_blocks_shell_command_before_execution() -> Result<()> {
    ...
    fs::create_dir_all(plugin_root.join(".peregrine-plugin"))
...
"#,
  ...
  "hooks": {
    ...
    let plugin_hook_sources = vec![PluginHookSource {
        ...
        hooks: serde_json::from_str::<peregrine_config::codex_compat::HooksFile>(plugin_hooks_json)
...
async fn pre_tool_use_merges_hooks_json_and_config_toml() -> Result<()> {
    ...
    let json_hook_inputs = read_pre_tool_use_hook_inputs(test.peregrine_home_path())?
        .into_iter()
        .map(|hook_input| {
            serde_json::json!({
...
async fn post_tool_use_records_additional_context_for_shell_command() -> Result<()> {
    ...
    let hook_inputs = read_post_tool_use_hook_inputs(test.peregrine_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
...
async fn post_tool_use_blocks_when_exec_session_completes_via_write_stdin() -> Result<()> {
    ...
    let pre_hook_inputs = read_pre_tool_use_hook_inputs(test.peregrine_home_path())?;
    ...
    let post_hook_inputs = read_post_tool_use_hook_inputs(test.peregrine_home_path())?;
