...
const REMOTE_MODEL_SLUG: &str = "peregrine-test";
...
async fn remote_models_get_model_info_uses_longest_matching_prefix() -> Result<()> {
    ...
    let auth = PeregrineAuth::create_dummy_chatgpt_auth_for_testing();
    let provider = ModelProviderInfo {
        base_url: Some(format!("{}/v1", server.uri())),
        ..built_in_model_providers(/* openai_base_url */ /*openai_base_url*/ None)["openai"].clone()
    };
    let manager = peregrine_core::test_support::models_manager_with_provider(
        peregrine_home.path().to_path_buf(),
        peregrine_core::test_support::auth_manager_from_auth(auth),
        provider,
    );
...
async fn remote_models_truncation_policy_without_override_preserves_remote() -> Result<()> {
    ...
    let slug = "peregrine-test-truncation-policy";
...
async fn remote_models_truncation_policy_with_tool_output_override() -> Result<()> {
    ...
    let slug = "peregrine-test-truncation-override";
    let remote_model = test_remote_model_with_policy(
        slug,
        ModelVisibility::List,
        /*priority*/ 1,
...
async fn remote_models_hide_picker_only_models() -> Result<()> {
    ...
    let hidden = available
        .iter()
        .find(|model| model.model == "peregrine-auto-balanced")
...
fn bundled_default_model_slug() -> String {
    peregrine_core::test_support::all_model_presets()
        .iter()
