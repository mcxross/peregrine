...
use peregrine_api::WS_REQUEST_HEADER_TRACEPARENT_CLIENT_METADATA_KEY;
use peregrine_api::WS_REQUEST_HEADER_TRACESTATE_CLIENT_METADATA_KEY;
use peregrine_core::ModelClient;
use peregrine_core::ModelClientSession;
use peregrine_core::Prompt;
use peregrine_core::ResponseEvent;
use peregrine_core::X_RESPONSESAPI_INCLUDE_TIMING_METRICS_HEADER;
...
async fn responses_websocket_streams_request() {
    ...
    assert_eq!(
        body["client_metadata"]["x-peregrine-installation-id"].as_str(),
        Some(TEST_INSTALLATION_ID)
    );
    let stream_request_start_ms = body["client_metadata"]
        [X_PEREGRINE_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY]
        .as_str()
...
async fn responses_websocket_sends_response_processed_after_remote_compaction_v2() {
    ...
    wait_for_event(&test.peregrine, |msg| {
...
async fn responses_websocket_usage_limit_error_emits_rate_limit_event() {
    ...
    let usage_limit_error = json!({
        ...
        "headers": {
            "x-peregrine-primary-used-percent": "100.0",
            "x-peregrine-secondary-used-percent": "87.5",
            "x-peregrine-primary-over-secondary-limit-percent": "95.0",
            "x-peregrine-primary-window-minutes": "15",
            "x-peregrine-secondary-window-minutes": "60"
        }
    });
    ...
    let token_event = wait_for_event(&test.peregrine, |msg| {
    ...
    let error_event =
        wait_for_event(&test.peregrine, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn responses_websocket_forwards_turn_metadata_on_initial_and_incremental_create() {
    ...
    assert_eq!(first["type"].as_str(), Some("response.create"));
    assert_eq!(
        first["client_metadata"]["x-peregrine-turn-metadata"].as_str(),
        Some(first_turn_metadata)
    );
    assert_eq!(second["type"].as_str(), Some("response.create"));
    assert_eq!(second["previous_response_id"].as_str(), Some("resp-1"));
    assert_eq!(
        second["client_metadata"]["x-peregrine-turn-metadata"].as_str(),
        Some(enriched_turn_metadata)
    );
...
async fn responses_websocket_preserves_custom_turn_metadata_fields() {
    ...
    assert_eq!(body["type"].as_str(), Some("response.create"));
    assert_eq!(
        body["client_metadata"]["x-peregrine-turn-metadata"]
            .as_str()
...
async fn websocket_harness_with_provider_options(
    ...
    let model_info = peregrine_core::test_support::construct_model_info_offline(MODEL, &config);
