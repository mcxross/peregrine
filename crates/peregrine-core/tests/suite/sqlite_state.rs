...
async fn new_thread_is_recorded_in_state_db() -> Result<()> {
    ...
    let db = test.peregrine.state_db().expect("state db enabled");
    assert!(
        !rollout_path.exists(),
...
async fn backfill_scans_existing_rollouts() -> Result<()> {
    ...
    let mut builder = test_codex()
        .with_pre_build_hook(move |peregrine_home| {
            let rollout_path = peregrine_home.join(&rollout_rel_path_for_hook);
            ...
            let session_meta_line = SessionMetaLine {
                meta: SessionMeta {
                    ...
                    cwd: peregrine_home.to_path_buf(),
    ...
    let db_path = codex_state::state_db_path(test.config.sqlite_home.as_path());
    let rollout_path = test.config.peregrine_home.join(&rollout_rel_path);
...
async fn mcp_call_marks_thread_memory_mode_polluted_when_configured() -> Result<()> {
    ...
    let test = builder.build(&server).await?;
    wait_for_mcp_server(&test.peregrine, server_name).await?;
    let db = test.peregrine.state_db().expect("state db enabled");
    let thread_id = test.session_configured.thread_id;
    ...
    .await;
    wait_for_event_match(&test.peregrine, |event| match event {
