...
use peregrine_core::spawn::PEREGRINE_SANDBOX_ENV_VAR;
...
fn skip_test() -> bool {
    if std::env::var(PEREGRINE_SANDBOX_ENV_VAR) == Ok("seatbelt".to_string()) {
        eprintln!("{PEREGRINE_SANDBOX_ENV_VAR} is set to 'seatbelt', skipping test.");
        return true;
    }
