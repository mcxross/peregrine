...
use peregrine_core::SESSIONS_SUBDIR;
use peregrine_core::personality_migration::PERSONALITY_MIGRATION_FILENAME;
use peregrine_core::personality_migration::PersonalityMigrationStatus;
use peregrine_core::personality_migration::maybe_migrate_personality;
...
async fn read_config_toml(peregrine_home: &Path) -> io::Result<ConfigToml> {
    let contents = tokio::fs::read_to_string(peregrine_home.join("config.toml")).await?;
    toml::from_str(&contents).map_err(|err| io::Error::new(io::ErrorKind::InvalidData, err))
}

async fn write_session_with_user_event(peregrine_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = peregrine_home
...
async fn write_archived_session_with_user_event(peregrine_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = peregrine_home.join(ARCHIVED_SESSIONS_SUBDIR);
    write_rollout_with_user_event(&dir, thread_id).await
}

async fn write_session_with_meta_only(peregrine_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = peregrine_home
        .join(SESSIONS_SUBDIR)
        .join("2025")
