...
//! request payload that Peregrine would send to the model and assert that the
...
use peregrine_core::spawn::PEREGRINE_SANDBOX_NETWORK_DISABLED_ENV_VAR;
...
fn network_disabled() -> bool {
    std::env::var(PEREGRINE_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
async fn start_test_conversation(
    ...
    (test.home, test.config, test.thread_manager, test.peregrine)
}

async fn user_turn(conversation: &Arc<PeregrineThread>, text: &str) {
    conversation
        .submit(Op::UserInput {
            environments: None,
...
async fn compact_conversation(conversation: &Arc<PeregrineThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<PeregrineThread>) -> std::path::PathBuf {
    conversation.rollout_path().expect("rollout path")
}

async fn shutdown_conversation(conversation: &Arc<PeregrineThread>) {
    conversation
        .shutdown_and_wait()
        .await
