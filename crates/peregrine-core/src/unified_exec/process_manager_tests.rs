...
fn unified_exec_env_injects_defaults() {
    ...
    let expected = HashMap::from([
        ...
        ("PEREGRINE_CI".to_string(), "1".to_string()),
    ]);
...
fn env_overlay_for_exec_server_keeps_runtime_changes_only() {
    ...
    let request_env = HashMap::from([
        ...
        ("SHELL_SET".to_string(), "policy".to_string()),
        ("PEREGRINE_THREAD_ID".to_string(), "thread-1".to_string()),
        (
            "PEREGRINE_SANDBOX_NETWORK_DISABLED".to_string(),
            "1".to_string(),
        ),
    ]);
...
fn exec_server_params_use_env_policy_overlay_contract() {
    ...
    let file_system_sandbox_policy =
        peregrine_types::permissions::FileSystemSandboxPolicy::unrestricted();
    let network_sandbox_policy = peregrine_types::permissions::NetworkSandboxPolicy::Restricted;
    let permission_profile = peregrine_types::models::PermissionProfile::Disabled;
    let request = ExecRequest {
        ...
        exec_server_env_config: Some(ExecServerEnvConfig {
            policy: codex_exec_server::ExecEnvPolicy {
                inherit: peregrine_types::config_types::ShellEnvironmentPolicyInherit::Core,
...
async fn network_denial_fallback_message_names_sandbox_network_proxy() {
    ...
    assert_eq!(
        message,
        "Network access was denied by the Peregrine sandbox network proxy."
    );
}
...
async fn failed_initial_end_for_unstored_process_uses_fallback_output() {
    ...
    let peregrine_types::protocol::EventMsg::ExecCommandEnd(end_event) = event.msg else {
        panic!("expected ExecCommandEnd event");
    };
    assert_eq!(end_event.call_id, "call-unified-denied");
    assert_eq!(
        end_event.status,
        peregrine_types::protocol::ExecCommandStatus::Failed
    );
