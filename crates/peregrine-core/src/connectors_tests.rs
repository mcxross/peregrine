...
use peregrine_config::AppRequirementToml;
use peregrine_config::AppToolRequirementToml;
use peregrine_config::AppToolsRequirementsToml;
...
use peregrine_config::types::AppsDefaultConfig;
...
fn accessible_connectors_from_mcp_tools_preserves_description() {
    let mcp_tools = vec![ToolInfo {
        ...
        callable_namespace: "mcp__peregrine_apps__calendar".to_string(),
...
async fn cloud_requirements_disable_connector_applies_without_user_apps_table() {
    let peregrine_home = tempdir().expect("tempdir should succeed");
    std::fs::write(peregrine_home.path().join(CONFIG_TOML_FILE), "").expect("write config");
...
async fn local_requirements_disable_connector_overrides_user_apps_config() {
    ...
    let config_toml_path =
        AbsolutePathBuf::try_from(peregrine_home.path().join(CONFIG_TOML_FILE)).expect("abs path");
    let mut config = ConfigBuilder::default()
        .peregrine_home(peregrine_home.path().to_path_buf())
        .fallback_cwd(Some(peregrine_home.path().to_path_buf()))
        .build()
