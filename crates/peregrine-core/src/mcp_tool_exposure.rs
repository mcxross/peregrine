...
pub(crate) fn build_mcp_tool_exposure(
    ...
    let mut deferred_tools = filter_non_peregrine_apps_mcp_tools_only(all_mcp_tools);
    if let Some(connectors) = connectors {
        deferred_tools.extend(filter_peregrine_apps_mcp_tools(
            all_mcp_tools,
            connectors,
            config,
        ));
...
fn filter_non_peregrine_apps_mcp_tools_only(mcp_tools: &[McpToolInfo]) -> Vec<McpToolInfo> {
    mcp_tools
        .iter()
        .filter(|tool| {
...
fn filter_peregrine_apps_mcp_tools(
    ...
    mcp_tools
        ...
        .filter(|tool| {
            ...
            allowed.contains(connector_id)
                && connectors::peregrine_app_tool_is_enabled(config, tool)
        })
        .cloned()
        .collect()
}
