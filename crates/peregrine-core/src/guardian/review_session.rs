...
use crate::peregrine_delegate::run_peregrine_thread_interactive;
...
struct GuardianReviewSessionReuseKey {
    ...
    peregrine_linux_sandbox_exe: Option<PathBuf>,
...
impl GuardianReviewSessionReuseKey {
    fn from_spawn_config(spawn_config: &Config) -> Self {
        Self {
            ...
            peregrine_linux_sandbox_exe: spawn_config.peregrine_linux_sandbox_exe.clone(),
...
impl GuardianReviewSession {
    async fn shutdown(&self) {
        ...
        let _ = self.peregrine.shutdown_and_wait().await;
    }
    ...
    async fn refresh_last_committed_fork_snapshot(&self) {
        match load_rollout_items_for_fork(&self.peregrine.session).await {
...
impl GuardianReviewSessionManager {
    pub(crate) async fn trunk_rollout_path(&self) -> Option<PathBuf> {
        ...
        trunk.peregrine.session.ensure_rollout_materialized().await;
        match trunk.peregrine.session.current_rollout_path().await {
            Ok(path) => path,
            Err(err) => {
    ...
    #[cfg(test)]
    pub(crate) async fn cache_for_test(&self, codex: Peregrine) {
    ...
    #[cfg(test)]
    pub(crate) async fn register_ephemeral_for_test(&self, codex: Peregrine) {
        let reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
            codex.session.get_config().await.as_ref(),
        );
        self.state
    ...
    pub(crate) async fn send_trunk_event_raw_for_test(&self, event: Event) {
        ...
        trunk.peregrine.session.send_event_raw(event).await;
    }
...
async fn spawn_guardian_review_session(
    ...
    let codex = Box::pin(run_peregrine_thread_interactive(
        spawn_config,
...
async fn run_review_on_session(
    ...
    let mut analytics_result = GuardianReviewAnalyticsResult::from_session(
        review_session.peregrine.session.conversation_id.to_string(),
        guardian_session_kind,
        params.model.clone(),
        ...
        Box::pin(async {
            params
                ...
                .sync_session_approved_hosts_to(
                    &review_session.peregrine.session.services.network_approval,
                )
                .await;
    ...
    let submit_result = run_before_review_deadline(
        deadline,
        params.external_cancel.as_ref(),
        Box::pin(review_session.peregrine.submit(Op::UserInput {
    ...
    if matches!(outcome.0, GuardianReviewSessionOutcome::Completed(_)) {
        if outcome.2
            && let Some(total_token_usage) =
                review_session.peregrine.session.total_token_usage().await
        {
...
async fn wait_for_guardian_review(
    ...
    loop {
        tokio::select! {
            ...
            event = review_session.peregrine.next_event() => {
                match event {
...
async fn interrupt_and_drain_turn(codex: &Peregrine, expected_turn_id: &str) -> anyhow::Result<()> {
...
mod tests {
    ...
    async fn interrupt_and_drain_turn_ignores_prior_turn_completion() {
        ...
        interrupt_and_drain_turn(&review_session.peregrine, "current-turn")
            .await
            .expect("drain current turn");

        assert!(review_session.peregrine.rx_event.try_recv().is_err());
    }
}
