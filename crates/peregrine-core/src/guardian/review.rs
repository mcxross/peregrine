...
use peregrine_types::protocol::GuardianAssessmentDecisionSource;
...
pub(crate) fn is_guardian_reviewer_source(
    session_source: &peregrine_types::protocol::SessionSource,
) -> bool {
    matches!(
        session_source,
        peregrine_types::protocol::SessionSource::SubAgent(SubAgentSource::Other(name))
    ...
    let (guardian_model, guardian_reasoning_effort) = if let Some(preset) = preferred_model {
        let reasoning_effort = preferred_reasoning_effort(
            preset.supported_reasoning_efforts.iter().any(|effort| {
                effort.effort == peregrine_types::openai_models::ReasoningEffort::Low
            }),
        ...
        let reasoning_effort = preferred_reasoning_effort(
            turn.model_info
                ...
                .iter()
                .any(|preset| {
                    preset.effort == peregrine_types::openai_models::ReasoningEffort::Low
                }),
