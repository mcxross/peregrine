...
use peregrine_api::RealtimeAudioFrame;
use peregrine_api::RealtimeEvent;
use peregrine_api::RealtimeEventParser;
use peregrine_api::RealtimeSessionConfig;
use peregrine_api::RealtimeSessionMode;
use peregrine_api::RealtimeWebsocketClient;
use peregrine_api::RealtimeWebsocketEvents;
use peregrine_api::RealtimeWebsocketWriter;
...
use peregrine_types::protocol::RealtimeConversationClosedEvent;
use peregrine_types::protocol::RealtimeConversationRealtimeEvent;
use peregrine_types::protocol::RealtimeConversationSdpEvent;
use peregrine_types::protocol::RealtimeConversationStartedEvent;
use peregrine_types::protocol::RealtimeHandoffRequested;
...
impl RealtimeConversationManager {
    ...
    async fn start(&self, start: RealtimeStart) -> PeregrineResult<RealtimeStartOutput> {
        let previous_state = {
    ...
    async fn start_inner(&self, start: RealtimeStart) -> PeregrineResult<RealtimeStartOutput> {
        let RealtimeStart {
            api_provider,
            extra_headers,
            session_config,
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> PeregrineResult<()> {
        let sender = {
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(PeregrineErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }

    pub(crate) async fn text_in(&self, text: String) -> PeregrineResult<()> {
        let sender = {
            let guard = self.state.lock().await;
            guard
                .as_ref()
        ...
        sender
            .send(text)
            .await
            .map_err(|_| PeregrineErr::InvalidRequest("conversation is not running".to_string()))?;
        Ok(())
    }

    pub(crate) async fn handoff_out(&self, output_text: String) -> PeregrineResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn handoff_complete(&self) -> PeregrineResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn shutdown(&self) -> PeregrineResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> PeregrineResult<PreparedRealtimeConversationStart> {
    ...
    let mut api_provider =
        peregrine_model_provider::api_provider_from_info(&provider, Some(AuthMode::ApiKey))?;
...
) -> PeregrineResult<RealtimeSessionConfig> {
    let config = sess.get_config().await;
...
fn realtime_api_key(
    ...
    Err(PeregrineErr::InvalidRequest(
...
fn realtime_request_headers(
    ...
    api_key: Option<&str>,
    version: RealtimeWsVersion,
) -> PeregrineResult<Option<HeaderMap>> {
    ...
    if let Some(api_key) = api_key {
        let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
            PeregrineErr::InvalidRequest(format!("invalid realtime api key header: {err}"))
        })?;
...
async fn send_conversation_error(
    ...
    sub_id: String,
    message: String,
    codex_error_info: PeregrineErrorInfo,
) {
    sess.send_event_raw(Event {
        id: sub_id,
