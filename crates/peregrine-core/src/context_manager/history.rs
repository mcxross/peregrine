...
use crate::tools::code_mode::PUBLIC_TOOL_NAME as CODE_MODE_PUBLIC_TOOL_NAME;
use base64::Engine;
...
impl ContextManager {
    ...
    {
        let mut code_mode_call_ids = self
            .items
            .iter()
            .filter_map(code_mode_custom_call_id)
            .collect::<HashSet<_>>();
        ...
        for item in items {
            ...
            if let Some(call_id) = code_mode_custom_call_id(item_ref) {
                code_mode_call_ids.insert(call_id);
            }

            let processed = self.process_item(item_ref, policy, &code_mode_call_ids);
    ...
    fn process_item(
        ...
        match item {
            ...
            ResponseItem::CustomToolCallOutput {
                ...
                let output = if code_mode_call_ids.contains(call_id) {
                    output.clone()
                } else {
...
fn code_mode_custom_call_id(item: &ResponseItem) -> Option<String> {
    match item {
        ResponseItem::CustomToolCall { call_id, name, .. }
            if name == CODE_MODE_PUBLIC_TOOL_NAME =>
        {
            Some(call_id.clone())
        }
        _ => None,
...
pub(crate) fn is_peregrine_generated_item(item: &ResponseItem) -> bool {
    matches!(
        item,
        ResponseItem::FunctionCallOutput { .. }
            | ResponseItem::ToolSearchOutput { .. }
