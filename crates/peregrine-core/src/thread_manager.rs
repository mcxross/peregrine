...
use crate::peregrine_thread::PeregrineThread;
use crate::rollout::truncation;
use crate::session::INITIAL_SUBMIT_ID;
use crate::session::Peregrine;
use crate::session::PeregrineSpawnArgs;
use crate::session::PeregrineSpawnOk;
...
struct TempPeregrineHomeGuard {
    path: PathBuf,
}

impl Drop for TempPeregrineHomeGuard {
    fn drop(&mut self) {
        let _ = std::fs::remove_dir_all(&self.path);
    }
}

/// Represents a newly created Peregrine thread (formerly called a conversation), including the first event
/// (which is [`EventMsg::SessionConfigured`]).
pub struct NewThread {
    pub thread_id: ThreadId,
    pub thread: Arc<PeregrineThread>,
...
pub struct ThreadManager {
    ...
    _test_peregrine_home_guard: Option<TempPeregrineHomeGuard>,
}
...
pub struct StartThreadOptions {
    ...
    pub dynamic_tools: Vec<peregrine_types::dynamic_tools::DynamicToolSpec>,
...
pub(crate) struct ThreadManagerState {
    threads: Arc<RwLock<HashMap<ThreadId, Arc<PeregrineThread>>>>,
...
impl ThreadManager {
    ...
    pub fn new(
        ...
        let peregrine_home = config.peregrine_home.clone();
        ...
        Self {
            ...
            _test_peregrine_home_guard: None,
        }
    }
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        let peregrine_home = std::env::temp_dir().join(format!(
            "peregrine-thread-manager-test-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&peregrine_home)
            .unwrap_or_else(|err| panic!("temp peregrine home dir create failed: {err}"));
        ...
        manager._test_peregrine_home_guard = Some(TempPeregrineHomeGuard {
            path: peregrine_home,
        });
        manager
    }

    /// Construct with a dummy AuthManager containing the provided CodexAuth and Peregrine home.
    /// Used for integration tests: should not be used by ordinary business logic.
    ...
    pub(crate) fn with_models_provider_home_and_state_for_tests(
        ...
        let skills_peregrine_home =
            match AbsolutePathBuf::from_absolute_path_checked(&peregrine_home) {
                Ok(peregrine_home) => peregrine_home,
                Err(err) => panic!("test peregrine_home should be absolute: {err}"),
            };
        let (thread_created_tx, _) = broadcast::channel(THREAD_CREATED_CHANNEL_CAPACITY);
        ...
        let thread_store: Arc<dyn ThreadStore> = Arc::new(LocalThreadStore::new(
            LocalThreadStoreConfig {
                ...
                sqlite_home: peregrine_home.clone(),
        ...
        Self {
            state: Arc::new(ThreadManagerState {
                ...
                models_manager: create_model_provider(provider, Some(auth_manager.clone()))
                    .models_manager(peregrine_home, /*config_model_catalog*/ None),
                environment_manager,
                skills_manager,
                plugins_manager,
                mcp_manager,
    ...
    pub async fn get_thread(&self, thread_id: ThreadId) -> PeregrineResult<Arc<PeregrineThread>> {
        self.state.get_thread(thread_id).await
    }

    /// Updates metadata for loaded and cold threads through one entrypoint.
    ///
    /// Loaded threads route through `PeregrineThread`/`LiveThread`, so metadata changes stay ordered
        ...
        self.state
            ...
            .map_err(|err| match err {
                ThreadStoreError::ThreadNotFound { thread_id } => {
                    PeregrineErr::ThreadNotFound(thread_id)
                }
    ...
    pub async fn start_thread(&self, config: Config) -> PeregrineResult<NewThread> {
    ...
    pub async fn start_thread_with_tools(
        &self,
        config: Config,
        dynamic_tools: Vec<peregrine_types::dynamic_tools::DynamicToolSpec>,
    ...
    /// as `Arc<PeregrineThread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<PeregrineThread>> {
    ...
    async fn initial_history_from_rollout_path(
        &self,
        rollout_path: PathBuf,
    ) -> PeregrineResult<InitialHistory> {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(
        &self,
        thread_id: ThreadId,
    ) -> PeregrineResult<Arc<PeregrineThread>> {
        let threads = self.threads.read().await;
        match threads.get(&thread_id) {
            Some(thread) if !thread.session_source.is_internal() => Ok(thread.clone()),
            Some(_) | None => Err(PeregrineErr::ThreadNotFound(thread_id)),
        }
    }
    ...
    pub(crate) async fn read_stored_thread(
        ...
        self.thread_store
            ...
            .map_err(|err| match err {
                ...
                err => PeregrineErr::Fatal(format!("failed to read stored thread {thread_id}: {err}")),
            })
    }

    /// Send an operation to a thread by ID.
    pub(crate) async fn send_op(&self, thread_id: ThreadId, op: Op) -> PeregrineResult<String> {
    ...
    pub(crate) async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<PeregrineThread>> {
    ...
    pub(crate) async fn spawn_thread_with_source(
        ...
        let PeregrineSpawnOk {
            codex, thread_id, ..
        } = Peregrine::spawn(PeregrineSpawnArgs {
            config,
    ...
    async fn finalize_thread_spawn(
        ...
        let session_configured = match event {
            ...
            _ => {
                return Err(PeregrineErr::SessionConfiguredNotFirstEvent);
            }
        };

        {
            ...
            if let std::collections::hash_map::Entry::Vacant(e) = threads.entry(thread_id) {
                let thread = Arc::new(PeregrineThread::new(
                    codex,
        ...
        Err(PeregrineErr::InvalidRequest(format!(
...
fn thread_store_rollout_read_error(err: ThreadStoreError) -> PeregrineErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => PeregrineErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => PeregrineErr::InvalidRequest(message),
        err => PeregrineErr::Fatal(format!("failed to read thread by rollout path: {err}")),
    }
}

fn thread_store_metadata_update_error(thread_id: ThreadId, err: ThreadStoreError) -> PeregrineErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => PeregrineErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => PeregrineErr::InvalidRequest(message),
        ThreadStoreError::Unsupported { operation } => PeregrineErr::UnsupportedOperation(format!(
            "thread metadata update is not supported by this store: {operation}"
        )),
        err => PeregrineErr::Fatal(format!(
