...
use peregrine_config::ConfigEditsBuilder;
use peregrine_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID;
...
use peregrine_config::load_global_mcp_servers;
...
pub(crate) async fn maybe_install_mcp_dependencies(
    ...
    let mut servers: HashMap<String, McpServerConfig> =
        match load_global_mcp_servers(&peregrine_home).await {
            Ok(servers) => codex_compat::mcp_server_config_map_to_codex(&servers),
            Err(err) => {
    ...
    let local_servers = codex_compat::mcp_server_config_btree_from_codex(&servers);
    if let Err(err) = ConfigEditsBuilder::new(&peregrine_home)
        .replace_mcp_servers(&local_servers)
        .apply()
        .await
    {
    ...
    for (name, server_config) in added {
        ...
        let first_attempt = perform_oauth_login(
            &name,
            &oauth_config.url,
            crate::config::codex_oauth_credentials_store_mode(
