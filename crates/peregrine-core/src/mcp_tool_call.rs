...
use peregrine_config::codex_compat::AppToolApproval;
...
use peregrine_types::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_TOOL_APPROVAL_KIND_KEY;
use peregrine_types::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_TOOL_APPROVAL_KIND_MCP_TOOL_CALL;
use peregrine_types::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_CONNECTOR_DESCRIPTION_KEY;
use peregrine_types::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_TOOL_APPROVAL_CONNECTOR_ID_KEY;
use peregrine_types::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_TOOL_APPROVAL_CONNECTOR_NAME_KEY;
use peregrine_types::mcp_approval_meta::PERSIST_ALWAYS as MCP_TOOL_APPROVAL_PERSIST_ALWAYS;
use peregrine_types::mcp_approval_meta::PERSIST_KEY as MCP_TOOL_APPROVAL_PERSIST_KEY;
use peregrine_types::mcp_approval_meta::PERSIST_SESSION as MCP_TOOL_APPROVAL_PERSIST_SESSION;
use peregrine_types::mcp_approval_meta::SOURCE_CONNECTOR as MCP_TOOL_APPROVAL_SOURCE_CONNECTOR;
use peregrine_types::mcp_approval_meta::SOURCE_KEY as MCP_TOOL_APPROVAL_SOURCE_KEY;
use peregrine_types::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_TOOL_DESCRIPTION_KEY;
use peregrine_types::mcp_approval_meta::TOOL_PARAMS_DISPLAY_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_DISPLAY_KEY;
use peregrine_types::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_KEY;
use peregrine_types::mcp_approval_meta::TOOL_TITLE_KEY as MCP_TOOL_APPROVAL_TOOL_TITLE_KEY;
...
use peregrine_types::request_user_input::RequestUserInputQuestion;
use peregrine_types::request_user_input::RequestUserInputQuestionOption;
...
pub(crate) async fn handle_mcp_tool_call(
    ...
    let approval_mode = if server == CODEX_APPS_MCP_SERVER_NAME {
        codex_compat::app_tool_approval(app_tool_policy.approval)
    } else {
...
async fn handle_approved_mcp_tool_call(
    ...
    .await;
    maybe_track_peregrine_app_used(sess, turn_context, &server, &tool_name).await;
...
async fn execute_mcp_tool_call(
    ...
    Ok(maybe_request_peregrine_apps_auth_elicitation(
        sess,
        turn_context,
        call_id,
        &invocation.server,
...
async fn maybe_request_peregrine_apps_auth_elicitation(
    sess: &Session,
    turn_context: &TurnContext,
    call_id: &str,
    server: &str,
    ...
    refresh_peregrine_apps_after_connector_auth(sess, turn_context).await;
...
async fn refresh_peregrine_apps_after_connector_auth(sess: &Session, turn_context: &TurnContext) {
    ...
    match mcp_tools_result {
        ...
        Err(err) => {
            tracing::warn!("failed to refresh Peregrine Apps tools after connector auth: {err:#}");
        }
    }
}
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    let sandbox_state = serde_json::to_value(SandboxState {
        ...
        codex_linux_sandbox_exe: turn_context.peregrine_linux_sandbox_exe.clone(),
        #[allow(deprecated)]
...
async fn maybe_track_peregrine_app_used(
    sess: &Session,
    turn_context: &TurnContext,
    server: &str,
    tool_name: &str,
...
pub(crate) struct McpToolApprovalMetadata {
    ...
    mcp_app_resource_uri: Option<String>,
    peregrine_apps_meta: Option<serde_json::Map<String, serde_json::Value>>,
...
async fn custom_mcp_tool_approval_mode(
    ...
    let user_configured_mode = turn_context
        ...
        .cloned()
        .and_then(|value| {
            HashMap::<String, peregrine_config::McpServerConfig>::deserialize(value).ok()
        })
        .and_then(|servers| {
    ...
    if let Some(user_configured_mode) = user_configured_mode {
        return codex_compat::app_tool_approval(user_configured_mode);
    }
...
fn build_mcp_tool_call_request_meta(
    ...
    {
        request_meta.insert(
            crate::X_PEREGRINE_TURN_METADATA_HEADER.to_string(),
            turn_metadata,
        );
    }

    if server == CODEX_APPS_MCP_SERVER_NAME {
        let mut peregrine_apps_meta = metadata
            .and_then(|metadata| metadata.peregrine_apps_meta.clone())
            .unwrap_or_default();
        peregrine_apps_meta.insert(
            "call_id".to_string(),
            serde_json::Value::String(call_id.to_string()),
        );
        request_meta.insert(
            MCP_TOOL_CODEX_APPS_META_KEY.to_string(),
            serde_json::Value::Object(peregrine_apps_meta),
        );
    }
...
pub(crate) const MCP_TOOL_APPROVAL_DECLINE_SYNTHETIC: &str = "__peregrine_mcp_decline__";
...
async fn maybe_persist_mcp_tool_approval(
    ...
    let persist_result = if key.server == CODEX_APPS_MCP_SERVER_NAME {
        ...
        persist_peregrine_app_tool_approval(&turn_context.config, &connector_id, &tool_name).await
    } else {
...
async fn persist_peregrine_app_tool_approval(
    config: &Config,
    connector_id: &str,
    tool_name: &str,
) -> anyhow::Result<()> {
...
fn user_mcp_server_is_configured(config: &Config, server: &str) -> anyhow::Result<bool> {
    ...
    let servers =
        HashMap::<String, peregrine_config::types::McpServerConfig>::deserialize(mcp_servers_toml)?;
    ...
    config
        ...
        .find_map(|layer| {
            ...
            let servers = layer
                ...
                .cloned()
                .and_then(|value| {
                    HashMap::<String, peregrine_config::types::McpServerConfig>::deserialize(value)
                        .ok()
                })?;
