...
pub(crate) struct SessionStartupPrewarmHandle {
    task: JoinHandle<PeregrineResult<ModelClientSession>>,
    started_at: Instant,
    timeout: Duration,
}
...
impl SessionStartupPrewarmHandle {
    ...
    fn resolution_from_join_result(
        result: std::result::Result<PeregrineResult<ModelClientSession>, tokio::task::JoinError>,
        started_at: Instant,
    ) -> SessionStartupPrewarmResolution {
        match result {
...
) -> PeregrineResult<ModelClientSession> {
