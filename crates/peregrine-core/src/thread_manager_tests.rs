...
async fn shutdown_all_threads_bounded_submits_shutdown_to_every_thread() {
    ...
    config.peregrine_home = temp_dir.path().join("peregrine-home").abs();
    config.cwd = config.peregrine_home.abs();
    std::fs::create_dir_all(&config.peregrine_home).expect("create peregrine home");
...
#[tokio::test]
async fn start_thread_uses_all_default_environments_from_peregrine_home() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.peregrine_home = temp_dir.path().join("peregrine-home").abs();
    config.cwd = config.peregrine_home.abs();
    std::fs::create_dir_all(&config.peregrine_home).expect("create peregrine home");
    std::fs::write(
        config.peregrine_home.join("environments.toml"),
        r#"
default = "dev"
...
"#,
    ...
    let prompt_items = crate::prompt_debug::build_prompt_input_from_session(
        thread.thread.peregrine.session.as_ref(),
        Vec::<UserInput>::new(),
    )
    .await
    .expect("prompt input");
...
#[tokio::test]
async fn explicit_installation_id_skips_peregrine_home_file() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.peregrine_home = temp_dir.path().join("peregrine-home").abs();
    config.cwd = config.peregrine_home.abs();
    ...
    assert_eq!(
        thread.thread.peregrine.session.installation_id,
        installation_id
    );
