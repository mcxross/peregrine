...
async fn global_doc_invalid_utf8_warns_and_uses_lossy_text() {
    let peregrine_home = tempfile::tempdir().expect("tempdir");
    let peregrine_home_abs = peregrine_home.abs();
    let path = peregrine_home_abs.join(DEFAULT_AGENTS_MD_FILENAME);
    fs::write(&path, b"global\xFF doc").unwrap();

    let mut warnings = Vec::new();
    let loaded = AgentsMdManager::load_global_instructions(
        LOCAL_FS.as_ref(),
        Some(&peregrine_home_abs),
        &mut warnings,
    )
    .await
...
async fn project_root_markers_are_honored_for_agents_discovery() {
    ...
    fs::write(root.path().join(".peregrine-root"), "").unwrap();
...
async fn instruction_sources_include_global_before_agents_md_docs() {
    ...
    let global_agents = cfg.peregrine_home.join(DEFAULT_AGENTS_MD_FILENAME);
    fs::create_dir_all(&cfg.peregrine_home).unwrap();
...
async fn skills_are_not_appended_to_agents_md() {
    ...
    let cfg = make_config(&tmp, /*limit*/ 4096, /*instructions*/ None).await;
    create_skill(
        cfg.peregrine_home.to_path_buf(),
        "pdf-processing",
        "extract from pdfs",
    );
...
fn create_skill(peregrine_home: PathBuf, name: &str, description: &str) {
    let skill_dir = peregrine_home.join(format!("skills/{name}"));
