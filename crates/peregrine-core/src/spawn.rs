...
///
/// 1. The process was spawned by Peregrine as part of a shell tool call.
/// 2. NetworkSandboxPolicy is restricted for the tool call.
///
/// We may try to have just one environment variable for all sandboxing
/// attributes, so this may change in the future.
pub const PEREGRINE_SANDBOX_NETWORK_DISABLED_ENV_VAR: &str = "PEREGRINE_SANDBOX_NETWORK_DISABLED";
...
pub const PEREGRINE_SANDBOX_ENV_VAR: &str = "PEREGRINE_SANDBOX";
...
/// `PEREGRINE_SANDBOX_NETWORK_DISABLED_ENV_VAR` environment variable.
...
pub(crate) async fn spawn_child_async(request: SpawnChildRequest<'_>) -> std::io::Result<Child> {
    ...
    if !network_sandbox_policy.is_enabled() {
        cmd.env(PEREGRINE_SANDBOX_NETWORK_DISABLED_ENV_VAR, "1");
    }

    // If this Peregrine process dies (including being killed via SIGKILL), we want
