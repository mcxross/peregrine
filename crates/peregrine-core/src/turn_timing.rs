    ...
    match item {
        ...
        ResponseItem::Reasoning {
            ...
            summary.iter().any(|entry| match entry {
                peregrine_types::models::ReasoningItemReasoningSummary::SummaryText { text } => {
                    !text.is_empty()
                }
            }) || content.as_ref().is_some_and(|entries| {
                entries.iter().any(|entry| match entry {
                    peregrine_types::models::ReasoningItemContent::ReasoningText { text }
                    | peregrine_types::models::ReasoningItemContent::Text { text } => {
                        !text.is_empty()
                    }
                })
            })
