...
async fn test_config_with_cli_overrides(
    ...
    let config = ConfigBuilder::default()
        .peregrine_home(home_path.clone())
...
async fn apply_role_preserves_unspecified_keys() {
    ...
    .await;
    config.peregrine_linux_sandbox_exe = Some(PathBuf::from("/tmp/peregrine-linux-sandbox"));
    config.main_execve_wrapper_exe = Some(PathBuf::from("/tmp/peregrine-execve-wrapper"));
    let role_path = write_role_config(
        &home,
        "effort-only.toml",
    ...
    assert_eq!(config.model_reasoning_effort, Some(ReasoningEffort::High));
    assert_eq!(
        config.peregrine_linux_sandbox_exe,
        Some(PathBuf::from("/tmp/peregrine-linux-sandbox"))
    );
    assert_eq!(
        config.main_execve_wrapper_exe,
        Some(PathBuf::from("/tmp/peregrine-execve-wrapper"))
    );
}
