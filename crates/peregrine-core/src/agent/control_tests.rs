...
impl AgentControlHarness {
    ...
    async fn start_thread(&self) -> (ThreadId, Arc<PeregrineThread>) {
        let new_thread = self
            .manager
...
async fn wait_for_subagent_notification(parent_thread: &Arc<PeregrineThread>) -> bool {
    let wait = async {
        loop {
...
async fn persist_thread_for_tree_resume(thread: &Arc<PeregrineThread>, message: &str) {
    thread
...
async fn send_input_errors_when_thread_missing() {
    ...
    assert_matches!(err, PeregrineErr::ThreadNotFound(id) if id == thread_id);
}
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            if thread
                ...
                .session
                .input_queue
                .has_pending_input(&thread.peregrine.session.active_turn)
                .await
            {
                break;
            }
...
async fn spawn_agent_can_fork_parent_thread_history_with_sanitized_items() {
    ...
    parent_thread
        ...
        .await;
    let turn_context = parent_thread.peregrine.session.new_default_turn().await;
    ...
    let history = child_thread.peregrine.session.clone_history().await;
...
async fn spawn_agent_fork_last_n_turns_keeps_only_recent_turns() {
    ...
    let queued_turn_context = parent_thread.peregrine.session.new_default_turn().await;
    parent_thread
        .peregrine
        .session
        .record_conversation_items(
    ...
    let triggered_turn_context = parent_thread.peregrine.session.new_default_turn().await;
    parent_thread
        .peregrine
        .session
        .record_conversation_items(
    ...
    parent_thread
        ...
        .await;
    let spawn_turn_context = parent_thread.peregrine.session.new_default_turn().await;
...
async fn spawn_agent_fork_last_n_turns_drops_parent_startup_prefix_when_under_limit() {
    let harness = AgentControlHarness::new().await;
    let (parent_thread_id, parent_thread) = harness.start_thread().await;
    let startup_turn_context = parent_thread.peregrine.session.new_default_turn().await;
    parent_thread
        .peregrine
        .session
        .record_conversation_items(
...
async fn spawn_agent_respects_max_threads_limit() {
    ...
    let PeregrineErr::AgentLimitReached {
        max_threads: seen_max_threads,
    } = err
    else {
        panic!("expected PeregrineErr::AgentLimitReached");
    };
...
async fn spawn_agent_limit_shared_across_clones() {
    ...
    let PeregrineErr::AgentLimitReached { max_threads } = err else {
        panic!("expected PeregrineErr::AgentLimitReached");
    };
...
async fn multi_agent_v2_completion_ignores_dead_direct_parent() {
    ...
    let tester_turn = tester_thread.peregrine.session.new_default_turn().await;
    tester_thread
        .peregrine
        .session
        .send_event(
