...
pub(crate) struct AgentControl {
    ...
    /// `ThreadManagerState -> PeregrineThread -> Session -> SessionServices -> ThreadManagerState`.
...
impl AgentControl {
    ...
    async fn spawn_agent_internal(
        ...
        )) = notification_source.as_ref()
        {
            ...
            let thread_config = new_thread.thread.peregrine.thread_config_snapshot().await;
            emit_subagent_session_started(
                ...
                client_metadata,
                new_thread.thread.peregrine.session.session_id(),
    ...
    ) -> PeregrineResult<crate::thread_manager::NewThread> {
        if options.fork_parent_spawn_call_id.is_none() {
            return Err(PeregrineErr::Fatal(
        ...
        let multi_agent_v2_usage_hint_texts_to_filter: Vec<String> =
            if let Some(parent_thread) = parent_thread.as_ref() {
                if parent_thread.enabled(Feature::MultiAgentV2) {
                    let parent_config = parent_thread.peregrine.session.get_config().await;
                    [
                        parent_config
                            .multi_agent_v2
    ...
    async fn resume_single_agent_from_rollout(
            ...
            SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
                ...
                let (resumed_agent_nickname, resumed_agent_role) =
                    if let Some(state_db_ctx) = state_db_ctx.as_ref() {
                        match Box::pin(state_db_ctx.get_thread(thread_id)).await {
        ...
        let inherited_shell_snapshot =
            Box::pin(self.inherited_shell_snapshot_for_source(&state, Some(&session_source))).await;
        let inherited_exec_policy =
            Box::pin(self.inherited_exec_policy_for_source(&state, Some(&session_source), &config))
                .await;
        let stored_thread = Box::pin(state.read_stored_thread(ReadThreadParams {
            thread_id,
            include_archived: true,
            include_history: true,
        }))
        ...
        let history = stored_thread
            .history
            .ok_or_else(|| PeregrineErr::ThreadNotFound(thread_id))?
            .items;

        let resumed_thread = Box::pin(state.resume_thread_with_history_with_source(
            ResumeThreadWithHistoryOptions {
                config: config.clone(),
        ...
        Box::pin(self.persist_thread_spawn_edge_for_source(
    ...
    pub(crate) async fn interrupt_agent(&self, agent_id: ThreadId) -> PeregrineResult<String> {
        let state = self.upgrade()?;
    ...
    async fn handle_thread_request_result(
        ...
        agent_id: ThreadId,
        state: &Arc<ThreadManagerState>,
        result: PeregrineResult<String>,
    ) -> PeregrineResult<String> {
        if matches!(result, Err(PeregrineErr::InternalAgentDied)) {
    ...
    pub(crate) async fn shutdown_live_agent(&self, agent_id: ThreadId) -> PeregrineResult<String> {
        let state = self.upgrade()?;
        let result = if let Ok(thread) = state.get_thread(agent_id).await {
            thread.peregrine.session.ensure_rollout_materialized().await;
            thread.peregrine.session.flush_rollout().await?;
    ...
    pub(crate) async fn close_agent(&self, agent_id: ThreadId) -> PeregrineResult<String> {
        let state = self.upgrade()?;
        ...
        match state.get_thread(agent_id).await {
            ...
            Err(PeregrineErr::ThreadNotFound(_)) if known_agent => {
    ...
    async fn shutdown_agent_tree(&self, agent_id: ThreadId) -> PeregrineResult<String> {
    ...
    pub(crate) async fn resolve_agent_reference(
        ...
        let agent_path = current_agent_path
            ...
            .map_err(PeregrineErr::UnsupportedOperation)?;
        if let Some(thread_id) = self.state.agent_id_for_path(&agent_path) {
            return Ok(thread_id);
        }
        Err(PeregrineErr::UnsupportedOperation(format!(
    ...
    pub(crate) async fn subscribe_status(
        &self,
        agent_id: ThreadId,
    ) -> PeregrineResult<watch::Receiver<AgentStatus>> {
        let state = self.upgrade()?;
    ...
    ) -> PeregrineResult<Vec<ListedAgent>> {
        let state = self.upgrade()?;
    ...
    ) -> PeregrineResult<(SessionSource, AgentMetadata)> {
        if depth == 1 {
    ...
    fn upgrade(&self) -> PeregrineResult<Arc<ThreadManagerState>> {
        self.manager
            .upgrade()
            .ok_or_else(|| PeregrineErr::UnsupportedOperation("thread manager dropped".to_string()))
    }
    ...
    async fn inherited_exec_policy_for_source(
        ...
        Some(Arc::clone(
            &parent_thread.peregrine.session.services.exec_policy,
        ))
    }
    ...
    async fn open_thread_spawn_children(
        &self,
        parent_thread_id: ThreadId,
    ) -> PeregrineResult<Vec<(ThreadId, AgentMetadata)>> {
    ...
    async fn live_thread_spawn_children(
        &self,
    ) -> PeregrineResult<HashMap<ThreadId, Vec<(ThreadId, AgentMetadata)>>> {
        let state = self.upgrade()?;
