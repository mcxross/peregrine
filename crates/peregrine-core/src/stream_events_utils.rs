...
use peregrine_types::items::ImageGenerationItem;
...
use futures::Future;
use peregrine_memories_read::citations::parse_memory_citation;
use peregrine_memories_read::citations::thread_ids_from_memory_citation;
...
fn strip_hidden_assistant_markup_and_parse_memory_citation(
    ...
    String,
    Option<peregrine_types::memory_citation::MemoryCitation>,
) {
        ...
        && role == "assistant"
    {
        let combined = content
            ...
            .filter_map(|ci| match ci {
                peregrine_types::models::ContentItem::OutputText { text } => Some(text.as_str()),
                _ => None,
            })
            .collect::<String>();
...
async fn save_image_generation_result(
    ...
    let bytes = BASE64_STANDARD
        ...
        .map_err(|err| {
            PeregrineErr::InvalidRequest(format!("invalid image generation payload: {err}"))
        })?;
    let path = image_generation_artifact_path(peregrine_home, session_id, call_id);
    ...
    .await
    {
        ...
        Err(err) => {
            ...
            let output_dir = output_path
                .parent()
                .unwrap_or_else(|| turn_context.config.peregrine_home.clone());
            tracing::warn!(
...
pub(crate) async fn handle_output_item_done(
    ...
    match ToolRouter::build_tool_call(item.clone()) {
        ...
        Err(FunctionCallError::Fatal(message)) => {
            return Err(PeregrineErr::Fatal(message));
        }
    }
        ...
        | ResponseItem::ImageGenerationCall { .. } => {
            ...
            if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                ...
                agent_message.content =
                    vec![peregrine_types::items::AgentMessageContent::Text { text: stripped }];
