...
use peregrine_config::ConfigLayerSource;
...
use peregrine_config::McpServerIdentity;
use peregrine_config::McpServerRequirement;
use peregrine_config::PluginRequirementsToml;
use peregrine_config::ProfileV2Name;
use peregrine_config::ResidencyRequirement;
use peregrine_config::SandboxModeRequirement;
use peregrine_config::Sourced;
use peregrine_config::ThreadConfigLoader;
use peregrine_config::codex_compat;
use peregrine_config::config_toml::ConfigLockfileToml;
use peregrine_config::config_toml::ConfigToml;
use peregrine_config::config_toml::DEFAULT_PROJECT_DOC_MAX_BYTES;
...
use peregrine_config::sandbox_mode_requirement_for_permission_profile;
...
use peregrine_config::types::McpServerDisabledReason;
...
use peregrine_config::types::OAuthCredentialsStoreMode;
use peregrine_config::types::SessionPickerViewMode;
use peregrine_config::types::ToolSuggestConfig;
...
use peregrine_types::config_types::SandboxMode;
use peregrine_types::config_types::ServiceTier;
use peregrine_types::config_types::ShellEnvironmentPolicy;
use peregrine_types::config_types::TrustLevel;
use peregrine_types::config_types::Verbosity;
use peregrine_types::config_types::WebSearchConfig;
...
use crate::config::permissions::get_readable_roots_required_for_peregrine_runtime;
...
pub use peregrine_config::ConfigLoadOptions;
pub use peregrine_config::Constrained;
pub use peregrine_config::ConstraintError;
pub use peregrine_config::ConstraintResult;
pub use peregrine_config::LoaderOverrides;
...
pub(crate) async fn test_config() -> Config {
    let peregrine_home = tempfile::tempdir().expect("create temp dir");
    Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides::default(),
        AbsolutePathBuf::from_absolute_path(peregrine_home.path())
...
impl Permissions {
    ...
    /// selection. Internal Peregrine-only writable roots are intentionally excluded.
...
// when Peregrine is still responsible for the network policy. `Disabled` means no
...
fn build_network_proxy_spec(
    ...
    network_requirements: Option<Sourced<peregrine_config::NetworkConstraints>>,
    permission_profile: &PermissionProfile,
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, Peregrine will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Peregrine
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.peregrine/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Peregrine"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Peregrine '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the Peregrine home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that Peregrine can reach out to for tool calls.
    pub mcp_servers: Constrained<HashMap<String, McpServerConfig>>,

    /// Preferred store for MCP OAuth credentials.
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by Peregrine unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/openai/codex/blob/main/peregrine-rs/rmcp-client/src/oauth.rs#L2
    /// file: PEREGRINE_HOME/.credentials.json
    ///       This file will be readable to Peregrine and other applications running as the same user.
    ...
    /// Directory containing all Peregrine state (defaults to `~/.peregrine` but can be
    /// overridden by the `PEREGRINE_HOME` environment variable).
    pub peregrine_home: AbsolutePathBuf,

    /// Directory where Peregrine stores the SQLite state DB.
    pub sqlite_home: PathBuf,

    /// Directory where Peregrine writes log files (defaults to `$PEREGRINE_HOME/log`).
    pub log_dir: PathBuf,

    /// Directory where Peregrine writes effective session config lock files.
    pub config_lock_export_dir: Option<AbsolutePathBuf>,

    /// Whether config lock replay ignores Peregrine version drift between the
    /// lock metadata and the regenerated lock.
    pub config_lock_allow_peregrine_version_mismatch: bool,
    ...
    /// Settings that govern if and what will be written to `~/.peregrine/history.jsonl`.
    pub history: History,
    ...
    /// Path to the current Peregrine executable. This cannot be set in the config
    /// file: it must be set in code via [`ConfigOverrides`].
    pub peregrine_self_exe: Option<PathBuf>,

    /// Path to the `peregrine-linux-sandbox` executable. This must be set if
    /// [`codex_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `peregrine-linux-sandbox`.
    pub peregrine_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `peregrine-execve-wrapper` executable used for shell
    ...
    /// When `true`, checks for Peregrine updates on startup and surfaces update prompts.
    /// Set to `false` only if your Peregrine updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across Peregrine product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across Peregrine product surfaces.
    /// Defaults to `true`.
    ...
    pub otel: peregrine_config::types::OtelConfig,
}
...
impl AuthManagerConfig for Config {
    fn codex_home(&self) -> PathBuf {
        self.peregrine_home.to_path_buf()
    }

    fn cli_auth_credentials_store_mode(&self) -> codex_login::AuthCredentialsStoreMode {
        match self.cli_auth_credentials_store_mode {
            AuthCredentialsStoreMode::File => codex_login::AuthCredentialsStoreMode::File,
            AuthCredentialsStoreMode::Keyring => codex_login::AuthCredentialsStoreMode::Keyring,
            AuthCredentialsStoreMode::Auto => codex_login::AuthCredentialsStoreMode::Auto,
            AuthCredentialsStoreMode::Ephemeral => codex_login::AuthCredentialsStoreMode::Ephemeral,
        }
    }
...
impl ConfigBuilder {
    pub fn peregrine_home(mut self, peregrine_home: PathBuf) -> Self {
        self.peregrine_home = Some(peregrine_home);
        self
    }
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let peregrine_home = match peregrine_home {
            Some(peregrine_home) => AbsolutePathBuf::from_absolute_path(peregrine_home)?,
            None => find_peregrine_home()?,
        };
        ...
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&peregrine_config::NoopThreadConfigLoader),
        )
        .await?;
        ...
        let config_toml: ConfigToml = match merged_toml.try_into() {
            ...
            Err(err) => {
                if let Some(config_error) =
                    peregrine_config::first_layer_config_error::<ConfigToml>(
                        &config_layer_stack,
                        peregrine_config::CONFIG_TOML_FILE,
                    )
                    .await
                {
                    return Err(peregrine_config::io_error_from_config_error(
        ...
        {
            let allow_peregrine_version_mismatch = config_lock_settings
                .and_then(|config_lock| config_lock.allow_peregrine_version_mismatch)
                .unwrap_or(false);
            ...
            config.config_lock_allow_peregrine_version_mismatch = allow_peregrine_version_mismatch;
...
impl Config {
    ...
    pub fn plugins_config_input(&self) -> PluginsConfigInput {
        PluginsConfigInput::new(
            self.compat_config_layer_stack(),
    ...
    pub fn compat_config_layer_stack(&self) -> codex_compat::ConfigLayerStack {
        codex_compat::config_layer_stack(&self.config_layer_stack)
    }
    ...
    pub async fn to_mcp_config(
        ...
        {
            let mut plugin_mcp_servers =
                codex_compat::mcp_server_config_map_from_codex(&plugin.mcp_servers);
        ...
        let configured_mcp_servers =
            codex_compat::mcp_server_config_map_to_codex(&configured_mcp_servers);

        McpConfig {
            chatgpt_base_url: self.chatgpt_base_url.clone(),
            apps_mcp_path_override: self.apps_mcp_path_override.clone(),
            apps_mcp_product_sku: self.apps_mcp_product_sku.clone(),
            codex_home: self.peregrine_home.to_path_buf(),
            mcp_oauth_credentials_store_mode: codex_compat::oauth_credentials_store_mode(
            ...
            approval_policy: codex_compat::approval_policy(&self.permissions.approval_policy),
            codex_linux_sandbox_exe: self.peregrine_linux_sandbox_exe.clone(),
    ...
    pub async fn rebuild_preserving_session_layers(
        ...
        Self::load_config_with_layer_stack(
            ...
            refreshed_config.peregrine_home.clone(),
            config_layer_stack,
        )
        .await
    }
    ...
    pub async fn load_default_with_cli_overrides(
        ...
        Self::load_default_with_cli_overrides_for_peregrine_home(
    ...
    /// Load a default configuration for a specific Peregrine home without reading
    /// user, project, or system config layers.
    pub async fn load_default_with_cli_overrides_for_peregrine_home(
        ...
        let cli_layer = peregrine_config::build_cli_overrides_layer(&cli_overrides);
        peregrine_config::merge_toml_values(&mut merged, &cli_layer);
        let peregrine_home = AbsolutePathBuf::from_absolute_path_checked(peregrine_home)?;
        let config_toml = deserialize_config_toml_with_base(merged, &peregrine_home)?;
        Self::load_config_with_layer_stack(
            LOCAL_FS.as_ref(),
            config_toml,
            ConfigOverrides::default(),
    ...
    /// in [ConfigToml], such as `cwd`, `peregrine_self_exe`, `peregrine_linux_sandbox_exe`, and
...
pub async fn load_config_as_toml_with_cli_and_load_options(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, peregrine_home).map_err(|e| {
...
pub fn codex_auth_credentials_store_mode(
...
pub fn codex_oauth_credentials_store_mode(
    mode: OAuthCredentialsStoreMode,
) -> codex_compat::OAuthCredentialsStoreMode {
    codex_compat::oauth_credentials_store_mode(mode)
}

pub(crate) fn codex_approval_policy(
    policy: &peregrine_config::Constrained<peregrine_types::protocol::AskForApproval>,
) -> codex_compat::ApprovalPolicy {
    codex_compat::approval_policy(policy)
}
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .peregrine/ folders.
...
/// Patch `PEREGRINE_HOME/config.toml` project state to set trust level.
/// Use with caution.
pub fn set_project_trust_level(
    ...
    ConfigEditsBuilder::new(peregrine_home)
        .set_project_trust_level(project_path, trust_level)
        .apply_blocking()
}

/// Save the default OSS provider preference to config.toml
pub fn set_default_oss_provider(peregrine_home: &Path, provider: &str) -> std::io::Result<()> {
    peregrine_config::config_toml::validate_oss_provider(provider)?;
    use toml_edit::value;
...
fn apply_managed_filesystem_constraints(
    ...
    filesystem_constraints: &peregrine_config::FilesystemConstraints,
) {
    for deny_read in &filesystem_constraints.deny_read {
        let deny_entry = if deny_read.contains_glob() {
            peregrine_types::permissions::FileSystemSandboxEntry {
                path: peregrine_types::permissions::FileSystemPath::GlobPattern {
                    pattern: deny_read.as_str().to_string(),
                },
                access: peregrine_types::permissions::FileSystemAccessMode::Deny,
            }
        } else {
            ...
            peregrine_types::permissions::FileSystemSandboxEntry {
                path: peregrine_types::permissions::FileSystemPath::Path { path },
                access: peregrine_types::permissions::FileSystemAccessMode::Deny,
            }
        };
...
impl Config {
    ...
    async fn load_from_base_config_with_overrides(
        ...
        peregrine_home: AbsolutePathBuf,
    ) -> std::io::Result<Self> {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let memories_root = memory_root(&peregrine_home);
        ...
        .map(add_peregrine_builtin_model_providers)
        ...
        let forced_chatgpt_workspace_id = cfg
            ...
            .clone()
            .map(peregrine_config::config_toml::ForcedChatgptWorkspaceIds::into_vec)
            .map(|values| {
                values
                    .into_iter()
        ...
        let log_dir = cfg
            ...
            .unwrap_or_else(|| peregrine_home.join("log").to_path_buf());
        let sqlite_home = cfg
            .sqlite_home
            .as_ref()
            .map(AbsolutePathBuf::to_path_buf)
            .or_else(|| resolve_sqlite_home_env(&resolved_cwd))
            .unwrap_or_else(|| peregrine_home.to_path_buf());
        ...
        let mut helper_readable_roots = get_readable_roots_required_for_peregrine_runtime(
            &peregrine_home,
            zsh_path.as_ref(),
        ...
        let config = Self {
            ...
            config_lock_allow_peregrine_version_mismatch: cfg
                .debug
                .as_ref()
                .and_then(|debug| debug.config_lockfile.as_ref())
                .and_then(|config_lock| config_lock.allow_peregrine_version_mismatch)
    ...
    pub fn bundled_skills_enabled(&self) -> bool {
        crate::manager::bundled_skills_enabled_from_stack(&self.compat_config_layer_stack())
    }
}
...
/// Returns the path to the Peregrine configuration directory, which can be
/// specified by the `PEREGRINE_HOME` environment variable. If not set, defaults to
/// `~/.peregrine`.
///
/// - If `PEREGRINE_HOME` is set, the value must exist and be a directory. The
///   value will be canonicalized and this function will Err otherwise.
/// - If `PEREGRINE_HOME` is not set, this function does not verify that the
///   directory exists.
pub fn find_peregrine_home() -> std::io::Result<AbsolutePathBuf> {
    peregrine_utils_home_dir::find_peregrine_home()
}

/// Returns the path to the folder where Peregrine logs are stored. Does not verify
