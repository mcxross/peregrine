...
use peregrine_config::ConfigRequirementsWithSources;
use peregrine_config::FilesystemDenyReadPattern;
use peregrine_config::LoaderOverrides;
use peregrine_config::RequirementSource;
use peregrine_config::SessionThreadConfig;
use peregrine_config::StaticThreadConfigLoader;
use peregrine_config::ThreadConfigSource;
use peregrine_config::config_error_from_ignored_toml_fields;
use peregrine_config::config_error_from_toml;
...
use peregrine_config::loader::load_requirements_toml;
...
async fn write_project_hook_config(
    dot_peregrine_folder: &Path,
    foo: Option<&str>,
    command: &str,
) -> std::io::Result<()> {
    tokio::fs::create_dir_all(dot_peregrine_folder).await?;
    let foo = foo
        .map(|value| format!("foo = \"{value}\"\n\n"))
        .unwrap_or_default();
    tokio::fs::write(
        dot_peregrine_folder.join(CONFIG_TOML_FILE),
        format!(
            r#"{foo}[hooks]
...
invalid = ["#,
    ...
    let user_layer = layers
        ...
        .expect("expected a user layer even when PEREGRINE_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let err = ConfigBuilder::default()
        .peregrine_home(tmp.path().to_path_buf())
    ...
    let expected_config_error =
        peregrine_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_active_user_layer()
        .expect("expected a user layer even when PEREGRINE_HOME/config.toml does not exist");
...
"#,
    ...
    assert_eq!(
        config_requirements_toml
            ...
            .as_deref()
            .cloned(),
        Some(vec![peregrine_config::WebSearchModeRequirement::Cached])
    );
    assert_eq!(
        config_requirements_toml
        ...
        Some(peregrine_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(peregrine_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
...
async fn load_config_layers_includes_cloud_requirements() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let peregrine_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&peregrine_home).await?;
...
async fn load_config_layers_includes_cloud_hook_requirements() -> anyhow::Result<()> {
    ...
    let requirements = ConfigRequirementsToml {
        hooks: Some(peregrine_config::ManagedHooksRequirementsToml {
            managed_dir: Some(managed_dir.clone()),
            windows_managed_dir: None,
            hooks: peregrine_config::HookEventsToml {
                pre_tool_use: vec![peregrine_config::MatcherGroup {
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            ...
            peregrine_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
    ...
    let err = load_config_layers_state(
        ...
        CloudRequirementsLoader::new(async {
            Err(CloudRequirementsLoadError::new(
                peregrine_config::CloudRequirementsLoadErrorCode::RequestFailed,
                /*status_code*/ None,
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".peregrine")).await?;
    tokio::fs::create_dir_all(project_root.join(".peregrine")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".peregrine").join(CONFIG_TOML_FILE),
        r#"foo = "root"
"#,
    )
    .await?;
    tokio::fs::write(
        nested.join(".peregrine").join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;
    ...
    assert_eq!(
        ...
        nested.join(".peregrine").as_path()
    );
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".peregrine").as_path()
    );
...
-> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".peregrine")).await?;
    tokio::fs::create_dir_all(worktree_child.join(".peregrine")).await?;
    write_linked_worktree_pointer(&repo_root, &worktree_root).await?;
    write_project_hook_config(
        &repo_root.join(".peregrine"),
        Some("repo-root"),
        "echo repo root hook",
    )
    .await?;
    write_project_hook_config(
        &repo_child.join(".peregrine"),
        Some("repo-child"),
        "echo repo child hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_root.join(".peregrine"),
        Some("worktree-root"),
        "echo worktree root hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_child.join(".peregrine"),
        Some("worktree-child"),
...
async fn nested_project_root_markers_do_not_redirect_regular_repo_hooks() -> std::io::Result<()> {
    ...
    .await?;
    write_project_hook_config(
        &project_root.join(".peregrine"),
        /*foo*/ None,
        "echo project root hook",
    )
    .await?;
    write_project_hook_config(
        &nested.join(".peregrine"),
        /*foo*/ None,
        "echo nested hook",
    )
    .await?;
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_peregrine_and_override_in_order()
-> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
"#;
    ...
    .await?;
    tokio::fs::write(nested.join(".peregrine").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".peregrine").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".peregrine").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let peregrine_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&peregrine_home).await?;
    tokio::fs::write(peregrine_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn project_layer_is_added_when_dot_peregrine_exists_without_config_toml()
-> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_peregrine_folder: AbsolutePathBuf::from_absolute_path(
                project_root.join(".peregrine"),
            )?,
        },
        TomlValue::Table(toml::map::Map::new()),
...
#[tokio::test]
async fn peregrine_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let peregrine_home = home_dir.join(".peregrine");
    tokio::fs::create_dir_all(&peregrine_home).await?;
    tokio::fs::write(
        peregrine_home.join(CONFIG_TOML_FILE),
        r#"foo = "user"
...
#[tokio::test]
async fn peregrine_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_codex = project_root.join(".peregrine");
...
"#,
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_peregrine_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_codex)?,
        },
        child_config,
    );
    assert_eq!(vec![&expected_project_layer], project_layers);
...
"#,
    ...
    let peregrine_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&peregrine_home_untrusted).await?;
    make_config_for_test(
        &peregrine_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let untrusted_config_path = peregrine_home_untrusted.join(CONFIG_TOML_FILE);
...
{untrusted_config_contents}"#
    ...
    let peregrine_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&peregrine_home_unknown).await?;
    tokio::fs::write(
        peregrine_home_unknown.join(CONFIG_TOML_FILE),
        r#"foo = "user"
"#,
    )
    .await?;
...
async fn project_layer_ignores_unsupported_config_keys() -> std::io::Result<()> {
    ...
    // `.peregrine` folder. The malformed profile value below would fail typed path
...
"#,
    ...
    // still use the project-local `.peregrine` folder as its relative-path base.
    assert_eq!(
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".peregrine")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".peregrine").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let peregrine_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&peregrine_home).await?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_level, expect_disabled) in cases {
        let peregrine_home = tmp.path().join(format!("home_no_config_{name}"));
        tokio::fs::create_dir_all(&peregrine_home).await?;
        if let Some(trust_level) = trust_level {
            make_config_for_test(
                &peregrine_home,
...
mod requirements_exec_policy_tests {
    ...
    use peregrine_config::ConfigRequirementsWithSources;
    use peregrine_config::RequirementSource;
    use peregrine_config::RequirementsExecPolicyDecisionToml;
    use peregrine_config::RequirementsExecPolicyParseError;
    use peregrine_config::RequirementsExecPolicyPatternTokenToml;
    use peregrine_config::RequirementsExecPolicyPrefixRuleToml;
    use peregrine_config::RequirementsExecPolicyToml;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
    use toml::Value as TomlValue;
    ...
    fn config_stack_for_dot_peregrine_folder_with_requirements(
        dot_peregrine_folder: &Path,
        requirements: ConfigRequirements,
    ) -> ConfigLayerStack {
        let dot_peregrine_folder = AbsolutePathBuf::from_absolute_path(dot_peregrine_folder)
            .expect("absolute dot_peregrine_folder");
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack =
            config_stack_for_dot_peregrine_folder_with_requirements(temp_dir.path(), requirements);
