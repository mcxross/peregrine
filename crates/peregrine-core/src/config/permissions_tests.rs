...
async fn restricted_read_implicitly_allows_helper_executables() -> std::io::Result<()> {
    ...
    let peregrine_home = temp_dir.path().join(".peregrine");
    let zsh_path = temp_dir.path().join("runtime").join("zsh");
    let arg0_root = peregrine_home.join("tmp").join("arg0");
    let allowed_arg0_dir = arg0_root.join("peregrine-arg0-session");
    let sibling_arg0_dir = arg0_root.join("peregrine-arg0-other-session");
    let execve_wrapper = allowed_arg0_dir.join("peregrine-execve-wrapper");
    ...
    let config = Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(&peregrine_home)?,
    )
    .await?;
