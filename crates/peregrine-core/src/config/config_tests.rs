...
use peregrine_config::ProfileV2Name;
use peregrine_config::RequirementSource;
use peregrine_config::config_toml::AgentRoleToml;
use peregrine_config::config_toml::AgentsToml;
use peregrine_config::config_toml::AutoReviewToml;
use peregrine_config::config_toml::ConfigToml;
use peregrine_config::config_toml::ExperimentalRequestUserInput;
use peregrine_config::config_toml::ProjectConfig;
use peregrine_config::config_toml::RealtimeConfig;
use peregrine_config::config_toml::RealtimeToml;
use peregrine_config::config_toml::RealtimeTransport;
use peregrine_config::config_toml::RealtimeWsMode;
use peregrine_config::config_toml::RealtimeWsVersion;
use peregrine_config::config_toml::ToolsToml;
use peregrine_config::loader::project_trust_key;
use peregrine_config::permissions_toml::FilesystemPermissionToml;
use peregrine_config::permissions_toml::FilesystemPermissionsToml;
use peregrine_config::permissions_toml::NetworkDomainPermissionToml;
use peregrine_config::permissions_toml::NetworkDomainPermissionsToml;
use peregrine_config::permissions_toml::NetworkMitmActionToml;
use peregrine_config::permissions_toml::NetworkMitmHookToml;
use peregrine_config::permissions_toml::NetworkMitmToml;
use peregrine_config::permissions_toml::NetworkToml;
use peregrine_config::permissions_toml::PermissionProfileToml;
use peregrine_config::permissions_toml::PermissionsToml;
use peregrine_config::permissions_toml::WorkspaceRootsToml;
use peregrine_config::profile_toml::ConfigProfile;
use peregrine_config::types::AppToolApproval;
use peregrine_config::types::ApprovalsReviewer;
use peregrine_config::types::BundledSkillsConfig;
use peregrine_config::types::FeedbackConfigToml;
use peregrine_config::types::HistoryPersistence;
use peregrine_config::types::McpServerEnvVar;
use peregrine_config::types::McpServerOAuthConfig;
use peregrine_config::types::McpServerToolConfig;
use peregrine_config::types::McpServerTransportConfig;
use peregrine_config::types::MemoriesConfig;
use peregrine_config::types::MemoriesToml;
use peregrine_config::types::ModelAvailabilityNuxConfig;
use peregrine_config::types::Notice;
use peregrine_config::types::NotificationCondition;
use peregrine_config::types::NotificationMethod;
use peregrine_config::types::Notifications;
use peregrine_config::types::OtelConfigToml;
use peregrine_config::types::OtelExporterKind;
use peregrine_config::types::SandboxWorkspaceWrite;
use peregrine_config::types::SessionPickerViewMode;
use peregrine_config::types::SkillsConfig;
use peregrine_config::types::ToolSuggestDisabledTool;
use peregrine_config::types::ToolSuggestDiscoverableType;
use peregrine_config::types::Tui;
use peregrine_config::types::TuiKeymap;
use peregrine_config::types::TuiNotificationSettings;
...
use peregrine_types::models::SandboxEnforcement;
...
use peregrine_types::protocol::NetworkAccess;
use peregrine_types::protocol::RealtimeVoice;
...
fn stdio_mcp(command: &str) -> McpServerConfig {
    McpServerConfig {
        ...
        environment_id: peregrine_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string(),
        enabled: true,
        required: false,
...
fn codex_mcp(config: McpServerConfig) -> peregrine_config::codex_compat::McpServerConfig {
    peregrine_config::codex_compat::mcp_server_config_map_to_codex(&HashMap::from([(
        "server".to_string(),
        config,
    )]))
    .remove("server")
    .expect("MCP config should convert to Codex shape")
}
...
async fn load_config_loads_global_agents_instructions() -> std::io::Result<()> {
    let peregrine_home = tempdir()?;
    std::fs::write(
        peregrine_home.path().join(DEFAULT_AGENTS_MD_FILENAME),
        "\n  global instructions  \n",
    )?;
...
async fn load_config_prefers_global_agents_override_instructions() -> std::io::Result<()> {
    ...
    let global_agents_override_path = peregrine_home.path().join(LOCAL_AGENTS_MD_FILENAME);
...
async fn experimental_network_requirements_enable_proxy_without_feature() -> std::io::Result<()> {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        ...
        .cloud_requirements(CloudRequirementsLoader::new(async {
            Ok(Some(peregrine_config::ConfigRequirementsToml {
                network: Some(peregrine_config::NetworkRequirementsToml {
                    enabled: Some(true),
                    ..Default::default()
                }),
                ..Default::default()
...
-> std::io::Result<()> {
    ...
    requirements.network = Some(Sourced::new(
        peregrine_config::NetworkConstraints {
            enabled: Some(true),
            ..Default::default()
        },
    ...
    requirements_toml.network = Some(peregrine_config::NetworkRequirementsToml {
        enabled: Some(true),
        ..Default::default()
    });
...
-> std::io::Result<()> {
    ...
    let memories_root = peregrine_home.path().join("memories").abs();
    assert!(
        !config
            .permissions
            .file_system_sandbox_policy()
...
-> std::io::Result<()> {
    ...
    let peregrine_home = temp_dir.path().join("peregrine-home");
    ...
    for root in [&cwd, &runtime_root, &profile_root] {
        ...
        std::fs::create_dir_all(root.join(".peregrine"))?;
    }
                        ...
                        filesystem: Some(FilesystemPermissionsToml {
                            ...
                            entries: BTreeMap::from([(
                                ...
                                FilesystemPermissionToml::Scoped(BTreeMap::from([
                                    ...
                                    (".peregrine".to_string(), FileSystemAccessMode::Read),
                                ])),
                            )]),
                        }),
                        network: None,
    ...
    for root in [cwd_abs, runtime_root_abs, profile_root_abs.clone()] {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&root.join(".peregrine"), cwd.as_path()),
            "expected .peregrine carveout under {root:?}, policy: {policy:?}"
        );
    }
    assert_eq!(
...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&cwd.path().join(".peregrine"), cwd.path()),
...
-> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    let extra_root = TempDir::new()?;
    for subpath in [".git", ".agents", ".peregrine"] {
...
async fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of Peregrine and will be ignored."
        )),
        "{:?}",
...
-> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of Peregrine and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
async fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `dev` does not define any recognized filesystem entries for this version of Peregrine."
        )),
        "{:?}",
        config.startup_warnings
    );
...
fn filter_plugin_mcp_servers_by_allowlist_enforces_plugin_and_identity_rules() {
    ...
    let requirements = Sourced::new(
        BTreeMap::from([(
            ...
            peregrine_config::PluginRequirementsToml {
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    let peregrine_home = TempDir::new()?;
    let user_file =
        AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, peregrine_home.path());
    let project_dot_codex =
        AbsolutePathBuf::resolve_path_against_base("project/.peregrine", peregrine_home.path());
    ...
    let requirements_toml = peregrine_config::ConfigRequirementsToml {
        mcp_servers: Some(mcp_requirements.clone()),
        ..Default::default()
    };
    let requirements = peregrine_config::ConfigRequirements {
    ...
    let refreshed_layer_stack = ConfigLayerStack::new(
        vec![
            ConfigLayerEntry::new(
                peregrine_app_server_protocol::ConfigLayerSource::User {
            ...
            ConfigLayerEntry::new(
                peregrine_app_server_protocol::ConfigLayerSource::Project {
                    dot_peregrine_folder: project_dot_codex.clone(),
                },
                toml::toml! {
            ...
            ConfigLayerEntry::new(
                peregrine_app_server_protocol::ConfigLayerSource::LegacyManagedConfigTomlFromMdm,
                toml::toml! {
                    [mcp_servers.managed_overrides_session]
    ...
    let thread_layer_stack = ConfigLayerStack::new(
        vec![
            ...
            ConfigLayerEntry::new(
                peregrine_app_server_protocol::ConfigLayerSource::Project {
                    dot_peregrine_folder: project_dot_codex,
                },
                toml::toml! {
                    [mcp_servers.fresh_project]
            ...
            ConfigLayerEntry::new(
                peregrine_app_server_protocol::ConfigLayerSource::SessionFlags,
                toml::toml! {
                    [mcp_servers.session_overrides_user]
...
-> anyhow::Result<()> {
    let peregrine_home = TempDir::new()?;
    let plugin_root = peregrine_home
        .path()
        .join("plugins/cache")
  ...
  "mcpServers": {
    ...
    let config = thread_config
        .rebuild_preserving_session_layers(&refreshed_config)
        .await?;
    let plugins_manager = PluginsManager::new(peregrine_home.path().to_path_buf());
    let mcp_config = config.to_mcp_config(&plugins_manager).await;
    let expected_mcp = codex_mcp(http_mcp("https://sample.example/mcp"));
...
"#,
    ...
    let plugins_manager = PluginsManager::new(peregrine_home.path().to_path_buf());
    let mcp_config = config.to_mcp_config(&plugins_manager).await;
    let expected_mcp = codex_mcp(http_mcp("https://user.example/mcp"));
...
"#,
    ...
    let requirements = peregrine_config::ConfigRequirementsToml {
    ...
    assert_eq!(
        ...
        Some((
            false,
            Some(
                peregrine_config::codex_compat::McpServerDisabledReason::Requirements {
                    source: peregrine_config::codex_compat::RequirementSource::CloudRequirements,
                },
            )
        ))
    );
...
async fn default_zsh_path_sets_runtime_zsh_path() -> std::io::Result<()> {
    let peregrine_home = TempDir::new()?;
    let default_zsh_path = peregrine_home.path().join("packaged-zsh");
...
#[tokio::test]
async fn sqlite_home_defaults_to_peregrine_home_for_workspace_write() -> std::io::Result<()> {
    let peregrine_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
    ...
    assert_eq!(config.sqlite_home, peregrine_home.path().to_path_buf());
...
-> std::io::Result<()> {
    let peregrine_home = TempDir::new()?;
    let memories_root = peregrine_home.path().join("memories");
    let writable_root = peregrine_home.path().join("writable").abs();
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            sandbox_workspace_write: Some(SandboxWorkspaceWrite {
...
"#,
    ...
    let project_config_dir = workspace.path().join(".peregrine");
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    let peregrine_home = TempDir::new()?;
    let managed_path = peregrine_home.path().join("managed_config.toml");
    let config_path = peregrine_home.path().join(CONFIG_TOML_FILE);
    ...
    let cwd = peregrine_home.path().abs();
    let config_layer_stack = load_config_layers_state(
        ...
        &peregrine_config::NoopThreadConfigLoader,
    )
    .await?;
...
async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
    ...
    let servers = load_global_mcp_servers(peregrine_home.path()).await?;
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    ...
    let loaded = load_global_mcp_servers(peregrine_home.path()).await?;
    ...
    match &docs.transport {
        McpServerTransportConfig::Stdio {
            ...
            assert_eq!(cwd, &Some(peregrine_home.path().to_path_buf()));
        }
...
"#,
    ...
    let err = load_global_mcp_servers(peregrine_home.path())
        .await
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    let cwd_path = PathBuf::from("/tmp/peregrine-mcp");
    let servers = BTreeMap::from([(
        "docs".to_string(),
        McpServerConfig {
            transport: McpServerTransportConfig::Stdio {
    ...
    let serialized = std::fs::read_to_string(&config_path)?;
    assert!(
        serialized.contains(r#"cwd = "/tmp/peregrine-mcp""#),
    ...
    match &docs.transport {
        McpServerTransportConfig::Stdio { cwd, .. } => {
            assert_eq!(cwd.as_deref(), Some(Path::new("/tmp/peregrine-mcp")));
        }
        other => panic!("unexpected transport {other:?}"),
    }
...
async fn replace_mcp_servers_streamable_http_serializes_oauth_resource() -> anyhow::Result<()> {
    ...
    let servers = BTreeMap::from([(
        ...
        McpServerConfig {
            ...
            oauth: Some(McpServerOAuthConfig {
                client_id: Some("eci-prd-pub-peregrine-123".to_string()),
            }),
    ...
    assert!(serialized.contains(r#"client_id = "eci-prd-pub-peregrine-123""#));
    ...
    assert_eq!(docs.oauth_client_id(), Some("eci-prd-pub-peregrine-123"));
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(peregrine_home.path())
        .set_model(Some("gpt-5.4"), Some(ReasoningEffort::High))
        .apply()
        .await?;

    let serialized =
        tokio::fs::read_to_string(peregrine_home.path().join(CONFIG_TOML_FILE)).await?;
...
async fn for_config_writes_selected_user_config_file() -> anyhow::Result<()> {
    let peregrine_home = TempDir::new()?;
    let base_config = peregrine_home.path().join(CONFIG_TOML_FILE);
    let selected_config = peregrine_home.path().join("work.config.toml");
...
fn profile_v2_config_path_resolves_validated_names() -> anyhow::Result<()> {
    ...
    assert_eq!(
        resolve_profile_v2_config_path(peregrine_home.path(), &profile_name),
        peregrine_home.path().join("work.config.toml").abs()
    );
    Ok(())
}
...
impl PrecedenceTestFixture {
    ...
    fn peregrine_home(&self) -> AbsolutePathBuf {
        self.peregrine_home.abs()
    }
}
...
async fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let peregrine_home = TempDir::new()?;
    let workspace = peregrine_home.path().join("workspace");
    ...
    let config =
        Config::load_from_base_config_with_overrides(cfg, overrides, peregrine_home.abs()).await?;
...
async fn load_config_uses_requirements_guardian_policy_config() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        Default::default(),
        peregrine_config::ConfigRequirementsToml {
...
async fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
    let peregrine_home = TempDir::new()?;
    let missing_path = peregrine_home.path().join("agents").join("researcher.toml");
    let cfg = ConfigToml {
...
async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
    let peregrine_home = TempDir::new()?;
    let role_config_path = peregrine_home.path().join("agents").join("researcher.toml");
...
"#,
    ...
    let config_layer_stack = peregrine_config::ConfigLayerStack::new(
        vec![peregrine_config::ConfigLayerEntry::new(
            peregrine_app_server_protocol::ConfigLayerSource::User {
                file: peregrine_home.path().join(CONFIG_TOML_FILE).abs(),
                profile: None,
            },
            layer_config,
        )],
        Default::default(),
        peregrine_config::ConfigRequirementsToml::default(),
    )
    .map_err(std::io::Error::other)?;
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".peregrine").join("agents");
...
async fn loads_legacy_split_agent_roles_from_config_toml() -> std::io::Result<()> {
    let peregrine_home = TempDir::new()?;
    let researcher_path = peregrine_home.path().join("agents").join("researcher.toml");
    let reviewer_path = peregrine_home.path().join("agents").join("reviewer.toml");
    tokio::fs::create_dir_all(
        researcher_path
            .parent()
            .expect("role config should have a parent directory"),
...
"#
    ...
    let home_agents_dir = peregrine_home.path().join("agents");
...
async fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let peregrine_home = TempDir::new()?;
    let catalog_path = peregrine_home.path().join("catalog.json");
...
"#;
    ...
    let peregrine_home_temp_dir = TempDir::new().unwrap();

    Ok(PrecedenceTestFixture {
        cwd: cwd_temp_dir,
        peregrine_home: peregrine_home_temp_dir,
        cfg,
    })
}
...
async fn trace_exporter_defaults_to_none_when_log_exporter_is_set() -> std::io::Result<()> {
    ...
    cfg.otel = Some(OtelConfigToml {
        exporter: Some(OtelExporterKind::OtlpHttp {
            ...
            headers: HashMap::new(),
            protocol: peregrine_config::types::OtelHttpProtocol::Binary,
            tls: None,
        }),
...
async fn config_toml_priority_service_tier_uses_priority_request_value() -> std::io::Result<()> {
    ...
    let peregrine_home = fixture.peregrine_home();
...
async fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()>
{
    ...
    let requirements_toml = peregrine_config::ConfigRequirementsToml {
        ...
        allowed_web_search_modes: Some(vec![peregrine_config::WebSearchModeRequirement::Cached]),
    ...
    let requirement_source = peregrine_config::RequirementSource::Unknown;
    ...
    let requirements = peregrine_config::ConfigRequirements {
        web_search_mode: peregrine_config::ConstrainedWithSource::new(
            constrained,
    ...
    let config_layer_stack =
        peregrine_config::ConfigLayerStack::new(Vec::new(), requirements, requirements_toml)
...
fn test_set_default_oss_provider() -> std::io::Result<()> {
    let temp_dir = TempDir::new()?;
    let peregrine_home = temp_dir.path();
    let config_path = peregrine_home.join(CONFIG_TOML_FILE);

    // Test setting valid provider on empty config
    set_default_oss_provider(peregrine_home, OLLAMA_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    assert!(content.contains("oss_provider = \"ollama\""));

    // Test updating existing config
    std::fs::write(&config_path, "model = \"gpt-4\"\n")?;
    set_default_oss_provider(peregrine_home, LMSTUDIO_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    ...
    let result = set_default_oss_provider(peregrine_home, "invalid_provider");
...
fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(peregrine_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
    assert!(
        ...
        .cloud_requirements(CloudRequirementsLoader::new(async {
            Ok(Some(peregrine_config::ConfigRequirementsToml {
                allowed_sandbox_modes: Some(vec![
                    peregrine_config::SandboxModeRequirement::ReadOnly,
                ]),
                ..Default::default()
            }))
        }))
...
"#,
    ...
    let requirements = peregrine_config::ConfigRequirementsToml {
        allowed_approval_policies: None,
        allowed_approvals_reviewers: None,
        allowed_sandbox_modes: Some(vec![peregrine_config::SandboxModeRequirement::ReadOnly]),
        allowed_permissions: None,
        remote_sandbox_config: None,
...
"#,
    ...
    let requirements = peregrine_config::ConfigRequirementsToml {
        windows: Some(peregrine_config::WindowsRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(peregrine_config::types::WindowsSandboxModeToml::Elevated)
    );
    assert!(
...
"#,
    ...
    assert_eq!(
        err.to_string(),
        "`approval_policy = \"never\"` cannot be used because requirements do not allow `sandbox_mode = \"danger-full-access\"`; Peregrine would fall back to read-only permissions with approvals disabled. Choose an `approval_policy` based on what you need, such as `on-request`, or choose an allowed sandbox mode."
    );
    Ok(())
}
...
async fn permission_profile_override_preserves_split_write_roots() -> std::io::Result<()> {
    let peregrine_home = TempDir::new()?;
    let cwd = peregrine_home.path().join("workspace");
    let outside_root = peregrine_home.path().join("outside-write");
        ...
        .cloud_requirements(CloudRequirementsLoader::new(async {
            Ok(Some(peregrine_config::ConfigRequirementsToml {
                allowed_web_search_modes: Some(vec![
                    peregrine_config::WebSearchModeRequirement::Cached,
                ]),
                ..Default::default()
            }))
        }))
...
async fn feature_requirements_normalize_effective_feature_values() -> std::io::Result<()> {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        ...
        .cloud_requirements(CloudRequirementsLoader::new(async {
            Ok(Some(peregrine_config::ConfigRequirementsToml {
                feature_requirements: Some(peregrine_config::FeatureRequirementsToml {
                    entries: BTreeMap::from([
...
export_dir = "locks"
allow_peregrine_version_mismatch = true
...
"#,
    ...
    assert!(config.config_lock_allow_peregrine_version_mismatch);
...
async fn debug_config_lockfile_load_path_loads_lock_from_nested_table() -> std::io::Result<()> {
    let peregrine_home = TempDir::new()?;
    let lock_path = peregrine_home.path().join("session.config.lock.toml");
    std::fs::write(
        &lock_path,
        format!(
            r#"version = {}
peregrine_version = "older-version"
