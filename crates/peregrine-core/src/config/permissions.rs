...
use peregrine_config::permissions_toml::NetworkUnixSocketPermissionToml;
use peregrine_config::permissions_toml::NetworkUnixSocketPermissionsToml;
use peregrine_config::permissions_toml::PermissionProfileToml;
use peregrine_config::permissions_toml::PermissionsToml;
use peregrine_config::permissions_toml::ResolvedPermissionProfileToml;
...
use peregrine_types::permissions::project_roots_glob_pattern;
...
/// for Peregrine to function. These should always be added to the
/// `FileSystemSandboxPolicy` for a thread.
pub(crate) fn get_readable_roots_required_for_peregrine_runtime(
    peregrine_home: &Path,
    zsh_path: Option<&PathBuf>,
    main_execve_wrapper_exe: Option<&PathBuf>,
) -> Vec<AbsolutePathBuf> {
    let arg0_root =
        AbsolutePathBuf::from_absolute_path(peregrine_home.join("tmp").join("arg0")).ok();
...
// Adding a new `:special_path` must not make older Peregrine versions reject the
...
fn missing_filesystem_entries_warning(profile_name: &str) -> String {
    format!(
        "Permissions profile `{profile_name}` does not define any recognized filesystem entries for this version of Peregrine. Filesystem access will remain restricted. Upgrade Peregrine if this profile expects filesystem permissions."
    )
}
...
fn maybe_push_unknown_special_path_warning(
    ...
    push_warning(
        ...
        match subpath.as_deref() {
            Some(subpath) => format!(
                "Configured filesystem path `{path}` with nested entry `{}` is not recognized by this version of Peregrine and will be ignored. Upgrade Peregrine if this path is required.",
                subpath.display()
            ),
            None => format!(
                "Configured filesystem path `{path}` is not recognized by this version of Peregrine and will be ignored. Upgrade Peregrine if this path is required."
            ),
        },
    );
}
