...
pub fn apply_blocking(peregrine_home: &Path, edits: &[ConfigEdit]) -> anyhow::Result<()> {
...
///
pub async fn apply(peregrine_home: &Path, edits: Vec<ConfigEdit>) -> anyhow::Result<()> {
...
impl ConfigEditsBuilder {
    pub fn new(peregrine_home: &Path) -> Self {
        Self::for_config_path(&peregrine_home.join(CONFIG_TOML_FILE))
    }
    ...
    pub fn for_config(config: &crate::config::Config) -> Self {
        let config_path = config
            ...
            .unwrap_or_else(|| config.peregrine_home.join(CONFIG_TOML_FILE).to_path_buf());
