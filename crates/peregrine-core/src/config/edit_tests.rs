...
fn keymap_bindings_edit_writes_multiple_bindings_as_array() {
    ...
    let raw = std::fs::read_to_string(peregrine_home.join(CONFIG_TOML_FILE)).expect("read config");
...
fn blocking_set_model_writes_through_symlink_chain() {
    ...
    let link_path = peregrine_home.join("config-link.toml");
...
fn blocking_set_model_replaces_symlink_on_cycle() {
    let tmp = tempdir().expect("tmpdir");
    let peregrine_home = tmp.path();
    let link_a = peregrine_home.join("a.toml");
    let link_b = peregrine_home.join("b.toml");
    let config_path = peregrine_home.join(CONFIG_TOML_FILE);
...
"#;
    std::fs::write(peregrine_home.join(CONFIG_TOML_FILE), original).expect("seed config");
...
#[test]
fn blocking_set_hide_gpt_5_1_peregrine_max_migration_prompt_preserves_table() {
    let tmp = tempdir().expect("tmpdir");
    let peregrine_home = tmp.path();
    std::fs::write(
        peregrine_home.join(CONFIG_TOML_FILE),
...
"#,
    ...
    apply_blocking(
        peregrine_home,
        &[ConfigEdit::SetNoticeHideModelMigrationPrompt(
            "hide_gpt-5.1-peregrine-max_migration_prompt".to_string(),
            true,
        )],
    )
    .expect("persist");
    ...
    let expected = r#"[notice]
existing = "value"
"hide_gpt-5.1-peregrine-max_migration_prompt" = true
"#;
    assert_eq!(contents, expected);
}
...
client_id = \"eci-prd-pub-peregrine-123\"
...
fn blocking_replace_mcp_servers_serializes_tool_approval_overrides() {
    ...
    apply_blocking(peregrine_home, &[ConfigEdit::ReplaceMcpServers(servers)]).expect("persist");
...
fn blocking_clear_path_noop_when_missing() {
    ...
    assert!(
        !peregrine_home.join(CONFIG_TOML_FILE).exists(),
...
"#;
    ...
    contents = std::fs::read_to_string(peregrine_home.join(CONFIG_TOML_FILE)).expect("read config");
