...
pub fn sandbox_setup_is_complete(peregrine_home: &Path) -> bool {
    codex_windows_sandbox::sandbox_setup_is_complete(peregrine_home)
}

#[cfg(not(target_os = "windows"))]
pub fn sandbox_setup_is_complete(_peregrine_home: &Path) -> bool {
    false
}
...
pub fn run_elevated_provisioning_setup(
    ...
    codex_windows_sandbox::run_elevated_provisioning_setup(peregrine_home, real_user)
}
...
async fn run_windows_sandbox_setup_and_persist(
    ...
    let peregrine_home = request.peregrine_home;
    let setup_peregrine_home = peregrine_home.clone();

    let setup_result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
        match mode {
            WindowsSandboxSetupMode::Elevated => {
                if !sandbox_setup_is_complete(setup_peregrine_home.as_path()) {
                    run_elevated_setup(
                        &permission_profile,
                        workspace_roots.as_slice(),
                        command_cwd.as_path(),
                        &env_map,
                        setup_peregrine_home.as_path(),
                    )?;
                }
            }
    ...
    ConfigEditsBuilder::new(peregrine_home.as_path())
