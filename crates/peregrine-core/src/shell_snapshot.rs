...
impl ShellSnapshot {
    ...
    async fn try_new(
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap_or(0);
        let path = peregrine_home
            .join(SNAPSHOT_DIR)
            .join(format!("{session_id}.{nonce}.{extension}"));
        let temp_path = peregrine_home
            .join(SNAPSHOT_DIR)
            .join(format!("{session_id}.tmp-{nonce}"));

        // Clean the (unlikely) leaked snapshot files.
        let peregrine_home = peregrine_home.clone();
        let cleanup_session_id = session_id;
        tokio::spawn(async move {
            if let Err(err) =
                cleanup_stale_snapshots(&peregrine_home, cleanup_session_id, state_db).await
            {
...
pub async fn cleanup_stale_snapshots(
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let rollout_path =
            find_thread_path_by_id_str(peregrine_home, session_id, state_db.as_deref()).await?;
