...
use peregrine_config::RequirementsExecPolicy;
...
fn config_stack_for_dot_peregrine_folder(dot_peregrine_folder: &Path) -> ConfigLayerStack {
...
async fn returns_empty_policy_when_no_policy_files_exist() {
    ...
    let config_stack = config_stack_for_dot_peregrine_folder(temp_dir.path());
...
async fn merges_requirements_exec_policy_network_rules() -> anyhow::Result<()> {
    ...
    let requirements = ConfigRequirements {
        exec_policy: Some(peregrine_config::Sourced::new(
            peregrine_config::RequirementsExecPolicy::new(requirements_exec_policy),
            peregrine_config::RequirementSource::Unknown,
        )),
        ..ConfigRequirements::default()
    };
    let dot_peregrine_folder = AbsolutePathBuf::from_absolute_path(temp_dir.path())?;
    let layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_peregrine_folder,
        },
...
async fn ignore_user_project_rules_keeps_system_policy_files() {
    ...
    .expect("write policy file");
    let config_file =
        AbsolutePathBuf::from_absolute_path(config_dir.join(peregrine_config::CONFIG_TOML_FILE))
...
async fn ignores_rules_from_untrusted_project_layers() -> anyhow::Result<()> {
    ...
    let project_dot_peregrine_folder = AbsolutePathBuf::from_absolute_path(project_dir.path())?;
    let layers = vec![ConfigLayerEntry::new_disabled(
        ConfigLayerSource::Project {
            dot_peregrine_folder: project_dot_peregrine_folder,
        },
        TomlValue::Table(Default::default()),
        "marked untrusted",
    )];
...
async fn ignore_user_config_keeps_user_policy_files() -> std::io::Result<()> {
    let temp = tempdir()?;
    let peregrine_home = temp.path().join("home_ignore_user_config");
    let rules_dir = peregrine_home.join(RULES_DIR_NAME);
...
async fn append_execpolicy_amendment_updates_policy_and_file() {
    let peregrine_home = tempdir().expect("create temp dir");
    let prefix = vec!["echo".to_string(), "hello".to_string()];
    let manager = ExecPolicyManager::default();

    manager
        .append_amendment_and_update(peregrine_home.path(), &ExecPolicyAmendment::from(prefix))
        .await
    ...
    let contents = fs::read_to_string(default_policy_path(peregrine_home.path()))
...
async fn append_execpolicy_amendment_rejects_empty_prefix() {
    ...
    let result = manager
        .append_amendment_and_update(peregrine_home.path(), &ExecPolicyAmendment::from(vec![]))
        .await;
...
async fn exec_policies_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let peregrine_home = temp.path().join("home_execpolicy_nested");
    let project_root = temp.path().join("project_execpolicy_nested");
    let nested = project_root.join("nested");
    let root_rules = project_root.join(".peregrine").join(RULES_DIR_NAME);
    let nested_rules = nested.join(".peregrine").join(RULES_DIR_NAME);

    fs::create_dir_all(&peregrine_home)?;
...
async fn exec_policies_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let nested = project_root.join("nested");
    let rules_dir = project_root.join(".peregrine").join(RULES_DIR_NAME);
    ...
    for (name, trust_entries, expected_decision) in cases {
        let peregrine_home = temp.path().join(format!("home_execpolicy_{name}"));
        fs::create_dir_all(&peregrine_home)?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_entries, expect_warning) in cases {
        let peregrine_home = temp.path().join(format!("home_execpolicy_warning_{name}"));
        fs::create_dir_all(&peregrine_home)?;
        write_project_trust_config(&peregrine_home, &trust_entries).await?;
