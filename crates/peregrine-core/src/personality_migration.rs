...
pub async fn maybe_migrate_personality(
    ...
    let marker_path = peregrine_home.join(PERSONALITY_MIGRATION_FILENAME);
    ...
    if !has_recorded_sessions(peregrine_home, model_provider_id.as_str(), state_db).await? {
        create_marker(&marker_path).await?;
...
async fn has_recorded_sessions(
    ...
    let store = LocalThreadStore::new(
        LocalThreadStoreConfig {
            codex_home: peregrine_home.to_path_buf(),
            sqlite_home: peregrine_home.to_path_buf(),
