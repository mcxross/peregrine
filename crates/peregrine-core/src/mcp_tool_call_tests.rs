...
use codex_mcp::MCP_TOOL_CODEX_APPS_META_KEY as MCP_TOOL_PEREGRINE_APPS_META_KEY;
...
use peregrine_config::types::AppConfig;
use peregrine_config::types::AppToolApproval as ConfigAppToolApproval;
use peregrine_config::types::AppToolConfig;
use peregrine_config::types::AppToolsConfig;
...
fn write_sample_plugin_mcp(peregrine_home: &std::path::Path) {
    let plugin_root = peregrine_home.join("plugins/cache/test/sample/local");
...
fn install_mcp_permission_request_hook(
    ...
    std::fs::create_dir_all(&turn_context.config.peregrine_home)
        .expect("create peregrine home for MCP permission hook");
    let script = format!(
        r#"import json
...
"#,
    ...
    std::fs::write(
        turn_context.config.peregrine_home.join("hooks.json"),
        serde_json::json!({
            "hooks": {
                "PermissionRequest": [{
                    "matcher": matcher,
    ...
    let hook_list = codex_hooks::list_hooks(HooksConfig {
        ...
        config_layer_stack: Some(turn_context.config.compat_config_layer_stack()),
        ..HooksConfig::default()
    });
...
#[test]
fn openai_file_params_are_only_honored_for_peregrine_apps() {
...
#[test]
fn peregrine_apps_tool_question_uses_fallback_app_label() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        CODEX_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn trusted_peregrine_apps_tool_question_offers_always_allow() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        CODEX_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn peregrine_apps_tool_question_without_elicitation_omits_always_allow() {
...
#[test]
fn peregrine_apps_connectors_support_persistent_approval() {
    let invocation = McpInvocation {
        server: CODEX_APPS_MCP_SERVER_NAME.to_string(),
...
async fn mcp_tool_call_request_meta_includes_turn_metadata_for_custom_server() {
    ...
    let turn_metadata = meta
        .get(crate::X_PEREGRINE_TURN_METADATA_HEADER)
    ...
    assert_eq!(
        meta,
        serde_json::json!({
            crate::X_PEREGRINE_TURN_METADATA_HEADER: expected_turn_metadata,
        })
    );
}
...
#[tokio::test]
async fn peregrine_apps_tool_call_request_meta_includes_turn_metadata_and_peregrine_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = turn_context
        .turn_metadata_state
        .current_meta_value_for_mcp_request(mcp_turn_metadata_context(&turn_context))
    ...
    let metadata = McpToolApprovalMetadata {
        ...
        peregrine_apps_meta: Some(
            ...
            .as_object()
            .cloned()
            .expect("_peregrine_apps metadata should be an object"),
        ),
        openai_file_input_params: None,
    };
    ...
    assert_eq!(
        ...
        Some(serde_json::json!({
            crate::X_PEREGRINE_TURN_METADATA_HEADER: expected_turn_metadata,
            MCP_TOOL_PEREGRINE_APPS_META_KEY: {
...
#[tokio::test]
async fn peregrine_apps_tool_call_request_meta_includes_call_id_without_existing_peregrine_apps_meta()
 {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = turn_context
        .turn_metadata_state
...
fn peregrine_apps_auth_failure_result() -> CallToolResult {
    CallToolResult {
        content: vec![serde_json::json!({
            "type": "text",
...
fn peregrine_apps_auth_failure_metadata() -> McpToolApprovalMetadata {
    approval_metadata(
...
async fn install_host_owned_peregrine_apps_manager(session: &Session, turn_context: &TurnContext) {
    let auth = session.services.auth_manager.auth().await;
    let (manager, _cancel_token) = codex_mcp::McpConnectionManager::new(
        &HashMap::new(),
        peregrine_config::codex_compat::oauth_credentials_store_mode(
            turn_context.config.mcp_oauth_credentials_store_mode,
        ),
        HashMap::new(),
        &peregrine_config::codex_compat::approval_policy(&turn_context.approval_policy),
        ...
        turn_context.config.peregrine_home.to_path_buf(),
...
#[tokio::test]
async fn peregrine_apps_auth_elicitation_feature_disabled_returns_original_result() {
    let (session, turn_context, rx_event) = make_session_and_context_with_rx().await;
    install_host_owned_peregrine_apps_manager(&session, &turn_context).await;
    let result = peregrine_apps_auth_failure_result();
    let metadata = peregrine_apps_auth_failure_metadata();

    let returned = maybe_request_peregrine_apps_auth_elicitation(
        &session,
        &turn_context,
        "call_123",
        CODEX_APPS_MCP_SERVER_NAME,
...
#[tokio::test]
async fn peregrine_apps_auth_elicitation_non_host_owned_server_returns_original_result() {
...
#[tokio::test]
async fn peregrine_apps_auth_elicitation_disallowed_by_policy_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    install_host_owned_peregrine_apps_manager(&session, &turn_context).await;
    let mut features = Features::with_defaults();
    features.enable(Feature::AuthElicitation);
...
#[tokio::test]
async fn peregrine_apps_auth_elicitation_granular_mcp_disabled_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    install_host_owned_peregrine_apps_manager(&session, &turn_context).await;
    let mut features = Features::with_defaults();
    features.enable(Feature::AuthElicitation);
...
#[tokio::test]
async fn peregrine_apps_auth_elicitation_feature_enabled_requests_elicitation() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    install_host_owned_peregrine_apps_manager(&session, &turn_context).await;
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let request_task = tokio::spawn({
        ...
        async move {
            maybe_request_peregrine_apps_auth_elicitation(
                &session,
                &turn_context,
                "call_123",
                CODEX_APPS_MCP_SERVER_NAME,
    ...
    assert_eq!(
        request.id,
        peregrine_types::mcp::RequestId::String("peregrine_apps_auth_call_123".to_string())
    );
    assert!(matches!(
        request.request,
        peregrine_types::approvals::ElicitationRequest::Url { .. }
    ));

    session
        .resolve_elicitation(
            CODEX_APPS_MCP_SERVER_NAME.to_string(),
            rmcp::model::RequestId::String("peregrine_apps_auth_call_123".into()),
            ElicitationResponse {
...
async fn guardian_review_decision_maps_to_mcp_tool_decision() {
    ...
    session.services.guardian_rejections.lock().await.insert(
        ...
        crate::guardian::GuardianRejection {
            ...
            source: peregrine_types::protocol::GuardianAssessmentDecisionSource::Agent,
        },
    );
...
#[test]
fn approval_elicitation_meta_includes_connector_source_for_peregrine_apps() {
    assert_eq!(
        build_mcp_tool_approval_elicitation_meta(
            CODEX_APPS_MCP_SERVER_NAME,
            Some(&approval_metadata(
...
#[tokio::test]
async fn persist_peregrine_app_tool_approval_writes_tool_override() {
    ...
    persist_peregrine_app_tool_approval(&config, "calendar", "calendar/list_events")
        .await
                ...
                AppConfig {
                    ...
                    tools: Some(AppToolsConfig {
                        tools: HashMap::from([(
                            ...
                            AppToolConfig {
                                enabled: None,
                                approval_mode: Some(ConfigAppToolApproval::Approve),
                            },
                        )]),
                    }),
                },
...
async fn custom_mcp_tool_approval_mode_uses_plugin_mcp_policy() {
    ...
    write_sample_plugin_mcp(peregrine_home.as_path());
    std::fs::write(
...
async fn maybe_persist_mcp_tool_approval_reloads_session_config_for_custom_server() {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .peregrine_home(peregrine_home.clone().to_path_buf())
        .build()
        .await
        .expect("load config");
    turn_context.config = Arc::new(config);
...
"#,
    ...
    let contents =
        std::fs::read_to_string(peregrine_home.join(CONFIG_TOML_FILE)).expect("read config");
    let parsed: ConfigToml = toml::from_str(&contents).expect("parse config");
    let tool = parsed
        .plugins
        .get("sample@test")
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    let project_peregrine_dir = project_dir.path().join(".peregrine");
    std::fs::create_dir_all(&project_peregrine_dir).expect("create project .peregrine dir");
    std::fs::write(
        project_peregrine_dir.join(CONFIG_TOML_FILE),
        "[mcp_servers.docs]\ncommand = \"docs-server\"\n",
    )
    ...
    ConfigEditsBuilder::new(&peregrine_home)
        .set_project_trust_level(
            project_dir.path(),
            peregrine_types::config_types::TrustLevel::Trusted,
        )
        .apply()
        .await
    ...
    let contents = std::fs::read_to_string(project_peregrine_dir.join(CONFIG_TOML_FILE))
