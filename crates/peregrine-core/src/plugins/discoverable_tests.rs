...
async fn list_tool_suggest_discoverable_plugins_returns_fallback_plugins_without_installed_apps() {
    let peregrine_home = tempdir().expect("tempdir should succeed");
    let curated_root = curated_plugins_repo_path(peregrine_home.path());
    write_openai_curated_marketplace(&curated_root, &["sample", "slack", "openai-developers"]);
    write_plugins_feature_config(peregrine_home.path());

    let config = load_plugins_config(peregrine_home.path()).await;
...
async fn list_tool_suggest_discoverable_plugins_filters_non_fallback_by_installed_apps() {
    ...
    write_plugins_feature_config(peregrine_home.path());
    install_marketplace_plugin(peregrine_home.path(), curated_root.as_path(), "slack").await;
...
async fn list_tool_suggest_discoverable_plugins_filters_microsoft_by_installed_apps() {
    ...
    write_plugins_feature_config(peregrine_home.path());
    install_marketplace_plugin(peregrine_home.path(), curated_root.as_path(), "teams").await;
...
async fn list_tool_suggest_discoverable_plugins_filters_sales_apps_by_marketplace() {
    ...
    let sales_marketplace_root = peregrine_home
        .path()
...
"#
    ...
    write_file(
        &peregrine_home.path().join(crate::config::CONFIG_TOML_FILE),
        &format!(
            r#"[features]
plugins = true
...
"#
    ...
    install_marketplace_plugin(peregrine_home.path(), curated_root.as_path(), "installed").await;
...
async fn list_tool_suggest_discoverable_plugins_normalizes_description() {
    ...
    write_plugins_feature_config(peregrine_home.path());
    write_file(
        &curated_root.join("plugins/slack/.peregrine-plugin/plugin.json"),
        r#"{
  "name": "slack",
  "description": "  Plugin\n   with   extra   spacing  "
}"#,
...
async fn list_tool_suggest_discoverable_plugins_omits_installed_curated_plugins() {
    ...
    let curated_root = curated_plugins_repo_path(peregrine_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);
    write_curated_plugin_sha(peregrine_home.path());
    write_plugins_feature_config(peregrine_home.path());

    PluginsManager::new(peregrine_home.path().to_path_buf())
    ...
    let refreshed_config = load_plugins_config(peregrine_home.path()).await;
...
async fn list_tool_suggest_discoverable_plugins_does_not_reload_marketplace_per_plugin() {
    ...
    for plugin_name in ["gmail", "openai-developers"] {
        write_file(
            &curated_root.join(format!(
                "plugins/{plugin_name}/.peregrine-plugin/plugin.json"
            )),
            &format!(
                r#"{{
  "name": "{plugin_name}",
  ...
  "interface": {{
    ...
    assert_eq!(
        normalized_logs
            .matches("gmail/.peregrine-plugin/plugin.json")
            .count(),
        4
    );
    assert_eq!(
        normalized_logs
            .matches("openai-developers/.peregrine-plugin/plugin.json")
            .count(),
        4
    );
}
...
async fn install_marketplace_plugin(
    ...
    write_curated_plugin_sha(peregrine_home);
    PluginsManager::new(peregrine_home.to_path_buf())
        .install_plugin(PluginInstallRequest {
