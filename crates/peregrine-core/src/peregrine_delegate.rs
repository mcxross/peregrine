...
/// Start an interactive sub-Peregrine thread and return IO channels.
///
/// The returned `events_rx` yields non-approval events emitted by the sub-agent.
/// Approval requests are handled via `parent_session` and are not surfaced.
/// The returned `ops_tx` allows the caller to submit additional `Op`s to the sub-agent.
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_peregrine_thread_interactive(
    config: Config,
...
) -> Result<Peregrine, PeregrineErr> {
    let (tx_sub, rx_sub) = async_channel::bounded(SUBMISSION_CHANNEL_CAPACITY);
    let (tx_ops, rx_ops) = async_channel::bounded(SUBMISSION_CHANNEL_CAPACITY);
    let PeregrineSpawnOk { codex, .. } = Box::pin(Peregrine::spawn(PeregrineSpawnArgs {
        config,
    ...
    let peregrine_for_events = Arc::clone(&codex);
    ...
    let peregrine_for_ops = Arc::clone(&codex);
    tokio::spawn(async move {
        forward_ops(peregrine_for_ops, rx_ops, cancel_token_ops).await;
    });
...
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_peregrine_thread_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: SharedModelsManager,
    input: Vec<UserInput>,
    ...
    let io = Box::pin(run_peregrine_thread_interactive(
        config,
        auth_manager,
        models_manager,
        parent_session,
...
async fn shutdown_delegate(codex: &Peregrine) {
    ...
    let guardian_decision = if routes_approval_to_guardian(parent_ctx) {
        ...
        let patch = changes
            ...
            .map(|(path, change)| match change {
                peregrine_types::protocol::FileChange::Add { content } => {
                    format!("*** Add File: {}\n{}", path.display(), content)
                }
                peregrine_types::protocol::FileChange::Delete { content } => {
                    format!("*** Delete File: {}\n{}", path.display(), content)
                }
                peregrine_types::protocol::FileChange::Update {
                    unified_diff,
                    move_path,
                } => {
...
async fn maybe_auto_review_mcp_request_user_input(
    ...
    Some(RequestUserInputResponse {
        answers: HashMap::from([(
            question.id.clone(),
            peregrine_types::request_user_input::RequestUserInputAnswer {
...
) -> peregrine_types::protocol::ReviewDecision
where
    F: core::future::Future<Output = peregrine_types::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            ...
            parent_session
                .notify_approval(approval_id, peregrine_types::protocol::ReviewDecision::Abort)
                .await;
            peregrine_types::protocol::ReviewDecision::Abort
        }
        decision = fut => {
            decision
        }
...
#[cfg(test)]
#[path = "peregrine_delegate_tests.rs"]
mod tests;
