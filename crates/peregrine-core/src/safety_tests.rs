...
use peregrine_types::protocol::FileSystemSpecialPath;
...
#[test]
fn missing_project_dot_peregrine_config_requires_approval() {
    let tmp = TempDir::new().unwrap();
    let cwd = tmp.path().abs();
    let config_path = cwd.join(".peregrine").join("config.toml");
