...
use peregrine_types::items::ContextCompactionItem;
...
async fn run_compact_task_inner_impl(
    ...
    loop {
        ...
        match attempt_result {
            ...
            Err(PeregrineErr::Interrupted) => {
                return Err(PeregrineErr::Interrupted);
            }
            Err(e @ PeregrineErr::ContextWindowExceeded) => {
                if turn_input_len > 1 {
...
pub(crate) fn compaction_status_from_result<T>(result: &PeregrineResult<T>) -> CompactionStatus {
    match result {
        Ok(_) => CompactionStatus::Completed,
        Err(PeregrineErr::Interrupted | PeregrineErr::TurnAborted) => CompactionStatus::Interrupted,
...
async fn drain_to_completed(
    ...
    loop {
        ...
        let Some(event) = maybe_event else {
            return Err(PeregrineErr::Stream(
