...
use peregrine_types::items::PlanItem;
...
use peregrine_types::protocol::AgentMessageContentDeltaEvent;
use peregrine_types::protocol::AgentReasoningSectionBreakEvent;
...
use peregrine_types::protocol::PlanDeltaEvent;
use peregrine_types::protocol::ReasoningContentDeltaEvent;
use peregrine_types::protocol::ReasoningRawContentDeltaEvent;
...
pub(crate) async fn run_turn(
    ...
    if let Err(err) = run_pre_sampling_compact(&sess, &turn_context, &mut client_session).await {
        ...
        sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
            .await;
        if error == PeregrineErrorInfo::UsageLimitExceeded
            && let Err(err) = sess
    ...
    loop {
        ...
        .await
        {
            ...
            Err(PeregrineErr::TurnAborted) => {
                // Aborted turn is reported via a different event.
                break;
            }
            Err(PeregrineErr::InvalidImageRequest()) => {
                {
                ...
                let error = PeregrineErrorInfo::BadRequest;
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
...
async fn run_sampling_request(
    ...
    input: Vec<ResponseItem>,
    cancellation_token: CancellationToken,
) -> PeregrineResult<SamplingRequestResult> {
    ...
    loop {
        ...
        .await
        {
            ...
            Err(PeregrineErr::ContextWindowExceeded) => {
                sess.set_total_tokens_full(&turn_context).await;
                return Err(PeregrineErr::ContextWindowExceeded);
            }
            Err(PeregrineErr::UsageLimitReached(e)) => {
                let rate_limits = e.rate_limits.clone();
                if let Some(rate_limits) = rate_limits {
                    sess.update_rate_limits(&turn_context, *rate_limits).await;
                }
                return Err(PeregrineErr::UsageLimitReached(e));
            }
            Err(err) => err,
        };
...
pub(crate) async fn built_tools(
    ...
    turn_context: &TurnContext,
    cancellation_token: &CancellationToken,
) -> PeregrineResult<Arc<ToolRouter>> {
...
fn agent_message_text(item: &peregrine_types::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            peregrine_types::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: peregrine_types::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(peregrine_types::items::AgentMessageItem {
...
async fn drain_in_flight(
    in_flight: &mut FuturesOrdered<BoxFuture<'static, PeregrineResult<ResponseInputItem>>>,
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
) -> PeregrineResult<()> {
...
async fn try_run_sampling_request(
    ...
    let mut stream = client_session
        ...
        .or_cancel(&cancellation_token)
        .await??;
    let mut in_flight: FuturesOrdered<BoxFuture<'static, PeregrineResult<ResponseInputItem>>> =
        FuturesOrdered::new();
    ...
    let outcome: PeregrineResult<SamplingRequestResult> = loop {
            ...
            .await
        {
            ...
            Err(codex_async_utils::CancelErr::Cancelled) => {
                break Err(PeregrineErr::TurnAborted);
            }
        };
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![peregrine_types::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
    ...
    if cancellation_token.is_cancelled() {
        return Err(PeregrineErr::TurnAborted);
    }
