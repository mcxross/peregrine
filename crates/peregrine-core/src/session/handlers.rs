...
use peregrine_types::protocol::ThreadSettingsAppliedEvent;
use peregrine_types::protocol::ThreadSettingsOverrides;
use peregrine_types::protocol::ThreadSettingsSnapshot;
...
use peregrine_types::mcp::RequestId as ProtocolRequestId;
...
pub async fn resolve_elicitation(
    ...
    decision: peregrine_types::approvals::ElicitationAction,
    content: Option<Value>,
    meta: Option<Value>,
) {
    let action = match decision {
        peregrine_types::approvals::ElicitationAction::Accept => ElicitationAction::Accept,
        peregrine_types::approvals::ElicitationAction::Decline => ElicitationAction::Decline,
        peregrine_types::approvals::ElicitationAction::Cancel => ElicitationAction::Cancel,
    };
    ...
    if let Err(err) = sess.flush_rollout().await {
        sess.send_event(
            ...
            EventMsg::Warning(WarningEvent {
                message: format!(
                    "Rolled the thread back, but failed to save the rollback marker. Peregrine will continue retrying. Error: {err}"
                ),
            }),
        )
        .await;
    ...
    if let Err(err) = persist_thread_memory_mode_update(sess, mode).await {
        ...
        let event = Event {
            ...
            msg: EventMsg::Error(ErrorEvent {
                ...
                codex_error_info: Some(PeregrineErrorInfo::Other),
            }),
        };
...
pub async fn shutdown(sess: &Arc<Session>, sub_id: String) -> bool {
    ...
    info!("Shutting down Peregrine instance");
