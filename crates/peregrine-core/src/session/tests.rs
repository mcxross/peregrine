...
use peregrine_config::NetworkConstraints;
use peregrine_config::NetworkDomainPermissionToml;
use peregrine_config::NetworkDomainPermissionsToml;
...
use peregrine_models_manager::model_info;
...
use peregrine_types::models::FileSystemPermissions;
use peregrine_types::models::FunctionCallOutputBody;
use peregrine_types::models::FunctionCallOutputPayload;
use peregrine_types::models::PermissionProfile;
use peregrine_types::models::SandboxEnforcement;
use peregrine_types::openai_models::ModelServiceTier;
...
use peregrine_types::protocol::NonSteerableTurnKind;
use peregrine_types::protocol::SandboxPolicy;
use peregrine_types::protocol::TurnEnvironmentSelection;
use peregrine_types::request_permissions::PermissionGrantScope;
use peregrine_types::request_permissions::RequestPermissionProfile;
use tracing::Span;
...
use peregrine_types::models::BaseInstructions;
use peregrine_types::models::ContentItem;
use peregrine_types::models::ResponseItem;
use peregrine_types::protocol::AskForApproval;
use peregrine_types::protocol::CompactedItem;
use peregrine_types::protocol::ConversationAudioParams;
use peregrine_types::protocol::CreditsSnapshot;
use peregrine_types::protocol::GranularApprovalConfig;
use peregrine_types::protocol::InitialHistory;
use peregrine_types::protocol::InterAgentCommunication;
use peregrine_types::protocol::NetworkApprovalProtocol;
use peregrine_types::protocol::PeregrineErrorInfo;
use peregrine_types::protocol::RateLimitSnapshot;
use peregrine_types::protocol::RateLimitWindow;
use peregrine_types::protocol::RealtimeAudioFrame;
use peregrine_types::protocol::RealtimeConversationListVoicesResponseEvent;
...
use peregrine_types::protocol::SkillScope;
use peregrine_types::protocol::Submission;
use peregrine_types::protocol::ThreadGoalStatus;
use peregrine_types::protocol::ThreadRolledBackEvent;
use peregrine_types::protocol::ThreadSettingsOverrides;
...
use peregrine_types::protocol::TurnStartedEvent;
use peregrine_types::protocol::UserMessageEvent;
use peregrine_types::protocol::W3cTraceContext;
use peregrine_types::request_user_input::RequestUserInputAnswer;
use peregrine_types::request_user_input::RequestUserInputResponse;
use std::path::Path;
...
use peregrine_types::mcp::CallToolResult as McpCallToolResult;
...
async fn regular_turn_emits_turn_started_with_trace_id_without_waiting_for_startup_prewarm() {
    let _trace_test_context = install_test_tracing("peregrine-core-tests");
    ...
    let response = session
        .request_mcp_server_elicitation(
            ...
            McpServerElicitationRequestParams {
                ...
                server_name: "peregrine_apps".to_string(),
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    ...
    crate::client::ModelClient::new(
        ...
        peregrine_types::protocol::SessionSource::Exec,
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        peregrine_home.join(peregrine_config::CONFIG_TOML_FILE),
...
) -> std::io::Result<Vec<peregrine_types::protocol::HookRunSummary>> {
    let hooks = Hooks::new(HooksConfig {
        feature_enabled: true,
        config_layer_stack: Some(config.compat_config_layer_stack()),
        ..HooksConfig::default()
    });
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    let (session, _turn_context) = make_session_and_context().await;
    let peregrine_home = session.peregrine_home().await;
    std::fs::create_dir_all(&peregrine_home).expect("create peregrine home");
    let config_toml_path = peregrine_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
    ...
    let apps_toml = config
        ...
        .cloned()
        .expect("apps table");
    let apps = peregrine_config::types::AppsConfigToml::deserialize(apps_toml)
...
async fn reload_user_config_layer_updates_base_and_selected_profile_layers() {
    ...
    let peregrine_home = session.peregrine_home().await;
    std::fs::create_dir_all(&peregrine_home).expect("create peregrine home");
    let base_config_path = peregrine_home.join(CONFIG_TOML_FILE);
    let profile_config_path = peregrine_home.join("work.config.toml");
    std::fs::write(
        &base_config_path,
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .peregrine_home(peregrine_home.to_path_buf())
...
async fn reload_user_config_layer_refreshes_hooks() -> anyhow::Result<()> {
    let session = make_session_with_config(|config| {
        config
            ...
            .expect("enable Peregrine hooks");
    })
    .await?;
    let peregrine_home = session.peregrine_home().await;
    std::fs::create_dir_all(&peregrine_home)?;
    let config_toml_path = peregrine_home.join(CONFIG_TOML_FILE);
    let user_config: peregrine_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
    ...
    let config = session.get_config().await;
    let hook_config_layer_stack = config
        .config_layer_stack
        .with_user_config(&config_toml_path, user_config.clone());
    let hook_list = codex_hooks::list_hooks(codex_hooks::HooksConfig {
        feature_enabled: true,
        config_layer_stack: Some(peregrine_config::codex_compat::config_layer_stack(
            &hook_config_layer_stack,
        )),
    ...
    assert_eq!(
        ...
        peregrine_types::protocol::HookTrustStatus::Untrusted
    );

    let trusted_user_config: peregrine_config::TomlValue =
...
async fn refresh_runtime_config_refreshes_hooks() -> anyhow::Result<()> {
    ...
    struct NormalizedHookIdentity {
        ...
        #[serde(flatten)]
        group: peregrine_config::MatcherGroup,
    }
    let trusted_hash = {
        let identity = NormalizedHookIdentity {
            ...
            group: peregrine_config::MatcherGroup {
                matcher: None,
                hooks: vec![peregrine_config::HookHandlerConfig::Command {
        ...
        let identity = peregrine_config::TomlValue::try_from(identity)?;
        peregrine_config::version_for_toml(&identity)
    };
...
async fn refresh_runtime_config_updates_runtime_refreshable_fields_and_keeps_session_static_settings()
 {
    ...
    std::fs::create_dir_all(&peregrine_home).expect("create peregrine home");
    std::fs::write(
        peregrine_home.join(CONFIG_TOML_FILE),
        r#"[apps.calendar]
enabled = false
destructive_enabled = false
...
async fn turn_error_lifecycle_exposes_error_and_stores() {
    ...
    let expected = RecordedTurnError {
        ...
        turn_level_id: turn_context.sub_id.clone(),
        turn_id: turn_context.sub_id.clone(),
        error: PeregrineErrorInfo::UsageLimitExceeded,
        saw_session_store: true,
        saw_thread_store: true,
    };

    session
        .emit_turn_error_lifecycle(&turn_context, PeregrineErrorInfo::UsageLimitExceeded)
        .await;
...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let sandbox_policy = SandboxPolicy::ExternalSandbox {
        network_access: peregrine_types::protocol::NetworkAccess::Restricted,
    };
...
async fn session_permission_profile_rebinds_runtime_workspace_roots() -> anyhow::Result<()> {
    let peregrine_home = tempfile::TempDir::new()?;
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_codex().with_config(|config| {
        config.permissions.approval_policy =
            peregrine_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    fork_config.permissions.approval_policy =
        peregrine_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "peregrine_core__peregrine_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let previous_context_item = TurnContextItem {
        ...
        summary: peregrine_types::config_types::ReasoningSummary::Auto,
    };
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            peregrine_types::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            peregrine_types::protocol::UserMessageEvent {
                client_id: None,
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            peregrine_types::protocol::TurnCompleteEvent {
                turn_id,
...
async fn thread_rollback_fails_without_persisted_thread_history() {
    ...
    assert_eq!(
        ...
        Some(PeregrineErrorInfo::ThreadRollbackFailed)
    );
...
async fn set_rate_limits_retains_previous_credits() {
    ...
    let config = build_test_config(peregrine_home.path()).await;
    ...
    let session_configuration = SessionConfiguration {
        ...
        peregrine_home: config.peregrine_home.clone(),
        thread_name: None,
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(peregrine_types::account::PlanType::Plus),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(initial.clone());

    let update = RateLimitSnapshot {
        limit_id: Some("peregrine_other".to_string()),
        limit_name: Some("peregrine_other".to_string()),
        primary: Some(RateLimitWindow {
            used_percent: 40.0,
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        secondary: None,
        credits: None,
        plan_type: Some(peregrine_types::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
    ...
    loop {
        ...
        match evt.msg {
            EventMsg::Error(payload)
                if payload.codex_error_info == Some(PeregrineErrorInfo::ThreadRollbackFailed) =>
            {
                return payload;
            }
            _ => continue,
...
async fn build_test_config(peregrine_home: &Path) -> Config {
...
async fn session_configuration_apply_permission_profile_preserves_existing_deny_read_entries() {
    ...
    let permission_profile = peregrine_types::models::PermissionProfile::from_runtime_permissions(
...
async fn active_profile_update_rebuilds_network_proxy_config() -> std::io::Result<()> {
    let peregrine_home = tempfile::tempdir().expect("create peregrine home");
    let cwd = tempfile::tempdir().expect("create cwd");
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let peregrine_home = parent_config.peregrine_home.clone();
    let skill_dir = peregrine_home.join("skills").join("demo");
    ...
    let role_path = peregrine_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_new_fails_when_zsh_fork_enabled_without_packaged_zsh() {
    let peregrine_home = tempfile::tempdir().expect("create temp dir");
    let mut config = build_test_config(peregrine_home.path()).await;
    config
        .features
    ...
    let plugins_manager = Arc::new(PluginsManager::new(config.peregrine_home.to_path_buf()));
    let mcp_manager = Arc::new(McpManager::new(Arc::clone(&plugins_manager)));
    let skills_manager = Arc::new(SkillsManager::new(
        config.peregrine_home.clone(),
    ...
    let services = SessionServices {
        mcp_connection_manager: Arc::new(RwLock::new(
            McpConnectionManager::new_uninitialized_with_permission_profile(
                &peregrine_config::codex_compat::approval_policy(
                    &config.permissions.approval_policy,
                ),
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            peregrine_types::request_permissions::RequestPermissionsResponse {
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = peregrine_types::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(peregrine_types::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            session
                .request_permissions(
                    &turn_context,
                    call_id,
                    peregrine_types::request_permissions::RequestPermissionsArgs {
...
async fn user_turn_updates_approvals_reviewer() {
    ...
    handlers::user_input_or_turn(
        ...
        Op::UserInput {
            ...
            responsesapi_client_metadata: None,
            additional_context: Default::default(),
            thread_settings: peregrine_types::protocol::ThreadSettingsOverrides {
                cwd: Some(config.cwd.to_path_buf()),
                approval_policy: Some(config.permissions.approval_policy.value()),
                approvals_reviewer: Some(peregrine_config::types::ApprovalsReviewer::AutoReview),
                sandbox_policy: Some(config.legacy_sandbox_policy()),
                summary: config.model_reasoning_summary,
                personality: config.personality,
                collaboration_mode: Some(peregrine_types::config_types::CollaborationMode {
                    mode: peregrine_types::config_types::ModeKind::Default,
                    settings: peregrine_types::config_types::Settings {
    ...
    assert_eq!(
        ...
        peregrine_config::types::ApprovalsReviewer::AutoReview
    );
}
...
async fn unknown_turn_environment_returns_error() {
    ...
    assert!(matches!(err, PeregrineErr::InvalidRequest(_)));
...
{
    let (tx_event, rx_event) = async_channel::unbounded();
    let mut config = build_test_config(peregrine_home).await;
...
async fn make_goal_session_and_context_with_rx() -> (
    ...
    (session, turn_context, rx, peregrine_home)
}
...
async fn handle_output_item_done_records_image_save_history_message() {
    ...
    let expected_saved_path = crate::stream_events_utils::image_generation_artifact_path(
        &turn_context.config.peregrine_home,
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    let text_element = peregrine_types::user_input::TextElement::new(
        peregrine_types::user_input::ByteRange { start: 5, end: 12 },
...
async fn interrupt_accounts_active_goal_without_pausing() -> anyhow::Result<()> {
    let (sess, tc, _rx, _peregrine_home) = make_goal_session_and_context_with_rx().await;
    ...
    assert_eq!(
        peregrine_types::protocol::ThreadGoalStatus::Active,
        goal.status
    );
...
async fn active_goal_continuation_runs_again_after_no_tool_turn() -> anyhow::Result<()> {
    ...
    tokio::time::timeout(std::time::Duration::from_secs(8), async {
        loop {
            let event = test.peregrine.next_event().await?;
    ...
    let goal_context_text = responses
        ...
        .find(|text| text.contains("<peregrine_internal_context source=\"goal\">"))
...
async fn pending_request_user_input_does_not_spawn_extra_goal_continuation() -> anyhow::Result<()> {
    ...
    let request_user_input_event = wait_for_event_match(&test.peregrine, |event| match event {
    ...
    assert!(
        timeout(Duration::from_millis(200), test.peregrine.next_event())
            .await
            .is_err(),
...
async fn budget_limited_accounting_steers_active_turn_without_aborting() -> anyhow::Result<()> {
    let (sess, tc, rx, _peregrine_home) = make_goal_session_and_context_with_rx().await;
    sess.set_thread_goal(
        tc.as_ref(),
        SetGoalRequest {
            objective: Some("Keep improving the benchmark".to_string()),
    ...
    assert!(text.starts_with("<peregrine_internal_context source=\"goal\">"));
    assert!(text.trim_end().ends_with("</peregrine_internal_context>"));
                    ...
                    if role == "user"
                        && content.iter().any(|content| matches!(
                            ...
                            ContentItem::InputText { text }
                                if text.starts_with("<peregrine_internal_context source=\"goal\">")
                                    && text.trim_end().ends_with("</peregrine_internal_context>")
...
async fn create_goal_tool_rejects_existing_goal() {
    let (session, turn_context, _rx, _peregrine_home) =
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let peregrine_home = temp.path().join("home");
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let root_dot_codex = project_root.join(".peregrine");
    let nested_dot_codex = nested.join(".peregrine");
    ...
    write_project_trust_config(&peregrine_home, &[(&nested, TrustLevel::Trusted)]).await?;

    let config = ConfigBuilder::default()
        .peregrine_home(peregrine_home)
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let dot_codex = project_root.join(".peregrine");
    ...
    for (name, trust_entries, expected_hooks) in cases {
        let peregrine_home = temp.path().join(format!("home_{name}"));
        std::fs::create_dir_all(&peregrine_home)?;
        write_project_trust_config(&peregrine_home, &trust_entries).await?;
