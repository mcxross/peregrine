...
use peregrine_types::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_ELICITATION_APPROVAL_KIND_KEY;
use peregrine_types::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_ELICITATION_APPROVAL_KIND_MCP_TOOL_CALL;
use peregrine_types::mcp_approval_meta::APPROVAL_KIND_TOOL_SUGGESTION as MCP_ELICITATION_APPROVAL_KIND_TOOL_SUGGESTION;
use peregrine_types::mcp_approval_meta::APPROVALS_REVIEWER_KEY as MCP_ELICITATION_APPROVALS_REVIEWER_KEY;
use peregrine_types::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_ELICITATION_CONNECTOR_DESCRIPTION_KEY;
use peregrine_types::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_ELICITATION_CONNECTOR_ID_KEY;
use peregrine_types::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_ELICITATION_CONNECTOR_NAME_KEY;
use peregrine_types::mcp_approval_meta::REQUEST_TYPE_APPROVAL_REQUEST as MCP_ELICITATION_REQUEST_TYPE_APPROVAL_REQUEST;
use peregrine_types::mcp_approval_meta::REQUEST_TYPE_KEY as MCP_ELICITATION_REQUEST_TYPE_KEY;
use peregrine_types::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_ELICITATION_TOOL_DESCRIPTION_KEY;
use peregrine_types::mcp_approval_meta::TOOL_NAME_KEY as MCP_ELICITATION_TOOL_NAME_KEY;
use peregrine_types::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_ELICITATION_TOOL_PARAMS_KEY;
use peregrine_types::mcp_approval_meta::TOOL_TITLE_KEY as MCP_ELICITATION_TOOL_TITLE_KEY;
...
impl Session {
    ...
    pub async fn request_mcp_server_elicitation(
        ...
        let params: peregrine_app_server_protocol::McpServerElicitationRequestParams =
            match serde_json::to_value(params).and_then(serde_json::from_value) {
                Ok(params) => params,
                Err(err) => {
                    warn!(
                        "failed to convert MCP elicitation params for app-server protocol: {err:#}"
                    );
                    return McpServerElicitationOutcome {
                        response: None,
                        sent: false,
        ...
        let request = match params.request {
            peregrine_app_server_protocol::McpServerElicitationRequest::Form {
                meta,
                message,
                requested_schema,
            } => {
                ...
                peregrine_types::approvals::ElicitationRequest::Form {
                    meta,
                    message,
                    requested_schema,
                }
            }
            peregrine_app_server_protocol::McpServerElicitationRequest::Url {
                meta,
                message,
                url,
                elicitation_id,
        ...
        let id = match request_id {
            rmcp::model::NumberOrString::String(value) => {
                peregrine_types::mcp::RequestId::String(value.to_string())
            }
            rmcp::model::NumberOrString::Number(value) => {
                peregrine_types::mcp::RequestId::Integer(value)
            }
        };
    ...
    async fn refresh_mcp_servers_inner(
        ...
        let mcp_config = self.services.mcp_manager.runtime_config(&config).await;
        ...
        let (refreshed_manager, cancel_token) = McpConnectionManager::new(
            ...
            store_mode,
            auth_statuses,
            &crate::config::codex_approval_policy(&turn_context.approval_policy),
...
fn plugin_install_elicitation_telemetry_metadata(
    ...
    let peregrine_types::approvals::ElicitationRequest::Form {
