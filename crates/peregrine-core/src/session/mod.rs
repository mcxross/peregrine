...
use futures::prelude::*;
use peregrine_config::codex_compat::McpServerConfig;
use peregrine_config::codex_compat::OAuthCredentialsStoreMode;
...
use peregrine_types::approvals::ElicitationRequestEvent;
use peregrine_types::approvals::ExecPolicyAmendment;
use peregrine_types::approvals::NetworkPolicyAmendment;
use peregrine_types::approvals::NetworkPolicyRuleAction;
...
use peregrine_types::dynamic_tools::DynamicToolResponse;
use peregrine_types::dynamic_tools::DynamicToolSpec;
use peregrine_types::items::TurnItem;
use peregrine_types::items::UserMessageItem;
...
use peregrine_types::models::format_allow_prefixes;
...
use peregrine_types::protocol::FileChange;
use peregrine_types::protocol::HasLegacyEvent;
...
use peregrine_types::protocol::RawResponseItemEvent;
...
use peregrine_types::protocol::TurnContextItem;
use peregrine_types::protocol::TurnContextNetworkItem;
...
use peregrine_types::request_permissions::RequestPermissionsArgs;
use peregrine_types::request_permissions::RequestPermissionsEvent;
use peregrine_types::request_permissions::RequestPermissionsResponse;
use peregrine_types::request_user_input::RequestUserInputArgs;
...
use crate::peregrine_thread::ThreadConfigSnapshot;
...
#[cfg(test)]
use peregrine_types::exec_output::StreamOutput;
        ...
        match self {
            ...
            Self::ActiveTurnNotSteerable { turn_kind } => {
                ...
                ErrorEvent {
                    ...
                    codex_error_info: Some(PeregrineErrorInfo::ActiveTurnNotSteerable {
                        turn_kind: *turn_kind,
                    }),
                }
            }
...
use peregrine_types::protocol::DeprecationNoticeEvent;
...
use peregrine_types::protocol::McpServerRefreshConfig;
use peregrine_types::protocol::ModelRerouteEvent;
use peregrine_types::protocol::ModelRerouteReason;
use peregrine_types::protocol::ModelVerification;
use peregrine_types::protocol::ModelVerificationEvent;
use peregrine_types::protocol::NetworkApprovalContext;
...
use peregrine_types::protocol::RequestUserInputEvent;
...
use peregrine_types::protocol::SessionNetworkProxyRuntime;
use peregrine_types::protocol::StreamErrorEvent;
...
use peregrine_types::protocol::WarningEvent;
use peregrine_types::user_input::UserInput;

/// The high-level interface to the Peregrine system.
...
/// Wrapper returned by [`Peregrine::spawn`] containing the spawned [`Peregrine`] and
/// the unique session id.
pub struct PeregrineSpawnOk {
    pub codex: Peregrine,
    pub thread_id: ThreadId,
}

pub(crate) struct PeregrineSpawnArgs {
...
impl Peregrine {
    ...
    pub(crate) async fn spawn(args: PeregrineSpawnArgs) -> PeregrineResult<PeregrineSpawnOk> {
        ...
        Self::spawn_internal(PeregrineSpawnArgs {
            parent_trace,
            ..args
        })
    ...
    async fn spawn_internal(args: PeregrineSpawnArgs) -> PeregrineResult<PeregrineSpawnOk> {
        let PeregrineSpawnArgs {
            mut config,
            installation_id,
            auth_manager,
            models_manager,
        ...
        let exec_policy = if crate::guardian::is_guardian_reviewer_source(&session_source) {
            ...
            Arc::new(
                ExecPolicyManager::load(&config.config_layer_stack)
                    .await
                    .map_err(|err| PeregrineErr::Fatal(format!("failed to load rules: {err}")))?,
            )
        };
        ...
        // Generate a unique ID for the lifetime of this Peregrine session.
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.peregrine_home)
        })?;
        ...
        Ok(PeregrineSpawnOk { codex, thread_id })
    }
    ...
    /// Use sparingly: prefer `submit()` so Peregrine is responsible for generating
    /// unique IDs for each submission.
    pub async fn submit_with_id(&self, mut sub: Submission) -> PeregrineResult<()> {
        ...
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| PeregrineErr::InternalAgentDied)?;
        Ok(())
    }
    ...
    pub async fn set_thread_memory_mode(
        &self,
        mode: peregrine_types::protocol::ThreadMemoryMode,
    ) -> anyhow::Result<()> {
    ...
    pub async fn shutdown_and_wait(&self) -> PeregrineResult<()> {
        ...
        match self.submit(Op::Shutdown).await {
            Ok(_) => {}
            Err(PeregrineErr::InternalAgentDied) => {}
...
fn session_permission_profile_state_from_config(
    config: &Config,
) -> PeregrineResult<PermissionProfileState> {
...
impl Session {
    ...
    /// Builds the `x-peregrine-beta-features` header value for this session.
    ///
    ...
    #[cfg(test)]
    pub(crate) async fn peregrine_home(&self) -> AbsolutePathBuf {
        let state = self.state.lock().await;
        state.session_configuration.peregrine_home().clone()
    }
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect Peregrine performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn update_settings(
        ...
        let (
            ...
            let peregrine_home = updated.peregrine_home.clone();
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &peregrine_types::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| peregrine_types::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&peregrine_home, amendment)
            .await?;
    ...
    pub(crate) async fn notify_stream_error(
        ...
        peregrine_error: PeregrineErr,
    ) {
        let additional_details = peregrine_error.to_string();
        let codex_error_info = PeregrineErrorInfo::ResponseStreamDisconnected {
            http_status_code: peregrine_error.http_status_code_value(),
        };
