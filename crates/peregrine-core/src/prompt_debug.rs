...
pub async fn build_prompt_input(
...
) -> PeregrineResult<Vec<ResponseItem>> {
    ...
    let thread_manager = ThreadManager::new(
        ...
        Arc::new(
            ...
            .await
            .map_err(|err| PeregrineErr::Fatal(err.to_string()))?,
        ),
    ...
    let output =
        build_prompt_input_from_session(thread.thread.peregrine.session.as_ref(), input).await;
