...
use peregrine_config::AppsRequirementsToml;
...
async fn plugin_apps_for_config(config: &Config) -> Vec<codex_plugin::AppConnectorId> {
    let plugins_input = config.plugins_config_input();
    PluginsManager::new(config.peregrine_home.to_path_buf())
...
async fn chatgpt_get_request_with_timeout<T: DeserializeOwned>(
    ...
    anyhow::ensure!(
        ...
        "ChatGPT account ID not available, please re-run `peregrine login`"
    );
...
pub async fn list_accessible_connectors_from_mcp_tools_with_options_and_status(
    ...
    let local_runtime_paths = ExecServerRuntimePaths::from_optional_paths(
        config.peregrine_self_exe.clone(),
        config.peregrine_linux_sandbox_exe.clone(),
    )?;
...
pub async fn list_accessible_connectors_from_mcp_tools_with_environment_manager(
    ...
    let auth_status_entries = compute_auth_statuses(
        mcp_servers.iter(),
        crate::config::codex_oauth_credentials_store_mode(config.mcp_oauth_credentials_store_mode),
        auth.as_ref(),
    )
    .await;
    ...
    let (mut mcp_connection_manager, cancel_token) = McpConnectionManager::new(
        ...
        crate::config::codex_oauth_credentials_store_mode(config.mcp_oauth_credentials_store_mode),
        auth_status_entries,
        &crate::config::codex_approval_policy(&config.permissions.approval_policy),
    ...
    let peregrine_apps_ready = if refreshed_tools_succeeded {
        true
    ...
    if peregrine_apps_ready || !accessible_connectors.is_empty() {
        write_cached_accessible_connectors(cache_key, &accessible_connectors);
    }
...
pub(crate) fn peregrine_app_tool_is_enabled(config: &Config, tool_info: &ToolInfo) -> bool {
