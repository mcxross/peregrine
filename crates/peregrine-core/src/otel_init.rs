...
use peregrine_config::types::OtelExporterKind as Kind;
use peregrine_config::types::OtelHttpProtocol as Protocol;
use std::error::Error;
...
pub fn build_provider(
    ...
    OtelProvider::from(&OtelSettings {
        ...
        codex_home: config.peregrine_home.to_path_buf(),
...
/// Filter predicate for exporting only Peregrine-owned events via OTEL.
/// Keeps events that originated from codex_otel module
pub fn peregrine_export_filter(meta: &tracing::Metadata<'_>) -> bool {
    OtelProvider::codex_export_filter(meta)
}
