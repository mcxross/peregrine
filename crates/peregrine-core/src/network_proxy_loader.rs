...
use peregrine_config::permissions_toml::overlay_network_domain_permissions;
...
async fn build_config_state_with_mtimes() -> Result<(ConfigState, Vec<LayerMtime>)> {
    ...
    .await
    .context("failed to load Peregrine config")?;
    ...
    stack
        ...
        .filter_map(|layer| {
            let path = match &layer.name {
                ...
                } => Some(dot_peregrine_folder.join(CONFIG_TOML_FILE)),
