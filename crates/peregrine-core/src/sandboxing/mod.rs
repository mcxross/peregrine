...
command transformation live in the peregrine-sandboxing crate; this module keeps
...
#[cfg(target_os = "macos")]
use crate::spawn::PEREGRINE_SANDBOX_ENV_VAR;
use crate::spawn::PEREGRINE_SANDBOX_NETWORK_DISABLED_ENV_VAR;
...
pub use peregrine_types::models::SandboxPermissions;
...
impl ExecRequest {
    ...
    pub(crate) fn from_sandbox_exec_request(
        ...
        if !network_sandbox_policy.is_enabled() {
            env.insert(
                PEREGRINE_SANDBOX_NETWORK_DISABLED_ENV_VAR.to_string(),
                "1".to_string(),
            );
        }
        #[cfg(target_os = "macos")]
        if sandbox == SandboxType::MacosSeatbelt {
            env.insert(
                PEREGRINE_SANDBOX_ENV_VAR.to_string(),
                "seatbelt".to_string(),
            );
        }
        Self {
...
) -> peregrine_types::error::Result<ExecToolCallOutput> {
