...
use peregrine_types::error::SandboxErr;
...
use peregrine_types::protocol::ExecCommandOutputDeltaEvent;
use peregrine_types::protocol::ExecOutputStream;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: peregrine_types::config_types::WindowsSandboxLevel,
...
fn select_process_exec_tool_sandbox_type(
    ...
    windows_sandbox_level: peregrine_types::config_types::WindowsSandboxLevel,
...
fn record_windows_sandbox_spawn_failure(
    ...
    let level = if matches!(
        windows_sandbox_level,
        peregrine_types::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_peregrine_home;
    ...
    let peregrine_home = find_peregrine_home().map_err(|err| {
        PeregrineErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve peregrine_home: {err}"
        )))
    })?;
        ...
        if use_elevated {
            run_windows_sandbox_capture_for_permission_profile_elevated(
                codex_windows_sandbox::ElevatedSandboxProfileCaptureRequest {
                    ...
                    peregrine_home: peregrine_home.as_ref(),
                    command,
                    cwd: &cwd,
                    env_map: env,
                    timeout_ms,
    ...
    let capture = match spawn_res {
        ...
        Ok(Err(err)) => {
            ...
            return Err(PeregrineErr::Io(io::Error::other(format!(
                "windows sandbox: {err}"
            ))));
        }
        Err(join_err) => {
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, PeregrineErr>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(PeregrineErr::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(PeregrineErr::Sandbox(SandboxErr::Timeout {
                    output: Box::new(exec_output),
                }));
            }

            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                return Err(PeregrineErr::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
...
/// PEREGRINE_SANDBOX_NETWORK_DISABLED=1 is added to the environment of the spawned
/// process.
///
...
fn has_reopened_writable_descendant(
    writable_roots: &[peregrine_types::protocol::WritableRoot],
) -> bool {
...
async fn consume_output(
    ...
    let stdout_reader = child.stdout.take().ok_or_else(|| {
        PeregrineErr::Io(io::Error::other(
