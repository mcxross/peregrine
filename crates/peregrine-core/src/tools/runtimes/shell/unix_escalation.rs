...
pub(super) async fn try_run_zsh_fork(
    ...
    let sandbox_exec_request = attempt
        ...
        .map_err(|err| ToolError::Peregrine(err.into()))?;
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        sandbox_policy_cwd,
        windows_sandbox_workspace_roots,
        codex_linux_sandbox_exe: ctx.turn.peregrine_linux_sandbox_exe.clone(),
...
impl ShellCommandExecutor for CoreShellCommandExecutor {
    async fn run(
        ...
        for var in ["PEREGRINE_ESCALATE_SOCKET", "EXEC_WRAPPER"] {
