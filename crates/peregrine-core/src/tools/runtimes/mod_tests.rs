...
fn apply_zsh_fork_path_prepend_uses_shell_parent() {
    ...
    apply_zsh_fork_path_prepend(
        ...
        PathBuf::from("/package/peregrine-resources/zsh/bin/zsh").as_path(),
    );

    let expected = "/package/peregrine-resources/zsh/bin:/usr/bin:/bin";
...
fn apply_zsh_fork_path_prepend_moves_existing_shell_parent_to_front() {
    let mut env = HashMap::from([(
        "PATH".to_string(),
        "/usr/bin:/package/peregrine-resources/zsh/bin:/bin:/package/peregrine-resources/zsh/bin"
            .to_string(),
    )]);
    let mut explicit_env_overrides = HashMap::new();
    ...
    assert_eq!(
        ...
        Some("/package/peregrine-resources/zsh/bin:/usr/bin:/bin")
    );
}

#[test]
fn explicit_escalation_keeps_user_proxy_env_without_peregrine_marker() {
...
#[test]
fn maybe_wrap_shell_lc_with_snapshot_restores_peregrine_thread_id_from_env() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport PEREGRINE_THREAD_ID='parent-thread'\n",
    )
    .expect("write snapshot");
    let session_shell = shell_with_snapshot(
        ShellType::Bash,
    ...
    let command = vec![
        "/bin/bash".to_string(),
        "-lc".to_string(),
        "printf '%s' \"$PEREGRINE_THREAD_ID\"".to_string(),
    ];
    let rewritten = maybe_wrap_shell_lc_with_snapshot(
        &command,
        &session_shell,
        ...
        &HashMap::new(),
        &HashMap::from([(
            "PEREGRINE_THREAD_ID".to_string(),
            "nested-thread".to_string(),
        )]),
    );
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env("PEREGRINE_THREAD_ID", "nested-thread")
        .output()
        .expect("run rewritten command");
...
#[cfg(target_os = "macos")]
#[test]
fn maybe_wrap_shell_lc_with_snapshot_refreshes_peregrine_proxy_git_ssh_command() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
...
#[cfg(target_os = "macos")]
#[test]
fn maybe_wrap_shell_lc_with_snapshot_clears_stale_peregrine_git_ssh_command_without_live_command() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    let stale_command = format!(
        "{CODEX_PROXY_GIT_SSH_COMMAND_MARKER}ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
...
fn maybe_wrap_shell_lc_with_snapshot_preserves_unset_override_variables() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport PEREGRINE_TEST_UNSET_OVERRIDE='snapshot-value'\n",
    )
    .expect("write snapshot");
    let session_shell = shell_with_snapshot(
        ShellType::Bash,
    ...
    let command = vec![
            "/bin/bash".to_string(),
            "-lc".to_string(),
            "if [ \"${PEREGRINE_TEST_UNSET_OVERRIDE+x}\" = x ]; then printf 'set:%s' \"$PEREGRINE_TEST_UNSET_OVERRIDE\"; else printf 'unset'; fi".to_string(),
        ];
    let explicit_env_overrides = HashMap::from([(
        "PEREGRINE_TEST_UNSET_OVERRIDE".to_string(),
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env_remove("PEREGRINE_TEST_UNSET_OVERRIDE")
        .output()
        .expect("run rewritten command");
    assert!(output.status.success(), "command failed: {output:?}");
    assert_eq!(String::from_utf8_lossy(&output.stdout), "unset");
