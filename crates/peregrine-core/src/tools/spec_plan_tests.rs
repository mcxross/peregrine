...
fn set_turn_workspace(turn: &mut TurnContext, workspace: &Path) {
    let workspace = workspace.abs();
    update_config(turn, |config| {
        config.cwd = workspace.clone();
        config.workspace_roots = vec![workspace.clone()];
    });
...
async fn code_mode_only_exposes_code_executor_and_hides_nested_tools() {
    ...
    assert_eq!(
        plain.namespace_function_names("peregrine_app"),
        &["lookup".to_string()]
    );
    ...
    assert_eq!(
        code_mode_only.namespace_function_names("peregrine_app"),
