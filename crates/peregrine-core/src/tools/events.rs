...
use peregrine_types::protocol::PatchApplyStatus;
...
impl ToolEmitter {
    ...
    pub async fn finish(
        ...
        let (event, result) = match out {
            ...
            Err(ToolError::Peregrine(PeregrineErr::Sandbox(SandboxErr::Timeout { output }))) => {
                let response = self.format_exec_output_for_model(&output, ctx);
                let event = ToolEventStage::Failure(ToolEventFailure::Output(*output));
                let result = Err(FunctionCallError::RespondToModel(response));
                (event, result)
            }
            Err(ToolError::Peregrine(PeregrineErr::Sandbox(SandboxErr::Denied {
                output, ..
            }))) => {
                let response = self.format_exec_output_for_model(&output, ctx);
...
mod tests {
    ...
    async fn denied_apply_patch_tracks_committed_delta() {
        ...
        assert_failed_apply_patch_tracks_committed_delta(
            Err(ToolError::Peregrine(PeregrineErr::Sandbox(
                SandboxErr::Denied {
