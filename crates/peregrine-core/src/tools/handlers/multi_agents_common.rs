...
use peregrine_types::protocol::CollabAgentRef;
use peregrine_types::protocol::CollabAgentStatusEntry;
...
pub(crate) fn collab_spawn_error(err: PeregrineErr) -> FunctionCallError {
    match err {
        PeregrineErr::UnsupportedOperation(message) if message == "thread manager dropped" => {
            FunctionCallError::RespondToModel("collab manager unavailable".to_string())
        }
        PeregrineErr::UnsupportedOperation(message) => FunctionCallError::RespondToModel(message),
        err => FunctionCallError::RespondToModel(format!("collab spawn failed: {err}")),
    }
}

pub(crate) fn collab_agent_error(agent_id: ThreadId, err: PeregrineErr) -> FunctionCallError {
    match err {
        PeregrineErr::ThreadNotFound(id) => {
            FunctionCallError::RespondToModel(format!("agent with id {id} not found"))
        }
        PeregrineErr::InternalAgentDied => {
            FunctionCallError::RespondToModel(format!("agent with id {agent_id} is closed"))
        }
        PeregrineErr::UnsupportedOperation(_) => {
            FunctionCallError::RespondToModel("collab manager unavailable".to_string())
        }
...
pub(crate) fn apply_spawn_agent_runtime_overrides(
    ...
    config.peregrine_linux_sandbox_exe = turn.peregrine_linux_sandbox_exe.clone();
    #[allow(deprecated)]
...
fn find_spawn_agent_model_name(
    available_models: &[peregrine_types::openai_models::ModelPreset],
    requested_model: &str,
