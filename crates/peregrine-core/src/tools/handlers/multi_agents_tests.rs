...
use peregrine_types::protocol::FileSystemAccessMode;
use peregrine_types::protocol::FileSystemPath;
use peregrine_types::protocol::FileSystemSandboxEntry;
use peregrine_types::protocol::FileSystemSandboxPolicy;
use peregrine_types::protocol::InitialHistory;
use peregrine_types::protocol::InterAgentCommunication;
use peregrine_types::protocol::NetworkSandboxPolicy;
...
async fn install_role_with_model_override(turn: &mut TurnContext) -> String {
    ...
    tokio::fs::create_dir_all(&turn.config.peregrine_home)
        .await
        .expect("peregrine home should be created");
        ...
        | ResponseInputItem::CustomToolCallOutput { output, .. } => {
            let content = match output.body {
                ...
                FunctionCallOutputBody::ContentItems(items) => {
                    peregrine_types::models::function_call_output_content_items_to_text(&items)
                        .unwrap_or_default()
                }
            };
...
async fn multi_agent_v2_list_agents_returns_completed_status_and_last_task_message() {
    ...
    let child_thread = manager
        ...
        .await
        .expect("child thread should exist");
    let child_turn = child_thread.peregrine.session.new_default_turn().await;
    child_thread
        .peregrine
        .session
        .send_event(
...
async fn multi_agent_v2_assign_task_completion_notifies_parent_on_every_turn() {
    ...
    let first_turn = thread.peregrine.session.new_default_turn().await;
    thread
        .peregrine
        .session
        .send_event(
    ...
    let second_turn = thread.peregrine.session.new_default_turn().await;
    thread
        .peregrine
        .session
        .send_event(
...
async fn multi_agent_v2_interrupted_turn_does_not_notify_parent() {
    ...
    let aborted_turn = thread.peregrine.session.new_default_turn().await;
    thread
        .peregrine
        .session
        .send_event(
...
async fn tool_handlers_cascade_close_and_resume_and_keep_explicitly_closed_subtrees_closed() {
    ...
    let parent_session = parent.thread.peregrine.session.clone();
    ...
    let child_thread = manager
        ...
        .await
        .expect("child thread should exist");
    let child_session = child_thread.peregrine.session.clone();
    ...
    let operator_session = operator.thread.peregrine.session.clone();
    let _ = manager
        .agent_control()
    ...
    let parent_resume_output = ResumeAgentHandler
        .handle(invocation(
            operator_session,
            operator.thread.peregrine.session.new_default_turn().await,
            "resume_agent",
...
async fn build_agent_spawn_config_uses_turn_context_values() {
    ...
    turn.peregrine_linux_sandbox_exe = Some(PathBuf::from("/bin/echo"));
    #[allow(deprecated)]
    ...
    expected.peregrine_linux_sandbox_exe = turn.peregrine_linux_sandbox_exe.clone();
    #[allow(deprecated)]
    {
