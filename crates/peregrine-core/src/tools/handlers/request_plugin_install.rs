...
impl ToolExecutor<ToolInvocation> for RequestPluginInstallHandler {
    ...
    async fn handle(
        ...
        if args.tool_type == DiscoverableToolType::Plugin
            && turn.app_server_client_name.as_deref() == Some("peregrine-tui")
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin install requests are not available in peregrine-tui yet".to_string(),
            ));
        }
...
async fn maybe_persist_disabled_install_request(
    ...
    if let Err(err) = persist_disabled_install_request(&turn.config.peregrine_home, tool).await {
        warn!(
            error = %err,
            tool_id = tool.id(),
...
async fn persist_disabled_install_request(
    peregrine_home: &codex_utils_absolute_path::AbsolutePathBuf,
    tool: &DiscoverableTool,
) -> anyhow::Result<()> {
