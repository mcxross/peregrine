...
pub struct Cli {
    ...
    /// Error out when config.toml contains fields that are not recognized by this version of Peregrine.
    #[arg(long = "strict-config", default_value_t = false)]
    pub strict_config: bool,

    // Internal controls set by the top-level `peregrine resume` subcommand.
    ...
    /// top-level `peregrine resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `peregrine fork` subcommand.
    // These are not exposed as user flags on the base `codex` command.
    #[clap(skip)]
    ...
    /// top-level `peregrine fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
...
fn mark_tui_args(cmd: clap::Command) -> clap::Command {
    cmd.mut_arg("raw_overrides", |arg| {
        arg.help("Override a configuration value that would otherwise be loaded from Peregrine config.toml")
            .long_help(
                "Override a configuration value that would otherwise be loaded from \
                 $PEREGRINE_HOME/config.toml, or ~/.peregrine/config.toml when \
                 PEREGRINE_HOME is unset. Use a dotted path (`foo.bar.baz`) to \
                 override nested values. The `value` portion is parsed as TOML. \
                 If it fails to parse as TOML, the raw string is used as a literal.\n\n\
                 Examples:\n  - `-c model=\"gpt-5.4\"`\n  - `-c \
    ...
    .mut_arg("config_profile_v2", |arg| {
        arg.help(
            "Layer $PEREGRINE_HOME/<name>.config.toml on top of the base user config",
        )
    })
