...
use crate::agent::shimmer::shimmer_spans;
            ...
            for (line_number, line) in contents.lines().enumerate() {
                ...
                if direct_spinner.is_match(code) {
                    violations.push(format!(
                        "{relative_path}:{} contains a direct `spinner(...)` call; use crate::agent::motion instead",
                        line_number + 1
                    ));
                }
                if direct_shimmer.is_match(code) {
                    violations.push(format!(
                        "{relative_path}:{} contains a direct `shimmer_spans(...)` call; use crate::agent::motion instead",
                        line_number + 1
                    ));
                }
            }
