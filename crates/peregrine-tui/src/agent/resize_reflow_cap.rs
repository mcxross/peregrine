...
//! the fallback bucket.
//!
//! These caps are deliberately conservative: Peregrine is rebuilding normal terminal scrollback, not an
...
use peregrine_config::types::DEFAULT_TERMINAL_RESIZE_REFLOW_FALLBACK_MAX_ROWS;

use crate::agent::legacy_core::config::TerminalResizeReflowConfig;
...
pub(crate) fn resize_reflow_max_rows(config: TerminalResizeReflowConfig) -> Option<usize> {
    resize_reflow_max_rows_for(
        config,
        &terminal_info(),
        crate::agent::tui::running_in_vscode_terminal(),
    )
}
