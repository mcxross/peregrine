...
//!
//! Peregrine uses the terminal scrollback itself for finalized chat history, so inserting a history
...
use crate::agent::terminal_hyperlinks::decorate_spans;
use crate::agent::terminal_hyperlinks::plain_hyperlink_lines;
use crate::agent::terminal_hyperlinks::remap_wrapped_line;
use crate::agent::wrapping::RtOptions;
use crate::agent::wrapping::adaptive_wrap_line;
use crate::agent::wrapping::line_contains_url_like;
use crate::agent::wrapping::line_has_mixed_url_and_non_url_tokens;
...
/// Zellij does not constrain soft-wrapped continuation rows to Peregrine's scroll region, so its raw
...
pub fn insert_history_lines<B>(
    terminal: &mut crate::agent::custom_terminal::Terminal<B>,
    lines: Vec<Line>,
) -> io::Result<()>
where
    B: Backend + Write,
...
mod tests {
    use super::*;
    use crate::agent::markdown_render::render_markdown_text;
    ...
    fn writes_semantic_web_link_without_changing_visible_text() {
        ...
        let line =
            crate::agent::terminal_hyperlinks::annotate_web_urls_in_line(Line::from(destination));
    ...
    fn vt100_terminal_wrap_policy_does_not_pre_wrap_long_paragraph() {
        ...
        assert!(
            ...
            "expected terminal soft-wrap instead of Peregrine word pre-wrap, rows: {rows:?}"
        );
    }
