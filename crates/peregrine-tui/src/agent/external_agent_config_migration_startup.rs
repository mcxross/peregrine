...
use crate::agent::external_agent_config_migration::ExternalAgentConfigMigrationOutcome;
use crate::agent::external_agent_config_migration::run_external_agent_config_migration_prompt;
...
fn external_agent_config_migration_success_message(
    ...
    if items.iter().any(|item| {
        item.item_type
            == peregrine_app_server_protocol::ExternalAgentConfigMigrationItemType::Plugins
    }) {
...
mod tests {
    ...
    async fn visible_external_agent_config_migration_items_omits_hidden_scopes() {
        let peregrine_home = tempdir().expect("temp peregrine home");
