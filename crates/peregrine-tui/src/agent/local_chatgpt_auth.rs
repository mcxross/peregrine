...
pub(crate) fn load_local_chatgpt_auth(
    ...
    let auth = load_auth_dot_json(peregrine_home, auth_credentials_store_mode)
...
mod tests {
    ...
    fn write_chatgpt_auth(peregrine_home: &Path, plan_type: &str) {
        ...
        save_auth(peregrine_home, &auth, AuthCredentialsStoreMode::File)
            .expect("chatgpt auth should save");
    }

    #[test]
    fn loads_local_chatgpt_auth_from_managed_auth() {
        let peregrine_home = TempDir::new().expect("tempdir");
        write_chatgpt_auth(peregrine_home.path(), "business");
    ...
    fn preserves_usage_based_plan_type_wire_name() {
        let peregrine_home = TempDir::new().expect("tempdir");
        write_chatgpt_auth(peregrine_home.path(), "self_serve_business_usage_based");
