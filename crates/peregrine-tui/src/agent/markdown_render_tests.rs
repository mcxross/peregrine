...
use crate::agent::markdown_render::COLON_LOCATION_SUFFIX_RE;
use crate::agent::markdown_render::HASH_LOCATION_SUFFIX_RE;
use crate::agent::markdown_render::render_markdown_text;
use crate::agent::markdown_render::render_markdown_text_with_width;
use crate::agent::markdown_render::render_markdown_text_with_width_and_cwd;
...
fn table_renders_app_style_rows_with_themed_bold_header() {
    ...
    assert!(text.lines[1].spans[0].style.fg.is_some());
    assert!(
        !text.lines[2]
            .style
            .add_modifier
...
fn table_separates_logical_rows_after_wrapped_content() {
    ...
    let text = crate::agent::markdown_render::render_markdown_text_with_width(md, Some(30));
    let lines: Vec<String> = text
        .lines
        .iter()
        .map(|line| line.spans.iter().map(|span| span.content.clone()).collect())
...
| [#23651: Zellij scrollback misses Peregrine transcript over SSH](https://github.com/openai/codex/issues/23651) | `+1` 3, human comments 2 | Medium | Clear regression and strong scrollback evidence. |
...
fn table_falls_back_to_key_value_records_if_grid_cannot_fit() {
    ...
    let text = crate::agent::markdown_render::render_markdown_text_with_width(md, Some(/*width*/ 20));
    let lines: Vec<String> = text
        .lines
        .iter()
        .map(|line| line.spans.iter().map(|span| span.content.clone()).collect())
...
fn table_key_value_fallback_preserves_rich_values_and_themed_labels() {
    ...
    let text = crate::agent::markdown_render::render_markdown_text_with_width(md, Some(/*width*/ 16));
    let lines = plain_lines(&text);
