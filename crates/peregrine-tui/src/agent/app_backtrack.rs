...
fn trim_transcript_cells_to_nth_user(
    transcript_cells: &mut Vec<Arc<dyn crate::agent::history_cell::HistoryCell>>,
...
pub(crate) fn user_count(cells: &[Arc<dyn crate::agent::history_cell::HistoryCell>]) -> usize {
    user_positions_iter(cells).count()
}

fn has_backtrack_target(cells: &[Arc<dyn crate::agent::history_cell::HistoryCell>]) -> bool {
    user_count(cells) > 0
}

fn nth_user_position(
    cells: &[Arc<dyn crate::agent::history_cell::HistoryCell>],
    nth: usize,
) -> Option<usize> {
...
fn user_positions_iter(
    ...
    let type_of = |cell: &Arc<dyn crate::agent::history_cell::HistoryCell>| cell.as_any().type_id();
...
#[cfg(test)]
fn agent_group_count(cells: &[Arc<dyn crate::agent::history_cell::HistoryCell>]) -> usize {
...
mod tests {
    ...
    fn backtrack_unavailable_info_message_snapshot() {
        let cell = crate::agent::history_cell::new_info_event(
