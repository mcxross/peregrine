...
use crate::agent::bottom_pane::pending_input_preview::PendingInputPreview;
use crate::agent::bottom_pane::pending_thread_approvals::PendingThreadApprovals;
use crate::agent::bottom_pane::unified_exec_footer::UnifiedExecFooter;
...
use crate::agent::status_indicator_widget::StatusIndicatorWidget;
...
impl BottomPane {
    ...
    pub(crate) fn list_keymap(&self) -> crate::agent::keymap::ListKeymap {
        self.keymap.list.clone()
    }
...
mod tests {
    ...
    fn test_pane_with_disable_paste_burst(
        ...
        BottomPane::new(BottomPaneParams {
            ...
            placeholder_text: "Ask Peregrine to do anything".to_string(),
            disable_paste_burst,
    ...
    fn exec_request() -> ApprovalRequest {
        ApprovalRequest::Exec {
            thread_id: peregrine_types::ThreadId::new(),
            thread_label: None,
            id: "1".to_string(),
    ...
    fn esc_with_skill_popup_does_not_interrupt_task() {
        ...
        while let Ok(ev) = rx.try_recv() {
            assert!(
                !matches!(ev, AppEvent::PeregrineOp(Op::Interrupt)),
    ...
    fn esc_interrupts_running_task_when_no_popup() {
        ...
        assert!(
            matches!(rx.try_recv(), Ok(AppEvent::PeregrineOp(Op::Interrupt))),
    ...
    fn selection_view_esc_respects_remapped_list_cancel() {
        ...
        let mut pane = test_pane(tx);
        let mut keymap = RuntimeKeymap::defaults();
        keymap.list.cancel = vec![crate::agent::key_hint::plain(KeyCode::Char('q'))];
        pane.set_keymap_bindings(&keymap);
        pane.show_selection_view(SelectionViewParams {
            title: Some("Agents".to_string()),
            items: vec![SelectionItem {
