...
use crate::agent::render::RectExt as _;
...
impl AppLinkViewParams {
    pub(crate) fn from_url_app_server_request(
        ...
        request: &peregrine_app_server_protocol::McpServerElicitationRequest,
    ) -> Option<Self> {
        let peregrine_app_server_protocol::McpServerElicitationRequest::Url {
            meta,
            message,
            url,
            elicitation_id,
...
impl crate::agent::render::renderable::Renderable for AppLinkView {
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        Paragraph::new(lines)
            .wrap(Wrap { trim: false })
            .render(inner, buf);
        crate::agent::terminal_hyperlinks::mark_url_hyperlink(buf, inner, &self.url);
...
mod tests {
    ...
    fn auth_url_request(url: &str) -> peregrine_app_server_protocol::McpServerElicitationRequest {
    ...
    fn non_codex_apps_url_elicitation_builds_generic_app_link_params() {
        ...
        let request = peregrine_app_server_protocol::McpServerElicitationRequest::Url {
            meta: None,
                ...
                (0..area.width)
                    .map(|x| {
                        ...
                        if symbol.is_empty() {
                            ...
                            crate::agent::terminal_hyperlinks::strip_osc8(symbol)
                                .chars()
                                .next()
                                .unwrap_or(' ')
                        }
    ...
    fn remapped_horizontal_list_keys_control_action_selection() {
        let (tx_raw, _rx) = unbounded_channel::<AppEvent>();
        let tx = AppEventSender::new(tx_raw);
        let mut list_keymap = crate::agent::keymap::RuntimeKeymap::defaults().list;
