...
impl RequestUserInputOverlay {
    ...
    fn footer_tips(&self) -> Vec<FooterTip> {
        ...
        if let Some(interrupt_key) = self.interrupt_turn_keys.first()
            && !(self.has_options()
                && notes_visible
                && *interrupt_key == crate::agent::key_hint::plain(KeyCode::Esc))
        {
...
mod tests {
    ...
    fn expect_interrupt_only(rx: &mut tokio::sync::mpsc::UnboundedReceiver<AppEvent>) {
        ...
        let AppEvent::PeregrineOp(op) = event else {
            panic!("expected PeregrineOp");
        };
    ...
    fn options_can_submit_empty_when_unanswered() {
        ...
        let AppEvent::PeregrineOp(Op::UserInputAnswer { id, response, .. }) = event else {
    ...
    fn enter_commits_default_selection_on_last_option_question() {
        ...
        let event = rx.try_recv().expect("expected AppEvent");
        let AppEvent::PeregrineOp(Op::UserInputAnswer { response, .. }) = event else {
            panic!("expected UserInputAnswer");
        };
        let answer = response.answers.get("q1").expect("answer missing");
    ...
    fn freeform_footer_shows_configured_submit_binding() {
        ...
        keymap.composer.submit = vec![crate::agent::key_hint::ctrl(KeyCode::Char('j'))];
    ...
    fn request_user_input_uses_remapped_interrupt_binding_while_notes_are_visible() {
        let (tx, mut rx) = test_sender();
        let mut keymap = RuntimeKeymap::defaults();
        keymap.chat.interrupt_turn = vec![crate::agent::key_hint::plain(KeyCode::F(12))];
    ...
    fn freeform_submit_binding_wins_over_question_navigation() {
        let (tx, _rx) = test_sender();
        let mut keymap = RuntimeKeymap::defaults();
        keymap.composer.submit = vec![crate::agent::key_hint::ctrl(KeyCode::Char('n'))];
        let mut overlay = RequestUserInputOverlay::new_with_keymap(
            request_event(
                "turn-1",
                vec![
