...
impl MemoriesSettingsView {
    ...
    fn settings_header(&self) -> ColumnRenderable<'_> {
        ...
        header.push(Line::from(
            "Choose how Peregrine uses and creates memories. Changes are saved to config.toml"
                .dim(),
        ));
        header
    }
    ...
    fn reset_confirmation_header(&self) -> ColumnRenderable<'_> {
        ...
        header.push(Line::from(
            "This clears local memory files and rollout summaries for the current Peregrine home."
                .dim(),
        ));
        header
    }
