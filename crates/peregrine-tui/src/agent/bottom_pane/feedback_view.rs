...
pub(crate) fn feedback_success_cell(
    ...
    let mut lines = vec![Line::from(match issue_url.as_ref() {
        Some(_) if feedback_audience == FeedbackAudience::OpenAiEmployee => {
            format!("{prefix} Please report this in the Peregrine feedback channel:")
        }
...
pub(crate) fn feedback_upload_consent_params(
    ...
    let mut header_lines: Vec<Box<dyn crate::agent::render::renderable::Renderable>> = vec![
        Line::from("Upload logs?".bold()).into(),
        Line::from("").into(),
        Line::from("The following files will be sent:".dim()).into(),
        Line::from(vec!["  • ".into(), "peregrine-logs.log".into()]).into(),
        Line::from(vec![
            "  • ".into(),
    ...
    super::SelectionViewParams {
        ...
        items: vec![
            super::SelectionItem {
                name: "Yes".to_string(),
                description: Some(
                    "Share the current Peregrine session logs and diagnostics with the team for troubleshooting."
                        .to_string(),
                ),
        ...
        header: Box::new(crate::agent::render::renderable::ColumnRenderable::with(
            header_lines,
        )),
        ..Default::default()
    }
...
mod tests {
    ...
    #[test]
    fn issue_url_is_not_generated_without_peregrine_feedback_target() {
        for category in [
            FeedbackCategory::Bug,
            FeedbackCategory::BadResult,
            FeedbackCategory::SafetyCheck,
            ...
            assert!(
                issue_url_for_category(category, "thread-1", FeedbackAudience::OpenAiEmployee)
                    .is_none()
            );
            assert!(
                issue_url_for_category(category, "thread-1", FeedbackAudience::External).is_none()
            );
        }
    }
    ...
    #[test]
    fn feedback_success_cell_uses_thread_id_for_remaining_categories() {
        for category in [
            FeedbackCategory::BadResult,
            FeedbackCategory::SafetyCheck,
            FeedbackCategory::Other,
            ...
            assert!(
                issue_url_for_category(category, "thread-4", FeedbackAudience::External).is_none()
            );
