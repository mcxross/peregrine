...
mod tests {
    ...
    fn make_selection_view(subtitle: Option<&str>) -> ListSelectionView {
        ...
        let items = vec![
            SelectionItem {
                ...
                description: Some("Peregrine can read files".to_string()),
                is_current: true,
            ...
            SelectionItem {
                ...
                description: Some("Peregrine can edit files".to_string()),
                is_current: false,
                dismiss_on_select: true,
                ..Default::default()
            },
    ...
    fn renders_blank_line_between_subtitle_and_items() {
        let view = make_selection_view(Some("Switch between Peregrine approval presets"));
        assert_snapshot!("list_selection_spacing_with_subtitle", render_lines(&view));
    }

    #[test]
    fn theme_picker_subtitle_uses_peregrine_theme_copy() {
        let (tx_raw, _rx) = unbounded_channel::<AppEvent>();
        let tx = AppEventSender::new(tx_raw);
        let home = dirs::home_dir().expect("home directory should be available");
        let peregrine_home = home.join(".peregrine");
        let params = crate::agent::theme_picker::build_theme_picker_params(
            /*current_name*/ None,
            Some(&peregrine_home),
            Some(94),
        );
        ...
        assert!(rendered.contains("Choose a Peregrine UI theme"));
    }
    ...
    fn c0_ctrl_p_respects_unbound_list_move_up() {
        let (tx_raw, _rx) = unbounded_channel::<AppEvent>();
        let tx = AppEventSender::new(tx_raw);
        let mut keymap = crate::agent::keymap::RuntimeKeymap::defaults().list;
    ...
    fn c0_ctrl_p_respects_remapped_list_move_down() {
        ...
        let mut keymap = crate::agent::keymap::RuntimeKeymap::defaults().list;
        keymap.move_up.clear();
        keymap.move_down = vec![crate::agent::key_hint::ctrl(KeyCode::Char('p'))];
        let mut view = ListSelectionView::new(
            SelectionViewParams {
                items: vec![
                    SelectionItem {
    ...
    fn page_and_jump_navigation_use_list_keymap() {
        ...
        let tx = AppEventSender::new(tx_raw);
        let mut keymap = crate::agent::keymap::RuntimeKeymap::defaults().list;
        keymap.page_down = vec![crate::agent::key_hint::ctrl(KeyCode::Char('d'))];
        keymap.page_up = vec![crate::agent::key_hint::ctrl(KeyCode::Char('u'))];
        keymap.jump_bottom = vec![crate::agent::key_hint::ctrl(KeyCode::Char('e'))];
        keymap.jump_top = vec![crate::agent::key_hint::ctrl(KeyCode::Char('a'))];
        let mut view = ListSelectionView::new(
            SelectionViewParams {
                items: (0..12)
                    .map(|idx| SelectionItem {
    ...
    fn width_changes_do_not_hide_rows() {
        ...
        let items = vec![
            SelectionItem {
                ...
                description: Some(
                    "Optimized for Peregrine. Balance of reasoning quality and coding ability."
                        .to_string(),
                ),
                is_current: true,
                dismiss_on_select: true,
            ...
            SelectionItem {
                ...
                description: Some(
                    "Optimized for Peregrine. Cheaper, faster, but less capable.".to_string(),
                ),
                dismiss_on_select: true,
                ..Default::default()
            },
