use crate::agent::history_cell::CompositeHistoryCell;
use crate::agent::history_cell::HistoryCell;
use crate::agent::history_cell::PlainHistoryCell;
use crate::agent::history_cell::plain_lines;
use crate::agent::history_cell::with_border_with_inner_width;
    ...
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let account_value = self.account.as_ref().map(|account| match account {
            ...
            StatusAccountDisplay::ApiKey => {
                "API key configured (run peregrine login to use ChatGPT)".to_string()
            }
        });
