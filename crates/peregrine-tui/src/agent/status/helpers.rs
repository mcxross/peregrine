...
use crate::agent::text_formatting;
use chrono::DateTime;
use chrono::Local;
...
mod tests {
    ...
    use crate::agent::legacy_core::LOCAL_AGENTS_MD_FILENAME;
    use crate::agent::legacy_core::config::ConfigBuilder;
    use codex_utils_absolute_path::test_support::PathBufExt;
    use pretty_assertions::assert_eq;
    use tempfile::TempDir;

    async fn test_config(peregrine_home: &TempDir, cwd: &TempDir) -> Config {
    ...
    async fn compose_agents_summary_includes_global_agents_path() {
        let peregrine_home = TempDir::new().expect("temp peregrine home");
        let cwd = TempDir::new().expect("temp cwd");
        let global_agents_path = peregrine_home.path().join(DEFAULT_AGENTS_MD_FILENAME);
        let config = test_config(&peregrine_home, &cwd).await;
    ...
    async fn compose_agents_summary_names_global_agents_override() {
        let peregrine_home = TempDir::new().expect("temp peregrine home");
        let cwd = TempDir::new().expect("temp cwd");
        let override_path = peregrine_home.path().join(LOCAL_AGENTS_MD_FILENAME);
        let config = test_config(&peregrine_home, &cwd).await;
