...
use crate::agent::key_hint::is_plain_text_key_event;
use crate::agent::keymap::ListKeymap;
use crate::agent::keymap::PagerKeymap;
...
use crate::agent::session_resume::resolve_session_thread_id;
use crate::agent::status::format_directory_display;
use crate::agent::terminal_palette::best_color;
use crate::agent::terminal_palette::default_bg;
            ...
            ThreadItem::UserMessage { content, .. } => Some(TranscriptPreviewLine {
                ...
                text: content
                    ...
                    .filter_map(|input| match input {
                        peregrine_app_server_protocol::UserInput::Text { text, .. } => {
                            Some(text.as_str())
                        }
                        _ => None,
                    })
...
impl PickerState {
    ...
    async fn persist_density(&self) -> Result<()> {
        ...
        ConfigEditsBuilder::new(&persistence.peregrine_home)
...
fn thread_list_params(
    ...
    ThreadListParams {
        ...
        source_kinds: Some(crate::agent::resume_source_kinds(include_non_interactive)),
        archived: Some(false),
...
fn render_picker_footer(
    frame: &mut crate::agent::custom_terminal::Frame,
    area: Rect,
    state: &PickerState,
    list_height: u16,
) {
...
fn render_transcript_loading_overlay(frame: &mut crate::agent::custom_terminal::Frame, area: Rect) {
    if area.width == 0 || area.height == 0 {
        return;
    }
...
fn render_list(frame: &mut crate::agent::custom_terminal::Frame, area: Rect, state: &PickerState) {
    if area.height == 0 {
        return;
    }
...
mod tests {
    ...
    fn footer_snapshot(state: &PickerState, width: u16, list_height: u16) -> String {
        use crate::agent::custom_terminal::Terminal;
        use crate::agent::test_backend::VT100Backend;
    ...
    fn remote_thread_list_params_can_include_non_interactive_sources() {
        ...
        assert_eq!(params.cursor, Some(String::from("cursor-1")));
        assert_eq!(params.model_providers, None);
        let source_kinds = crate::agent::resume_source_kinds(/*include_non_interactive*/ true);
...
"#
    ...
    async fn ctrl_o_keeps_toggled_density_when_persistence_fails() {
        let tmp = tempdir().expect("tmpdir");
        let peregrine_home_file = tmp.path().join("codex-home-file");
        std::fs::write(&peregrine_home_file, "not a directory").expect("write peregrine home file");
        let loader = page_only_loader(|_| {});
        let mut state = PickerState::new(
            FrameRequester::test_dummy(),
            loader,
        ...
        state.view_persistence = Some(SessionPickerViewPersistence {
            peregrine_home: peregrine_home_file,
        });
    ...
    async fn page_and_jump_navigation_use_list_keymap() {
        ...
        state.list_keymap.page_down = vec![crate::agent::key_hint::ctrl(KeyCode::Char('d'))];
        state.list_keymap.page_up = vec![crate::agent::key_hint::ctrl(KeyCode::Char('u'))];
        state.list_keymap.jump_bottom = vec![crate::agent::key_hint::ctrl(KeyCode::Char('y'))];
        state.list_keymap.jump_top = vec![crate::agent::key_hint::ctrl(KeyCode::Char('a'))];
    ...
    async fn ctrl_c_exits_even_when_cancel_is_remapped_to_ctrl_c() {
        ...
        state.list_keymap.cancel = vec![crate::agent::key_hint::ctrl(KeyCode::Char('c'))];
    ...
    fn thread_to_transcript_cells_renders_core_message_types() {
        ...
        let thread = Thread {
            ...
            git_info: None,
            name: None,
            turns: vec![peregrine_app_server_protocol::Turn {
