...
use crate::agent::render::renderable::RenderableExt as _;
use crate::agent::selection_list::selection_option_row;
...
pub(crate) fn migration_copy_for_models(
    ...
    let heading_text = Span::from(format!(
        "Peregrine just got an upgrade. Introducing {target_display_name}."
    ))
    .bold();
...
impl ModelMigrationScreen {
    ...
    fn render_menu(&self, column: &mut ColumnRenderable) {
        column.push(Line::from(""));
        column.push(
            Paragraph::new("Choose how you'd like Peregrine to proceed.")
                .wrap(Wrap { trim: false })
                .inset(Insets::tlbr(
                    /*top*/ 0, /*left*/ 2, /*bottom*/ 0, /*right*/ 0,
                )),
    ...
    fn prompt_snapshot() {
        ...
        let screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                ...
                /*model_link*/ None,
                Some(
                    "Upgrade to gpt-5.2-peregrine for the latest and greatest agentic coding model."
                        .to_string(),
                ),
                /*migration_markdown*/ None,
                "gpt-5.1-codex-max".to_string(),
                Some("Peregrine-optimized flagship for deep and fast reasoning.".to_string()),
                /*can_opt_out*/ true,
            ),
        );
