...
//!
//! The picker lists Peregrine UI themes. It provides:
//!
...
use crate::agent::bottom_pane::SideContentWidth;
use crate::agent::bottom_pane::popup_consts::standard_popup_hint_line;
use crate::agent::diff_render::DiffLineType;
use crate::agent::diff_render::current_diff_render_style_context;
use crate::agent::diff_render::line_number_width;
use crate::agent::diff_render::push_wrapped_diff_line_with_style_context;
use crate::agent::diff_render::push_wrapped_diff_line_with_syntax_and_style_context;
use crate::agent::render::highlight;
...
fn theme_picker_subtitle(_peregrine_home: Option<&Path>, _terminal_width: Option<u16>) -> String {
    "Choose a Peregrine UI theme".to_string()
}

/// Builds [`SelectionViewParams`] for the `/theme` picker dialog.
///
/// Lists Peregrine UI themes, with live preview on cursor movement and
/// cancel-restore.
///
...
pub(crate) fn build_theme_picker_params(
    ...
    peregrine_home: Option<&Path>,
    terminal_width: Option<u16>,
) -> SelectionViewParams {
    // Snapshot the current theme so we can restore on cancel.
    let original_theme = highlight::current_syntax_theme();

    let entries = highlight::list_available_themes(peregrine_home);
    let peregrine_home_owned = peregrine_home.map(Path::to_path_buf);
    ...
    let preview_home = peregrine_home_owned.clone();
    let on_selection_changed = Some(Box::new(
        move |idx: usize, tx: &crate::agent::app_event_sender::AppEventSender| {
        ...
        as Box<dyn Fn(usize, &crate::agent::app_event_sender::AppEventSender) + Send + Sync>);

    // Restore original theme on cancel.
    let on_cancel = Some(
        Box::new(move |tx: &crate::agent::app_event_sender::AppEventSender| {
            highlight::set_syntax_theme(original_theme.clone());
            tx.send(AppEvent::SyntaxThemePreviewed);
        }) as Box<dyn Fn(&crate::agent::app_event_sender::AppEventSender) + Send + Sync>,
    );
    SelectionViewParams {
        title: Some("Select Syntax Theme".to_string()),
        subtitle: Some(theme_picker_subtitle(
            peregrine_home_owned.as_deref(),
            terminal_width,
        )),
...
mod tests {
    ...
    fn theme_picker_uses_half_width_with_stacked_fallback_preview() {
        let params = build_theme_picker_params(
            /*current_name*/ None, /*peregrine_home*/ None, /*terminal_width*/ None,
        );
    ...
    #[test]
    fn subtitle_uses_peregrine_theme_copy() {
        let subtitle =
            theme_picker_subtitle(/*peregrine_home*/ None, /*terminal_width*/ None);
        assert_eq!(subtitle, "Choose a Peregrine UI theme");
    }
