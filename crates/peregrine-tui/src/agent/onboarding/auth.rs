...
use crate::agent::onboarding::keys;
use crate::agent::onboarding::onboarding_screen::KeyboardHandler;
use crate::agent::onboarding::onboarding_screen::StepStateProvider;
...
fn onboarding_request_id() -> peregrine_app_server_protocol::RequestId {
    peregrine_app_server_protocol::RequestId::String(Uuid::new_v4().to_string())
}
...
pub(super) async fn cancel_login_attempt(
    ...
    let _ = request_handle
        .request_typed::<peregrine_app_server_protocol::CancelLoginAccountResponse>(
...
impl AuthModeWidget {
    ...
    fn render_pick_mode(&self, area: Rect, buf: &mut Buffer) {
        let mut lines: Vec<Line> = vec![
            Line::from(vec![
                "  ".into(),
                "Sign in with ChatGPT to use Peregrine as part of your paid plan".into(),
            ]),
            Line::from(vec![
                "  ".into(),
    ...
    fn render_api_key_configured(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            ...
            "".into(),
            "  Peregrine will use usage-based billing with your API key.".into(),
        ];
...
mod tests {
    ...
    async fn widget_forced_chatgpt() -> (AuthModeWidget, TempDir) {
        ...
        let config = ConfigBuilder::default()
            .peregrine_home(peregrine_home_path.clone())
            .build()
            .await
            .unwrap();
        let client = InProcessAppServerClient::start(InProcessClientStartArgs {
            ...
            environment_manager: Arc::new(
                peregrine_app_server_client::EnvironmentManager::default_for_tests(),
            ),
