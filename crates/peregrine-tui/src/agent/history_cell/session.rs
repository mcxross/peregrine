    ...
    pub(crate) fn format_directory_inner(directory: &Path, max_width: Option<usize>) -> String {
        ...
        if let Some(max_width) = max_width {
            ...
            if UnicodeWidthStr::width(formatted.as_str()) > max_width {
                return crate::agent::text_formatting::center_truncate_path(&formatted, max_width);
            }
        }
...
impl HistoryCell for SessionHeaderHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        // Title line rendered inside the box: ">_ OpenAI Peregrine (vX)"
        ...
        const CHANGE_PROVIDER_HINT_EXPLANATION: &str = " to change provider";
        ...
        let model_spans: Vec<Span<'static>> = {
            ...
            spans.push("   ".dim());
            spans.push(CHANGE_PROVIDER_HINT_COMMAND.cyan());
            spans.push(CHANGE_PROVIDER_HINT_EXPLANATION.dim());
            spans
        };
    ...
    fn raw_lines(&self) -> Vec<Line<'static>> {
        let mut lines = vec![
            Line::from(format!("OpenAI Peregrine (v{})", self.version)),
            Line::from(format!(
                "model: {}{}",
                self.model,
