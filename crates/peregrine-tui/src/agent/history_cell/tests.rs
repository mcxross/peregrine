...
use crate::agent::exec_cell::ExecCall;
...
async fn test_config() -> Config {
    let peregrine_home = std::env::temp_dir();
...
fn composite_cell_preserves_child_web_links() {
    ...
    assert_eq!(
        lines[2].hyperlinks,
        vec![crate::agent::terminal_hyperlinks::TerminalHyperlink {
...
fn prefixed_wrapped_history_cell_indents_wrapped_lines() {
    ...
    assert_eq!(
        rendered,
        vec![
            "✔ You approved peregrine to".to_string(),
...
fn session_header_includes_reasoning_level_when_present() {
    ...
    assert!(model_line.contains("/provider to change provider"));
}
...
fn multiline_command_wraps_with_extra_indent_on_subsequent_lines() {
    ...
    let cmd = "set -o pipefail\ncargo test -p peregrine-tui --quiet".to_string();
    let call_id = "c1".to_string();
    let mut cell = ExecCell::new(
        ExecCall {
            call_id: call_id.clone(),
