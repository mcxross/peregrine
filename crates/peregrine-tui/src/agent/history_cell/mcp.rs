...
pub(crate) struct McpToolCallCell {
    ...
    result: Option<Result<peregrine_types::mcp::CallToolResult, String>>,
...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<peregrine_types::mcp::CallToolResult, String>,
...
impl HistoryCell for McpToolCallCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if let Some(result) = &self.result {
            match result {
                Ok(peregrine_types::mcp::CallToolResult { content, .. }) => {
                    if !content.is_empty() {
...
///   register it as an MCP server via `peregrine mcp add`.
...
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, peregrine_types::mcp::Tool>,
...
pub(crate) fn new_mcp_tools_output_from_statuses(
    ...
    for status in statuses {
        ...
        let auth_status = match status.auth_status {
            peregrine_app_server_protocol::McpAuthStatus::Unsupported => McpAuthStatus::Unsupported,
            peregrine_app_server_protocol::McpAuthStatus::NotLoggedIn => McpAuthStatus::NotLoggedIn,
            peregrine_app_server_protocol::McpAuthStatus::BearerToken => McpAuthStatus::BearerToken,
