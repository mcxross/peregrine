...
impl ChatWidget {
            ...
            .map(|details| {
                ...
                match details_capitalization {
                    StatusDetailsCapitalization::CapitalizeFirst => {
                        crate::agent::text_formatting::capitalize_first(trimmed)
                    }
    ...
    pub(crate) fn add_status_output(
        ...
        let rate_limit_snapshots: Vec<RateLimitSnapshotDisplay> = self
            ...
            .cloned()
            .collect();
        let agents_summary = crate::agent::status::compose_agents_summary(
            &self.config,
            &self.instruction_source_paths,
        );
        let (cell, handle) = crate::agent::status::new_status_output_with_rate_limits_handle(
            &self.config,
