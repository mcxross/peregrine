...
pub(super) fn app_server_rate_limit_error_kind(
    info: &AppServerPeregrineErrorInfo,
) -> Option<RateLimitErrorKind> {
    match info {
        AppServerPeregrineErrorInfo::ServerOverloaded => Some(RateLimitErrorKind::ServerOverloaded),
        AppServerPeregrineErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        AppServerPeregrineErrorInfo::ResponseTooManyFailedAttempts {
...
pub(super) fn is_app_server_cyber_policy_error(info: &AppServerPeregrineErrorInfo) -> bool {
    matches!(info, AppServerPeregrineErrorInfo::CyberPolicy)
}
...
impl ChatWidget {
    pub(crate) fn on_rate_limit_snapshot(&mut self, snapshot: Option<RateLimitSnapshot>) {
        if let Some(mut snapshot) = snapshot {
            let limit_id = snapshot
                .limit_id
                .clone()
                .unwrap_or_else(|| "peregrine".to_string());
            ...
            let is_codex_limit = limit_id.eq_ignore_ascii_case("peregrine");
            if is_codex_limit
    ...
    fn open_rate_limit_switch_prompt(&mut self, preset: ModelPreset) {
        ...
        let switch_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
            tx.send(AppEvent::PeregrineOp(AppCommand::override_turn_context(
                /*cwd*/ None,
    ...
    pub(super) fn open_workspace_owner_nudge_prompt(
        ...
        let (title, prompt) = match credit_type {
            ...
            AddCreditsNudgeCreditType::UsageLimit => (
                "Usage limit reached",
                "Request a limit increase from your owner to continue using Peregrine. Request increase?",
            ),
        };
