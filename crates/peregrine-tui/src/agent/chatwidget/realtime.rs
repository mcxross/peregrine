...
pub(super) struct RealtimeConversationUiState {
    ...
    #[cfg(not(target_os = "linux"))]
    capture: Option<crate::agent::voice::VoiceCapture>,
    #[cfg(not(target_os = "linux"))]
    audio_player: Option<crate::agent::voice::RealtimeAudioPlayer>,
}
...
impl ChatWidget {
    ...
    pub(crate) fn on_realtime_webrtc_offer_created(
        &mut self,
        result: Result<crate::agent::app_event::RealtimeWebrtcOffer, String>,
    ) {
        ...
        #[cfg(not(target_os = "linux"))]
        {
            if self.realtime_conversation.audio_player.is_none() {
                self.realtime_conversation.audio_player =
                    crate::agent::voice::RealtimeAudioPlayer::start(&self.config).ok();
            }
    ...
    fn start_realtime_local_audio(&mut self) {
        ...
        let capture = match crate::agent::voice::VoiceCapture::start_realtime(
            &self.config,
    ...
    pub(crate) fn restart_realtime_audio_device(&mut self, kind: RealtimeAudioDeviceKind) {
        ...
        match kind {
            ...
            RealtimeAudioDeviceKind::Speaker => {
                ...
                match crate::agent::voice::RealtimeAudioPlayer::start(&self.config) {
                    Ok(player) => {
    ...
    std::thread::spawn(move || {
        let result = match RealtimeWebrtcSession::start() {
            Ok(started) => {
                ...
                Ok(crate::agent::app_event::RealtimeWebrtcOffer {
...
fn start_realtime_meter_task(
    ...
    std::thread::spawn(move || {
        let mut meter = crate::agent::voice::RecordingMeterState::new();
