...
impl ChatWidget {
    ...
    pub(super) fn maybe_apply_ide_context(&mut self, items: &mut Vec<UserInput>) {
        ...
        match crate::agent::ide_context::fetch_ide_context(&self.config.cwd) {
            Ok(context) => {
                self.ide_context.mark_available();
                self.sync_ide_context_status_indicator();
                crate::agent::ide_context::apply_ide_context_to_user_input(&context, items);
            }
            Err(err) => {
                self.sync_ide_context_status_indicator();
    ...
    fn add_ide_context_status_message(&mut self) {
        ...
        match crate::agent::ide_context::fetch_ide_context(&self.config.cwd) {
            Ok(context) => {
                self.ide_context.mark_available();
                self.sync_ide_context_status_indicator();
                if crate::agent::ide_context::has_prompt_context(&context) {
                    self.add_info_message(
