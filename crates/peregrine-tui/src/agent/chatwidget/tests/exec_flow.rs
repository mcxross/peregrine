...
async fn disabled_slash_command_while_task_running_snapshot() {
    ...
    // Dispatch a command that is unavailable while a task runs.
...
async fn approval_modal_exec_snapshot() -> anyhow::Result<()> {
    ...
    let height = chat.desired_height(width);
    let mut terminal =
        crate::agent::custom_terminal::Terminal::with_options(VT100Backend::new(width, height))
            .expect("create terminal");
...
async fn apply_patch_approval_sends_op_with_call_id() {
    ...
    while let Ok(app_ev) = rx.try_recv() {
        ...
        } = app_ev
        {
            ...
            assert_matches!(
                decision,
                peregrine_app_server_protocol::FileChangeApprovalDecision::Accept
            );
            found = true;
            break;
        }
...
async fn apply_patch_full_flow_integration_like() {
    ...
    let forwarded = op_rx
        .try_recv()
        .expect("expected op forwarded to peregrine channel");
    match forwarded {
