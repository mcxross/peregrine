...
async fn submission_preserves_text_elements_and_local_images() {
    ...
    let configured = crate::agent::session_state::ThreadSessionState {
        thread_id,
        forked_from_id: None,
...
async fn alt_up_edits_most_recent_queued_message() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.chat_keymap.edit_queued_message = vec![crate::agent::key_hint::alt(KeyCode::Up)];
    chat.queued_message_edit_hint_binding = Some(crate::agent::key_hint::alt(KeyCode::Up));
    chat.bottom_pane
...
fn queued_message_edit_binding_mapping_covers_special_terminals_and_tmux() {
    assert_eq!(
        ...
        crate::agent::key_hint::shift(KeyCode::Left)
    );
    assert_eq!(
        queued_message_edit_binding_for_terminal(TerminalInfo {
            name: TerminalName::WarpTerminal,
    ...
    assert_eq!(
        ...
        crate::agent::key_hint::alt(KeyCode::Up)
    );
}
