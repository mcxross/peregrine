...
async fn mode_switch_surfaces_reasoning_change_notification_when_model_stays_same() {
    ...
    let plan_messages = drain_insert_history(&mut rx)
        ...
        .join("\n");
    assert!(
        plan_messages.contains("Model changed to gpt-5.3-peregrine medium for Plan mode."),
...
async fn collaboration_modes_defaults_to_code_on_startup() {
    ...
    assert_eq!(
        chat.current_model(),
        crate::agent::legacy_core::test_support::get_model_offline(chat.config.model.as_deref())
    );
}
