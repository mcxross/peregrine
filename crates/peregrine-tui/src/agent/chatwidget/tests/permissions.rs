use super::*;
use crate::agent::legacy_core::config::CustomPermissionProfileSummary;
...
fn windows_sandbox_requirements_stack(
    ...
    let mut requirements_with_sources = peregrine_config::ConfigRequirementsWithSources::default();
    requirements_with_sources
        .merge_unset_fields(RequirementSource::Unknown, requirements_toml.clone());
    let requirements = peregrine_config::ConfigRequirements::try_from(requirements_with_sources)
...
async fn startup_windows_sandbox_prompt_blocks_disallowed_unelevated_fallback() {
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 120);
    assert!(
        popup.contains("Your organization requires the default Peregrine agent sandbox"),
...
async fn permissions_selection_sends_approvals_reviewer_in_override_turn_context() {
    ...
    let op = std::iter::from_fn(|| rx.try_recv().ok())
        .find_map(|event| match event {
            AppEvent::PeregrineOp(op @ Op::OverrideTurnContext { .. }) => Some(op),
            _ => None,
        })
