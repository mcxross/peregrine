use super::*;
use crate::agent::app_event::ConnectorsSnapshot;
use crate::agent::chatwidget::connectors::ConnectorsCacheState;
use codex_features::Stage;
use peregrine_app_server_protocol::AppInfo;
use peregrine_app_server_protocol::HookErrorInfo;
    ...
    chat.on_plugin_detail_loaded(
        ...
        Ok(PluginReadResponse {
            plugin: plugins_test_detail(
                ...
                &[
                    (peregrine_app_server_protocol::HookEventName::PreToolUse, 1),
                    (peregrine_app_server_protocol::HookEventName::Stop, 2),
                ],
...
async fn plugins_popup_openai_curated_tab_omits_marketplace_in_rows() {
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 100);
    assert!(
        popup.contains("Curated marketplace."),
        "expected Curated tab header, got:\n{popup}"
    );
    assert!(
        popup.contains("Calendar") && !popup.contains("Repo Plugin"),
        "expected Curated tab to show only bundled marketplace plugins, got:\n{popup}"
    );
    assert!(
...
async fn experimental_features_popup_snapshot() {
    ...
    let view = ExperimentalFeaturesView::new(
        ...
        crate::agent::keymap::RuntimeKeymap::defaults().list,
    );
...
async fn provider_selection_popup_snapshot() {
    ...
    chat.open_provider_popup(test_provider_list_response());

    let popup = render_bottom_popup(&chat, /*width*/ 100);
    assert!(popup.contains("Select Provider"));
    assert!(popup.contains("OpenAI"));
    assert!(popup.contains("Ollama"));
    assert!(popup.contains("Amazon Bedrock"));
    assert!(popup.contains("Anthropic"));
    assert!(popup.contains("Anthropic Messages"));
    assert!(popup.contains("claude-sonnet-4-6"));
    assert!(popup.contains("Set ANTHROPIC_API_KEY"));
    assert_chatwidget_snapshot!("provider_selection_popup", popup);
}

#[tokio::test]
async fn provider_selection_for_ollama_opens_model_picker() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.open_provider_popup(ModelProviderListResponse {
        selected_provider_id: "ollama".to_string(),
        data: vec![test_provider_entry(
            "ollama",
            "Ollama",
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::OpenProviderModelPicker {
            provider_id,
            provider_display_name,
        }) if provider_id == "ollama" && provider_display_name == "Ollama"
    );
}

#[tokio::test]
async fn provider_selection_for_bedrock_persists_default_model() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.open_provider_popup(ModelProviderListResponse {
        ...
        data: vec![test_provider_entry(
            "amazon-bedrock",
            "Amazon Bedrock",
            ModelProviderKind::AmazonBedrock,
            ModelProviderWireApi::Responses,
            ModelProviderAuthStrategy::Aws,
            /*selected*/ false,
            /*selectable*/ true,
            None,
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::PersistProviderSelection { provider_id, model })
            if provider_id == "amazon-bedrock" && model.as_deref() == Some("openai.gpt-5.4")
    );
}

#[tokio::test]
async fn provider_selection_for_anthropic_persists_default_model() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.open_provider_popup(ModelProviderListResponse {
        selected_provider_id: "openai".to_string(),
        data: vec![test_provider_entry(
            ...
            ModelProviderWireApi::AnthropicMessages,
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::PersistProviderSelection { provider_id, model })
            if provider_id == "anthropic" && model.as_deref() == Some("claude-sonnet-4-6")
    );
}

#[test]
fn provider_setup_warning_uses_app_server_credential_metadata() {
    let provider = missing_anthropic_api_key_provider_entry();
...
#[tokio::test]
async fn provider_model_popup_persists_selected_ollama_model() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.open_provider_model_popup(
        "ollama".to_string(),
        "Ollama".to_string(),
        ModelProviderModelsListResponse {
            provider_id: "ollama".to_string(),
            data: vec![
                ModelProviderModel {
                    id: "llama3.2".to_string(),
                    model: "llama3.2".to_string(),
                    display_name: "llama3.2".to_string(),
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 80);
    assert_chatwidget_snapshot!("provider_model_selection_popup", popup);
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::PersistProviderSelection { provider_id, model })
            if provider_id == "ollama" && model.as_deref() == Some("llama3.2")
    );
}
...
async fn server_overloaded_error_does_not_switch_models() {
    ...
    handle_error(
        &mut chat,
        "server overloaded",
        Some(PeregrineErrorInfo::ServerOverloaded),
    );
...
async fn feedback_selection_popup_snapshot() {
    ...
    chat.show_selection_view(crate::agent::bottom_pane::feedback_selection_params(
        chat.app_event_tx.clone(),
    ));
...
async fn feedback_upload_consent_popup_snapshot() {
    ...
    chat.show_selection_view(crate::agent::bottom_pane::feedback_upload_consent_params(
        chat.app_event_tx.clone(),
        crate::agent::app_event::FeedbackCategory::Bug,
...
async fn feedback_good_result_consent_popup_includes_connectivity_diagnostics_filename() {
    ...
    chat.show_selection_view(crate::agent::bottom_pane::feedback_upload_consent_params(
        chat.app_event_tx.clone(),
        crate::agent::app_event::FeedbackCategory::GoodResult,
        chat.current_rollout_path.clone(),
        Some("auto-review-rollout-thread-1.jsonl".to_string()),
...
fn test_provider_list_response() -> ModelProviderListResponse {
    ModelProviderListResponse {
        ...
        data: vec![
            ...
            missing_anthropic_api_key_provider_entry(),
            test_provider_entry(
                "ollama",
                "Ollama",
                ModelProviderKind::Ollama,
...
fn test_provider_entry(
    id: &str,
    display_name: &str,
    kind: ModelProviderKind,
    wire_api: ModelProviderWireApi,
    ...
    let configured = selectable || disabled_reason.is_some();
    let credential_state = match auth_strategy {
        ModelProviderAuthStrategy::AccountOrApiKey => ModelProviderCredentialState::NeedsLogin,
        ModelProviderAuthStrategy::ApiKey => ModelProviderCredentialState::MissingApiKey,
        ModelProviderAuthStrategy::Aws
        | ModelProviderAuthStrategy::External
        | ModelProviderAuthStrategy::Unsupported => ModelProviderCredentialState::Unknown,
        ModelProviderAuthStrategy::None => ModelProviderCredentialState::NotRequired,
    };
    ModelProviderEntry {
        id: id.to_string(),
        display_name: display_name.to_string(),
        description: format!("Use {display_name} for agent turns."),
        kind,
        wire_api,
        auth_strategy,
        selected,
...
fn missing_anthropic_api_key_provider_entry() -> ModelProviderEntry {
    let mut entry = test_provider_entry(
        "anthropic",
        "Anthropic",
        ModelProviderKind::Anthropic,
        ModelProviderWireApi::AnthropicMessages,
    ...
    entry.credential_state = Some(ModelProviderCredentialState::MissingApiKey);
    entry.setup_hint = Some("Set ANTHROPIC_API_KEY before starting a turn.".to_string());
    entry
}
