...
impl ChatWidget {
    ...
    pub(crate) fn world_writable_warning_details(&self) -> Option<(Vec<String>, usize, bool)> {
        ...
        match codex_windows_sandbox::apply_world_writable_scan_and_denies_for_permissions(
            ...
            &env_map,
            &permissions,
            Some(self.config.peregrine_home.as_path()),
        ) {
            Ok(_) => None,
    ...
    pub(crate) fn open_windows_sandbox_fallback_prompt(
        ...
        if allow_unelevated {
            lines.push(line![
                "You can still use Peregrine in a non-admin sandbox. It carries greater risk if prompt injected."
            ]);
        } else {
            lines.push(line![
        ...
        if allow_unelevated {
            items.push(SelectionItem {
                name: "Use Peregrine with non-admin sandbox".to_string(),
                description: None,
                actions: vec![Box::new({
                    let otel = self.session_telemetry.clone();
