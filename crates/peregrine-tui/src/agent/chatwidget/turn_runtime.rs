...
impl ChatWidget {
    ...
    pub(super) fn on_task_started(&mut self) {
        ...
        self.set_ambient_pet_notification(
            crate::agent::pets::PetNotificationKind::Running,
            /*body*/ None,
        );
        self.request_redraw();
    }
    ...
    pub(super) fn on_task_complete(
                ...
                let elapsed_seconds = if show_work_separator {
                    duration_ms
                        ...
                        .or_else(|| {
                            self.bottom_pane
                                .status_widget()
                                .map(crate::agent::status_indicator_widget::StatusIndicatorWidget::elapsed_seconds)
                        })
                } else {
                    None
                };
        ...
        if !from_replay {
            ...
            self.set_ambient_pet_notification(
                crate::agent::pets::PetNotificationKind::Review,
                body,
            );
        }
        self.request_redraw();
    ...
    pub(super) fn handle_app_server_steer_rejected_error(
        &mut self,
        peregrine_error_info: &AppServerPeregrineErrorInfo,
    ) -> bool {
        matches!(
            peregrine_error_info,
            AppServerPeregrineErrorInfo::ActiveTurnNotSteerable { .. }
    ...
    pub(super) fn on_server_overloaded_error(&mut self, message: String) {
        ...
        let message = if message.trim().is_empty() {
            "Peregrine is currently experiencing high load.".to_string()
        } else {
            message
        };
    ...
    pub(super) fn on_error(&mut self, message: String) {
        ...
        self.set_ambient_pet_notification(
            crate::agent::pets::PetNotificationKind::Failed,
            /*body*/ None,
        );
        self.request_redraw();
    ...
    pub(super) fn handle_non_retry_error(
        &mut self,
        message: String,
        peregrine_error_info: Option<AppServerPeregrineErrorInfo>,
    ) {
        if peregrine_error_info
            .as_ref()
            .is_some_and(|info| self.handle_app_server_steer_rejected_error(info))
        {
        } else if peregrine_error_info
            .as_ref()
            .is_some_and(is_app_server_cyber_policy_error)
        {
            self.on_cyber_policy_error();
        } else if let Some(info) = peregrine_error_info
            .as_ref()
