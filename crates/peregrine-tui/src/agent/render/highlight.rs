...
//! | `PEREGRINE_HOME` | `OnceLock<Option<PathBuf>>` | Root for custom `.tmTheme` discovery |
//!
...
use crate::theme::ThemeName as PeregrineThemeName;
...
static PEREGRINE_HOME: OnceLock<Option<PathBuf>> = OnceLock::new();
...
/// Set the user-configured syntax theme override and peregrine home path.
///
/// Call this with the **final resolved config** (after onboarding, resume, and
/// fork reloads complete). The first call persists `name` and `peregrine_home` in
...
pub(crate) fn set_theme_override(
    ...
    let warning = validate_theme_name(name.as_deref(), peregrine_home.as_deref());
    let style_warning = crate::agent::style::set_agent_theme(name.as_deref());
    let override_set_ok = THEME_OVERRIDE.set(name.clone()).is_ok();
    let peregrine_home_set_ok = PEREGRINE_HOME.set(peregrine_home.clone()).is_ok();
    if THEME.get().is_some() {
        set_syntax_theme(resolve_theme_with_override(
            name.as_deref(),
            peregrine_home.as_deref(),
        ));
    }
    if !override_set_ok || !peregrine_home_set_ok {
...
pub(crate) fn validate_theme_name(
    name: Option<&str>,
    _peregrine_home: Option<&Path>,
) -> Option<String> {
    let name = name?;
    if parse_peregrine_theme_name(name).is_some() {
        return None;
    }
...
fn parse_peregrine_theme_name(name: &str) -> Option<PeregrineThemeName> {
    PeregrineThemeName::from_str(name).ok()
}

fn embedded_theme_for_peregrine_theme(theme: PeregrineThemeName) -> EmbeddedThemeName {
    match theme {
        PeregrineThemeName::PeregrineNight => EmbeddedThemeName::TwoDark,
        PeregrineThemeName::FalconWatch => EmbeddedThemeName::Nord,
        PeregrineThemeName::AuditDark => EmbeddedThemeName::GruvboxDark,
        PeregrineThemeName::ThreatMatrix => EmbeddedThemeName::DarkNeon,
        PeregrineThemeName::BytecodeEmber => EmbeddedThemeName::MonokaiExtended,
...
#[cfg(test)]
fn custom_theme_path(name: &str, peregrine_home: &Path) -> PathBuf {
    peregrine_home
        .join("themes")
        .join(format!("{name}.tmTheme"))
}

/// Try to load a custom `.tmTheme` file from `{peregrine_home}/themes/{name}.tmTheme`.
#[cfg(test)]
fn load_custom_theme(name: &str, peregrine_home: &Path) -> Option<Theme> {
    ThemeSet::get_theme(custom_theme_path(name, peregrine_home)).ok()
}

fn adaptive_default_theme_selection() -> (EmbeddedThemeName, &'static str) {
    let theme = PeregrineThemeName::default();
    (embedded_theme_for_peregrine_theme(theme), theme.slug())
}
...
fn resolve_theme_with_override(name: Option<&str>, _peregrine_home: Option<&Path>) -> Theme {
    let ts = two_face::theme::extra();

    // Honor user-configured theme if valid.
    if let Some(name) = name {
        if let Some(theme_name) = parse_peregrine_theme_name(name) {
            return ts
                .get(embedded_theme_for_peregrine_theme(theme_name))
                .clone();
        }
...
fn build_default_theme() -> Theme {
    ...
    let peregrine_home = PEREGRINE_HOME
        .get()
        .and_then(|peregrine_home| peregrine_home.as_deref());
    resolve_theme_with_override(name, peregrine_home)
}
...
/// depth.  [`diff_render`](crate::agent::diff_render) converts them to ratatui
...
pub(crate) fn configured_theme_name() -> String {
    ...
    if let Some(Some(name)) = THEME_OVERRIDE.get() {
        if let Some(theme) = parse_peregrine_theme_name(name) {
            return theme.slug().to_string();
        }
    }
...
pub(crate) fn resolve_theme_by_name(name: &str, _peregrine_home: Option<&Path>) -> Option<Theme> {
    let ts = two_face::theme::extra();
    if let Some(theme) = parse_peregrine_theme_name(name) {
        crate::agent::style::set_agent_theme(Some(theme.slug()));
        return Some(ts.get(embedded_theme_for_peregrine_theme(theme)).clone());
    }
    None
}

/// A theme available in the picker, either bundled or loaded from a custom
/// `.tmTheme` file under `{PEREGRINE_HOME}/themes/`.
...
/// found in `{peregrine_home}/themes/`.
pub(crate) fn list_available_themes(peregrine_home: Option<&Path>) -> Vec<ThemeEntry> {
    let _ = peregrine_home;
    PeregrineThemeName::all()
        .iter()
        .map(|theme| ThemeEntry {
            name: theme.slug().to_string(),
...
/// Highlight code using the file extension or file name as the language hint.
///
/// Returns `None` when the path does not resolve to a bundled syntax or when
/// the input exceeds the highlighting guardrails. Callers that need an
/// unconditional fallback should render plain text themselves so they can still
/// distinguish unsupported files from successfully highlighted files.
pub(crate) fn highlight_code_for_path_to_lines(
    code: &str,
    path: &Path,
) -> Option<Vec<Line<'static>>> {
    let hints = syntax_hints_for_path(path);
    for hint in hints {
        if let Some(line_spans) = highlight_to_line_spans(code, &hint) {
            return Some(line_spans.into_iter().map(Line::from).collect());
        }
    }
    None
}

fn syntax_hints_for_path(path: &Path) -> Vec<String> {
    let mut hints = Vec::with_capacity(3);
    if let Some(extension) = path.extension().and_then(|extension| extension.to_str()) {
        hints.push(extension.to_ascii_lowercase());
    }
...
<key>background</key><string>{deleted_background}</string>
    ...
    fn unique_foreground_colors_for_theme(theme_name: &str) -> Vec<String> {
        let theme = resolve_theme_by_name(theme_name, /*peregrine_home*/ None)
    ...
    #[test]
    fn legacy_codex_theme_aliases_do_not_resolve() {
        for theme_name in ["ansi", "base16", "base16-256", "github"] {
            assert!(
                resolve_theme_by_name(theme_name, /*peregrine_home*/ None).is_none(),
                "legacy Codex theme alias {theme_name} should not resolve in Peregrine TUI"
            );
        }
    }
    ...
    #[test]
    fn peregrine_theme_can_resolve_for_diff_highlighting() {
        let theme = resolve_theme_by_name("zero-day", /*peregrine_home*/ None)
            .expect("expected Peregrine theme to load");
        let _ = diff_scope_background_rgbs_for_theme(&theme);
    }

    #[test]
    fn custom_tmtheme_diff_scope_backgrounds_are_ignored() {
        ...
        assert!(
            resolve_theme_by_name("custom-diff", Some(dir.path())).is_none(),
            "Peregrine TUI should not resolve custom tmTheme files from CODEX_HOME-style paths"
        );
    }
    ...
    fn validate_theme_name_none_for_bundled() {
        assert!(validate_theme_name(Some("zero-day"), /*peregrine_home*/ None).is_none());
        assert!(
            validate_theme_name(Some("Peregrine Night"), Some(Path::new("/nonexistent"))).is_none()
        );
    }

    #[test]
    fn validate_theme_name_none_when_no_override() {
        assert!(validate_theme_name(/*name*/ None, /*peregrine_home*/ None).is_none());
    }

    #[test]
    fn validate_theme_name_warns_for_unknown_peregrine_theme() {
        let dir = tempfile::tempdir().unwrap();
        let warning = validate_theme_name(Some("my-fancy"), Some(dir.path()));
        assert!(warning.is_some(), "should warn when theme is unknown");
    ...
    #[test]
    fn validate_theme_name_ignores_custom_tmtheme_files() {
        let dir = tempfile::tempdir().unwrap();
        let themes_dir = dir.path().join("themes");
        std::fs::create_dir(&themes_dir).unwrap();
        write_minimal_tmtheme(&themes_dir.join("my-fancy.tmTheme"));
        assert!(
            validate_theme_name(Some("my-fancy"), Some(dir.path())).is_some(),
            "agent mode only accepts Peregrine theme names"
        );
    }

    #[test]
    fn list_available_themes_uses_peregrine_theme_names() {
        let dir = tempfile::tempdir().unwrap();
        let themes_dir = dir.path().join("themes");
        std::fs::create_dir(&themes_dir).unwrap();
        ...
        assert!(entries.iter().all(|entry| !entry.is_custom));
    }

    #[test]
    fn resolve_theme_by_name_accepts_peregrine_theme_names() {
        assert!(resolve_theme_by_name("zero-day", None).is_some());
        assert!(resolve_theme_by_name("dracula", None).is_none());
    }
