...
//! local spritesheets. The TUI resolves them from the public Peregrine pets CDN on
//! first use, verifies that the downloaded file has the expected spritesheet
//! geometry, and installs it into a versioned cache under PEREGRINE_HOME.
//!
...
pub(crate) fn builtin_spritesheet_path(peregrine_home: &Path, file: &str) -> PathBuf {
    pack_dir(peregrine_home).join("assets").join(file)
}
...
pub(crate) fn ensure_builtin_pet(peregrine_home: &Path, pet: catalog::BuiltinPet) -> Result<()> {
    let destination = builtin_spritesheet_path(peregrine_home, pet.spritesheet_file);
...
fn pack_dir(peregrine_home: &Path) -> PathBuf {
    peregrine_home.join(PET_PACK_DIR).join(PET_PACK_VERSION)
}
...
#[cfg(test)]
pub(crate) fn write_test_pack(peregrine_home: &Path) {
    let assets_dir = pack_dir(peregrine_home).join("assets");
