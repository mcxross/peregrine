...
impl App {
    pub(super) fn disable_ambient_pet_before_shutdown(&mut self, tui: &mut tui::Tui) -> Result<()> {
        ...
        if let Err(clear_err) = tui.clear_ambient_pet_image() {
            match clear_err {
                crate::agent::pets::PetImageRenderError::Terminal(err) => return Err(err.into()),
                crate::agent::pets::PetImageRenderError::Asset(err) => {
                    tracing::warn!(
                        error = %err,
    ...
    pub(super) fn handle_ambient_pet_image_render_error(
        &mut self,
        tui: &mut tui::Tui,
        err: crate::agent::pets::PetImageRenderError,
    ) -> Result<()> {
        match err {
            crate::agent::pets::PetImageRenderError::Terminal(err) => Err(err.into()),
            crate::agent::pets::PetImageRenderError::Asset(err) => {
                tracing::warn!(
                    error = %err,
                ...
                if let Err(clear_err) = tui.clear_ambient_pet_image() {
                    match clear_err {
                        crate::agent::pets::PetImageRenderError::Terminal(err) => {
                            return Err(err.into());
                        }
                        crate::agent::pets::PetImageRenderError::Asset(err) => {
                            tracing::warn!(
    ...
    pub(super) async fn handle_pet_disabled(&mut self, tui: &mut tui::Tui) {
        let edit = crate::agent::legacy_core::config::edit::tui_pet_edit(
            crate::agent::pets::DISABLED_PET_ID,
        );
        let apply_result = ConfigEditsBuilder::new(&self.config.peregrine_home)
            .with_edits([edit])
            .apply()
            .await;
        match apply_result {
    ...
    pub(super) async fn handle_pet_selection_loaded(
        ...
        match result {
            Ok(ambient_pet) => {
                let edit = crate::agent::legacy_core::config::edit::tui_pet_edit(&pet_id);
                match ConfigEditsBuilder::new(&self.config.peregrine_home)
                    .with_edits([edit])
                    .apply()
                    .await
                {
