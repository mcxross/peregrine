...
impl App {
    ...
    pub(super) async fn apply_permission_profile_selection(
        ...
        self.sync_active_thread_permission_settings_to_cached_session()
            .await;
        self.app_event_tx
            .send(AppEvent::PeregrineOp(AppCommand::override_turn_context(
                /*cwd*/ None,
                approval_policy,
                approvals_reviewer,
    ...
    pub(super) async fn read_effective_config_after_overridden_write(
        ...
        match crate::agent::config_update::read_effective_config(app_server.request_handle(), cwd)
            .await
        {
            ...
            Err(err) => {
                ...
                self.chat_widget.add_error_message(format!(
                    "{setting} were saved, but Peregrine could not refresh the effective config: {err}"
                ));
                None
            }
        }
    ...
    pub(super) async fn rebuild_config_for_resume_or_fallback(
        ...
        match self.rebuild_config_for_cwd(resume_cwd.clone()).await {
            Ok(config) => Ok(config),
            Err(err) => {
                if crate::agent::session_resume::cwds_differ(current_cwd, &resume_cwd) {
                    Err(err)
                } else {
    ...
    pub(super) async fn update_feature_flags(
        ...
        for (feature, enabled) in updates {
            ...
            if feature == Feature::GuardianApproval {
                ...
                if effective_enabled {
                    ...
                    feature_edits.push(crate::agent::config_update::replace_config_value(
                        "approvals_reviewer",
                    ...
                    feature_edits.push(crate::agent::config_update::clear_config_value(
                        "approvals_reviewer",
                    ));
            ...
            config_edits.push(crate::agent::config_update::build_feature_enabled_edit(
                feature_key,
                effective_enabled,
            ));
        }
        ...
        let write_response = match crate::agent::config_update::write_config_batch(
    ...
    pub(super) async fn update_memory_settings(
        ...
        let edits = crate::agent::config_update::build_memory_settings_edits(
            use_memories,
            generate_memories,
        );
    ...
    pub(crate) fn token_usage(&self) -> crate::agent::token_usage::TokenUsage {
    ...
    pub(super) fn sync_tui_pet_disabled(&mut self) {
        let pet = crate::agent::pets::DISABLED_PET_ID.to_string();
        self.config.tui_pet = Some(pet.clone());
        self.chat_widget.set_tui_pet(Some(pet));
    }
    ...
    pub(super) fn restore_runtime_theme_from_config(&self) {
        if let Some(name) = self.config.tui_theme.as_deref()
            && let Some(theme) = crate::agent::render::highlight::resolve_theme_by_name(
                name,
        ...
        let auto_theme_name = crate::agent::render::highlight::adaptive_default_theme_name();
...
fn windows_sandbox_mode_from_effective_config(
    effective_config: &ConfigReadResponse,
) -> Option<peregrine_config::types::WindowsSandboxModeToml> {
...
mod tests {
    use super::*;
    use crate::agent::app::test_support::app_enabled_in_effective_config;
    use crate::agent::app::test_support::make_test_app;
    use crate::agent::legacy_core::config::edit::ConfigEdit;
    ...
    -> Result<()> {
        ...
        let peregrine_home = tempdir()?;
        app.config.peregrine_home = peregrine_home.path().to_path_buf().abs();
        std::fs::write(peregrine_home.path().join("config.toml"), "[broken")?;
...
"#,
        ...
        assert_eq!(
            ...
            crate::agent::legacy_core::config::TerminalResizeReflowMaxRows::Limit(9000)
        );
        Ok(())
    }
    ...
    async fn sync_tui_pet_disabled_updates_chat_widget_config_copy() {
        ...
        assert_eq!(
            ...
            Some(crate::agent::pets::DISABLED_PET_ID)
        );
        assert_eq!(
            app.chat_widget.config_ref().tui_pet.as_deref(),
            Some(crate::agent::pets::DISABLED_PET_ID)
