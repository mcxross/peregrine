...
use super::*;
use crate::agent::session_resume::read_session_model;
...
impl App {
    ...
    pub(super) fn ignore_same_thread_resume(
        &mut self,
        target_session: &crate::agent::resume_picker::SessionTarget,
    ) -> bool {
            ...
            ServerRequest::FileChangeRequestApproval { params, .. } => Some(
                ThreadInteractiveRequest::Approval(ApprovalRequest::ApplyPatch {
                    ...
                    changes: self
                        ...
                        .await
                        .map(crate::agent::app_server_approval_conversions::file_update_changes_to_display)
                        .unwrap_or_default(),
                }),
            ),
                    ...
                    match &params.request {
                        ...
                        peregrine_app_server_protocol::McpServerElicitationRequest::Url { .. } => {
                            self.app_event_tx.resolve_elicitation(
                                ...
                                request_id.clone(),
                                peregrine_app_server_protocol::McpServerElicitationAction::Decline,
                                /*content*/ None,
                                /*meta*/ None,
                            );
                            None
    ...
    pub(super) fn append_message_history_entry(&self, thread_id: ThreadId, text: String) {
        let history_config = crate::agent::message_history::HistoryConfig::new(
            self.chat_widget.config_ref().peregrine_home.clone(),
            &self.chat_widget.config_ref().history,
        );
        tokio::spawn(async move {
            if let Err(err) =
                crate::agent::message_history::append_entry(&text, thread_id, &history_config).await
            {
                tracing::warn!(
                    thread_id = %thread_id,
                    error = %err,
    ...
    pub(super) async fn lookup_message_history_entry(
        ...
        tokio::spawn(async move {
            let entry_opt = tokio::task::spawn_blocking(move || {
                crate::agent::message_history::lookup(log_id, offset, &history_config)
            })
            .await
            .unwrap_or_else(|err| {
        ...
        match op {
            ...
            AppCommand::ListSkills { cwds, force_reload } => {
                self.handle_skills_list_result(
                    app_server
                        .skills_list(peregrine_app_server_protocol::SkillsListParams {
                            cwds: cwds.clone(),
