...
use super::*;
use crate::agent::app_backtrack::BacktrackSelection;
use crate::agent::app_backtrack::BacktrackState;
use crate::agent::app_backtrack::user_count;

use crate::agent::chatwidget::ChatWidgetInit;
use crate::agent::chatwidget::create_initial_user_message;
use crate::agent::chatwidget::tests::make_chatwidget_manual_with_sender;
use crate::agent::chatwidget::tests::set_chatgpt_auth;
use crate::agent::chatwidget::tests::set_fast_mode_test_catalog;
use crate::agent::file_search::FileSearchManager;
use crate::agent::history_cell::AgentMarkdownCell;
use crate::agent::history_cell::AgentMessageCell;
use crate::agent::history_cell::HistoryCell;
use crate::agent::history_cell::PlainHistoryCell;
use crate::agent::history_cell::UserHistoryCell;
use crate::agent::history_cell::new_session_info;
use crate::agent::multi_agents::AgentPickerThreadEntry;
use assert_matches::assert_matches;

use crate::agent::app_command::AppCommand as Op;
...
use crate::agent::legacy_core::config::TerminalResizeReflowMaxRows;
use codex_otel::SessionTelemetry;
use codex_utils_absolute_path::AbsolutePathBuf;
use crossterm::event::KeyModifiers;
use insta::assert_snapshot;
use peregrine_app_server_protocol::AdditionalFileSystemPermissions;
use peregrine_app_server_protocol::AdditionalNetworkPermissions;
use peregrine_app_server_protocol::AdditionalPermissionProfile;
use peregrine_app_server_protocol::AgentMessageDeltaNotification;
...
use peregrine_app_server_protocol::FileUpdateChange;
...
use peregrine_app_server_protocol::NetworkApprovalContext as AppServerNetworkApprovalContext;
use peregrine_app_server_protocol::NetworkApprovalProtocol as AppServerNetworkApprovalProtocol;
use peregrine_app_server_protocol::NetworkPolicyAmendment as AppServerNetworkPolicyAmendment;
use peregrine_app_server_protocol::NetworkPolicyRuleAction as AppServerNetworkPolicyRuleAction;
use peregrine_app_server_protocol::NonSteerableTurnKind as AppServerNonSteerableTurnKind;
use peregrine_app_server_protocol::PatchChangeKind;
...
use peregrine_app_server_protocol::SessionSource;
...
use peregrine_app_server_protocol::TokenUsageBreakdown;
...
use peregrine_app_server_protocol::TurnError as AppServerTurnError;
...
use peregrine_app_server_protocol::UserInput as AppServerUserInput;
...
use peregrine_types::models::NetworkPermissions;
...
async fn next_thread_settings_updated(
    ...
    for _ in 0..20 {
        ...
        if let peregrine_app_server_client::AppServerEvent::ServerNotification(
...
async fn handle_mcp_inventory_result_respects_origin_thread() {
    ...
    app.handle_mcp_inventory_result(
        Ok(vec![McpServerStatus {
            ...
            auth_status: peregrine_app_server_protocol::McpAuthStatus::Unsupported,
        }]),
...
async fn enqueue_primary_thread_session_replays_turns_before_initial_prompt_submit() -> Result<()> {
    ...
    let model = crate::agent::legacy_core::test_support::get_model_offline(config.model.as_deref());
    app.chat_widget = ChatWidget::new_with_app_event(ChatWidgetInit {
        config,
        frame_requester: crate::agent::tui::FrameRequester::test_dummy(),
    ...
    while let Ok(event) = app_event_rx.try_recv() {
        match event {
            ...
            AppEvent::PeregrineOp(Op::UserTurn { items, .. }) => {
                submitted_items = Some(items);
            }
            _ => {}
        }
...
async fn token_usage_update_refreshes_status_line_with_runtime_context_window() {
    ...
    app.chat_widget.setup_status_line(
        vec![crate::agent::bottom_pane::StatusLineItem::ContextWindowSize],
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::CollabAgentToolCall {
                id: "wait-1".to_string(),
                tool: peregrine_app_server_protocol::CollabAgentTool::Wait,
                status: peregrine_app_server_protocol::CollabAgentToolCallStatus::InProgress,
...
async fn collab_receiver_notification_does_not_cache_not_found_thread() {
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(
        ServerNotification::ItemCompleted(
            peregrine_app_server_protocol::ItemCompletedNotification {
                thread_id: ThreadId::new().to_string(),
                turn_id: "turn-1".to_string(),
                completed_at_ms: 0,
                item: ThreadItem::CollabAgentToolCall {
                    id: "send-1".to_string(),
                    tool: peregrine_app_server_protocol::CollabAgentTool::SendInput,
                    status: peregrine_app_server_protocol::CollabAgentToolCallStatus::Failed,
                    sender_thread_id: ThreadId::new().to_string(),
                    receiver_thread_ids: vec![receiver_thread_id.to_string()],
                    ...
                    agents_states: HashMap::from([(
                        ...
                        peregrine_app_server_protocol::CollabAgentState {
                            status: peregrine_app_server_protocol::CollabAgentStatus::NotFound,
                            message: None,
                        },
                    )]),
                },
...
async fn open_agent_picker_keeps_missing_threads_for_replay() -> Result<()> {
    ...
    let mut app_server = Box::pin(crate::agent::start_embedded_app_server_for_picker(
...
fn attach_live_thread_for_selection_rejects_empty_non_ephemeral_fallback_threads() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        let mut app_server = crate::agent::start_embedded_app_server_for_picker(&config)
            .await
            .expect("embedded app server");
...
fn attach_live_thread_for_selection_rejects_unmaterialized_fallback_threads() -> Result<()> {
    ...
    runtime.block_on(async {
        let mut app = make_test_app().await;
        let mut app_server =
            crate::agent::start_embedded_app_server_for_picker(app.chat_widget.config_ref())
                .await?;
...
async fn update_memory_settings_persists_and_updates_widget_config() -> Result<()> {
    ...
    app.config.peregrine_home = peregrine_home.path().to_path_buf().abs();
    let mut app_server = Box::pin(crate::agent::start_embedded_app_server_for_picker(
        &app.config,
    ))
    .await?;
    ...
    let config = std::fs::read_to_string(peregrine_home.path().join("config.toml"))?;
...
fn update_memory_settings_updates_current_thread_memory_mode() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        let peregrine_home = tempdir()?;
        app.config.peregrine_home = peregrine_home.path().to_path_buf().abs();
        app.config.sqlite_home = peregrine_home.path().to_path_buf();
        ...
        let state_db = codex_state::StateRuntime::init(
            peregrine_home.path().to_path_buf(),
...
async fn reset_memories_clears_local_memory_directories() -> Result<()> {
    Box::pin(async {
        ...
        let memory_root = peregrine_home.path().join("memories");
...
"#,
    ...
    let op = match app_event_rx.try_recv() {
        Ok(AppEvent::PeregrineOp(op)) => op,
        other => panic!("expected PeregrineOp event, got {other:?}"),
    };
    assert_eq!(
        op,
...
-> Result<()> {
    ...
    let peregrine_home = tempdir()?;
    app.config.peregrine_home = peregrine_home.path().to_path_buf().abs();
    let config_toml_path = peregrine_home.path().join("config.toml").abs();
...
async fn inactive_thread_exec_approval_preserves_context() {
    ...
    assert_eq!(
        available_decisions,
        vec![
            peregrine_app_server_protocol::CommandExecutionApprovalDecision::Accept,
            peregrine_app_server_protocol::CommandExecutionApprovalDecision::AcceptForSession,
            peregrine_app_server_protocol::CommandExecutionApprovalDecision::ApplyNetworkPolicyAmendment {
                network_policy_amendment: AppServerNetworkPolicyAmendment {
                    host: "example.com".to_string(),
                    action: AppServerNetworkPolicyRuleAction::Allow,
                },
            },
            peregrine_app_server_protocol::CommandExecutionApprovalDecision::Cancel,
        ]
    );
}
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::FileChange {
                ...
                status: peregrine_app_server_protocol::PatchApplyStatus::InProgress,
            },
        }),
    )
    .await
...
async fn inactive_thread_permissions_approval_preserves_file_system_permissions() {
    ...
    let request = ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: peregrine_app_server_protocol::RequestPermissionProfile {
                network: Some(AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: Some(AdditionalFileSystemPermissions {
...
async fn inactive_thread_url_elicitation_routes_to_app_link() {
    ...
    assert_eq!(
        ...
        Some(crate::agent::bottom_pane::AppLinkElicitationTarget {
            thread_id,
            server_name: "payments".to_string(),
            request_id: AppServerRequestId::Integer(9),
        })
    ...
    assert_matches!(
        ...
        Ok(AppEvent::SubmitThreadOp {
            ...
            op: Op::ResolveElicitation {
                server_name,
                request_id: AppServerRequestId::Integer(10),
                decision: peregrine_app_server_protocol::McpServerElicitationAction::Decline,
                content: None,
                meta: None,
            },
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadStarted(ThreadStartedNotification {
            thread: Thread {
                ...
                created_at: 1,
                updated_at: 2,
                status: peregrine_app_server_protocol::ThreadStatus::Idle,
                path: Some(rollout_path.clone()),
                cwd: test_path_buf("/tmp/agent").abs(),
                cli_version: "0.0.0".to_string(),
                source: peregrine_app_server_protocol::SessionSource::Unknown,
                thread_source: None,
                agent_nickname: Some("Robie".to_string()),
                agent_role: Some("explorer".to_string()),
                git_info: None,
...
async fn side_parent_status_prioritizes_input_over_approval() -> Result<()> {
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ServerRequestResolved(
            peregrine_app_server_protocol::ServerRequestResolvedNotification {
...
async fn side_thread_ignores_global_mcp_startup_notifications() {
    ...
    app.handle_app_server_event(
        &app_server,
        peregrine_app_server_client::AppServerEvent::ServerNotification(
...
async fn side_restore_user_message_puts_inline_question_back_in_composer() {
    let mut app = make_test_app().await;
    let user_message = crate::agent::chatwidget::UserMessage::from("side question");
...
async fn active_non_primary_shutdown_target_returns_none_for_non_shutdown_event() -> Result<()> {
    ...
    assert_eq!(
        app.active_non_primary_shutdown_target(&ServerNotification::SkillsChanged(
            peregrine_app_server_protocol::SkillsChangedNotification {},
        )),
        None
    );
    Ok(())
...
async fn render_clear_ui_header_after_long_transcript_for_snapshot() -> String {
    ...
    app.transcript_cells = vec![
        make_header(true),
        Arc::new(crate::agent::history_cell::new_info_event(
...
async fn clear_ui_header_shows_fast_status_for_fast_capable_models() {
    ...
    app.chat_widget.set_service_tier(Some(
        peregrine_types::config_types::ServiceTier::Fast
            .request_value()
            .to_string(),
    ));
...
async fn make_test_app() -> App {
    ...
    App {
        ...
        enhanced_keys_supported: false,
        keymap: crate::agent::keymap::RuntimeKeymap::defaults(),
        ...
        app_server_target: crate::agent::AppServerTarget::Embedded,
...
fn rendered_line_text(line: &crate::agent::terminal_hyperlinks::HyperlinkLine) -> String {
    line.line
        .spans
        .iter()
        .map(|span| span.content.as_ref())
...
async fn resize_reflow_wraps_transcript_early_when_pet_is_enabled() {
    ...
    app.chat_widget.set_pet_image_support_for_tests(
        crate::agent::pets::PetImageSupport::Supported(crate::agent::pets::ImageProtocol::Kitty),
    );
    app.chat_widget
...
async fn height_shrink_schedules_resize_reflow() {
    let (mut app, _rx, _op_rx) = make_test_app_with_channels().await;
    enable_terminal_resize_reflow(&mut app);
    let frame_requester = crate::agent::tui::FrameRequester::test_dummy();
...
fn test_session_telemetry(config: &Config, model: &str) -> SessionTelemetry {
    let model_info =
        crate::agent::legacy_core::test_support::construct_model_info_offline(model, config);
    SessionTelemetry::new(
        ...
        "test".to_string(),
        crate::agent::test_support::session_source_cli(),
    )
}
...
fn active_turn_not_steerable_turn_error_extracts_structured_server_error() {
    let turn_error = AppServerTurnError {
        ...
        peregrine_error_info: Some(AppServerPeregrineErrorInfo::ActiveTurnNotSteerable {
...
fn session_start_error_surfaces_archived_guidance_without_rollout_path() {
    ...
    let expected = format!(
        "session {thread_id} is archived. Run `peregrine unarchive {thread_id}` to unarchive it first."
    );
...
async fn backtrack_selection_with_duplicate_history_targets_unique_turn() {
    ...
    app.chat_widget
        .handle_thread_session(crate::agent::session_state::ThreadSessionState {
            thread_id: base_id,
            forked_from_id: None,
            fork_parent_title: None,
            thread_name: None,
        ...
        ThreadEventSnapshot {
            ...
            events: vec![ThreadBufferedEvent::Notification(
                ServerNotification::ItemStarted(
                    peregrine_app_server_protocol::ItemStartedNotification {
                        ...
                        item: ThreadItem::CollabAgentToolCall {
                            ...
                            tool: peregrine_app_server_protocol::CollabAgentTool::Wait,
                            status:
                                peregrine_app_server_protocol::CollabAgentToolCallStatus::InProgress,
                            sender_thread_id: ThreadId::new().to_string(),
                            receiver_thread_ids: vec![receiver_thread_id.to_string()],
                            prompt: None,
                            model: None,
...
async fn new_session_requests_shutdown_for_previous_conversation() {
    Box::pin(async {
        ...
        let thread_id = ThreadId::new();
        let event = crate::agent::session_state::ThreadSessionState {
            thread_id,
            forked_from_id: None,
            fork_parent_title: None,
            thread_name: None,
    ...
    Box::pin(async {
        ...
        let op = AppCommand::override_turn_context(
            ...
            Some(ActivePermissionProfile::new(
                peregrine_types::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE,
            )),
            /*windows_sandbox_level*/ None,
...
async fn inactive_thread_settings_notification_updates_cached_collaboration_mode() {
    ...
    let notification = ThreadSettingsUpdatedNotification {
        ...
        thread_settings: ThreadSettings {
            ...
            approval_policy: AskForApproval::OnRequest,
            approvals_reviewer: peregrine_app_server_protocol::ApprovalsReviewer::AutoReview,
            sandbox_policy: peregrine_app_server_protocol::SandboxPolicy::ReadOnly {
                network_access: false,
            },
            active_permission_profile: Some(
                peregrine_app_server_protocol::ActivePermissionProfile::read_only(),
            ),
            model: "gpt-plan".to_string(),
            model_provider: "openai".to_string(),
            service_tier: None,
...
async fn clear_only_ui_reset_preserves_chat_session_state() {
    ...
    app.overlay = Some(Overlay::new_transcript(
        app.transcript_cells.clone(),
        crate::agent::keymap::RuntimeKeymap::defaults().pager,
    ));
    app.deferred_history_lines = vec![Line::from("stale buffered line").into()];
    app.has_emitted_history_lines = true;
    app.backtrack.primed = true;
...
async fn start_config_write_test_app_server(app: &App) -> Result<AppServerSession> {
    Box::pin(crate::agent::start_embedded_app_server_for_picker(
        &app.config,
    ))
    .await
}
