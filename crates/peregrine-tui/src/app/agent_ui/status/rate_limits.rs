...
use crate::agent::chatwidget::fallback_limit_label;
use crate::agent::chatwidget::limit_label_for_window;
use crate::agent::text_formatting::capitalize_first;
...
use chrono::Local;
use chrono::Utc;
use peregrine_app_server_protocol::CreditsSnapshot as CoreCreditsSnapshot;
...
pub(crate) fn rate_limit_snapshot_display(
    ...
    rate_limit_snapshot_display_for_limit(snapshot, "peregrine".to_string(), captured_at)
}
...
pub(crate) fn compose_rate_limit_data_many(
    ...
    for snapshot in snapshots {
        ...
        let show_limit_prefix = !limit_bucket_label.eq_ignore_ascii_case("peregrine");
...
mod tests {
    ...
    fn non_codex_single_limit_renders_combined_row() {
        let now = Local::now();
        let peregrine = RateLimitSnapshotDisplay {
            limit_name: "peregrine".to_string(),
            captured_at: now,
        ...
        let rows = match compose_rate_limit_data_many(&[peregrine, other], now) {
