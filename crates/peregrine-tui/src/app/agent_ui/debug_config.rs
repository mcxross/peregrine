...
use peregrine_config::NetworkUnixSocketPermissionToml;
...
use peregrine_config::WebSearchModeRequirement;
        ...
        let all_proxy =
            session_all_proxy_url(
                ...
                config.permissions.network.as_ref().is_some_and(
                    crate::agent::legacy_core::config::NetworkProxySpec::socks_enabled,
                ),
            );
...
fn format_config_layer_source(source: &ConfigLayerSource) -> String {
    match source {
        ...
        ConfigLayerSource::Project {
            ...
            format!(
                "project ({}/config.toml)",
                dot_peregrine_folder.as_path().display()
            )
        }
...
mod tests {
    ...
    use peregrine_config::HookHandlerConfig;
    use peregrine_config::ManagedHooksRequirementsToml;
    use peregrine_config::MatcherGroup;
    ...
    use peregrine_config::NetworkUnixSocketPermissionToml;
    use peregrine_config::NetworkUnixSocketPermissionsToml;
    use peregrine_config::RequirementSource;
    use peregrine_config::ResidencyRequirement;
    use peregrine_config::SandboxModeRequirement;
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        ...
        let layers = vec![
            ...
            ConfigLayerEntry::new_disabled(
                ConfigLayerSource::Project {
                    dot_peregrine_folder: project_folder,
                },
                empty_toml_table(),
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            absolute_path("C:\\ProgramData\\Peregrine\\requirements.toml")
        } else {
            absolute_path("/etc/peregrine/requirements.toml")
        };
