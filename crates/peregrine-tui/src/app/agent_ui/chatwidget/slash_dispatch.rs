...
fn security_scan_goal_objective(scope: Option<&str>) -> String {
    let mut objective = String::from(
        "Run a full Sui Move security scan of the loaded package as Peregrine's security-focused harness.\n\
         ...
         If `tool_search` is available, first use it to discover Sui analysis tools relevant to package insight, static analysis, scanner reports, graphs, bytecode, fuzzing, and formal verification. Call returned tools as harness/model tools with their JSON arguments. They are not shell commands; never run MCP tool names through the shell.\n\
        ...
        match cmd {
            ...
            SlashCommand::Archive => {
                self.bottom_pane.show_selection_view(SelectionViewParams {
                    ...
                    subtitle: Some(
                        "Are you sure? This will archive the current session and exit Peregrine"
                            .to_string(),
                    ),
            ...
            SlashCommand::ElevateSandbox => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    if !windows_degraded_sandbox_enabled
                        || !crate::agent::legacy_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
                    {
            ...
            SlashCommand::Workbench => {
                self.app_event_tx.send(AppEvent::SwitchToWorkbench);
            }
            SlashCommand::Copy => {
    ...
    fn dispatch_prepared_command_with_args(
        ...
        match cmd {
            ...
            SlashCommand::Keymap => match trimmed.to_ascii_lowercase().as_str() {
                "" => self.open_keymap_picker(),
                "debug" => {
                    match crate::agent::keymap::RuntimeKeymap::from_config(&self.config.tui_keymap)
                    {
            ...
            SlashCommand::Rename if !trimmed.is_empty() => {
                ...
                self.session_telemetry
                    .counter("codex.thread.rename", /*inc*/ 1, &[]);
                let Some(name) = crate::agent::legacy_core::util::normalize_thread_name(&args)
                else {
