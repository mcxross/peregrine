...
use crate::agent::bottom_pane::SelectionRowDisplay;
use crate::agent::bottom_pane::SelectionTab;
use crate::agent::bottom_pane::SelectionToggle;
...
use crate::agent::motion::shimmer_text;
use crate::agent::onboarding::mark_url_hyperlink;
...
impl ChatWidget {
    ...
    fn marketplace_remove_confirmation_popup_params(
        ...
        header.push(Line::from(
            "This removes the configured marketplace from Peregrine.".dim(),
        ));
    ...
    fn plugins_popup_params(
        ...
        tabs.push(SelectionTab {
            ...
            header: plugins_header(
                "Curated marketplace.".to_string(),
                format!("Installed {curated_installed} of {curated_total} curated plugins."),
            ),
            items: self.plugin_selection_items(
                curated_entries,
                /*include_marketplace_names*/ false,
                "No curated plugins available",
                "No curated plugins available.",
            ),
        });
            ...
            let actions: Vec<SelectionAction> = if let Some(marketplace_path) = marketplace_path {
                vec![Box::new(move |tx| {
                    ...
                    tx.send(AppEvent::FetchPluginDetail {
                        cwd: cwd.clone(),
                        params: peregrine_app_server_protocol::PluginReadParams {
...
fn plugin_hook_summary(plugin: &PluginDetail) -> String {
    if plugin.hooks.is_empty() {
        ...
        let mut event_counts = Vec::<(peregrine_app_server_protocol::HookEventName, usize)>::new();
