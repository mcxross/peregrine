use super::*;
use crate::agent::bottom_pane::goal_status_indicator_line;
use crate::agent::chatwidget::rate_limits::NUDGE_MODEL_SLUG;
use crate::agent::chatwidget::rate_limits::get_limits_duration;
use pretty_assertions::assert_eq;
use ratatui::backend::TestBackend;
use serial_test::serial;

fn enable_test_ambient_pet(chat: &mut ChatWidget) {
    chat.set_pet_image_support_for_tests(crate::agent::pets::PetImageSupport::Supported(
        crate::agent::pets::ImageProtocol::Kitty,
    ));
...
async fn app_server_cyber_policy_error_renders_dedicated_notice() {
    ...
    handle_error(
        ...
        Some(PeregrineErrorInfo::CyberPolicy),
    );
    ...
    assert!(rendered.contains("adjust the task scope"));
    assert!(!rendered.contains("server fallback message"));
    let removed_program_name = ["Trusted Access", " for Cyber"].concat();
    assert!(!rendered.contains(&removed_program_name));
    assert!(!rendered.contains("chatgpt.com"));
}
...
async fn token_usage_update_uses_runtime_context_window() {
    ...
    assert_eq!(
        chat.status_line_value_for_item(
            crate::agent::bottom_pane::StatusLineItem::ContextWindowSize
        ),
...
async fn status_line_git_summary_items_render_values() {
    ...
    chat.status_line_git_summary = Some(StatusLineGitSummary {
        pull_request: Some(crate::agent::branch_summary::StatusLinePullRequest {
            number: 20_252,
            url: "https://github.com/openai/codex/pull/20252".to_string(),
        }),
        branch_change_stats: Some(crate::agent::branch_summary::GitBranchDiffStats {
            additions: 143,
            deletions: 22,
        }),
    });
    ...
    assert_eq!(
        chat.status_line_value_for_item(
            crate::agent::bottom_pane::StatusLineItem::PullRequestNumber
        ),
        Some("PR #20252".to_string())
    );
    assert_eq!(
        chat.status_line_value_for_item(crate::agent::bottom_pane::StatusLineItem::BranchChanges),
...
async fn raw_output_status_line_value_only_shows_when_enabled() {
    ...
    assert_eq!(
        chat.status_line_value_for_item(crate::agent::bottom_pane::StatusLineItem::RawOutput),
        None
    );
...
async fn flush_answer_stream_keeps_default_reflow_for_plain_text_tail() {
    ...
    let mut controller = crate::agent::streaming::controller::StreamController::new(
        Some(80),
        cwd.as_path(),
    ...
    while let Ok(event) = rx.try_recv() {
        match event {
            ...
            AppEvent::ConsolidateAgentMessage {
                ...
                assert_eq!(
                    scrollback_reflow,
                    crate::agent::app_event::ConsolidationScrollbackReflow::IfResizeReflowRan
                );
    ...
    while let Ok(event) = rx.try_recv() {
        match event {
            ...
            AppEvent::ConsolidateAgentMessage {
                ...
                assert_eq!(
                    scrollback_reflow,
                    crate::agent::app_event::ConsolidationScrollbackReflow::Required
                );
                assert!(
...
async fn completed_plan_table_tail_skips_provisional_history_insert() {
    ...
    let mut controller = crate::agent::streaming::controller::PlanStreamController::new(
        Some(80),
        cwd.as_path(),
        HistoryRenderMode::Rich,
    );
    ...
    controller.push("| --- | --- |\n");
    controller.push("| Verify | Peregrine |\n");
    assert!(
        controller.has_live_tail(),
...
async fn configured_pet_load_is_deferred_until_after_construction() {
    ...
    cfg.tui_pet = Some(crate::agent::pets::DEFAULT_PET_ID.to_string());
    crate::agent::pets::write_test_pack(&cfg.peregrine_home);
    let resolved_model =
        crate::agent::legacy_core::test_support::get_model_offline(cfg.model.as_deref());
    ...
    assert_matches!(
        ...
        AppEvent::ConfiguredPetLoaded { pet_id, result } => {
            assert_eq!(pet_id, crate::agent::pets::DEFAULT_PET_ID);
...
async fn status_line_uses_secondary_fallback_for_unsupported_window() {
    ...
    assert_eq!(
        chat.status_line_value_for_item(crate::agent::bottom_pane::StatusLineItem::WeeklyLimit),
...
async fn status_line_legacy_limit_items_prefer_matching_windows() {
    ...
    assert_eq!(
        chat.status_line_value_for_item(crate::agent::bottom_pane::StatusLineItem::FiveHourLimit),
...
async fn rate_limit_snapshots_keep_separate_entries_per_limit_id() {
    ...
    chat.on_rate_limit_snapshot(Some(RateLimitSnapshot {
        limit_id: Some("peregrine".to_string()),
        limit_name: Some("peregrine".to_string()),
        primary: Some(RateLimitWindow {
            used_percent: 20,
            window_duration_mins: Some(300),
            resets_at: Some(100),
    ...
    let peregrine = chat
        .rate_limit_snapshots_by_limit_id
        .get("peregrine")
        .expect("peregrine snapshot should exist");
    let other = chat
        .rate_limit_snapshots_by_limit_id
        .get("codex_other")
    ...
    assert_eq!(
        peregrine.primary.as_ref().map(|w| w.used_percent),
        Some(20.0)
    );
    assert_eq!(
        peregrine
...
async fn workspace_owner_limit_states_render_state_specific_messages() {
    let cases = [
        (
            RateLimitReachedType::WorkspaceOwnerCreditsDepleted,
            RateLimitErrorKind::Generic,
            "You're out of credits. Your workspace is out of credits. Add credits to continue using Peregrine.",
        ),
        (
            RateLimitReachedType::WorkspaceOwnerUsageLimitReached,
            RateLimitErrorKind::UsageLimit,
            "Usage limit reached. You've reached your usage limit. Increase your limits to continue using Peregrine.",
        ),
    ];
...
async fn ctrl_c_interrupt_pauses_active_goal_turn() {
    ...
    let mut goal = test_thread_goal(
        peregrine_app_server_protocol::ThreadGoalStatus::Active,
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadGoalUpdated(
            peregrine_app_server_protocol::ThreadGoalUpdatedNotification {
...
async fn ambient_pet_stays_hidden_until_a_pet_is_selected() {
    ...
    crate::agent::pets::write_test_pack(&chat.config.peregrine_home);
    chat.set_tui_pet(Some("peregrine".to_string()));
...
async fn ambient_pet_can_be_disabled() {
    ...
    chat.set_tui_pet(Some(crate::agent::pets::DISABLED_PET_ID.to_string()));
...
async fn ambient_pet_reduces_stream_width_and_composer_text_width() {
    ...
    let (mut disabled, _disabled_rx, _disabled_op_rx) =
        make_chatwidget_manual(/*model_override*/ None).await;
    disabled.set_tui_pet(Some(crate::agent::pets::DISABLED_PET_ID.to_string()));
    ...
    assert_eq!(
        ...
        crate::agent::width::usable_content_width(
            /*total_width*/ 69, /*reserved_cols*/ 2
        )
    );
    assert_eq!(
        stream_width_without_pet,
        crate::agent::width::usable_content_width(
            /*total_width*/ 80, /*reserved_cols*/ 2
        )
    );
...
async fn ambient_pet_hides_notification_text_overlay() {
    ...
    enable_test_ambient_pet(&mut chat);
    for (kind, label) in [
        (crate::agent::pets::PetNotificationKind::Running, "Running"),
        (
            crate::agent::pets::PetNotificationKind::Waiting,
            "Needs input",
        ),
        (crate::agent::pets::PetNotificationKind::Review, "Ready"),
        (crate::agent::pets::PetNotificationKind::Failed, "Blocked"),
    ] {
...
impl crate::agent::workspace_command::WorkspaceCommandExecutor for NoopWorkspaceCommandRunner {
    fn run(
        &self,
        _command: crate::agent::workspace_command::WorkspaceCommand,
    ) -> std::pin::Pin<
        Box<
            dyn std::future::Future<
                    Output = Result<
                        crate::agent::workspace_command::WorkspaceCommandOutput,
                        crate::agent::workspace_command::WorkspaceCommandError,
                    >,
                > + Send
                + '_,
        >,
    > {
        Box::pin(async {
            Ok(crate::agent::workspace_command::WorkspaceCommandOutput {
                exit_code: 1,
                stdout: String::new(),
                stderr: String::new(),
            })
...
async fn interrupted_turn_clears_visible_running_hook() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            ...
            peregrine_app_server_protocol::HookEventName::PreToolUse,
...
async fn status_line_model_with_reasoning_includes_fast_for_fast_capable_models() {
    ...
    assert_eq!(
        status_line_text(&chat),
        Some(format!(
            "gpt-5.3-peregrine xhigh · Context 0% used · {test_cwd}"
        ))
    );
}
...
async fn status_line_model_with_reasoning_updates_on_mode_switch_without_manual_refresh() {
    ...
    assert_eq!(
        status_line_text(&chat),
        Some("gpt-5.3-peregrine high".to_string())
    );
    ...
    assert_eq!(
        status_line_text(&chat),
        Some("gpt-5.3-peregrine medium".to_string())
    );
...
async fn renamed_thread_footer_title_snapshot() {
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadNameUpdated(
            peregrine_app_server_protocol::ThreadNameUpdatedNotification {
                thread_id: thread_id.to_string(),
...
async fn status_line_goal_complete_elapsed_footer_snapshot() {
    ...
    chat.refresh_status_line();
    let mut goal = test_thread_goal(
        peregrine_app_server_protocol::ThreadGoalStatus::Complete,
        /*token_budget*/ None,
        /*tokens_used*/ 40_000,
    );
...
async fn thread_goal_update_for_other_thread_is_ignored() {
    ...
    let mut goal = test_thread_goal(
        peregrine_app_server_protocol::ThreadGoalStatus::BudgetLimited,
        /*token_budget*/ Some(50_000),
        /*tokens_used*/ 50_000,
    );
...
fn goal_status_indicator_formats_statuses_and_budgets() {
    ...
    assert_eq!(
        goal_status_indicator_from_app_goal(&test_thread_goal(
            peregrine_app_server_protocol::ThreadGoalStatus::Blocked,
            /*token_budget*/ None,
            /*tokens_used*/ 0,
        )),
    ...
    assert_eq!(
        goal_status_indicator_from_app_goal(&test_thread_goal(
            peregrine_app_server_protocol::ThreadGoalStatus::UsageLimited,
            /*token_budget*/ None,
            /*tokens_used*/ 0,
        )),
...
fn test_thread_goal(
    status: peregrine_app_server_protocol::ThreadGoalStatus,
    token_budget: Option<i64>,
    tokens_used: i64,
) -> peregrine_app_server_protocol::ThreadGoal {
    peregrine_app_server_protocol::ThreadGoal {
        thread_id: "thread-1".to_string(),
    ...
    chat.handle_server_notification(
        ServerNotification::HookStarted(AppServerHookStartedNotification {
            ...
            run: AppServerHookRunSummary {
                ...
                source: peregrine_app_server_protocol::HookSource::User,
                display_order: 0,
...
async fn post_tool_use_hook_events_render_snapshot() {
    assert_hook_events_snapshot(
        peregrine_app_server_protocol::HookEventName::PostToolUse,
...
async fn completed_hook_with_no_entries_stays_out_of_history() {
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            "post-tool-use:0:/tmp/hooks.json",
            peregrine_app_server_protocol::HookEventName::PostToolUse,
            peregrine_app_server_protocol::HookRunStatus::Completed,
            Vec::new(),
        ),
    );
...
async fn blocked_and_failed_hooks_render_feedback_and_errors() {
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            "pre-tool-use:0:/tmp/hooks.json",
            peregrine_app_server_protocol::HookEventName::PreToolUse,
            peregrine_app_server_protocol::HookRunStatus::Blocked,
            vec![peregrine_app_server_protocol::HookOutputEntry {
                kind: peregrine_app_server_protocol::HookOutputEntryKind::Feedback,
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            ...
            peregrine_app_server_protocol::HookEventName::PostToolUse,
            peregrine_app_server_protocol::HookRunStatus::Failed,
            vec![peregrine_app_server_protocol::HookOutputEntry {
                kind: peregrine_app_server_protocol::HookOutputEntryKind::Error,
...
async fn completed_same_id_hook_output_survives_restart() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            hook_id,
            peregrine_app_server_protocol::HookEventName::Stop,
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            hook_id,
            peregrine_app_server_protocol::HookEventName::Stop,
            peregrine_app_server_protocol::HookRunStatus::Stopped,
            vec![peregrine_app_server_protocol::HookOutputEntry {
                kind: peregrine_app_server_protocol::HookOutputEntryKind::Stop,
...
async fn hook_completed_before_reveal_renders_completed_without_running_flash() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            ...
            peregrine_app_server_protocol::HookEventName::SessionStart,
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            ...
            peregrine_app_server_protocol::HookRunStatus::Completed,
            vec![peregrine_app_server_protocol::HookOutputEntry {
                kind: peregrine_app_server_protocol::HookOutputEntryKind::Context,
...
fn hook_started_run(
    id: &str,
    event_name: peregrine_app_server_protocol::HookEventName,
    status_message: Option<&str>,
) -> peregrine_app_server_protocol::HookRunSummary {
    hook_run_summary(
        id,
        event_name,
        peregrine_app_server_protocol::HookRunStatus::Running,
        status_message,
        Vec::new(),
    )
}
...
fn hook_completed_run(
    id: &str,
    event_name: peregrine_app_server_protocol::HookEventName,
    status: peregrine_app_server_protocol::HookRunStatus,
    entries: Vec<peregrine_app_server_protocol::HookOutputEntry>,
) -> peregrine_app_server_protocol::HookRunSummary {
    hook_run_summary(
...
fn hook_run_summary(
    ...
    peregrine_app_server_protocol::HookRunSummary {
        id: id.to_string(),
        event_name,
        handler_type: peregrine_app_server_protocol::HookHandlerType::Command,
        execution_mode: peregrine_app_server_protocol::HookExecutionMode::Sync,
        scope: peregrine_app_server_protocol::HookScope::Turn,
        source_path: PathBuf::from(test_path_display("/tmp/hooks.json")).abs(),
        source: peregrine_app_server_protocol::HookSource::User,
        display_order: 0,
        status,
        ...
        started_at: 1,
        completed_at: (status != peregrine_app_server_protocol::HookRunStatus::Running)
            .then_some(2),
        duration_ms: (status != peregrine_app_server_protocol::HookRunStatus::Running).then_some(1),
        entries,
    }
}
...
async fn chatwidget_exec_and_status_layout_vt100_snapshot() {
    ...
    let mut term =
        crate::agent::custom_terminal::Terminal::with_options(backend).expect("terminal");
    term.set_viewport_area(viewport);

    for lines in drain_insert_history(&mut rx) {
        crate::agent::insert_history::insert_history_lines(&mut term, lines)
