...
fn force_tmux_pet_image_unsupported(chat: &mut ChatWidget) {
    chat.set_pet_image_support_for_tests(crate::agent::pets::PetImageSupport::Unsupported(
        crate::agent::pets::PetImageUnsupportedReason::Tmux,
    ));
}

fn force_terminal_pet_image_unsupported(chat: &mut ChatWidget) {
    chat.set_pet_image_support_for_tests(crate::agent::pets::PetImageSupport::Unsupported(
        crate::agent::pets::PetImageUnsupportedReason::Terminal,
    ));
}

fn force_old_iterm2_pet_image_unsupported(chat: &mut ChatWidget) {
    chat.set_pet_image_support_for_tests(crate::agent::pets::PetImageSupport::Unsupported(
        crate::agent::pets::PetImageUnsupportedReason::Iterm2TooOld,
    ));
}
...
async fn slash_compact_eagerly_queues_follow_up_before_turn_start() {
    ...
    match rx.try_recv() {
        Ok(AppEvent::PeregrineOp(Op::Compact)) => {}
...
async fn queued_slash_compact_dispatches_after_active_turn() {
    ...
    assert!(
        events
            .iter()
            .any(|event| matches!(event, AppEvent::PeregrineOp(Op::Compact))),
...
async fn queued_bare_rename_drains_next_input_after_name_update() {
    ...
    assert!(
        events.iter().any(|event| matches!(
            event,
            AppEvent::PeregrineOp(Op::SetThreadName { name }) if name == "Queued rename"
        )),
...
async fn slash_init_skips_when_project_doc_exists() {
    ...
    match op_rx.try_recv() {
        Err(TryRecvError::Empty) => {}
        other => panic!("expected no Peregrine op to be sent, got {other:?}"),
    }
...
async fn goal_slash_command_emits_set_goal_event() {
    ...
    assert_eq!(
        mode,
        crate::agent::app_event::ThreadGoalSetMode::ConfirmIfExists
    );
...
#[tokio::test]
async fn bare_scan_slash_command_emits_security_goal_event() {
    let (mut chat, mut rx, mut op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_feature_enabled(Feature::Goals, /*enabled*/ true);
    let thread_id = ThreadId::new();
    chat.thread_id = Some(thread_id);

    submit_composer_text(&mut chat, "/scan");

    let event = rx.try_recv().expect("expected scan goal objective event");
    let AppEvent::SetThreadGoalObjective {
        thread_id: actual_thread_id,
        objective,
        mode,
    ...
    assert_eq!(actual_thread_id, thread_id);
    assert!(objective.contains("Sui Move security scan"), "{objective}");
    assert!(
        objective.contains("Peregrine's security-focused harness"),
        "{objective}"
    );
    assert!(
        objective.contains("If `tool_search` is available"),
        "{objective}"
    );
    assert!(
        objective
    ...
    assert!(
        objective.contains("First infer and state what the package intends to do"),
        "{objective}"
    );
    assert!(
        objective.contains("build your own analysis plan and execute it"),
        "{objective}"
    );
    assert!(
        !objective.contains("User-provided scan scope or focus"),
        "{objective}"
    );
    assert_eq!(
        mode,
    ...
    assert_no_submit_op(&mut op_rx);
    assert_eq!(recall_latest_after_clearing(&mut chat), "/scan");
}

#[tokio::test]
async fn scan_slash_command_includes_inline_scope_in_security_goal() {
    let (mut chat, mut rx, mut op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_feature_enabled(Feature::Goals, /*enabled*/ true);
    let thread_id = ThreadId::new();
    chat.thread_id = Some(thread_id);
    let command = "/scan focus on capability handoff and mint flows";
    ...
    assert!(
        objective.contains(
            "User-provided scan scope or focus:\nfocus on capability handoff and mint flows"
        ),
        "{objective}"
    );
    assert_no_submit_op(&mut op_rx);
...
#[tokio::test]
async fn queued_scan_slash_command_emits_set_goal_event_after_thread_starts() {
    let (mut chat, mut rx, mut op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_feature_enabled(Feature::Goals, /*enabled*/ true);
...
async fn slash_rename_prefills_existing_thread_name() {
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::PeregrineOp(Op::SetThreadName { name })) if name == "Current project title"
    );
}
...
async fn unavailable_slash_command_is_available_from_local_recall() {
    ...
    let rendered = cells
        ...
        .join("\n");
    assert!(
        rendered.contains("'/permissions' is disabled while a task is in progress."),
        "expected disabled-command message, got: {rendered:?}"
    );
    assert_eq!(recall_latest_after_clearing(&mut chat), "/permissions");
}
...
async fn keymap_capture_can_capture_current_copy_shortcut() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let runtime_keymap = crate::agent::keymap::RuntimeKeymap::defaults();
    chat.open_keymap_capture(
        "composer".to_string(),
        "submit".to_string(),
        crate::agent::app_event::KeymapEditIntent::ReplaceAll,
        &runtime_keymap,
    );
...
async fn copy_shortcut_can_be_remapped() {
    ...
    keymap_config.global.copy = Some(peregrine_config::types::KeybindingsSpec::One(
        peregrine_config::types::KeybindingSpec("ctrl-x".to_string()),
    ));
    let runtime_keymap =
        crate::agent::keymap::RuntimeKeymap::from_config(&keymap_config).expect("valid copy remap");
...
async fn slash_copy_stores_clipboard_lease_and_preserves_it_on_failure() {
    ...
    chat.copy_last_agent_markdown_with(|markdown| {
        ...
        Ok(Some(crate::agent::clipboard_copy::ClipboardLease::test()))
    });
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadGoalUpdated(
            peregrine_app_server_protocol::ThreadGoalUpdatedNotification {
                ...
                turn_id: None,
                goal: peregrine_app_server_protocol::ThreadGoal {
                    thread_id: "thread-1".to_string(),
                    objective: "finish the benchmark".to_string(),
                    status: peregrine_app_server_protocol::ThreadGoalStatus::Active,
                    token_budget: None,
                    tokens_used: 0,
                    time_used_seconds: 0,
                    created_at: 1,
...
async fn fast_slash_command_updates_and_persists_local_service_tier() {
    ...
    assert!(
        events.iter().any(|event| matches!(
            event,
            AppEvent::PeregrineOp(Op::OverrideTurnContext {
...
async fn compact_queues_user_messages_snapshot() {
    ...
    handle_error(
        ...
        Some(PeregrineErrorInfo::ActiveTurnNotSteerable {
