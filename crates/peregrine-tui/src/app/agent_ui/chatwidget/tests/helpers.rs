...
pub(super) async fn test_config() -> Config {
    ...
    .await
    .expect("config");
    config.peregrine_home = peregrine_home.abs();
    config.sqlite_home = peregrine_home.clone();
    config.log_dir = peregrine_home.join("log");
...
pub(super) fn test_model_catalog(_config: &Config) -> Arc<ModelCatalog> {
    Arc::new(ModelCatalog::new(
        crate::agent::legacy_core::test_support::all_model_presets().clone(),
    ))
}
...
pub(super) async fn make_chatwidget_manual(
    ...
    let mut widget =
        ChatWidget::new_with_op_target(common, super::PeregrineOpTarget::Direct(op_tx));
    widget.transcript.active_cell = None;
    widget.transcript.active_cell_revision = 0;
    widget.normal_placeholder_text = "Ask Peregrine to do anything".to_string();
...
fn token_usage_breakdown(usage: TokenUsage) -> peregrine_app_server_protocol::TokenUsageBreakdown {
    peregrine_app_server_protocol::TokenUsageBreakdown {
        ...
        Some(info) => {
            chat.handle_server_notification(
                ServerNotification::ThreadTokenUsageUpdated(
                    peregrine_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                        ...
                        token_usage: peregrine_app_server_protocol::ThreadTokenUsage {
...
pub(super) fn handle_error(
    ...
    peregrine_error_info: Option<PeregrineErrorInfo>,
) {
    chat.handle_server_notification(
...
pub(super) fn terminal_interaction(
    ...
    chat.handle_server_notification(
        ServerNotification::TerminalInteraction(
            peregrine_app_server_protocol::TerminalInteractionNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .turn_lifecycle
                    .last_turn_id
...
pub(super) fn plugins_test_detail(
    ...
    description: Option<&str>,
    skills: &[&str],
    hooks: &[(peregrine_app_server_protocol::HookEventName, usize)],
    apps: &[(&str, bool)],
    mcp_servers: &[&str],
) -> PluginDetail {
    PluginDetail {
        ...
        hooks: hooks
            ...
            .flat_map(|(event_index, (event_name, handler_count))| {
                (0..*handler_count).map(move |handler_index| {
                    peregrine_app_server_protocol::PluginHookSummary {
...
pub(super) async fn assert_hook_events_snapshot(
    ...
    handle_hook_completed(
        ...
        hook_run(
            ...
            status_message,
            vec![
                peregrine_app_server_protocol::HookOutputEntry {
                    kind: peregrine_app_server_protocol::HookOutputEntryKind::Warning,
...
fn hook_event_label(event_name: peregrine_app_server_protocol::HookEventName) -> &'static str {
    match event_name {
        peregrine_app_server_protocol::HookEventName::PreToolUse => "PreToolUse",
        peregrine_app_server_protocol::HookEventName::PermissionRequest => "PermissionRequest",
        peregrine_app_server_protocol::HookEventName::PostToolUse => "PostToolUse",
        peregrine_app_server_protocol::HookEventName::PreCompact => "PreCompact",
        peregrine_app_server_protocol::HookEventName::PostCompact => "PostCompact",
        peregrine_app_server_protocol::HookEventName::SessionStart => "SessionStart",
        peregrine_app_server_protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
        peregrine_app_server_protocol::HookEventName::SubagentStart => "SubagentStart",
        peregrine_app_server_protocol::HookEventName::SubagentStop => "SubagentStop",
        peregrine_app_server_protocol::HookEventName::Stop => "Stop",
    }
}
