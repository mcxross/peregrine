...
impl ChatWidget {
    pub(super) fn open_theme_picker(&mut self) {
        let peregrine_home = peregrine_utils_home_dir::find_peregrine_home().ok();
    ...
    pub(crate) fn open_personality_popup(&mut self) {
        ...
        if !self.current_model_supports_personality() {
            ...
            self.add_error_message(format!(
                "Current model ({current_model}) doesn't support personalities. Choose a compatible provider or model before changing personality."
            ));
            return;
        }
    ...
    fn open_personality_popup_for_current_model(&mut self) {
        ...
        header.push(Line::from(
            "Choose a communication style for Peregrine.".dim(),
        ));
    ...
    pub(crate) fn open_realtime_audio_popup(&mut self) {
        ...
        self.bottom_pane.show_selection_view(SelectionViewParams {
            ...
            subtitle: Some("Configure settings for Peregrine.".to_string()),
            footer_hint: Some(standard_popup_hint_line()),
            items,
            ..Default::default()
        });
