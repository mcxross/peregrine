...
impl ChatWidget {
        ...
        if approvals_reviewer == ApprovalsReviewer::User && preset.id == "auto" {
            #[cfg(target_os = "windows")]
            {
                if WindowsSandboxLevel::from_config(&self.config) == WindowsSandboxLevel::Disabled {
                    let preset = preset.clone();
                    if crate::agent::legacy_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
    ...
    pub(crate) fn open_full_access_confirmation(
        ...
        let info_line = Line::from(vec![
            "When Peregrine runs with full access, it can edit any file on your computer and run commands with network, without your approval. "
                .into(),
