...
// The standalone `peregrine-tui` binary prints a short help message before the
// alternate‑screen mode starts; that file opts‑out locally via `allow`.
#![deny(clippy::print_stdout, clippy::print_stderr)]
#![deny(clippy::disallowed_methods)]
#![allow(dead_code, unused_imports, unused_mut)]
use crate::agent::legacy_core::check_execpolicy_for_warnings;
...
use crate::agent::legacy_core::config::load_config_as_toml_with_cli_and_load_options;
use crate::agent::legacy_core::config::resolve_oss_provider;
use crate::agent::legacy_core::config::resolve_profile_v2_config_path;
use crate::agent::legacy_core::format_exec_policy_error_with_source;
#[cfg(target_os = "windows")]
use crate::agent::legacy_core::windows_sandbox::WindowsSandboxLevelExt;
use crate::agent::session_resume::ResolveCwdOutcome;
...
use peregrine_app_server_client::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
use peregrine_app_server_client::InProcessAppServerClient;
use peregrine_app_server_client::InProcessClientStartArgs;
use peregrine_app_server_client::RemoteAppServerClient;
use peregrine_app_server_client::RemoteAppServerConnectArgs;
pub use peregrine_app_server_client::RemoteAppServerEndpoint;
use peregrine_app_server_protocol::Account as AppServerAccount;
use peregrine_app_server_protocol::AskForApproval;
use peregrine_app_server_protocol::AuthMode as AppServerAuthMode;
use peregrine_app_server_protocol::ConfigWarningNotification;
use peregrine_app_server_protocol::Thread as AppServerThread;
use peregrine_app_server_protocol::ThreadListCwdFilter;
use peregrine_app_server_protocol::ThreadListParams;
use peregrine_app_server_protocol::ThreadSortKey as AppServerThreadSortKey;
use peregrine_app_server_protocol::ThreadSourceKind;
use peregrine_config::CloudRequirementsLoader;
use peregrine_config::ConfigLoadError;
use peregrine_config::LoaderOverrides;
use peregrine_config::format_config_error_with_source;
...
pub(crate) use peregrine_app_server_client::legacy_core;

fn get_default_model_for_oss_provider(provider_id: &str) -> Option<&'static str> {
    match provider_id {
        codex_model_provider_info::LMSTUDIO_OSS_PROVIDER_ID => Some("openai/gpt-oss-20b"),
        codex_model_provider_info::OLLAMA_OSS_PROVIDER_ID => Some("gpt-oss:20b"),
        _ => None,
    }
}
...
pub(crate) use render::highlight::{
    highlight_code_for_path_to_lines, resolve_theme_by_name, set_syntax_theme,
};
...
use crate::agent::startup_hooks_review::StartupHooksReviewOutcome;
use crate::agent::startup_hooks_review::maybe_run_startup_hooks_review;
use crate::agent::tui::Tui;
pub use cli::Cli;
...
const TUI_LOG_FILE_NAME: &str = "peregrine-tui.log";
...
fn remove_legacy_tui_log_file(peregrine_home: &Path) {
    ...
    let _ = std::fs::remove_file(peregrine_home.join("log").join(TUI_LOG_FILE_NAME));
}
...
pub fn resolve_remote_addr(addr: &str) -> color_eyre::Result<RemoteAppServerEndpoint> {
    if let Some(socket_path) = addr.strip_prefix("unix://") {
        let socket_path = if socket_path.is_empty() {
            let peregrine_home =
                find_peregrine_home().wrap_err("failed to resolve PEREGRINE_HOME")?;
            peregrine_app_server_client::app_server_control_socket_path(&peregrine_home)
...
async fn connect_remote_app_server(
    ...
    let app_server = RemoteAppServerClient::connect(RemoteAppServerConnectArgs {
        endpoint,
        client_name: "peregrine-tui".to_string(),
...
#[cfg(unix)]
async fn maybe_probe_default_daemon_socket(peregrine_home: &Path) -> Option<AbsolutePathBuf> {
    let socket_path =
        peregrine_app_server_client::app_server_control_socket_path(peregrine_home).ok()?;
...
#[cfg(not(unix))]
async fn maybe_probe_default_daemon_socket(_peregrine_home: &Path) -> Option<AbsolutePathBuf> {
    None
}
...
pub(crate) async fn run_main_with_session(
    ...
    #[allow(clippy::print_stderr)]
    let peregrine_home = match find_peregrine_home() {
        Ok(peregrine_home) => peregrine_home.to_path_buf(),
        Err(err) => {
            eprintln!("Error finding peregrine home: {err}");
            std::process::exit(1);
        }
    };
    ...
    if let Some(profile_v2) = cli.config_profile_v2.as_ref() {
        let user_config_path = resolve_profile_v2_config_path(&peregrine_home, profile_v2);
    ...
    let default_daemon = if explicit_remote_endpoint.is_none() && reuse_implicit_local_daemon {
        maybe_probe_default_daemon_socket(&peregrine_home).await
    } else {
        None
    };
    ...
    let environment_manager =
        if should_load_configured_environments(&loader_overrides, &app_server_target) {
            EnvironmentManager::from_codex_home(peregrine_home.clone(), Some(local_runtime_paths))
                .await
        } else {
    ...
    let overrides = ConfigOverrides {
        ...
        peregrine_self_exe: arg0_paths.codex_self_exe.clone(),
        peregrine_linux_sandbox_exe: arg0_paths.codex_linux_sandbox_exe.clone(),
    ...
    let otel_originator = "peregrine-tui";
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        crate::agent::legacy_core::otel_init::build_provider(
            &config,
    ...
    crate::agent::legacy_core::otel_init::record_process_start(otel.as_ref(), otel_originator);
    crate::agent::legacy_core::otel_init::install_sqlite_telemetry(otel.as_ref(), otel_originator);
    let state_db = init_state_db_for_app_server_target(&config, &app_server_target).await?;

    let effective_toml = config.config_layer_stack.effective_config();
    match effective_toml.try_into() {
        Ok(config_toml) => {
            match crate::agent::legacy_core::personality_migration::maybe_migrate_personality(
            ...
            {
                Ok(
                    crate::agent::legacy_core::personality_migration::PersonalityMigrationStatus::Applied,
                ) => {
                ...
                Ok(
                    crate::agent::legacy_core::personality_migration::PersonalityMigrationStatus::SkippedMarker
                    | crate::agent::legacy_core::personality_migration::PersonalityMigrationStatus::SkippedExplicitPersonality
                    | crate::agent::legacy_core::personality_migration::PersonalityMigrationStatus::SkippedNoSessions,
                ) => {}
                Err(err) => {
    ...
    let (tui_file_layer, _tui_file_log_guard) = if config_toml.log_dir.is_some() {
        ...
        let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("peregrine_core=info,peregrine_tui=info,codex_rmcp_client=info")
        });
...
async fn run_ratatui_app(
    ...
    let config = if should_show_trust_screen_flag {
        let Some(startup_app_server) = app_server.as_mut() else {
            unreachable!("app server should be initialized before trust consent");
        };
        let trust_result =
            run_trust_consent_app(startup_app_server, &mut tui, &initial_config).await?;

        if trust_result.should_exit {
        ...
        #[cfg(target_os = "windows")]
        {
            trust_decision_was_made = trust_result.directory_trust_persisted;
        }

        if trust_result.directory_trust_persisted {
            load_config_or_exit(
                cli_kv_overrides.clone(),
                overrides.clone(),
                loader_overrides.clone(),
    ...
    if let Some(w) = crate::agent::render::highlight::set_theme_override(
    ...
    let required_elevated_sandbox_needs_setup = windows_sandbox_level
        ...
        && config
            ...
            .source
            .is_some()
        && !crate::agent::legacy_core::windows_sandbox::sandbox_setup_is_complete(
            config.peregrine_home.as_path(),
        );
    #[cfg(target_os = "windows")]
...
impl TerminalRestoreGuard {
    ...
    fn restore(&mut self) -> color_eyre::Result<()> {
        if self.active {
            crate::agent::tui::restore_after_exit()?;
            self.active = false;
        }
        Ok(())
    }
...
struct TrustConsentResult {
    directory_trust_persisted: bool,
    should_exit: bool,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum TrustConsentSelection {
    Trust,
    Quit,
}

async fn run_trust_consent_app(
    app_server: &mut AppServerSession,
    tui: &mut Tui,
    config: &Config,
) -> color_eyre::Result<TrustConsentResult> {
    use tokio_stream::StreamExt;

    let trust_target = codex_git_utils::resolve_root_git_project_for_trust(
    ...
    let mut selection = TrustConsentSelection::Trust;
        ...
        tokio::select! {
            event = tui_events.next() => {
                ...
                match event {
                    tui::TuiEvent::Key(key_event) => {
                        ...
                        match key_event.code {
                            ...
                            | ratatui::crossterm::event::KeyCode::Char('1') => {
                                selection = TrustConsentSelection::Trust;
                                error = None;
                            }
                            ratatui::crossterm::event::KeyCode::Down
                            | ratatui::crossterm::event::KeyCode::Char('j')
                            | ratatui::crossterm::event::KeyCode::Char('2') => {
                                selection = TrustConsentSelection::Quit;
                                error = None;
                            }
                            ratatui::crossterm::event::KeyCode::Esc
                            | ratatui::crossterm::event::KeyCode::Char('q') => {
                                return Ok(TrustConsentResult {
                            ...
                            ratatui::crossterm::event::KeyCode::Enter => {
                                match selection {
                                    TrustConsentSelection::Trust => {
                                        match config_update::write_trusted_project(
                                        ...
                                        .await
                                        {
                                            Ok(_) => {
                                                return Ok(TrustConsentResult {
                                                    directory_trust_persisted: true,
                                                    should_exit: false,
                                                });
                                            }
                                            Err(err) => {
                    ...
                    tui::TuiEvent::Paste(_) => {}
                    tui::TuiEvent::Draw | tui::TuiEvent::Resize => {
                        draw_trust_consent(
                            tui,
                            config,
                            trust_target.as_path(),
            ...
            event = app_server.next_event() => {
                match event {
                    Some(peregrine_app_server_client::AppServerEvent::Disconnected { message }) => {
                        return Err(color_eyre::eyre::eyre!(message));
                    }
                    Some(
                        peregrine_app_server_client::AppServerEvent::Lagged { .. }
                        | peregrine_app_server_client::AppServerEvent::ServerNotification(_)
                        | peregrine_app_server_client::AppServerEvent::ServerRequest(_)
                    ) => {}
                    None => {}
                }
            }
...
fn draw_trust_consent(
    tui: &mut Tui,
    config: &Config,
    trust_target: &Path,
    selection: TrustConsentSelection,
    error: Option<&str>,
) -> color_eyre::Result<()> {
    let palette = config
        .tui_theme
        .as_deref()
        .and_then(|name| name.parse::<crate::theme::ThemeName>().ok())
        .unwrap_or_default()
        .palette();
    ...
    tui.draw(u16::MAX, |frame| {
        ...
        if cwd.as_path() != trust_target {
            lines.push(Line::from(Span::styled(
                format!(
                    "Note: this is inside a Git repository. Trusting applies to {}.",
                    trust_target.display()
                ),
        ...
        lines.push(Line::from(Span::styled(
            "Do you trust the contents of this directory? Trusting allows project-local config, hooks, and exec policies to load.",
            Style::default().fg(palette.fg),
        )));
        lines.push(Line::from(""));
        lines.push(trust_option_line(
            "1. Yes, continue",
            selection == TrustConsentSelection::Trust,
            palette,
        ));
        ...
        lines.push(Line::from(Span::styled(
            "Press Enter to continue, arrows to choose.",
...
mod tests {
    ...
    fn session_target_display_label_falls_back_to_thread_id() {
        ...
        let target = crate::agent::resume_picker::SessionTarget {
            path: None,
            thread_id,
        };
    ...
    fn resolve_remote_addr_accepts_default_socket() -> color_eyre::Result<()> {
        let peregrine_home = find_peregrine_home().wrap_err("failed to resolve PEREGRINE_HOME")?;
        assert_eq!(
            resolve_remote_addr("unix://")?,
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: peregrine_app_server_client::app_server_control_socket_path(
                    &peregrine_home
                )?,
            }
        );
    ...
    async fn default_daemon_auto_connect_skips_missing_socket() -> color_eyre::Result<()> {
        ...
        assert!(
            maybe_probe_default_daemon_socket(peregrine_home.path())
                .await
                .is_none()
        );
        Ok(())
    ...
    async fn default_daemon_auto_connect_probes_socket_only() -> color_eyre::Result<()> {
        let peregrine_home = TempDir::new()?;
        let socket_path =
            peregrine_app_server_client::app_server_control_socket_path(peregrine_home.path())?;
        ...
        assert_eq!(
            maybe_probe_default_daemon_socket(peregrine_home.path()).await,
            Some(socket_path)
        );
        Ok(())
    }
    ...
    async fn fork_last_filters_latest_session_by_cwd_unless_show_all() -> color_eyre::Result<()> {
        fn write_session_rollout(
            ...
            let rollout_path = peregrine_home
                .join("sessions")
                .join(year)
                .join(month)
                .join(day)
            ...
            let session_meta = peregrine_types::protocol::SessionMeta {
                id: thread_id,
                timestamp: meta_rfc3339.to_string(),
                cwd: cwd.to_path_buf(),
                originator: "peregrine".to_string(),
                cli_version: "0.0.0".to_string(),
                source: peregrine_types::protocol::SessionSource::Cli,
                model_provider: Some(model_provider.to_string()),
                ..Default::default()
            };
            let session_meta = serde_json::to_value(peregrine_types::protocol::SessionMetaLine {
                meta: session_meta,
                git: None,
            })?;
            let lines = [
        ...
        let mut app_server = AppServerSession::new(
            peregrine_app_server_client::AppServerClient::InProcess(
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let peregrine_home = temp_dir.path().to_path_buf();
...
"#
        ...
        let trusted_config = ConfigBuilder::default()
            ...
            .peregrine_home(peregrine_home.clone())
    ...
    async fn theme_warning_uses_final_config() -> std::io::Result<()> {
        use crate::agent::render::highlight::validate_theme_name;
