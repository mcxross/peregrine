...
use crate::agent::hooks_rpc::fetch_hooks_list;
use crate::agent::hooks_rpc::write_hook_trust;
use crate::agent::hooks_rpc::write_hook_trusts;
...
impl App {
    ...
    pub(super) fn handle_feedback_thread_event(&mut self, event: FeedbackThreadEvent) {
        match event.result {
            Ok(thread_id) => {
                self.chat_widget
                    .add_to_history(crate::agent::bottom_pane::feedback_success_cell(
                        event.category,
                        event.include_logs,
                        &thread_id,
...
pub(super) async fn send_add_credits_nudge_email(
    request_handle: AppServerRequestHandle,
    credit_type: AddCreditsNudgeCreditType,
) -> Result<peregrine_app_server_protocol::AddCreditsNudgeEmailStatus> {
    let request_id = RequestId::String(format!("add-credits-nudge-{}", Uuid::new_v4()));
    let response: peregrine_app_server_protocol::SendAddCreditsNudgeEmailResponse = request_handle
    ...
    request_handle
        .request_typed(ClientRequest::ConfigBatchWrite {
            ...
            params: ConfigBatchWriteParams {
                edits: vec![peregrine_app_server_protocol::ConfigEdit {
...
pub(super) fn build_feedback_upload_params(
    ...
    FeedbackUploadParams {
        classification: crate::agent::bottom_pane::feedback_classification(category).to_string(),
        reason,
...
pub(super) type McpInventoryMaps = (
    HashMap<String, peregrine_types::mcp::Tool>,
    HashMap<String, Vec<peregrine_types::mcp::Resource>>,
    HashMap<String, Vec<peregrine_types::mcp::ResourceTemplate>>,
    ...
    for status in statuses {
        ...
        auth_statuses.insert(
            ...
            match status.auth_status {
                peregrine_app_server_protocol::McpAuthStatus::Unsupported => {
                    McpAuthStatus::Unsupported
                }
                peregrine_app_server_protocol::McpAuthStatus::NotLoggedIn => {
                    McpAuthStatus::NotLoggedIn
                }
                peregrine_app_server_protocol::McpAuthStatus::BearerToken => {
                    McpAuthStatus::BearerToken
                }
                peregrine_app_server_protocol::McpAuthStatus::OAuth => McpAuthStatus::OAuth,
            },
        );
