...
impl App {
        ...
        match event {
            ...
            AppEvent::ClearUiAndSubmitUserMessage { text } => {
                ...
                self.start_fresh_session_with_summary_hint(
                    ...
                    app_server,
                    Some(ThreadStartSource::Clear),
                    crate::agent::chatwidget::create_initial_user_message(
                        Some(text),
                        Vec::new(),
                        Vec::new(),
                    ),
            ...
            AppEvent::OpenResumePicker => {
                let picker_app_server = match crate::agent::start_app_server_for_picker(
                    &self.config,
                ...
                match crate::agent::resume_picker::run_resume_picker_from_existing_session_with_app_server(
                    tui,
                    &self.config,
                    /*show_all*/ false,
            ...
            AppEvent::ResumeSessionByIdOrName(id_or_name) => {
                match crate::agent::lookup_session_target_with_app_server(app_server, &id_or_name)
                    .await?
                {
                    Some(target_session) => {
                        return self
            ...
            AppEvent::SwitchToWorkbench => {
                ...
                if matches!(
                    ...
                    return Ok(AppRunControl::Exit(ExitReason::SwitchToWorkbench));
                }
                return Ok(shutdown_result);
            }
            ...
            AppEvent::OpenAppLink {
                ...
                self.chat_widget
                    .open_app_link_view(crate::agent::bottom_pane::AppLinkViewParams {
                        app_id,
                        title,
                        description,
                        instructions,
            ...
            AppEvent::OpenProviderPicker => {
                let request_id = RequestId::String(format!("provider-list-{}", Uuid::new_v4()));
                match app_server
                    .request_handle()
                ...
                {
                    Ok(providers) => {
                        self.chat_widget.open_provider_popup(providers);
                    }
                    Err(err) => {
                        tracing::error!(
                            error = %err,
                            "failed to load model providers from app server"
                        );
                        self.chat_widget
            ...
            AppEvent::OpenProviderModelPicker {
                ...
                let request_id =
                    RequestId::String(format!("provider-models-list-{}", Uuid::new_v4()));
                match app_server
                    .request_handle()
                ...
                {
                    Ok(models) => {
                        self.chat_widget.open_provider_model_popup(
                            provider_id,
                            provider_display_name,
                            models,
                        );
                    ...
                    Err(err) => {
                        tracing::error!(
                            error = %err,
                            provider_id,
                            "failed to load provider models from app server"
                        );
                        self.chat_widget
                            .add_error_message(format!("Failed to load provider models: {err}"));
                    }
                }
            }
            ...
            AppEvent::BeginWindowsSandboxElevatedSetup {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    let peregrine_home = self.config.peregrine_home.clone();
                    let tx = self.app_event_tx.clone();

                    // If the elevated setup already ran on this machine, don't prompt for
                    // elevation again - just flip the config to use the elevated path.
                    if crate::agent::legacy_core::windows_sandbox::sandbox_setup_is_complete(
                    ...
                    tokio::task::spawn_blocking(move || {
                        let result = crate::agent::legacy_core::windows_sandbox::run_elevated_setup(
                            &permission_profile,
                        ...
                        let event = match result {
                            ...
                            Err(err) => {
                                ...
                                if let Some((code, message)) =
                                    crate::agent::legacy_core::windows_sandbox::elevated_setup_failure_details(
                                        &err,
                                    )
                                {
                                    code_tag = Some(code);
                                ...
                                session_telemetry.counter(
                                    crate::agent::legacy_core::windows_sandbox::elevated_setup_failure_metric_name(
                                        &err,
                                    ),
                                    /*inc*/ 1,
                                    &tags,
            ...
            AppEvent::BeginWindowsSandboxLegacySetup {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    tokio::task::spawn_blocking(move || {
                        if let Err(err) =
                            crate::agent::legacy_core::windows_sandbox::run_legacy_setup_preflight(
                                &permission_profile,
                                workspace_roots.as_slice(),
                                command_cwd.as_path(),
                                &env_map,
            ...
            AppEvent::BeginWindowsSandboxGrantReadRoot { path } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    tokio::task::spawn_blocking(move || {
                        ...
                        let event = match crate::agent::legacy_core::grant_read_root_non_elevated(
                            &permission_profile,
                            workspace_roots.as_slice(),
                            command_cwd.as_path(),
                            &env_map,
            ...
            AppEvent::EnableWindowsSandboxForAgentMode {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    let edits = crate::agent::config_update::build_windows_sandbox_mode_edits(
                        elevated_enabled,
                    );
                    match crate::agent::config_update::write_config_batch(
                    ...
                    .await
                    {
                        ...
                        Ok(_) => {
                            ...
                            {
                                self.app_event_tx.send(AppEvent::PeregrineOp(
                                ...
                                self.chat_widget.add_plain_history_lines(vec![
                                    ...
                                    Line::from(vec![
                                        "  ".into(),
                                        "Peregrine can now safely edit files and execute commands in your computer"
                                            .dark_gray(),
                                    ]),
                                ]);
                            } else {
            ...
            AppEvent::PersistModelSelection { model, effort } => {
                match crate::agent::config_update::write_config_batch(
                    app_server.request_handle(),
                    crate::agent::config_update::build_model_selection_edits(
                        model.as_str(),
                        effort,
                    ),
                )
            ...
            AppEvent::PersistProviderSelection { provider_id, model } => {
                let request_id = RequestId::String(format!("provider-select-{}", Uuid::new_v4()));
                match app_server
                    .request_handle()
                    .request_typed::<ModelProviderSelectResponse>(
                ...
                {
                    Ok(response) => {
                        let selected_provider = response.selected_provider.clone();
                        self.config.model_provider_id = selected_provider.id.clone();
                        let mut provider = self
                            ...
                            .unwrap_or_else(|| codex_model_provider_info::ModelProviderInfo {
                                name: selected_provider.display_name.clone(),
                                base_url: selected_provider.runtime_base_url.clone(),
                                ..Default::default()
                            });
                        provider.name = selected_provider.display_name.clone();
                        provider.requires_openai_auth = selected_provider.requires_openai_auth;
                        self.config.model_provider = provider;
                        self.chat_widget
                            .apply_model_provider_selection(selected_provider);
                        self.config.model = response.model.clone();
                        if let Some(model) = response.model.as_deref() {
                            self.chat_widget.set_model(model);
                        }

                        let mut message =
                            format!("Provider changed to {}", response.provider.display_name);
                        if let Some(model) = response.model.as_deref() {
                        ...
                        self.chat_widget.add_info_message(message, /*hint*/ None);
                        if let Some(warning) =
                            ChatWidget::provider_setup_warning(&response.provider)
                        {
                            self.chat_widget.add_info_message(
            ...
            AppEvent::PersistPersonalitySelection { personality } => {
                match crate::agent::config_update::write_config_batch(
                    app_server.request_handle(),
                    vec![crate::agent::config_update::replace_config_value(
                        "personality",
            ...
            AppEvent::PersistServiceTierSelection { service_tier } => {
                ...
                self.sync_active_thread_service_tier_to_cached_session()
                    .await;
                let edits = crate::agent::config_update::build_service_tier_selection_edits(
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    if should_check {
                        ...
                        let env_map: std::collections::HashMap<String, String> =
                            std::env::vars().collect();
                        let tx = self.app_event_tx.clone();
                        let logs_base_dir = self.config.peregrine_home.clone();
            ...
            AppEvent::UpdateApprovalsReviewer(policy) => {
                ...
                self.sync_active_thread_permission_settings_to_cached_session()
                    .await;
                if let Err(err) = crate::agent::config_update::write_config_batch(
                    app_server.request_handle(),
                    vec![crate::agent::config_update::replace_config_value(
                        "approvals_reviewer",
            ...
            AppEvent::PersistPlanModeReasoningEffort(effort) => {
                ...
                let edit = if let Some(effort) = effort {
                    crate::agent::config_update::replace_config_value(
                        key_path,
                        serde_json::json!(effort.to_string()),
                    )
                } else {
                    crate::agent::config_update::clear_config_value(key_path)
                };
                if let Err(err) = crate::agent::config_update::write_config_batch(
                    app_server.request_handle(),
                    vec![edit],
            ...
            AppEvent::SetSkillEnabled { path, enabled } => {
                match crate::agent::config_update::write_skill_enabled(
                    app_server.request_handle(),
                    path.clone(),
                    enabled,
                )
            ...
            AppEvent::SetAppEnabled { id, enabled } => {
                let edits = if enabled {
                    vec![
                        crate::agent::config_update::clear_config_value(
                            crate::agent::config_update::app_scoped_key_path(&id, "enabled"),
                        ),
                        crate::agent::config_update::clear_config_value(
                            crate::agent::config_update::app_scoped_key_path(
                                &id,
                                "disabled_reason",
                            ),
                        ),
            ...
            AppEvent::FullScreenApprovalRequest(request) => match request {
                ...
                ApprovalRequest::Permissions {
                    ...
                    if let Some(rule_line) =
                        crate::agent::bottom_pane::format_requested_permissions_rule(&permissions)
                    {
            ...
            AppEvent::StatusLineSetup {
                ...
                let items_edit =
                    crate::agent::legacy_core::config::edit::status_line_items_edit(&ids);
                let colors_edit =
                    crate::agent::legacy_core::config::edit::status_line_use_colors_edit(
                        use_theme_colors,
                    );
            ...
            AppEvent::TerminalTitleSetup { items } => {
                let ids = items.iter().map(ToString::to_string).collect::<Vec<_>>();
                let edit = crate::agent::legacy_core::config::edit::terminal_title_items_edit(&ids);
                let apply_result = ConfigEditsBuilder::for_config(&self.config)
                    .with_edits([edit])
                    .apply()
                    .await;
            ...
            AppEvent::SyntaxThemeSelected { name } => {
                ...
                match apply_result {
                    Ok(()) => {
                        ...
                        if let Some(theme) = crate::agent::render::highlight::resolve_theme_by_name(
                            &name,
                            Some(&self.config.peregrine_home),
                        ) {
                            crate::agent::render::highlight::set_syntax_theme(theme);
                        }
    ...
    async fn apply_keymap_capture(
        ...
        action: String,
        key: String,
        intent: crate::agent::app_event::KeymapEditIntent,
    ) {
        let outcome = match crate::agent::keymap_setup::keymap_with_edit(
        ...
        let (keymap_config, bindings, message) = match outcome {
            crate::agent::keymap_setup::KeymapEditOutcome::Updated {
                keymap_config,
                bindings,
                message,
            } => (*keymap_config, bindings, message),
            crate::agent::keymap_setup::KeymapEditOutcome::Unchanged { message } => {
                self.chat_widget.add_info_message(message, /*hint*/ None);
                return;
            }
        };
        ...
        let runtime_keymap = match RuntimeKeymap::from_config(&keymap_config) {
            ...
            Err(err) => {
                let params = crate::agent::keymap_setup::build_keymap_conflict_params(
        ...
        let edit = crate::agent::legacy_core::config::edit::keymap_bindings_edit(
    ...
    async fn apply_keymap_clear(&mut self, context: String, action: String) {
        let keymap_config = match crate::agent::keymap_setup::keymap_without_custom_binding(
            &self.config.tui_keymap,
            &context,
            &action,
        ) {
        ...
        let edit =
            crate::agent::legacy_core::config::edit::keymap_binding_clear_edit(&context, &action);
        match ConfigEditsBuilder::for_config(&self.config)
            .with_edits([edit])
            .apply()
            .await
