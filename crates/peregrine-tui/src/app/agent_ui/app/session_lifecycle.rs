...
impl App {
            ...
            .await
        {
            Ok(thread) => {
                self.upsert_agent_picker_thread(
                    ...
                    matches!(
                        thread.status,
                        peregrine_app_server_protocol::ThreadStatus::NotLoaded
                    ),
                );
                true
            }
    ...
    pub(super) async fn resume_target_session(
        ...
        let resume_cwd = if self.app_server_target.uses_remote_workspace() {
            ...
            match crate::agent::session_resume::resolve_cwd_for_resume_or_fork(
                tui,
            ...
            .await?
            {
                crate::agent::session_resume::ResolveCwdOutcome::Continue(Some(cwd)) => cwd,
                crate::agent::session_resume::ResolveCwdOutcome::Continue(None) => {
                    current_cwd.clone()
                }
                crate::agent::session_resume::ResolveCwdOutcome::Exit => {
