...
use crate::agent::hooks_rpc::hook_needs_review;
...
use crate::agent::style::accent_style;
...
impl HooksBrowserView {
    ...
    fn event_rows(&self) -> Vec<EventRow> {
        peregrine_types::protocol::HookEventName::iter()
            .map(|event_name| {
    ...
    fn selected_event(&self) -> Option<HookEventName> {
        self.state
            .selected_idx
            .and_then(|idx| peregrine_types::protocol::HookEventName::iter().nth(idx))
            .map(Into::into)
    }
    ...
    fn page_len(&self) -> usize {
        match self.page {
            HooksBrowserPage::Events => peregrine_types::protocol::HookEventName::iter().count(),
...
fn event_description(event_name: HookEventName) -> &'static str {
    match event_name {
        ...
        HookEventName::Stop => "Right before Peregrine ends its turn",
    }
}
