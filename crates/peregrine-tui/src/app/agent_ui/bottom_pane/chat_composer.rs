...
use crate::agent::key_hint::has_ctrl_or_alt;
use crate::agent::line_truncation::truncate_line_with_ellipsis_if_overflow;
use crate::agent::ui_consts::FOOTER_INDENT_COLS;
...
use crate::agent::bottom_pane::paste_burst::FlushResult;
use crate::agent::key_hint::KeyBindingListExt;
use crate::agent::keymap::EditorKeymap;
use crate::agent::keymap::RuntimeKeymap;
use crate::agent::keymap::VimNormalKeymap;
use crate::agent::keymap::primary_binding;
use crate::agent::onboarding::mark_underlined_hyperlink;
use crate::agent::render::Insets;
use crate::agent::render::RectExt;
...
use crate::agent::bottom_pane::textarea::TextArea;
use crate::agent::clipboard_paste::normalize_pasted_path;
use crate::agent::clipboard_paste::pasted_image_format;
use crate::agent::history_cell;
use crate::agent::skills_helpers::skill_display_name;
        ...
        let is_wsl = {
            #[cfg(target_os = "linux")]
            {
                mode == FooterMode::ShortcutOverlay
                    && crate::agent::clipboard_paste::is_probably_wsl()
            }
            #[cfg(not(target_os = "linux"))]
            {
                false
    ...
    fn mention_items(&self) -> Vec<MentionItem> {
            ...
            && let Some(snapshot) = self.connectors_snapshot.as_ref()
        {
            for connector in &snapshot.connectors {
                ...
                let display_name =
                    crate::agent::connector_metadata::connector_display_label(connector);
                let description = Some(Self::connector_brief_description(connector));
                let slug = crate::agent::connector_metadata::connector_mention_slug(connector);
...
mod tests {
    ...
    use crate::agent::bottom_pane::AppEventSender;
    use crate::agent::bottom_pane::ChatComposer;
    use crate::agent::bottom_pane::InputResult;
    use crate::agent::bottom_pane::chat_composer::LARGE_PASTE_CHAR_THRESHOLD;
    use crate::agent::bottom_pane::textarea::TextArea;
    ...
    fn footer_hint_row_is_separated_from_composer() {
        ...
        let composer = ChatComposer::new(
            ...
            "Ask Peregrine to do anything".to_string(),
    ...
    fn set_skill_mentions_refreshes_open_mention_popup() {
        ...
        composer.set_skill_mentions(Some(vec![SkillMetadata {
            name: "peregrine".to_string(),
            description: "Primary personal Peregrine repo skill.".to_string(),
            short_description: None,
            interface: None,
            dependencies: None,
            policy: None,
            path_to_skills_md: skill_path.clone(),
            scope: crate::agent::test_support::skill_scope_user(),
            plugin_id: None,
        }]));
    ...
    fn mention_items_show_plugin_owned_skill_and_app_duplicates() {
        ...
        composer.set_skill_mentions(Some(vec![SkillMetadata {
            ...
            policy: None,
            path_to_skills_md: skill_path.clone(),
            scope: crate::agent::test_support::skill_scope_repo(),
            plugin_id: None,
        }]));
        composer.set_plugin_mentions(Some(vec![PluginCapabilitySummary {
...
程序员 与 Unicode 同行";
    ...
    #[test]
    fn slash_popup_provider_first_for_pr_ui() {
        use ratatui::Terminal;
        use ratatui::backend::TestBackend;
        ...
        // Type "/pr" humanlike so paste-burst doesn't interfere.
        type_chars_humanlike(&mut composer, &['/', 'p', 'r']);
        ...
        // Visual snapshot should show the slash popup with /provider as the first entry.
        insta::assert_snapshot!("slash_popup_provider", terminal.backend());
    }

    #[test]
    fn slash_popup_provider_first_for_pr_logic() {
        ...
        match &composer.popups.active {
            ActivePopup::Command(popup) => match popup.selected_item() {
                Some(CommandItem::Builtin(cmd)) => {
                    assert_eq!(cmd.command(), "provider")
                }
                Some(CommandItem::ServiceTier(command)) => {
                    panic!("expected provider command, got service tier {command:?}")
                }
                None => panic!("no selected command for '/pr'"),
            },
            _ => panic!("slash popup not active after typing '/pr'"),
        }
    }
    ...
    fn slash_tab_completion_wins_over_queueing_while_task_running() {
        ...
        assert_eq!(result, InputResult::None);
        assert_eq!(composer.draft.textarea.text(), "/provider ");
        assert_eq!(
            composer.draft.textarea.cursor(),
            composer.draft.textarea.text().len()
        );
    ...
    fn pasting_filepath_attaches_image() {
        ...
        let tmp_path: PathBuf = tmp.path().join("peregrine_tui_test_paste_image.png");
