...
use crate::agent::bottom_pane::BottomPaneView;
use crate::agent::bottom_pane::CancellationEvent;
use crate::agent::bottom_pane::list_selection_view::ListSelectionView;
use crate::agent::bottom_pane::list_selection_view::SelectionItem;
use crate::agent::bottom_pane::list_selection_view::SelectionViewParams;
use crate::agent::bottom_pane::popup_consts::accept_cancel_hint_line;
use crate::agent::diff_model::FileChange;
use crate::agent::exec_command::strip_bash_lc_and_escape;
use crate::agent::history_cell;
use crate::agent::history_cell::ReviewDecision;
use crate::agent::key_hint;
use crate::agent::key_hint::KeyBinding;
use crate::agent::key_hint::KeyBindingListExt;
use crate::agent::keymap::ApprovalKeymap;
use crate::agent::keymap::ListKeymap;
use crate::agent::keymap::primary_binding;
...
use peregrine_app_server_protocol::FileSystemAccessMode;
use peregrine_app_server_protocol::FileSystemPath;
use peregrine_app_server_protocol::FileSystemSandboxEntry;
use peregrine_app_server_protocol::FileSystemSpecialPath;
use peregrine_app_server_protocol::McpServerElicitationAction;
use peregrine_app_server_protocol::NetworkApprovalContext;
use peregrine_app_server_protocol::NetworkApprovalProtocol;
    ...
    fn handle_permissions_decision(
        ...
        if request.thread_label().is_none() {
            ...
            self.app_event_tx.send(AppEvent::InsertHistoryCell(Box::new(
                crate::agent::history_cell::PlainHistoryCell::new(vec![message.into()]),
            )));
        }
        let thread_id = request.thread_id();
    ...
    available_decisions
        ...
        .filter_map(|decision| match decision {
            ...
            CommandExecutionApprovalDecision::Cancel => Some(ApprovalOption {
                label: "No, and tell Peregrine what to do differently".to_string(),
...
pub(crate) fn format_requested_permissions_rule(
    ...
    let permissions =
        crate::agent::app_server_approval_conversions::granted_permission_profile_from_request(
            permissions.clone(),
        );
...
mod tests {
    ...
    use peregrine_app_server_protocol::ExecPolicyAmendment;
    use peregrine_app_server_protocol::NetworkApprovalProtocol;
    use peregrine_app_server_protocol::NetworkPolicyAmendment;
    ...
    fn render_history_cell_lines(
        cell: &dyn crate::agent::history_cell::HistoryCell,
        width: u16,
    ) -> Vec<String> {
    ...
    fn make_overlay(
        ...
        let keymap = crate::agent::keymap::RuntimeKeymap::defaults();
    ...
    fn configured_list_cancel_aborts_exec_approval() {
        ...
        let mut keymap = crate::agent::keymap::RuntimeKeymap::defaults();
    ...
    fn network_exec_options_use_expected_labels_and_hide_execpolicy_amendment() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "Yes, and allow this host for this conversation".to_string(),
                "Yes, and allow this host in the future".to_string(),
                "No, and tell Peregrine what to do differently".to_string(),
            ]
        );
    }
    ...
    fn exec_history_cell_wraps_with_two_space_indent() {
        ...
        let rendered: Vec<String> = lines
            ...
            .collect();
        let expected = vec![
            "✔ You approved peregrine to run".to_string(),
    ...
    fn network_access_command_history_uses_target_without_structured_context() {
        ...
        assert_eq!(
            ...
            vec![
                "✔ You approved peregrine network access to https://example.com:8443 this time"
                    .to_string(),
            ]
        );
    }
