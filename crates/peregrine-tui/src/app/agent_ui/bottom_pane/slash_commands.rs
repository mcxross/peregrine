...
use crate::agent::slash_command::built_in_slash_commands;
...
pub(crate) fn commands_for_input(
    ...
    for (_, cmd) in builtins_for_input(flags) {
        ...
        if cmd == SlashCommand::Provider && tiers_enabled {
            commands.extend(
...
mod tests {
    ...
    #[test]
    fn all_service_tiers_are_exposed_as_commands_after_provider() {
        let commands = vec![
            ServiceTierCommand {
                id: "priority".to_string(),
                name: "fast".to_string(),
        ...
        let provider_idx = items
            .iter()
            .position(|item| matches!(item, SlashCommandItem::Builtin(SlashCommand::Provider)))
            .expect("provider command should be visible");
        let inserted = items
            .into_iter()
            .skip(provider_idx + 1)
            .take(commands.len())
