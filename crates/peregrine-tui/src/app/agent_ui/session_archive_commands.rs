//! Shared implementation for `peregrine archive` and `peregrine unarchive`.
//!
...
use peregrine_config::ConfigLoadOptions;
...
async fn start_app_server_for_archive_command(
    ...
    let peregrine_home = find_peregrine_home().wrap_err("failed to find Peregrine home")?;
    ...
    let default_daemon = if explicit_remote_endpoint.is_none() && reuse_implicit_local_daemon {
        super::maybe_probe_default_daemon_socket(peregrine_home.as_path()).await
    } else {
        None
    };
    ...
    let model = cli.model.clone().or_else(|| {
        model_provider
            .as_deref()
            .and_then(super::get_default_model_for_oss_provider)
