...
use crate::agent::app_event::FeedbackCategory;
...
use crate::agent::app_server_session::TurnPermissionsOverride;
use crate::agent::app_server_session::app_server_rate_limit_snapshots;
use crate::agent::bottom_pane::AppLinkViewParams;
...
use crate::agent::chatwidget::ChatWidget;
use crate::agent::chatwidget::ExternalEditorState;
use crate::agent::chatwidget::ReplayKind;
use crate::agent::chatwidget::ThreadInputState;
use crate::agent::cwd_prompt::CwdPromptAction;
use crate::agent::diff_render::DiffSummary;
use crate::agent::exec_command::split_command_string;
use crate::agent::exec_command::strip_bash_lc_and_escape;
use crate::agent::external_agent_config_migration_startup::ExternalAgentConfigMigrationStartupOutcome;
use crate::agent::external_agent_config_migration_startup::handle_external_agent_config_migration_prompt_if_needed;
...
use crate::agent::history_cell::UpdateAvailableHistoryCell;
use crate::agent::hooks_rpc::HookTrustUpdate;
...
use crate::agent::legacy_core::config::edit::ConfigEditsBuilder;
#[cfg(target_os = "windows")]
use crate::agent::legacy_core::windows_sandbox::WindowsSandboxLevelExt;
use crate::agent::model_catalog::ModelCatalog;
use crate::agent::model_migration::ModelMigrationOutcome;
use crate::agent::model_migration::migration_copy_for_models;
use crate::agent::model_migration::run_model_migration_prompt;
use crate::agent::multi_agents::agent_picker_status_dot_spans;
use crate::agent::multi_agents::format_agent_picker_item_name;
use crate::agent::multi_agents::next_agent_shortcut_matches;
use crate::agent::multi_agents::previous_agent_shortcut_matches;
use crate::agent::pager_overlay::Overlay;
use crate::agent::render::highlight::highlight_bash_to_lines;
use crate::agent::render::renderable::Renderable;
use crate::agent::resume_picker::SessionSelection;
use crate::agent::resume_picker::SessionTarget;
...
#[cfg(test)]
use crate::agent::test_support::test_path_display;
use crate::agent::token_usage::TokenUsage;
use crate::agent::transcript_reflow::TranscriptReflowState;
use crate::agent::tui;
use crate::agent::tui::TuiEvent;
use crate::agent::update_action::UpdateAction;
use crate::agent::version::CODEX_CLI_VERSION;
use crate::agent::workspace_command::AppServerWorkspaceCommandRunner;
...
use peregrine_app_server_protocol::SandboxMode as AppServerSandboxMode;
...
use peregrine_app_server_protocol::SkillErrorInfo;
...
#[cfg(target_os = "windows")]
use peregrine_types::permissions::FileSystemSandboxKind;
        ...
        ServerNotification::ItemCompleted(notification) => match &notification.item {
            ThreadItem::CollabAgentToolCall { agents_states, .. } => {
                agents_states.get(receiver_thread_id).is_some_and(|state| {
                    matches!(
                        &state.status,
                        peregrine_app_server_protocol::CollabAgentStatus::NotFound
                    )
                })
            }
            _ => false,
...
fn default_exec_approval_decisions(
    network_approval_context: Option<&peregrine_app_server_protocol::NetworkApprovalContext>,
    proposed_execpolicy_amendment: Option<&peregrine_app_server_protocol::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<
        &[peregrine_app_server_protocol::NetworkPolicyAmendment],
    >,
    additional_permissions: Option<&peregrine_app_server_protocol::AdditionalPermissionProfile>,
) -> Vec<peregrine_app_server_protocol::CommandExecutionApprovalDecision> {
    use peregrine_app_server_protocol::CommandExecutionApprovalDecision;
    use peregrine_app_server_protocol::NetworkPolicyRuleAction;
...
struct InitialHistoryReplayBuffer {
    retained_lines: VecDeque<crate::agent::terminal_hyperlinks::HyperlinkLine>,
...
pub(crate) struct App {
    ...
    pub(crate) deferred_history_lines: Vec<crate::agent::terminal_hyperlinks::HyperlinkLine>,
    ...
    pub(crate) backtrack: crate::agent::app_backtrack::BacktrackState,
...
struct RuntimePermissionProfileOverride {
    ...
    network: Option<crate::agent::legacy_core::config::NetworkProxySpec>,
}
...
fn active_turn_not_steerable_turn_error(error: &TypedRequestError) -> Option<AppServerTurnError> {
    ...
    matches!(
        turn_error.peregrine_error_info,
        Some(AppServerPeregrineErrorInfo::ActiveTurnNotSteerable { .. })
    )
...
fn spawn_startup_thread_start(
    ...
    tokio::spawn(async move {
        let result = crate::agent::app_server_session::start_thread_with_request_handle(
            request_handle,
            config,
            thread_params_mode,
            remote_cwd_override,
...
fn archived_session_guidance(err: &color_eyre::eyre::Report) -> Option<String> {
    ...
    if !message.contains(" is archived. Run `peregrine unarchive ") {
        return None;
    }
    let message = message
        .split_once(" (code ")
...
impl App {
    pub fn chatwidget_init_for_forked_or_resumed_thread(
        &self,
        tui: &mut tui::Tui,
        cfg: crate::agent::legacy_core::config::Config,
        initial_user_message: Option<crate::agent::chatwidget::UserMessage>,
    ) -> crate::agent::chatwidget::ChatWidgetInit {
        crate::agent::chatwidget::ChatWidgetInit {
            config: cfg,
    ...
    pub async fn run(
        ...
        let remote_connection =
            crate::agent::status::remote_connection::remote_connection_status_value(
                &app_server_target,
        ...
        let runtime_model_provider_base_url = config.model_provider.base_url.clone();
            ...
            SessionSelection::StartFresh | SessionSelection::Exit => {
                ...
                let startup_tooltip_override =
                    prepare_startup_tooltip_override(&mut config, &available_models, is_first_run)
                        .await;
                let init = crate::agent::chatwidget::ChatWidgetInit {
                    config: config.clone(),
                    frame_requester: tui.frame_requester(),
                    app_event_tx: app_event_tx.clone(),
                    workspace_command_runner: Some(workspace_command_runner.clone()),
                    initial_user_message: crate::agent::chatwidget::create_initial_user_message(
...
See the Peregrine keymap documentation for supported actions and examples."
            )
        })?;
        #[cfg(not(debug_assertions))]
        let upgrade_version = crate::agent::updates::get_upgrade_version(&config);
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if should_check {
                ...
                let logs_base_dir = app.config.peregrine_home.clone();
            ...
            let control = app
                .handle_event(
                    ...
                    AppEvent::InsertHistoryCell(Box::new(UpdateAvailableHistoryCell::new(
                        latest_version,
                        crate::agent::update_action::get_update_action(),
                    ))),
                )
                .await?;
            match control {
