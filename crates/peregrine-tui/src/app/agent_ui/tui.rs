...
use crate::agent::custom_terminal;
use crate::agent::custom_terminal::Terminal as CustomTerminal;
use crate::agent::insert_history::HistoryLineWrapPolicy;
use crate::agent::insert_history::InsertHistoryMode;
use crate::agent::notifications::DesktopNotificationBackend;
use crate::agent::notifications::detect_backend;
use crate::agent::terminal_hyperlinks::HyperlinkLine;
use crate::agent::terminal_hyperlinks::plain_hyperlink_lines;
use crate::agent::tui::event_stream::EventBroker;
use crate::agent::tui::event_stream::TuiEventStream;
#[cfg(unix)]
use crate::agent::tui::job_control::SuspendContext;
...
/// Restore the terminal after Peregrine is exiting.
///
...
pub(crate) fn init() -> Result<InitializedTerminal> {
    ...
    let mut restore_guard = InitRestoreGuard::new();
    set_modes()?;
    ...
    #[cfg(unix)]
    let startup_probe = {
        use crate::agent::terminal_probe::StartupKeyboardEnhancementProbe;
        ...
        match crate::agent::terminal_probe::startup(
            crate::agent::terminal_probe::DEFAULT_TIMEOUT,
            keyboard_probe,
        ) {
            Ok(probe) => {
                tracing::info!(
            ...
            Err(err) => {
                ...
                crate::agent::terminal_probe::StartupProbe {
    ...
    #[cfg(unix)]
    crate::agent::terminal_palette::set_default_colors_from_startup_probe(
...
struct InitRestoreGuard {
    active: bool,
}

impl InitRestoreGuard {
    fn new() -> Self {
        Self { active: true }
    }
...
impl Drop for InitRestoreGuard {
    fn drop(&mut self) {
        if self.active {
...
pub struct Tui {
    ...
    ambient_pet_image_state: crate::agent::pets::PetImageRenderState,
    pet_picker_preview_image_state: crate::agent::pets::PetImageRenderState,
...
impl Tui {
    pub(crate) fn new(
        ...
        let _ = crate::agent::terminal_palette::default_colors();
        ...
        Self {
            ...
            ambient_pet_image_state: crate::agent::pets::PetImageRenderState::default(),
            pet_picker_preview_image_state: crate::agent::pets::PetImageRenderState::default(),
    ...
    /// terminal modes and stderr (optionally keeping raw mode enabled), then re-applies Peregrine TUI
    ...
    fn flush_pending_history_lines(
        ...
        for batch in pending_history_lines.iter() {
            ...
            crate::agent::insert_history::insert_history_hyperlink_lines_with_mode_and_wrap_policy(
                terminal,
                batch.lines.clone(),
                mode,
                batch.wrap_policy,
    ...
    pub fn draw_ambient_pet_image(
        &mut self,
        request: Option<crate::agent::pets::AmbientPetDraw>,
    ) -> std::result::Result<(), crate::agent::pets::PetImageRenderError> {
        if let Err(err) = ensure_virtual_terminal_processing() {
            return Err(crate::agent::pets::PetImageRenderError::Terminal(err));
        }

        let terminal = &mut self.terminal;
        let state = &mut self.ambient_pet_image_state;
        stdout().sync_update(|_| {
            match crate::agent::pets::render_ambient_pet_image(
                terminal.backend_mut(),
                state,
                request,
            ) {
                Ok(()) => Ok(Ok(())),
                Err(crate::agent::pets::PetImageRenderError::Terminal(err)) => Err(err),
                Err(err @ crate::agent::pets::PetImageRenderError::Asset(_)) => Ok(Err(err)),
            }
        })??
    }
    ...
    pub fn draw_pet_picker_preview_image(
        ...
        stdout().sync_update(|_| {
            match crate::agent::pets::render_pet_picker_preview_image(
                terminal.backend_mut(),
                state,
                request,
            ) {
    ...
    pub fn clear_ambient_pet_image(
        ...
        crate::agent::pets::render_ambient_pet_image(
