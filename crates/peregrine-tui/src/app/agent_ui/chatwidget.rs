//! The main Peregrine TUI chat surface.
//!
...
use crate::agent::app::app_server_requests::ResolvedAppServerRequest;
use crate::agent::app_command::AppCommand;
use crate::agent::app_event::HistoryLookupResponse;
use crate::agent::app_event::RealtimeAudioDeviceKind;
use crate::agent::app_server_approval_conversions::file_update_changes_to_display;
use crate::agent::approval_events::ApplyPatchApprovalRequestEvent;
use crate::agent::approval_events::ExecApprovalRequestEvent;
#[cfg(not(target_os = "linux"))]
use crate::agent::audio_device::list_realtime_audio_device_names;
use crate::agent::bottom_pane::StatusLineItem;
use crate::agent::bottom_pane::StatusLineSetupView;
use crate::agent::bottom_pane::StatusSurfacePreviewData;
use crate::agent::bottom_pane::StatusSurfacePreviewItem;
use crate::agent::bottom_pane::TerminalTitleItem;
use crate::agent::bottom_pane::TerminalTitleSetupView;
...
use crate::agent::mention_codec::LinkedMention;
use crate::agent::mention_codec::encode_history_mentions;
use crate::agent::model_catalog::ModelCatalog;
use crate::agent::multi_agents;
use crate::agent::multi_agents::AgentMetadata;
use crate::agent::session_state::SessionNetworkProxyRuntime;
use crate::agent::session_state::ThreadSessionState;
use crate::agent::status::RateLimitWindowDisplay;
use crate::agent::status::StatusAccountDisplay;
use crate::agent::status::StatusHistoryHandle;
use crate::agent::status::format_directory_display;
use crate::agent::status::format_tokens_compact;
use crate::agent::status::rate_limit_snapshot_display_for_limit;
use crate::agent::terminal_hyperlinks::HyperlinkLine;
use crate::agent::terminal_title::SetTerminalTitleResult;
use crate::agent::terminal_title::clear_terminal_title;
use crate::agent::terminal_title::set_terminal_title;
use crate::agent::text_formatting::proper_join;
use crate::agent::token_usage::TokenUsage;
use crate::agent::token_usage::TokenUsageInfo;
...
use peregrine_app_server_protocol::CollabAgentTool;
use peregrine_app_server_protocol::CollabAgentToolCallStatus;
use peregrine_app_server_protocol::CommandExecutionRequestApprovalParams;
use peregrine_app_server_protocol::CommandExecutionSource as ExecCommandSource;
use peregrine_app_server_protocol::ConfigLayerSource;
use peregrine_app_server_protocol::CreditsSnapshot;
use peregrine_app_server_protocol::ErrorNotification;
use peregrine_app_server_protocol::FileChangeRequestApprovalParams;
use peregrine_app_server_protocol::GuardianApprovalReviewAction;
use peregrine_app_server_protocol::ItemCompletedNotification;
use peregrine_app_server_protocol::ItemStartedNotification;
use peregrine_app_server_protocol::McpServerElicitationRequest;
...
use peregrine_app_server_protocol::ModelVerification as AppServerModelVerification;
use peregrine_app_server_protocol::PeregrineErrorInfo as AppServerPeregrineErrorInfo;
use peregrine_app_server_protocol::RateLimitReachedType;
use peregrine_app_server_protocol::RateLimitSnapshot;
use peregrine_app_server_protocol::RequestId as AppServerRequestId;
use peregrine_app_server_protocol::ReviewTarget;
use peregrine_app_server_protocol::ServerNotification;
use peregrine_app_server_protocol::ServerRequest;
use peregrine_app_server_protocol::SkillMetadata as ProtocolSkillMetadata;
use peregrine_app_server_protocol::SkillsListResponse;
use peregrine_app_server_protocol::ThreadGoal as AppThreadGoal;
use peregrine_app_server_protocol::ThreadGoalStatus as AppThreadGoalStatus;
...
use peregrine_types::approvals::GuardianAssessmentAction;
use peregrine_types::approvals::GuardianAssessmentDecisionSource;
use peregrine_types::approvals::GuardianAssessmentEvent;
use peregrine_types::approvals::GuardianAssessmentStatus;
...
use peregrine_types::config_types::Settings;
#[cfg(any(target_os = "windows", test))]
use peregrine_types::config_types::WindowsSandboxLevel;
use peregrine_types::items::AgentMessageContent;
use peregrine_types::items::AgentMessageItem;
use peregrine_types::models::MessagePhase;
use peregrine_types::plan_tool::PlanItemArg as UpdatePlanItemArg;
use peregrine_types::plan_tool::StepStatus as UpdatePlanItemStatus;
use peregrine_types::request_permissions::RequestPermissionsEvent;
use peregrine_types::user_input::ByteRange;
...
use crate::agent::app_event::AppEvent;
use crate::agent::app_event::ExitMode;
use crate::agent::app_event::PermissionProfileSelection;
use crate::agent::app_event::RateLimitRefreshOrigin;
#[cfg(any(target_os = "windows", test))]
use crate::agent::app_event::WindowsSandboxEnableMode;
use crate::agent::app_event_sender::AppEventSender;
use crate::agent::auto_review_denials;
use crate::agent::auto_review_denials::RecentAutoReviewDenials;
use crate::agent::bottom_pane::ApprovalRequest;
use crate::agent::bottom_pane::BottomPane;
use crate::agent::bottom_pane::BottomPaneParams;
use crate::agent::bottom_pane::CancellationEvent;
use crate::agent::bottom_pane::CollaborationModeIndicator;
use crate::agent::bottom_pane::ColumnWidthMode;
use crate::agent::bottom_pane::DOUBLE_PRESS_QUIT_SHORTCUT_ENABLED;
use crate::agent::bottom_pane::ExperimentalFeatureItem;
use crate::agent::bottom_pane::ExperimentalFeaturesView;
use crate::agent::bottom_pane::GoalStatusIndicator;
use crate::agent::bottom_pane::HistoryEntry;
use crate::agent::bottom_pane::InputResult;
use crate::agent::bottom_pane::LocalImageAttachment;
use crate::agent::bottom_pane::McpServerElicitationFormRequest;
use crate::agent::bottom_pane::MemoriesSettingsView;
use crate::agent::bottom_pane::MentionBinding;
use crate::agent::bottom_pane::QUIT_SHORTCUT_TIMEOUT;
use crate::agent::bottom_pane::QueuedInputAction;
use crate::agent::bottom_pane::SelectionAction;
use crate::agent::bottom_pane::SelectionItem;
use crate::agent::bottom_pane::SelectionViewParams;
use crate::agent::bottom_pane::custom_prompt_view::CustomPromptView;
use crate::agent::bottom_pane::popup_consts::standard_popup_hint_line;
use crate::agent::clipboard_paste::paste_image_to_temp_png;
use crate::agent::collaboration_modes;
use crate::agent::diff_render::display_path_for;
use crate::agent::exec_cell::CommandOutput;
use crate::agent::exec_cell::ExecCell;
use crate::agent::exec_cell::new_active_exec_command;
use crate::agent::exec_command::split_command_string;
use crate::agent::exec_command::strip_bash_lc_and_escape;
use crate::agent::get_git_diff::get_git_diff;
use crate::agent::history_cell;
use crate::agent::history_cell::HistoryCell;
use crate::agent::history_cell::HistoryRenderMode;
use crate::agent::history_cell::HookCell;
use crate::agent::history_cell::McpInvocation;
use crate::agent::history_cell::McpToolCallCell;
use crate::agent::history_cell::PlainHistoryCell;
use crate::agent::history_cell::WebSearchCell;
use crate::agent::key_hint;
use crate::agent::key_hint::KeyBinding;
use crate::agent::key_hint::KeyBindingListExt;
use crate::agent::keymap::ChatKeymap;
use crate::agent::keymap::RuntimeKeymap;
use crate::agent::render::Insets;
use crate::agent::render::renderable::ColumnRenderable;
use crate::agent::render::renderable::FlexRenderable;
use crate::agent::render::renderable::Renderable;
use crate::agent::render::renderable::RenderableExt;
use crate::agent::render::renderable::RenderableItem;
use crate::agent::slash_command::SlashCommand;
use crate::agent::status::RateLimitSnapshotDisplay;
use crate::agent::status::remote_connection::RemoteConnectionStatus;
use crate::agent::status_indicator_widget::STATUS_DETAILS_DEFAULT_MAX_LINES;
use crate::agent::status_indicator_widget::StatusDetailsCapitalization;
...
pub(crate) use crate::agent::branch_summary::StatusLineGitSummary;
use crate::agent::streaming::chunking::AdaptiveChunkingPolicy;
use crate::agent::streaming::commit_tick::CommitTickScope;
use crate::agent::streaming::commit_tick::run_commit_tick;
use crate::agent::streaming::controller::PlanStreamController;
use crate::agent::streaming::controller::StreamController;
use crate::agent::workspace_command::WorkspaceCommandRunner;
...
pub(crate) struct ChatWidget {
    ...
    codex_op_target: PeregrineOpTarget,
    ...
    clipboard_lease: Option<crate::agent::clipboard_copy::ClipboardLease>,
    ...
    ambient_pet: Option<crate::agent::pets::AmbientPet>,
    pet_picker_preview_state: crate::agent::pets::PetPickerPreviewState,
    pet_picker_preview_pet: Option<crate::agent::pets::AmbientPet>,
    pet_picker_preview_request_id: u64,
    pet_picker_preview_image_visible: std::cell::Cell<bool>,
    pet_selection_load_request_id: u64,
    #[cfg(test)]
    pet_image_support_override: Option<crate::agent::pets::PetImageSupport>,
    ...
    // Feedback sink for app-server feedback submissions.
...
fn request_permissions_from_params(
    params: peregrine_app_server_protocol::PermissionsRequestApprovalParams,
...
impl ChatWidget {
    ...
    pub(crate) fn open_feedback_note(
        &mut self,
        category: crate::agent::app_event::FeedbackCategory,
        include_logs: bool,
    ) {
    ...
    fn show_feedback_note(
        ...
        let view = crate::agent::bottom_pane::FeedbackNoteView::new(
            category,
    ...
    pub(crate) fn open_app_link_view(
        &mut self,
        params: crate::agent::bottom_pane::AppLinkViewParams,
    ) {
        let view = crate::agent::bottom_pane::AppLinkView::new_with_keymap(
            params,
            self.app_event_tx.clone(),
    ...
    pub(crate) fn open_feedback_consent(
        ...
        let include_windows_sandbox_log =
            codex_windows_sandbox::current_log_file_path_for_peregrine_home(
                &self.config.peregrine_home,
            )
            .is_file();
        #[cfg(not(target_os = "windows"))]
        let include_windows_sandbox_log = false;
        let params = crate::agent::bottom_pane::feedback_upload_consent_params(
            self.app_event_tx.clone(),
            category,
                ...
                .filter_map(|item| match item {
                    ...
                    UserInput::Mention { name, path } => {
                        ...
                        let mention = if let Some(plugin_id) = plugin_id {
                            ...
                            crate::agent::connector_metadata::connector_mention_slug_from_name(name)
                        } else {
                            name.clone()
                        };
    ...
    pub(crate) fn add_debug_config_output(&mut self) {
        self.add_to_history(crate::agent::debug_config::new_debug_config_output(
            &self.config,
    ...
    fn current_stream_width(&self, reserved_cols: usize) -> Option<usize> {
        self.last_rendered_width.get().and_then(|width| {
            if width == 0 {
                ...
                Some(crate::agent::width::usable_content_width(width, reserved_cols).unwrap_or(1))
            }
        })
    }
        ...
        T: Into<AppCommand>,
    {
        ...
        match &self.codex_op_target {
            PeregrineOpTarget::Direct(codex_op_tx) => {
                crate::agent::session_log::log_outbound_op(&op);
                if let Err(e) = codex_op_tx.send(op) {
                    tracing::error!("failed to submit op: {e}");
                    return false;
                }
            }
            PeregrineOpTarget::AppEvent => {
                self.app_event_tx.send(AppEvent::PeregrineOp(op));
            }
        }
        true
    }
    ...
    pub(crate) fn active_cell_transcript_lines(&self, width: u16) -> Option<Vec<Line<'static>>> {
        self.active_cell_transcript_hyperlink_lines(width)
            .map(crate::agent::terminal_hyperlinks::visible_lines)
    }
