...
/// has a sensible starting point without implying that Peregrine is already the
...
pub(crate) fn build_pet_picker_params(
    ...
    let mut entries = available_pet_entries(peregrine_home);
    ...
    let preview_pet_ids = entries
        ...
        .collect::<Vec<_>>();
    let on_selection_changed: crate::agent::bottom_pane::OnSelectionChangedCallback =
        Some(Box::new(
...
fn available_pet_entries(peregrine_home: &Path) -> Vec<PetPickerEntry> {
    ...
    entries.extend(custom_pet_entries(peregrine_home));
    entries
}

fn custom_pet_entries(peregrine_home: &Path) -> Vec<PetPickerEntry> {
    let mut entries_by_selector = HashMap::new();
    for (directory_name, manifest_file) in [("avatars", "avatar.json"), ("pets", "pet.json")] {
        let Ok(children) = fs::read_dir(peregrine_home.join(directory_name)) else {
            continue;
        };
        for child in children.flatten() {
            ...
            let Ok(pet) = Pet::load_with_peregrine_home(
                &selector,
                /*peregrine_home*/ Some(peregrine_home),
            ) else {
                continue;
            };
...
mod tests {
    ...
    fn picker_lists_app_bundled_and_custom_pets() {
        ...
        write_pet(peregrine_home.path(), "chefito", "Chefito");
    ...
    fn picker_preselects_codex_without_marking_it_current_when_no_pet_is_configured() {
        ...
        assert_eq!(params.initial_selected_idx, Some(2));
        assert_eq!(params.items[2].name, "Peregrine");
    ...
    fn picker_imports_legacy_avatar_manifests() {
        let peregrine_home = tempfile::tempdir().unwrap();
        write_legacy_avatar(peregrine_home.path(), "legacy", "Legacy");
