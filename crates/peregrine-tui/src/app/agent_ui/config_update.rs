    ...
    let service_tier_edit = service_tier.map_or_else(
        ...
        |service_tier| {
            let config_value = if service_tier == SERVICE_TIER_DEFAULT_REQUEST_VALUE {
                ...
                match peregrine_types::config_types::ServiceTier::from_request_value(service_tier) {
                    Some(peregrine_types::config_types::ServiceTier::Fast) => "fast",
                    Some(peregrine_types::config_types::ServiceTier::Flex) => "flex",
                    None => service_tier,
                }
            };
