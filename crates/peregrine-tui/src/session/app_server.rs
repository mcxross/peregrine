...
use crate::agent::bottom_pane::FeedbackAudience;
use crate::agent::legacy_core::config::Config;
use crate::agent::permission_compat::legacy_compatible_permission_profile;
use crate::agent::service_tier_resolution;
use crate::agent::session_state::MessageHistoryMetadata;
use crate::agent::session_state::ThreadSessionState;
use crate::agent::status::StatusAccountDisplay;
use crate::agent::status::plan_type_display_name;
use codex_otel::TelemetryAuthMode;
use codex_utils_absolute_path::AbsolutePathBuf;
use color_eyre::eyre::ContextCompat;
use color_eyre::eyre::Result;
use color_eyre::eyre::WrapErr;
use peregrine_app_server_client::AppServerClient;
...
use peregrine_app_server_protocol::ExternalAgentConfigDetectParams;
use peregrine_app_server_protocol::ExternalAgentConfigDetectResponse;
use peregrine_app_server_protocol::ExternalAgentConfigImportParams;
use peregrine_app_server_protocol::ExternalAgentConfigImportResponse;
use peregrine_app_server_protocol::ExternalAgentConfigMigrationItem;
...
use peregrine_app_server_protocol::Model as ApiModel;
...
use peregrine_app_server_protocol::ReviewDelivery;
...
use peregrine_app_server_protocol::ThreadMemoryMode;
...
use peregrine_app_server_protocol::ThreadRealtimeAudioChunk;
...
use peregrine_types::openai_models::ModelAvailabilityNux;
use peregrine_types::openai_models::ModelPreset;
use peregrine_types::openai_models::ModelServiceTier;
use peregrine_types::openai_models::ModelUpgrade;
...
pub(crate) struct AppServerBootstrap {
    ...
    pub(crate) plan_type: Option<peregrine_types::account::PlanType>,
...
impl AppServerSession {
    ...
    pub(crate) async fn turn_start(
        ...
        approvals_reviewer: peregrine_types::config_types::ApprovalsReviewer,
        permissions_override: TurnPermissionsOverride,
        workspace_roots: &[AbsolutePathBuf],
        model: String,
        effort: Option<peregrine_types::openai_models::ReasoningEffort>,
        summary: Option<peregrine_types::config_types::ReasoningSummary>,
        service_tier: Option<Option<String>>,
        collaboration_mode: Option<peregrine_types::config_types::CollaborationMode>,
        personality: Option<peregrine_types::config_types::Personality>,
...
fn approvals_reviewer_override_from_config(
    config: &Config,
) -> Option<peregrine_app_server_protocol::ApprovalsReviewer> {
...
fn sandbox_mode_from_permission_profile(
    ...
    cwd: &std::path::Path,
) -> Option<peregrine_app_server_protocol::SandboxMode> {
    match permission_profile {
        PermissionProfile::Disabled => {
            Some(peregrine_app_server_protocol::SandboxMode::DangerFullAccess)
        }
        ...
        PermissionProfile::Managed { .. } => {
            ...
            if file_system_policy.has_full_disk_write_access() {
                permission_profile
                    ...
                    .is_enabled()
                    .then_some(peregrine_app_server_protocol::SandboxMode::DangerFullAccess)
            } else if file_system_policy.can_write_path_with_cwd(cwd, cwd) {
                Some(peregrine_app_server_protocol::SandboxMode::WorkspaceWrite)
            } else {
                Some(peregrine_app_server_protocol::SandboxMode::ReadOnly)
            }
        }
    }
}
...
fn turn_permissions_overrides(
    ...
    Option<peregrine_app_server_protocol::SandboxPolicy>,
    Option<String>,
) {
...
fn display_permission_profile_from_thread_response(
    sandbox: &peregrine_app_server_protocol::SandboxPolicy,
    cwd: &std::path::Path,
    config: &Config,
    thread_params_mode: ThreadParamsMode,
) -> PermissionProfile {
...
async fn thread_session_state_from_thread_response(
    ...
    reasoning_effort: Option<peregrine_types::openai_models::ReasoningEffort>,
    config: &Config,
) -> Result<ThreadSessionState, String> {
    ...
    let history_config = crate::agent::message_history::HistoryConfig::new(
        config.peregrine_home.clone(),
        &config.history,
    );
    let (log_id, entry_count) =
        crate::agent::message_history::history_metadata(&history_config).await;
...
mod tests {
    use super::*;
    use crate::agent::legacy_core::config::ConfigBuilder;
    use crate::agent::legacy_core::config::ConfigOverrides;
    ...
    async fn build_config(temp_dir: &TempDir) -> Config {
        ConfigBuilder::default()
            .peregrine_home(temp_dir.path().to_path_buf())
            .build()
            .await
    ...
    fn rate_limit_snapshot(limit_id: &str) -> RateLimitSnapshot {
        RateLimitSnapshot {
            ...
            limit_name: None,
            primary: Some(peregrine_app_server_protocol::RateLimitWindow {
                used_percent: 0,
    ...
    fn app_server_rate_limit_snapshots_deduplicates_top_level_limit_from_map() {
        let response = GetAccountRateLimitsResponse {
            rate_limits: rate_limit_snapshot("peregrine"),
            rate_limits_by_limit_id: Some(HashMap::from([
                ("peregrine".to_string(), rate_limit_snapshot("peregrine")),
    ...
    fn legacy_turn_permissions_project_to_sandbox_when_explicitly_overridden() {
        ...
        assert_eq!(
            sandbox_policy,
            Some(peregrine_app_server_protocol::SandboxPolicy::ReadOnly {
    ...
    async fn resume_response_restores_turns_from_thread_items() {
        ...
        let response = ThreadResumeResponse {
            thread: peregrine_app_server_protocol::Thread {
                ...
                source: peregrine_app_server_protocol::SessionSource::Cli,
                thread_source: None,
                agent_nickname: None,
                agent_role: None,
                git_info: None,
                ...
                turns: vec![Turn {
                    id: "turn-1".to_string(),
                    items_view: peregrine_app_server_protocol::TurnItemsView::Full,
                    items: vec![
                        peregrine_app_server_protocol::ThreadItem::UserMessage {
                            id: "user-1".to_string(),
                            client_id: None,
                            content: vec![peregrine_app_server_protocol::UserInput::Text {
                                text: "hello from history".to_string(),
                                text_elements: Vec::new(),
                            }],
                        },
                        peregrine_app_server_protocol::ThreadItem::AgentMessage {
            ...
            approval_policy: peregrine_app_server_protocol::AskForApproval::Never,
            approvals_reviewer: peregrine_app_server_protocol::ApprovalsReviewer::User,
        ...
        let embedded_config = ConfigBuilder::default()
            .peregrine_home(temp_dir.path().join("embedded-codex-home"))
            .harness_overrides(ConfigOverrides {
                default_permissions: Some(BUILT_IN_PERMISSION_PROFILE_WORKSPACE.to_string()),
                ..ConfigOverrides::default()
            })
    ...
    async fn embedded_thread_response_uses_local_config_profile() {
        ...
        assert_eq!(
            display_permission_profile_from_thread_response(
                &peregrine_app_server_protocol::SandboxPolicy::DangerFullAccess,
                cwd.as_path(),
                &config,
                ThreadParamsMode::Embedded,
            ),
    ...
    async fn session_configured_populates_history_metadata() {
        ...
        crate::agent::message_history::append_entry("older", &thread_id, &history_config)
            .await
            .expect("history append should succeed");
        crate::agent::message_history::append_entry("newer", &thread_id, &history_config)
            .await
            .expect("history append should succeed");
        ...
        let session = thread_session_state_from_thread_response(
            ...
            peregrine_types::config_types::ApprovalsReviewer::User,
            PermissionProfile::read_only(),
            /*active_permission_profile*/ None,
            test_path_buf("/tmp/project").abs(),
            Vec::new(),
    ...
    fn status_account_display_from_auth_mode_uses_remapped_plan_labels() {
        let business = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(peregrine_types::account::PlanType::EnterpriseCbpUsageBased),
        );
        assert!(matches!(
            business,
            Some(StatusAccountDisplay::ChatGpt {
        ...
        let team = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(peregrine_types::account::PlanType::SelfServeBusinessUsageBased),
        );
        assert!(matches!(
            team,
            Some(StatusAccountDisplay::ChatGpt {
