...
use peregrine_core::content_items_to_text;
...
pub(crate) struct SpawnedConsolidationAgent {
    ...
    pub(crate) thread: Arc<PeregrineThread>,
}
...
impl MemoryStartupContext {
    pub(crate) fn new(
        ...
        let auth_mode = auth.map(|auth| {
            if matches!(auth, PeregrineAuth::ApiKey(_)) {
                TelemetryAuthMode::ApiKey
            } else {
                TelemetryAuthMode::Chatgpt
            }
        });
        let account_id = auth.and_then(PeregrineAuth::get_account_id);
        let account_email = auth.and_then(PeregrineAuth::get_account_email);
    ...
    pub(crate) async fn stage_one_request_context(
        ...
        let turn_metadata_header =
            peregrine_core::build_turn_metadata_header(&config.cwd, /*sandbox*/ None).await;
        ...
        while let Some(message) = stream.next().await.transpose()? {
            match message {
                ...
                ResponseEvent::OutputItemDone(item) => {
                    if result.is_empty()
                        && let peregrine_types::models::ResponseItem::Message { content, .. } = item
