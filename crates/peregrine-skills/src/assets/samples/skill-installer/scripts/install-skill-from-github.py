#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $PEREGRINE_HOME/skills."""
...
def _peregrine_home() -> str:
    return os.environ.get("PEREGRINE_HOME", os.path.expanduser("~/.peregrine"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "peregrine")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "peregrine-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_peregrine_home(), "skills")
