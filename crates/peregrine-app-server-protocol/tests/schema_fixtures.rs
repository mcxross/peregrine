use anyhow::Context;
use anyhow::Result;
use peregrine_app_server_protocol::generate_json_with_experimental;
use peregrine_app_server_protocol::generate_typescript_schema_fixture_subtree_for_tests;
use peregrine_app_server_protocol::read_schema_fixture_subtree;
...
fn schema_root() -> Result<PathBuf> {
    ...
    let json_bundle = codex_utils_cargo_bin::find_resource!(
        "schema/json/peregrine_app_server_protocol.schemas.json"
    )
    .context("resolve JSON schema bundle")?;
    let json_root = json_bundle.parent().and_then(|p| p.parent()).context(
        "derive schema root from schema/json/peregrine_app_server_protocol.schemas.json",
    )?;
    anyhow::ensure!(
