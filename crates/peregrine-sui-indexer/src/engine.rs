...
use crate::{
    analysis_input::merge_normalized_analysis,
...
use peregrine_analysis::{ArtifactBundle, PropertyGraph};
...
impl SuiMoveIndexer {
    ...
    pub fn index_package(
        ...
        self.index_package_inner(root.as_ref(), run_id, None)
    }

    pub fn index_package_with_analysis(
        &self,
        root: impl AsRef<Path>,
        run_id: String,
        ...
        self.index_package_inner(root.as_ref(), run_id, Some((artifacts, graphs)))
    }

    fn index_package_inner(
        &self,
        root: &Path,
        run_id: String,
        analysis: Option<(&ArtifactBundle, &[PropertyGraph])>,
    ) -> IndexerResult<IndexReport> {
        let loaded = self.adapter.load_package(root)?;
        ...
        if let Some((artifacts, graphs)) = analysis {
            merge_normalized_analysis(&mut program, artifacts, graphs)?;
        }
    ...
    fn materialize_summary(
        ...
        self.adapter
            .materialize_summary_context(crate::model::SummaryMaterializationRequest {
