...
use peregrine_analysis::{
    AnalysisDiagnostic, AnalysisStage, Artifact, ArtifactBundle, ChainId, GraphEdge, GraphKind,
    PropertyGraph,
};
...
#[test]
fn normalized_analysis_outputs_are_persisted_as_structure_and_evidence() {
    let root = copy_fixture("package_summaries_pointer_basic");
    let artifacts = ArtifactBundle {
        chain: ChainId::new("sui"),
        target_id: "fixture-target".to_string(),
        package_root: Some(root.path().to_path_buf()),
        artifacts: vec![Artifact {
            id: "artifact::fixture".to_string(),
            kind: "module".to_string(),
            name: "fixture".to_string(),
            path: Some("sources/fixture.move".to_string()),
            metadata: Value::Null,
        }],
        symbols: Vec::new(),
        evidence: Vec::new(),
    ...
    let graphs = vec![PropertyGraph {
        ...
        nodes: Vec::new(),
        edges: vec![GraphEdge {
            id: "edge::fixture".to_string(),
            from: "value::source".to_string(),
            to: "value::sink".to_string(),
            kind: "flowsTo".to_string(),
            spans: Vec::new(),
    ...
    SuiMoveIndexer::new(IndexerConfig::default())
        .index_package_with_analysis(root.path(), "analysis-run".to_string(), &artifacts, &graphs)
        .expect("index package with normalized analysis");

    let connection = Connection::open(root.path().join(".peregrine/index.sqlite")).expect("db");
    let metadata: String = connection
        .query_row("SELECT metadata_json FROM packages", [], |row| row.get(0))
        .expect("package metadata");
    let metadata: Value = serde_json::from_str(&metadata).expect("parse package metadata");
    assert_eq!(
        metadata["normalizedAnalysis"],
        json!({
    ...
    let persisted_edge: (String, String, String) = connection
        .query_row(
            "SELECT from_id, to_id, edge_type FROM edges WHERE edge_type = 'DataFlow'",
            [],
            |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?)),
        )
        .expect("persisted data-flow edge");
    assert_eq!(
        persisted_edge,
        (
    ...
    let diagnostic_source: String = connection
        .query_row(
            "SELECT source FROM diagnostics WHERE message = 'fixture diagnostic'",
            [],
            |row| row.get(0),
        )
        .expect("persisted analysis diagnostic");
    assert_eq!(diagnostic_source, "sui.scanner");
}
...
fn find_function(
    ...
    package_id: &str,
    query: &str,
) -> peregrine_sui_indexer::model::SymbolResult {
    let reader = SqliteIndexReader::open(db_path).expect("reader");
    reader
...
fn operation_position(
    operations: &[peregrine_sui_indexer::core::Operation],
    kind: OperationKind,
) -> usize {
    operations
        .iter()
...
fn is_transfer_operation(operation: &peregrine_sui_indexer::core::Operation) -> bool {
