...
RELEASE="${PEREGRINE_RELEASE:-latest}"
NON_INTERACTIVE="${PEREGRINE_NON_INTERACTIVE:-false}"

BIN_DIR="${PEREGRINE_INSTALL_DIR:-$HOME/.local/bin}"
BIN_PATH="$BIN_DIR/peregrine"
PEREGRINE_HOME_DIR="${PEREGRINE_HOME:-$HOME/.peregrine}"
STANDALONE_ROOT="$PEREGRINE_HOME_DIR/standalone"
...
validate_version() {
  ...
  if ! printf '%s\n' "$version" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+(-(alpha|beta)(\.[0-9]+)?)?$'; then
    echo "Invalid Peregrine release version: $version. Expected latest or x.y.z[-alpha[.N]|-beta[.N]]." >&2
    exit 1
  fi
}
...
Environment:
  PEREGRINE_RELEASE          Version to install; overridden by --release.
  PEREGRINE_NON_INTERACTIVE  Set to 1, true, or yes to skip prompts.
EOF
        exit 0
        ;;
      *)
...
download_file() {
  ...
  echo "curl or wget is required to install Peregrine." >&2
  exit 1
}
...
release_url_for_asset() {
  ...
  printf 'https://github.com/mcxross/peregrine/releases/download/v%s/%s\n' "$resolved_version" "$asset"
}

release_metadata_url() {
  resolved_version="$1"

  printf 'https://api.github.com/repos/mcxross/peregrine/releases/tags/v%s\n' "$resolved_version"
}
...
release_asset_digest() {
  ...
  digest="$(printf '%s\n' "$digest_text" | awk '{print $1}')"
  if [ -z "$digest" ]; then
    echo "Could not parse SHA-256 digest for release asset $asset." >&2
    exit 1
  fi
...
file_sha256() {
  ...
  echo "sha256sum, shasum, or openssl is required to verify the Peregrine download." >&2
  exit 1
}
...
verify_archive_digest() {
  ...
  if [ "$actual_digest" != "$expected_digest" ]; then
    echo "Downloaded Peregrine archive checksum did not match expected digest." >&2
...
require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "$1 is required to install Peregrine." >&2
    exit 1
  fi
}
...
resolve_version() {
  ...
  release_json="$(download_text "https://api.github.com/repos/mcxross/peregrine/releases/latest")"
  resolved="$(printf '%s\n' "$release_json" | sed -n 's/.*"tag_name":[[:space:]]*"v\([^"]*\)".*/\1/p' | head -n 1)"

  if [ -z "$resolved" ]; then
    echo "Failed to resolve the latest Peregrine release version." >&2
    exit 1
  fi
...
add_to_path() {
  ...
  begin_marker="# >>> Peregrine installer >>>"
  end_marker="# <<< Peregrine installer <<<"
...
cleanup_stale_install_artifacts() {
  ...
  if [ -d "$BIN_DIR" ]; then
    find "$BIN_DIR" -mindepth 1 -maxdepth 1 -name '.peregrine.*' -exec rm -f {} +
  fi
}
...
version_from_binary() {
  ...
  if [ ! -x "$peregrine_path" ]; then
    return 1
  fi

  "$peregrine_path" --version 2>/dev/null | sed -n 's/.* \([0-9][0-9A-Za-z.+-]*\)$/\1/p' | head -n 1
}

current_installed_version() {
  version="$(version_from_binary "$CURRENT_LINK/peregrine-tui" || true)"
  if [ -n "$version" ]; then
    printf '%s\n' "$version"
    return 0
  fi
...
resolve_existing_peregrine() {
  command -v peregrine 2>/dev/null || true
}

classify_existing_peregrine() {
  existing_path="$1"
...
print_launch_instructions() {
  case "$path_action" in
    added)
      step "Current terminal: export PATH=\"$BIN_DIR:\$PATH\" && peregrine"
      step "Future terminals: open a new terminal and run: peregrine"
    ...
    configured)
      ...
      ;;
    *)
      step "Current terminal: peregrine"
      step "Future terminals: open a new terminal and run: peregrine"
      ;;
  esac
}
...
detect_conflicting_install() {
  ...
  step "Detected existing $manager-managed Peregrine at $existing_path"
  warn "Multiple managed Peregrine installs can be ambiguous because PATH order decides which one runs."
}
...
handle_conflicting_install() {
  ...
  case "$conflict_manager" in
    brew)
      uninstall_cmd="brew uninstall --cask peregrine"
      ;;
    *)
      uninstall_cmd="rm $conflict_path"
      ;;
  esac

  if prompt_yes_no "Uninstall the existing $conflict_manager-managed Peregrine now?"; then
    step "Running: $uninstall_cmd"
    if ! sh -c "$uninstall_cmd"; then
      warn "Failed to uninstall the existing $conflict_manager-managed Peregrine. Continuing with the standalone install."
    fi
  else
    warn "Leaving the existing $conflict_manager-managed Peregrine installed. PATH order will determine which peregrine runs."
  fi
}
...
release_dir_is_complete() {
  ...
  [ -x "$release_dir/peregrine-tui" ] || return 1
}
...
update_visible_command() {
  release_dir="$1"
  mkdir -p "$BIN_DIR"
  tmp_link="$BIN_DIR/.peregrine.$$"

  replace_path_with_symlink "$BIN_PATH" "$CURRENT_LINK/peregrine-tui" "$tmp_link"
}
...
asset="peregrine-tui-$vendor_target.tar.gz"
...
if [ -n "$current_version" ] && [ "$current_version" != "$resolved_version" ]; then
  step "Updating Peregrine CLI from $current_version to $resolved_version"
elif [ -n "$current_version" ]; then
  step "Updating Peregrine CLI"
else
  step "Installing Peregrine CLI"
fi
