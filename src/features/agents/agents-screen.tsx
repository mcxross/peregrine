...
export function AgentsScreen({
  ...
  const [settings, setSettings] = React.useState<AgentServerSettings>(() => loadAgentServerSettings());
  ...
  const selectedRunDetail = selectedAgent ? runDetailsByAgentId[selectedAgent.id] : undefined;
  ...
  React.useEffect(() => {
    ...
    void loadProjectMetadata(projectRootPath)
      .then((metadata) => {
        if (!cancelled) {
          setState(ensurePrimaryAgentThreadState(loadAgentStudioStateFromProjectMetadata(metadata)));
          setIsProjectStateLoaded(true);
        }
      })
      .catch((error) => {
  ...
  const refreshModels = React.useCallback(() => {
    ...
    void listAgentServerModels({
      ...
      .then((response) => {
        const defaultModel = response.models.data.find((model) => model.isDefault)
          ?? response.models.data[0];
        const selectedProvider = response.providers.data.find((provider) => provider.selected);
        setModelSummary({
          label: defaultModel
        ...
        {activeMainTab === "details" ? (
          ...
          <section className="grid min-h-0 grid-rows-[auto_minmax(0,1fr)] overflow-hidden">
            <RunSummarySection
              ...
              packageName={projectContext?.packageName}
              target={settings.target}
            />
            <AgentsTable
              ...
              onRunAgent={(agent) => {
                const workflow = state.workflows.find((candidate) => candidate.id === agent.workflowId)
                  ?? selectedWorkflow;
                void runWorkflowFor(agent, workflow);
              }}
  ...
  async function runWorkflowFor(runAgent: AgentDefinition, runWorkflowState: AgentWorkflow) {
    ...
    try {
      ...
      setRunCompletedState(runAgent, runWorkflowState, previousStatus, "completed", "info");
    } catch (error) {
  ...
  function appendRunDetailEvent(
    ...
    setRunDetailsByAgentId((current) => {
      ...
      return {
        ...
        [agentId]: {
          ...detail,
          events: [...detail.events, createRunEvent(event, timestamp)].slice(-160),
        },
      };
    });
  }
  ...
  function setRunCompletedState(
    ...
    setState((current) => ({
      ...
      workflows: current.workflows.map((candidate) =>
        candidate.id === workflow.id ? markWorkflowStatus(candidate, workflowStatus) : candidate,
      ),
      logs: [
        ...current.logs,
        createExecutionLog({
  ...
  return (
    <header className="flex flex-wrap items-center justify-between gap-3 border-b border-[color:var(--app-border)] bg-[var(--app-chrome)] px-4 py-3">
      ...
      <div className="flex flex-wrap items-center justify-end gap-2">
        ...
        <Button className="h-8 gap-1.5 text-xs" onClick={onRefreshModels} type="button" variant="outline">
          <Server className="size-3.5" />
          Models
        </Button>
        {isRunInProgress ? (
...
function RunSummarySection({
  ...
  const cards = [
    ...
    { icon: Bot, label: "Package", value: packageName ? displayMovePackageName(packageName) : "None" },
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-end" onClick={(event) => event.stopPropagation()}>
        <Button
          aria-label={isAgentRunning ? `Stop ${agent.name}` : `Run ${agent.name}`}
          className="size-7"
        ...
        >
          {isAgentRunning ? <Square className="size-3.5" /> : <Play className="size-3.5" />}
        </Button>
      </div>
      <div className="flex justify-end pr-3" onClick={(event) => event.stopPropagation()}>
        <DropdownMenu>
      ...
      <header className="flex min-w-0 items-center border-b border-[color:var(--app-border)] bg-[var(--app-chrome)] px-4 py-2">
        <div className="flex min-w-0 flex-1 items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-2">
            ...
            <span className="truncate text-sm font-semibold">Agent Threads</span>
            <ChevronRight className="size-3 shrink-0 text-muted-foreground" aria-hidden="true" />
            <span className="truncate text-sm text-muted-foreground">{agent.name}</span>
          </div>
          {agentIsRunning ? (
            <Button className="h-8 shrink-0 gap-1.5 text-xs" onClick={onStopRun} type="button" variant="outline">
              <Square className="size-3.5" />
              Stop
            </Button>
          ) : (
            <Button className="h-8 shrink-0 gap-1.5 text-xs" disabled={isRunInProgress} onClick={onRunWorkflow} type="button">
              <Play className="size-3.5" />
              Run
            </Button>
          )}
  ...
  return (
    <section className="grid min-h-0 min-w-0 grid-rows-[auto_minmax(0,1fr)] overflow-hidden border-r border-[color:var(--app-border)] bg-[var(--app-window)]">
      <header className="flex items-center justify-between gap-3 border-b border-[color:var(--app-border)] px-3 py-3">
        ...
        {agentIsRunning ? <span className="text-[11px] text-sky-300">Streaming</span> : null}
      </header>
      <div className="min-h-0 overflow-auto p-3">
        {responseText ? (
          ...
          <div className="grid min-h-full place-items-center rounded border border-dashed border-[color:var(--app-border)] bg-black/10 p-6 text-center text-xs leading-5 text-muted-foreground">
            {agentIsRunning ? "Waiting for streamed app-server text." : "Run this thread to stream a response here."}
          </div>
        )}
        {reasoningText ? (
        ...
        {events.length ? (
          <div className="grid gap-2">
            {events.slice().reverse().map((event) => (
              <div className="rounded border border-[color:var(--app-border)] bg-[var(--app-window)] p-2" key={event.id}>
                <div className="flex items-center justify-between gap-2">
                  <span className={cn("text-xs font-semibold", eventLevelClass(event.level))}>{event.title}</span>
                  <span className="text-[10px] text-muted-foreground">{timeLabel(event.timestamp)}</span>
                </div>
    ...
    <aside className="grid min-h-0 min-w-0 grid-rows-[auto_minmax(0,1fr)] overflow-hidden border-l border-[color:var(--app-border)] bg-[var(--app-panel)]">
      ...
      <ScrollArea className="min-h-0 min-w-0">
        <div className="space-y-4 px-4 pb-4 pt-3">
          ...
          <SettingsBlock title="App Server">
            <LabelledField label="Target">
              <select
                className="h-8 w-full min-w-0 rounded-md border border-[color:var(--app-border)] bg-[var(--app-panel)] px-3 text-xs outline-none"
                onChange={(event) => {
                  const mode = event.target.value as AgentServerTarget["mode"];
                  onSettingsChange((current) => ({
                    target:
                      mode === "remote"
                        ? { mode, endpoint: current.target.endpoint ?? "ws://127.0.0.1:8910" }
                        : mode === "localDaemon"
                          ? { mode, endpoint: current.target.endpoint }
                          : { mode },
                  }));
                }}
                value={settings.target.mode}
              >
                <option value="embedded">Embedded</option>
                <option value="localDaemon">Local daemon</option>
                <option value="remote">Remote</option>
              </select>
            </LabelledField>
            {settings.target.mode !== "embedded" ? (
              <LabelledField label="Endpoint">
                <Input
                  className="h-8 text-xs"
                  onChange={(event) =>
                    onSettingsChange({
                      target: settings.target.mode === "remote"
                        ? { mode: "remote", endpoint: event.target.value }
                        : { mode: "localDaemon", endpoint: event.target.value || undefined },
                    })
                  }
                  placeholder={settings.target.mode === "remote" ? "ws://127.0.0.1:8910" : "unix://"}
                  value={settings.target.endpoint ?? ""}
                />
              </LabelledField>
            ) : null}
            <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-2 rounded-md border border-[color:var(--app-border)] bg-[var(--app-surface)] px-2 py-2">
              <p className="min-w-0 truncate text-[10px] leading-4 text-muted-foreground" title={modelSummary.error ?? modelSummary.label}>
                {modelSummary.status === "loading"
                  ? "Reading app-server model config..."
                  : modelSummary.error ?? modelSummary.label}
              </p>
              <Button className="h-6 shrink-0 px-2 text-[10px]" onClick={onRefreshModels} type="button" variant="outline">
                Refresh
              </Button>
            </div>
          </SettingsBlock>

          <SettingsBlock title="Instructions">
            <pre className="min-h-24 whitespace-pre-wrap break-words rounded-md border border-[color:var(--app-border)] bg-[var(--app-panel)] px-3 py-2 font-mono text-xs leading-5 text-muted-foreground">
              {agent.systemPrompt || agent.description}
            </pre>
          </SettingsBlock>

          <SettingsBlock title="Workflow">
            <p className="text-xs leading-5 text-muted-foreground">{workflow.description}</p>
            <Button className="h-8 w-full gap-1.5 text-xs" onClick={onOpenDetails} type="button" variant="outline">
...
function LabelledField({ children, label }: { children: React.ReactNode; label: string }) {
  return (
...
function AgentIcon({ agent }: { agent: AgentDefinition }) {
  ...
  return (
    <div className={cn(
      ...
      metadata.category === "Primary" && "border-sky-400/30 bg-sky-400/10 text-sky-200",
      metadata.category === "Subagent" && "border-emerald-400/30 bg-emerald-400/10 text-emerald-200",
    )}>
...
function CategoryBadge({ category }: { category: AgentCategory }) {
  return (
    <Badge className="w-fit rounded px-2 py-0.5 text-[10px]" variant="secondary">
      {category}
    </Badge>
  );
}
...
function serverRequestSummary(request: ServerRequest) {
  switch (request.method) {
    case "item/commandExecution/requestApproval":
      return request.params.command ?? request.params.reason ?? "Command approval requested.";
...
function resolveServerRequestWithPrompt(request: ServerRequest): AgentServerRequestResolution {
  switch (request.method) {
    case "item/commandExecution/requestApproval": {
      const command = request.params.command ?? "command";
      const approved = window.confirm(`Allow app-server command?\n\n${command}`);
      return { type: "resolve", result: { decision: approved ? "accept" : "decline" } };
    }
    case "item/fileChange/requestApproval": {
      const approved = window.confirm(`Allow app-server file change?\n\n${request.params.reason ?? "File change requested."}`);
      return { type: "resolve", result: { decision: approved ? "accept" : "decline" } };
    }
    case "item/tool/requestUserInput": {
